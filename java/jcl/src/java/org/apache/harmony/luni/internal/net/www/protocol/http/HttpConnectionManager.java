...
/**
 * <code>HttpConnectionManager</code> manages a pool of
 * <code>HttpConnection</code>s that are not currently in use and is used to get
 * hold of persistent <code>HttpConnection</code>s. Clients should return an
 * <code>HttpConnection</code> to the pool after use by calling
 * <code>returnConnectionToPool</code>
 * 
 * Two system properties affect the behaviour of this class -
 * <code>http.maxConnections</code> and <code>http.keepAlive</code>.
 * <code>http.keepAlive</code> determines whether or not connections should be
 * persisted and <code>http.maxConnections</code> determines the maximum number
 * of connections to each individual host that should be kept in the pool.
 */
public class HttpConnectionManager {
	...
	public HttpConnection getConnection(URI uri, int connectTimeout)
			throws IOException {
	...
	public HttpConnection getConnection(URI uri, Proxy proxy, int connectTimeout)
			throws IOException {
		checkSystemProperties();
	...
	private void checkSystemProperties() {
		String httpMaxConnections = AccessController
				.doPrivileged(new PriviAction<String>("http.maxConnections"));
		String httpKeepAlive = AccessController
				.doPrivileged(new PriviAction<String>("http.keepAlive"));
	...
	private static class ConnectionPool {
		...
		private Map<HttpConfiguration, List<HttpConnection>> freeConnectionMap = new HashMap<HttpConfiguration, List<HttpConnection>>(); // Map
																																			// of
																																			// free
																																			// Sockets

		public synchronized void clear() {
			for (Iterator<List<HttpConnection>> iter = freeConnectionMap
					.values().iterator(); iter.hasNext();) {
				List<HttpConnection> connections = iter.next();
				for (Iterator<HttpConnection> iterator = connections.iterator(); iterator
						.hasNext();) {
		...
		public synchronized void returnConnection(HttpConnection connection) {
			if (!connection.getSocket().isClosed() && keepAlive) {
				...
				List<HttpConnection> connections = freeConnectionMap
						.get(config);
				if (connections == null) {
		...
		public synchronized HttpConnection getHttpConnection(
				HttpConfiguration config, int connectTimeout)
				throws IOException {
			...
			if (!keepAlive || connections.isEmpty()) {
				...
				if (!connection.isStale()) {
					...
					if (security != null) {
						security.checkConnect(connection.getSocket()
								.getInetAddress().getHostName(), connection
								.getSocket().getPort());
					}
					return connection;
				} else {

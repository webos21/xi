...
public class HttpURLConnectionImpl extends HttpURLConnection {
	...
	private class LimitedInputStream extends InputStream {
		...
		public void close() throws IOException {
			if (bytesRemaining > 0) {
				bytesRemaining = 0;
				disconnect(true); // Should close the socket if client hasn't
									// read all the data
			} else {
				disconnect(false);
			}
		...
		public int read(byte[] buf, int offset, int length) throws IOException {
			...
			if (result > 0) {
				...
				// if user has set useCache to true and cache exists,
				// writes to it
				if (useCaches && null != cacheOut) {
	...
	protected HttpConnection getHTTPConnection(Proxy proxy) throws IOException {
		...
		if (proxy == null || proxy.type() == Proxy.Type.DIRECT) {
			...
			connection = HttpConnectionManager.getDefault().getConnection(uri,
					getConnectTimeout());
		} else {
			connection = HttpConnectionManager.getDefault().getConnection(uri,
					proxy, getConnectTimeout());
		}
		return connection;
	}
	 ...
	 */
	protected void setUpTransportIO(HttpConnection connection)
			throws IOException {
	...
	private boolean getFromCache() throws IOException {
		if (useCaches && null != responseCache && !hasTriedCache) {
			...
			cacheResponse = responseCache.get(uri, method,
					resHeader.getFieldMap());
	...
	public OutputStream getOutputStream() throws IOException {
		...
		if (sentRequest) {
			throw new ProtocolException(
					"can't open OutputStream after reading from an inputStream");
		}
	...
	private String getAuthorizationCredentials(String challenge)
		...
		String usernameAndPassword = pa.getUserName() + ":"

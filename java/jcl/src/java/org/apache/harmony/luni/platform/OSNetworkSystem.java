...
/**
 * This wraps native code that implements the INetworkSystem interface. Address
 * length was changed from long to int for performance reasons.
 */
final class OSNetworkSystem implements INetworkSystem {
	...
	public native void bind(FileDescriptor fd, InetAddress inetAddress, int port)
			throws SocketException;

	public native void connect(FileDescriptor fd, InetAddress inetAddress,
			int port, int timeout) throws SocketException;

	public native boolean connectNonBlocking(FileDescriptor fd,
			InetAddress inetAddress, int port) throws IOException;

	public native boolean isConnected(FileDescriptor fd, int timeout)
			throws IOException;

	public native void socket(FileDescriptor fd, boolean stream)
			throws SocketException;
	...
	public native void listen(FileDescriptor fd, int backlog)
			throws SocketException;

	public native int read(FileDescriptor fd, byte[] data, int offset, int count)
			throws IOException;

	public native int readDirect(FileDescriptor fd, int address, int count)
			throws IOException;

	public native int recv(FileDescriptor fd, DatagramPacket packet,
			byte[] data, int offset, int length, boolean peek, boolean connected)
			throws IOException;

	public native int recvDirect(FileDescriptor fd, DatagramPacket packet,
			int address, int offset, int length, boolean peek, boolean connected)
			throws IOException;
	...
	public boolean select(FileDescriptor[] readFDs, FileDescriptor[] writeFDs,
		...
		return selectImpl(readFDs, writeFDs, numReadable, numWritable, flags,
				timeout);
	}
	...
	public native int send(FileDescriptor fd, byte[] data, int offset,
			int length, int port, InetAddress inetAddress) throws IOException;

	public native int sendDirect(FileDescriptor fd, int address, int offset,
			int length, int port, InetAddress inetAddress) throws IOException;
	...
	public native int writeDirect(FileDescriptor fd, int address, int offset,
			int count) throws IOException;
}

...
public interface INetworkSystem {
	...
	public void bind(FileDescriptor fd, InetAddress inetAddress, int port)
	...
	public boolean connectNonBlocking(FileDescriptor fd,
			InetAddress inetAddress, int port) throws IOException;

	public boolean isConnected(FileDescriptor fd, int timeout)
			throws IOException;

	public int send(FileDescriptor fd, byte[] data, int offset, int length,
			int port, InetAddress inetAddress) throws IOException;

	public int sendDirect(FileDescriptor fd, int address, int offset,
			int length, int port, InetAddress inetAddress) throws IOException;

	public int recv(FileDescriptor fd, DatagramPacket packet, byte[] data,
			int offset, int length, boolean peek, boolean connected)
			throws IOException;
	...
	public void socket(FileDescriptor fd, boolean stream)
			throws SocketException;
	...
	public void connect(FileDescriptor fd, InetAddress inetAddress, int port,
	 ...
	 * 
	 * <p>
	 * The first {@code numReadable} file descriptors of {@code readFDs} will be
	 * selected for read-ready operations. The first {@code numWritable} file
	 ...
	 *            array. See {@code SelectorImpl.READABLE} and
	 *            {@code SelectorImpl.WRITEABLE}
	 * @return true unless selection timed out or was interrupted
	 * @throws SocketException
	 */

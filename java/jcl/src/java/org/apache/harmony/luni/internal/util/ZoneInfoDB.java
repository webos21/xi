 ...
 * the 'zoneinfo' database as the source of time zone information. However, to
 * conserve disk space the data for all time zones are concatenated into a
 * single file, and a second file is used to indicate the starting position of
 * each time zone record. A third file indicates the version of the zoneinfo
 * database used to generate the data.
 * 
 * {@hide}
 */
public final class ZoneInfoDB {
	 ...
	 */
	private static final String ZONE_DIRECTORY_NAME = (System
			.getenv("ICU_DATA") != null
			&& !"".equals(System.getenv("ICU_DATA")) ? System
			.getenv("ICU_DATA") + "/zoneinfo/" : "../res/icu/zoneinfo/");
	// "../res/icu/zoneinfo/";
	// System.getenv("ANDROID_ROOT") + "/usr/share/zoneinfo/"; by jshwang

	/**
	 * The name of the file containing the concatenated time zone records.
	 */
	private static final String ZONE_FILE_NAME = ZONE_DIRECTORY_NAME
			+ "zoneinfo.dat";
	 ...
	 */
	private static final String INDEX_FILE_NAME = ZONE_DIRECTORY_NAME
			+ "zoneinfo.idx";
	...
	/**
	 * The 'ids' array contains time zone ids sorted alphabetically, for binary
	 * searching. The other two arrays are in the same order. 'byteOffsets'
	 * gives the byte offset into "zoneinfo.dat" of each time zone, and
	 * 'rawUtcOffsets' gives the time zone's raw UTC offset.
	 */
	...
	private static String readVersion() {
		...
		try {
			versionFile = new RandomAccessFile(ZONE_DIRECTORY_NAME
					+ "zoneinfo.version", "r");
	...
	/**
	 * Traditionally, Unix systems have one file per time zone. We have one big
	 * data file, which is just a concatenation of regular time zone files. To
	 * allow random access into this big data file, we also have an index. We
	 * read the index at startup, and keep it in memory so we can binary search
	 * by id when we need time zone data.
	 * 
	...
	private static void readIndex() {
		...
		try {
			...
			for (int i = 0; i < numEntries; i++) {
				...
				// System.out.println("idBytes = " + new String(idBytes));
				// System.out.println("byteOffsets = " + byteOffsets[i]);
				// System.out.println("rawUtcOffsets[i] = " + rawUtcOffsets[i]);
			...
			// We create one string containing all the ids, and then break that
			// into substrings.
	...
	/**
	 * Rather than open, read, and close the big data file each time we look up
	 * a time zone, we map the big data file during startup, and then just use
	 * the ByteBuffer.
	 * 
	 * At the moment, this "big" data file is about 160 KiB. At some point, that
	 * will be small enough that we'll just keep the byte[] in memory.
	 */
	...
	private static TimeZone makeTimeZone(String id) throws IOException {
		...
		// We duplicate the ByteBuffer to allow unsynchronized access to this
		// shared data,

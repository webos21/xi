...
final class OSMemory {
	 ...
	 * <code>destAddress</code>. Where any part of the source memory block and
	 * the destination memory block overlap <code>memmove()</code> ensures that
	 * the original source bytes in the overlapping region are copied before
	 * being overwritten.
	 * <p>
	 * The behavior is unspecified if
	 ...
	 */
	public static native void memmove(int destAddress, int srcAddress,
			long length);

	/**
	 * Copies <code>length</code> bytes from the memory block at
	 * <code>address</code> into the byte array <code>bytes</code> starting at
	 * element <code>offset</code> within the byte array.
	 * <p>
	 * The behavior of this method is undefined if the range
	 * <code>(address ... address + length)</code> is not within a memory block
	 * that was allocated using {@link #malloc(int) malloc(int)}.
	 * </p>
	 * 
	 * @param address
	 ...
	 * @param offset
	 *            the index of the first element in <code>bytes</code> that will
	 *            be overwritten.
	 * @param length
	 ...
	 */
	public static native void getByteArray(int address, byte[] bytes,
			int offset, int length) throws NullPointerException,
	 ...
	 *            be read.
	 * @param length
	 *            the total number of bytes to copy from <code>bytes</code> into
	 *            the memory block.
	 * @throws NullPointerException
	 *             if <code>bytes</code> is <code>null</code>.
	 * @throws IndexOutOfBoundsException
	 *             if <code>offset + length > bytes.length</code>.
	 */
	public static native void setByteArray(int address, byte[] bytes,
			int offset, int length) throws NullPointerException,
			IndexOutOfBoundsException;

	/**
	 * Copies <code>length</code> shorts from the short array
	 * <code>shorts</code> into the memory block at <code>address</code>,
	 * starting at element <code>offset</code> within the short array.
	 * <p>
	 * The behavior of this method is undefined if the range
	 ...
	 *             if <code>offset + length > bytes.length</code>.
	 */
	public static native void setShortArray(int address, short[] shorts,
			int offset, int length, boolean swap) throws NullPointerException,
			IndexOutOfBoundsException;

	/**
	 * Copies <code>length</code> ints from the int array <code>ints</code> into
	 * the memory block at <code>address</code>, starting at element
	 ...
	 * @param offset
	 *            the index of the first element in <code>ints</code> that will
	 *            be read.
	 * @param length
	 *            the total number of ints to copy from <code>ints</code> into
	 *            the memory block.
	 * @param swap
	...
	public static native void setFloatArray(int address, float[] floats,
			int offset, int length, boolean swap) throws NullPointerException,
			IndexOutOfBoundsException;
	...
	public static short getShort(int address, ByteOrder byteOrder) {
		return (byteOrder == NATIVE_ORDER) ? getShort(address) : Short
				.reverseBytes(getShort(address));
	}
	 ...
	 * <p>
	 * The behavior is unspecified if <code>(address ... address + 4)</code> is
	 * not wholly within the range that was previously allocated using
	 * <code>malloc()</code>.
	 * </p>
	 * 
	...
	public static int getInt(int address, ByteOrder byteOrder) {
		return (byteOrder == NATIVE_ORDER) ? getInt(address) : Integer
				.reverseBytes(getInt(address));
	}
	 ...
	 * order at the given address.
	 * <p>
	 * The behavior is unspecified if <code>(address ... address + 8)</code> is
	 * not wholly within the range that was previously allocated using
	 * <code>malloc()</code>.
	 * </p>
	 * 
	...
	public static long getLong(int address, ByteOrder byteOrder) {
		return (byteOrder == NATIVE_ORDER) ? getLong(address) : Long
				.reverseBytes(getLong(address));
	}
	...
	private static native int mmapImpl(int fd, long offset, long size,
			int mapMode);

	public static int mmap(int fd, long offset, long size, MapMode mapMode)
			throws IOException {
		// Check just those errors mmap(2) won't detect.
		if (offset < 0 || size < 0 || offset > Integer.MAX_VALUE
				|| size > Integer.MAX_VALUE) {
			throw new IllegalArgumentException("offset=" + offset + " size="
					+ size);
		}

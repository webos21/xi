...
final class ZoneInfo extends TimeZone {
	...
	private static final long MILLISECONDS_PER_400_YEARS = MILLISECONDS_PER_DAY
			* (400 * 365 + 100 - 3);

	private static final long UNIX_OFFSET = 62167219200000L;

	private static final int[] NORMAL = new int[] { 0, 31, 59, 90, 120, 151,
			181, 212, 243, 273, 304, 334, };

	private static final int[] LEAP = new int[] { 0, 31, 60, 91, 121, 152, 182,
			213, 244, 274, 305, 335, };
	...
	ZoneInfo(String name, int[] transitions, byte[] type, int[] gmtoff,
			byte[] isdst, byte[] abbreviationIndexes, byte[] abbreviationList) {
		...
		if (lastdst >= 0) {
			mDaylightName = nullName(abbreviationList,
					abbreviationIndexes[mTypes[lastdst] & 0xFF]);
		} else {
			mDaylightName = null;
		}
		if (laststd >= 0) {
			mStandardName = nullName(abbreviationList,
					abbreviationIndexes[mTypes[laststd] & 0xFF]);
		} else {
			mStandardName = null;
		}
		...
		// We don't care if they've historically used it: most places have at
		// least once.
		// We want to know whether the last "schedule info" (the unix times in
		// the mTransitions
		// array) is in the future. If it is, DST is still relevant.
		// See http://code.google.com/p/android/issues/detail?id=877.
		// This test means that for somewhere like Morocco, which tried DST in
		// 2009 but has
		// no future plans (and thus no future schedule info) will report "true"
		// from
		// useDaylightTime at the start of 2009 but "false" at the end. This
		// seems appropriate.
		boolean usesDst = false;
		long currentUnixTime = System.currentTimeMillis() / 1000;
		if (mTransitions.length > 0) {
			// (We're really dealing with uint32_t values, so long is most
			// convenient in Java.)
	...
	public int hashCode() {
		...
		int result = 1;
		result = prime * result
				+ ((mDaylightName == null) ? 0 : mDaylightName.hashCode());
		result = prime * result + Arrays.hashCode(mGmtOffs);
		result = prime * result + Arrays.hashCode(mIsDsts);
		result = prime * result + mRawOffset;
		result = prime * result
				+ ((mStandardName == null) ? 0 : mStandardName.hashCode());
	...
	public String toString() {
		return getClass().getName() + "[\"" + mStandardName + "\",mRawOffset="
				+ mRawOffset + ",mUseDst=" + mUseDst + "]";
	}
}

...
public class PlatformAddressFactory {
	 ...
	 * power of two. Caching PlatformAddress objects minimizes the creation of
	 * garbage and reduces the number of GC-hiccups in OpenGL animations.
	 */
	...
	/**
	 * Defines the maximum number of probes taken per hash, used for looking up
	 * an empty cache slot or a previously stored PlatformAddress.
	 */
	private final static int MAX_PROBES = 5;

	/**
	 * A cycling index (0 to MAX_PROBES-1) used to replace elements in the
	 * cache.
	 */
	 ...
	 * 
	 * @param address
	 *            the start address for the memory; {@code 0} means {@code null}
	 * @param size
	...
	public synchronized static PlatformAddress on(int value, long size) {
		...
		for (int probe = 0; probe < MAX_PROBES; probe++) {
			...
			if (cachedObj == null) {
				return cache[(idx + probe) & CACHE_MASK] = new PlatformAddress(
						value, size);
			}
		...
		return cache[(idx + replacementIndex) & CACHE_MASK] = new PlatformAddress(
				value, size);
	}
	...
	public static PlatformAddress allocMap(int fd, long start, long size,
			MapMode mode) throws IOException {
		if (size == 0) {
	...
	/**
	 * Allocates a contiguous block of OS heap memory and initializes it to a
	 * given value.
	 * 
	 * @param size
	 *            The number of bytes to allocate from the system heap.
	 * @param init
	 *            The value to initialize the memory.
	...
	public static PlatformAddress alloc(int size, byte init) {
		...
		/*
		 * We use make() and not on() here, for a couple reasons: First and
		 * foremost, doing so means that if the client uses address.autoFree()
		 * (to enable auto-free on gc) the cache won't prevent the freeing
		 * behavior. Second, this avoids polluting the cache with addresses that
		 * aren't likely to be reused anyway.
		 */

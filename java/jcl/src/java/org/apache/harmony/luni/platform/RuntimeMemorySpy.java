...
final class RuntimeMemorySpy {
	...
	final class AddressWrapper {
		...
		AddressWrapper(PlatformAddress address) {
			...
			this.wrAddress = new PhantomReference<PlatformAddress>(address,
					notifyQueue);
		}
	}

	// TODO: figure out how to prevent this being a synchronization bottleneck
	private final Map<PlatformAddress, AddressWrapper> memoryInUse = new HashMap<PlatformAddress, AddressWrapper>(); // Shadow
																														// to
																														// Wrapper

	private final Map<Reference, PlatformAddress> refToShadow = new HashMap<Reference, PlatformAddress>(); // Reference
																											// to
																											// Shadow
	...
	public boolean free(PlatformAddress address) {
		...
		if (wrapper == null) {
			...
			System.err
					.println("Memory Spy! Fixed attempt to free memory that was not allocated "
							+ address);
		}
		return wrapper != null;
	}
	...
	private void orphanedMemory(Reference ref) {
		synchronized (this) {
			...
			if (wrapper != null) {
				...
				if (!wrapper.autoFree) {
					System.err
							.println("Memory Spy! Fixed memory leak by freeing "
									+ wrapper.shadow);
				}
				wrapper.shadow.free();
			}

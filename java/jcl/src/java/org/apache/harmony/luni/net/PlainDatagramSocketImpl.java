...
public class PlainDatagramSocketImpl extends DatagramSocketImpl {
	...
	public void joinGroup(SocketAddress addr, NetworkInterface netInterface)
		...
		if (addr instanceof InetSocketAddress) {
			...
			setOption(MCAST_JOIN_GROUP, new MulticastGroupRequest(groupAddr,
					netInterface));
		}
	}
	...
	public void leaveGroup(SocketAddress addr, NetworkInterface netInterface)
		...
		if (addr instanceof InetSocketAddress) {
			InetAddress groupAddr = ((InetSocketAddress) addr).getAddress();
			setOption(MCAST_LEAVE_GROUP, new MulticastGroupRequest(groupAddr,
					netInterface));
		}
	}
	...
	protected int peek(InetAddress sender) throws IOException {
		// We don't actually want the data: we just want the DatagramPacket's
		// filled-in address.
	...
	private void doRecv(DatagramPacket pack, boolean peek) throws IOException {
		netImpl.recv(fd, pack, pack.getData(), pack.getOffset(),
				pack.getLength(), peek, isNativeConnected);
	...
	public void send(DatagramPacket packet) throws IOException {
		...
		netImpl.send(fd, packet.getData(), packet.getOffset(),
				packet.getLength(), port, address);
	}

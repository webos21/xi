...
/**
 * This class extends the MessageDigestSpi class implementing all its abstract
 * methods; it overrides the "Object clone()" and "int engineGetDigestLength()"
 * methods. <BR>
 * The class implements the Cloneable interface.
 */

public class SHA1_MessageDigestImpl extends MessageDigestSpi implements
		Cloneable, SHA1_Data {
	...
	private byte[] oneByte; // one byte buffer needed to use in
							// engineUpdate(byte)
							// having buffer as private field is just
							// optimization

	private int messageLength; // total length of bytes supplied by user

	/**
	 * The constructor creates needed buffers and sets the engine at initial
	 * state
	 */
	...
	/**
	 * The method performs final actions and invokes the "computeHash(int[])"
	 * method. In case if there is no enough words in current frame after
	 * processing its data, extra frame is prepared and the "computeHash(int[])"
	 * method is invoked second time. <BR>
	 * 
	...
	private void processDigest(byte[] digest, int offset) {
		...
		long nBits = messageLength << 3; // length has to be calculated before
											// padding
		...
		lastWord = (buffer[BYTES_OFFSET] + 3) >> 2; // computing of # of full
													// words by shifting
													// # of bytes
		...
		//
		// - lastWord < 14 - two last, these are 14 & 15, words in 16 word frame
		// are free;
		// no extra frame needed
		// - lastWord = 14 - only one last, namely 15-th, word in frame doesn't
		// contain bytes;
	...
	public Object clone() throws CloneNotSupportedException {
		...
		SHA1_MessageDigestImpl cloneObj = (SHA1_MessageDigestImpl) super
				.clone();
	...
	/**
	 * Computes message digest value. Upon return, the value is stored in "buf"
	 * buffer beginning "offset" byte. <BR>
	 * 
	 * The method resets the engine. <BR>
	 * 
	 * The method overrides "engineDigest(byte[],int,int) in class
	 * MessageDigestSpi.
	 * 
	 * @param buf
	 *            byte array to store a message digest returned
	 * @param offset
	 *            a position in the array for first byte of the message digest
	 * @param len
	 *            number of bytes within buffer allotted for the message digest;
	 *            as this implementation doesn't provide partial digests, len
	 *            should be >= 20, DigestException is thrown otherwise
	 * @return the length of the message digest stored in the "buf" buffer; in
	 *         this implementation the length=20
	 * 
	...
	protected int engineDigest(byte[] buf, int offset, int len)
		...
		if (offset > buf.length || len > buf.length
	 ...
	 * 
	 * @param input
	 *            byte to add to current message
	 */
	 ...
	 * 
	 * The method overrides "engineUpdate(byte[],int,int)" in class
	 * MessageDigestSpi. <BR>
	 * 
	 ...
	 *            a byte array
	 * @param offset
	 *            a number of first byte in the "input" array to use for
	 *            updating
	 * @param len
	 *            a number of bytes to use
	 * 
	 * @throws NullPointerException
	 *             if null is passed to the "buf" argument
	 * 
	 * @throws IllegalArgumentException
	 *             if offset > buf.length or len > buf.length or (len + offset)
	 *             > buf.length
	 * @throws ArrayIndexOutOfBoundsException
	 *             offset < 0
	 */
	protected void engineUpdate(byte[] input, int offset, int len) {
		...
		if (offset > input.length || len > input.length
				|| (len + offset) > input.length) {
			throw new IllegalArgumentException();
		}

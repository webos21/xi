...
public class BerInputStream {
	...
	public BerInputStream(InputStream in, int initialSize) throws IOException {
		...
		if (length != INDEFINIT_LENGTH) {
			...
			throw new ASN1Exception(
					"Decoding indefinite length encoding is not supported");
		}
	}
	...
	public void readBitString() throws IOException {
		...
		if (tag == ASN1Constants.TAG_BITSTRING) {
			...
			throw new ASN1Exception(
					"Decoding constructed ASN.1 bitstring  type is not provided");
		} else {
	...
	public void readGeneralizedTime() throws IOException {
		...
		if (tag == ASN1Constants.TAG_GENERALIZEDTIME) {
			...
			throw new ASN1Exception(
					"Decoding constructed ASN.1 GeneralizedTime type is not supported");
		} else {
	...
	public void readUTCTime() throws IOException {
		...
		if (tag == ASN1Constants.TAG_UTCTIME) {
			...
			throw new ASN1Exception(
					"Decoding constructed ASN.1 UTCTime type is not supported");
		} else {
	...
	public void readOctetString() throws IOException {
		...
		if (tag == ASN1Constants.TAG_OCTETSTRING) {
			...
			throw new ASN1Exception(
					"Decoding constructed ASN.1 octet string type is not supported");
		} else {
	...
	private ASN1Exception expected(String what) throws ASN1Exception {
		throw new ASN1Exception("ASN.1 " + what + " identifier expected at ["
				+ tagOffset + "], got " + Integer.toHexString(tag));
	}
	...
	public void readString(ASN1StringType type) throws IOException {
		...
		if (tag == type.id) {
			...
			throw new ASN1Exception(
					"Decoding constructed ASN.1 string type is not provided");
		} else {
			...
			if (bytesRead != length) {
				...
				do {
					if (c < 1 || bytesRead > length) {
						throw new ASN1Exception(
								"Failed to read encoded content");
					}

	...
	public ASN1Choice(ASN1Type[] type) {
		...
		if (type.length == 0) {
			throw new IllegalArgumentException(
					"ASN.1 choice type MUST have at least one alternative: "
							+ getClass().getName());
		}
		...
		for (int index = 0; index < type.length; index++) {
			...
			if (t instanceof ASN1Any) {
				...
				throw new IllegalArgumentException(
						"ASN.1 choice type MUST have alternatives with distinct tags: "
	...
	public Object decode(BerInputStream in) throws IOException {
		...
		if (index < 0) {
			throw new ASN1Exception(
					"Failed to decode ASN.1 choice type.  No alternatives were found for "

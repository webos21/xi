...
/**
 * This class extends the SecureRandomSpi class implementing all its abstract
 * methods. <BR>
 * <BR>
 * To generate pseudo-random bits, the implementation uses technique described
 * in the "Random Number Generator (RNG) algorithms" section, Appendix A, JavaTM
 * Cryptography Architecture, API Specification&Reference <BR>
 * <BR>
...
public class SHA1PRNG_SecureRandomImpl extends SecureRandomSpi implements
	...
	// constants to use in expressions operating on bytes in int and long
	// variables:
	...
	// COUNTER_BASE - initial value to set to "counter" before computing
	// "nextBytes(..)";
	...
	// MAX_BYTES - maximum # of seed bytes processing which doesn't require
	// extra frame
	...
	// The "seed" array is used to compute both "current seed hash" and
	// "next bytes".
	//
	// As the "SHA1" algorithm computes a hash of entire seed by splitting it
	// into
	// a number of the 512-bit length frames (512 bits = 64 bytes = 16 words),
	// "current seed hash" is a hash (5 words, 20 bytes) for all previous full
	// frames;
	...
	// remaining bytes are copied into the 21-36 word frame of the "copies"
	// array;
	...
	/*
	 * The method invokes the SHA1Impl's "updateHash(..)" method to update
	 * current seed frame and to compute new intermediate hash value if the
	 * frame is full.
	 * 
	...
	private void updateSeed(byte[] bytes) {
		...
		// on return: "seed" contains new current bytes and possibly new current
		// hash
	...
	/**
	 * Changes current seed by supplementing a seed argument to the current
	 * seed, if this already set; the argument is used as first seed otherwise. <BR>
	 * 
	...
	protected void engineSetSeed(byte[] seed) {
		...
		if (state == NEXT_BYTES) { // first setSeed after NextBytes; restoring
									// hash
	 ...
	 * 
	 * @param numBytes
	 *            - number of bytes to return; should be >= 0.
	 ...
	 * that is, the current seed with a 64-bit counter appended is used to
	 * compute new bits. The counter is incremented by 1 for each 20-byte
	 * output. <BR>
	 * 
	...
	protected void engineNextBytes(byte[] bytes) {
		...
		final int extrabytes = 7;// # of bytes to add in order to computer # of
									// 8 byte words
		...
		if (state == UNDEFINED) {
			...
			// no seed supplied by user, hence it is generated thus randomizing
			// internal state
			...
			//
			// seed bytes < 48 - remaining bytes are enough for all, 8 counter
			// bytes,
			// 0x80, and 8 seedLength bytes; no extra frame required
			// 48 < seed bytes < 56 - remaining 9 bytes are for 0x80 and 8
			// counter bytes
			...
			bits = (seedLength << 3) + 64; // transforming # of bytes into # of
											// bits
		...
		for (;;) {
			...
			for (i = 0; i < EXTRAFRAME_OFFSET; i++) {
				...
				nextBytes[j] = (byte) (k >>> 24); // getting first byte from
													// left
				nextBytes[j + 1] = (byte) (k >>> 16); // getting second byte
														// from left
	...
	private void writeObject(ObjectOutputStream oos) throws IOException {
		...
		int nRemaining = (seed[BYTES_OFFSET] + 3) >> 2; // converting bytes in
														// words
		// result may be 0
		if (state != NEXT_BYTES) {

			// either the state is UNDEFINED or previous method was
			// "setSeed(..)"
			// so in "seed[]" to serialize are remaining bytes
			// (seed[0-nRemaining]) and
			...
			// so, data to serialize are all the above (two first are in
			// "copies" array)

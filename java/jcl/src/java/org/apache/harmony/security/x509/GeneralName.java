...
public class GeneralName {
	...
	public GeneralName(int tag, String name) throws IOException {
		...
		case X400_ADDR:
		case EDIP_NAME:
			throw new IOException("Unknown string representation for type ["
					+ tag + "]");
		case DNS_NAME:
	...
	public boolean isAcceptable(GeneralName gname) {
		...
		case UR_ID:
			...
			String host = (end == -1) ? uri.substring(begin) : uri.substring(
					begin, end);
			uri = (String) gname.getName();
			begin = uri.indexOf("://") + 3;
			end = uri.indexOf('/', begin);
			String _host = (end == -1) ? uri.substring(begin) : uri.substring(
					begin, end);
			if (host.startsWith(".")) {
	...
	public String toString() {
		...
		switch (tag) {
		case OTHER_NAME:
			result = "otherName[0]: " + getBytesAsString(getEncoded());
			break;
		case RFC822_NAME:
		...
		case UR_ID:
			...
			break;
		case REG_ID:
			result = "registeredID[8]: "
					+ ObjectIdentifier.toString((int[]) name);
			break;
		case X400_ADDR:
			result = "x400Address[3]: " + getBytesAsString(getEncoded());
			break;
		case DIR_NAME:
			...
			break;
		case EDIP_NAME:
			result = "ediPartyName[5]: " + getBytesAsString(getEncoded());
			break;
		case IP_ADDR:
	...
	public static void checkURI(String uri) throws IOException {
		try {
			...
			if (ur.getScheme() == null
					|| ur.getRawSchemeSpecificPart().isEmpty()) {
				throw new IOException(
						"No scheme or scheme-specific-part in uniformResourceIdentifier: "
								+ uri);
			}
			if (!ur.isAbsolute()) {
				throw new IOException("Relative uniformResourceIdentifier: "
						+ uri);
			}
		} catch (URISyntaxException e) {
			throw (IOException) new IOException(
					"Bad representation of uniformResourceIdentifier: " + uri)
					.initCause(e);
	...
	public static int[] oidStrToInts(String oid) throws IOException {
		...
		if (number < 2) {
			throw new IOException(
					"OID should consist of no less than 2 components: " + oid);
		}
		...
		if (isIPv4) {
			...
			while (i < ip_bytes.length) {
				...
				while ((i < ip_bytes.length) && (ip_bytes[i] >= '0')
					...
					if (digits > 3) {
						throw new IOException(
								"Component of IPv4 address should consist of no more than 3 decimal digits: "
										+ ip);
					}
				...
				if (ip_bytes[i] == '/') {
					...
					if (component != 4) {
						throw new IOException(
								"IPv4 address should consist of 4 decimal numbers: "
										+ ip);
					}
				...
				if ((bytik >= '0') && (bytik <= '9')) {
					...
					if (bytik == '/') {
						...
						if (component != 16) {
							// check the number of read components
							throw new IOException(
									"IPv6 address should consist of 8 hexadecimal numbers: "
											+ ip);
						}
						reading_second_bound = true;
					}
	...
	public static final ASN1Choice ASN1 = new ASN1Choice(new ASN1Type[] {
		...
		public Object getDecodedObject(BerInputStream in) throws IOException {
			...
			case REG_ID: // registeredID
				...
				break;
			default:
				throw new IOException("GeneralName: unknown tag: "
						+ in.choiceIndex);
			}

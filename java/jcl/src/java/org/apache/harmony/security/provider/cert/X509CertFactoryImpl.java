...
public class X509CertFactoryImpl extends CertificateFactorySpi {
	...
	public Collection<? extends Certificate> engineGenerateCertificates(
		...
		try {
			...
			while ((ch = inStream.read()) != -1) {
				...
				BerInputStream in = (encoding == null) ? new BerInputStream(
	...
	private static final byte[] PEM_BEGIN = "-----BEGIN"
	 ...
	 */
	private static final byte[] CERT_BOUND_SUFFIX = " CERTIFICATE-----"
			.getBytes(Charsets.UTF_8);
	...
	private byte[] decodePEM(InputStream inStream, byte[] boundary_suffix)
		...
		if (boundary_suffix == null) {
			...
			while ((ch = inStream.read()) != '\n') {
				if (ch == -1) {
					throw new IOException(
							"Incorrect PEM encoding: EOF before content");
				}
			}
		} else {
			...
			if (ch != '\n') {
				throw new IOException(
						"Incorrect PEM encoding: newline expected after "
		...
		while ((ch = inStream.read()) != '-') {
			if (ch == -1) {
				throw new IOException(
						"Incorrect Base64 encoding: EOF without closing delimiter");
			}
			buff[index++] = (byte) ch;
			if (index == size) {
				// enlarge the buffer
		...
		if (buff[index - 1] != '\n') {
			throw new IOException(
					"Incorrect Base64 encoding: newline expected before "
	...
	private IOException badEnd(byte[] boundary_suffix) throws IOException {
		...
		throw new IOException("Incorrect PEM encoding: '-----END" + s
				+ "' is expected as closing delimiter boundary.");
	}
	...
	private static Certificate getCertificate(byte[] encoding)
		...
		if (encoding.length < CERT_CACHE_SEED_LENGTH) {
			throw new CertificateException(
					"encoding.length < CERT_CACHE_SEED_LENGTH");
		}
	...
	private static Certificate getCertificate(InputStream inStream)
		...
		synchronized (CERT_CACHE) {
			...
			if (buff == null) {
				throw new CertificateException(
						"InputStream doesn't contain enough data");
			}

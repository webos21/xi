 ...
 * The methods support the algorithm described in "SECURE HASH STANDARD", FIPS
 * PUB 180-2, <BR>
 ...
 * <BR>
 * The "updateHash(..)" method appends new bytes to existing ones within limit
 * of a frame of 64 bytes (16 words). Once a length of accumulated bytes reaches
 * the limit the "computeHash(int[])" method is invoked on the frame to compute
 * updated hash, and the number of bytes in the frame is set to 0. Thus, after
 * appending all bytes, the frame contain only those bytes that were not used in
 * computing final hash value yet. <BR>
 * <BR>
 * The "computeHash(..)" method generates a 160 bit hash value using a 512 bit
 * message stored in first 16 words of int[] array argument and current hash
 * value stored in five words, beginning HASH_OFFSET, of the array argument.
...
public class SHA1Impl implements SHA1_Data {
	...
	/**
	 * The method generates a 160 bit hash value using a 512 bit message stored
	 * in first 16 words of int[] array argument and current hash value stored
	 * in five words, beginning OFFSET+1, of the array argument. Computation is
	 * done according to SHA-1 algorithm.
	 * 
	 * The resulting hash value replaces the previous hash value in the array;
	 * original bits of the message are not preserved.
	 * 
	 * No checks on argument supplied, that is, a calling method is responsible
	 * for such checks. In case of incorrect array passed to the method either
	 * NPE or IndexOutOfBoundException gets thrown by JVM.
	 * 
	...
	/**
	 * The method appends new bytes to existing ones within limit of a frame of
	 * 64 bytes (16 words).
	 * 
	 * Once a length of accumulated bytes reaches the limit the
	 * "computeHash(int[])" method is invoked on the array to compute updated
	 * hash, and the number of bytes in the frame is set to 0. Thus, after
	 * appending all bytes, the array contain only those bytes that were not
	 * used in computing final hash value yet.
	 * 
	 * No checks on arguments passed to the method, that is, a calling method is
	 ...
	 * @params to - a number of the last byte in the input array to use, that
	 *         is, for first byte "to"==0, for last byte "to"==input.length-1
	 */
	static void updateHash(int[] intArray, byte[] byteInput, int fromByte,
			int toByte) {
		...
		maxWord = (toByte - i + 1) >> 2; // # of remaining full words, may be
											// "0"

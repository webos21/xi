...
/**
 * The class provides DSAPublicKey functionality by extending a class
 * implementing PublicKey and implementing methods defined in both interfaces,
 * DSAKey and DSAPublicKey
 */
public class DSAPublicKeyImpl extends PublicKeyImpl implements DSAPublicKey {
	...
	public DSAPublicKeyImpl(DSAPublicKeySpec keySpec) {
		...
		ThreeIntegerSequence threeInts = new ThreeIntegerSequence(
				p.toByteArray(), q.toByteArray(), g.toByteArray());

		AlgorithmIdentifier ai = new AlgorithmIdentifier(
				AlgNameMapper.map2OID("DSA"), threeInts.getEncoded());
	 ...
	 * 
	 * @param keySpec
	 *            - a X509EncodedKeySpec object
	 * 
	 * @throws InvalidKeySpecException
	 *             - if key data cannot be obtain from encoded format
	 */
	public DSAPublicKeyImpl(X509EncodedKeySpec keySpec)
		...
		try {
			...
			throw new InvalidKeySpecException(
					"Failed to decode keySpec encoding: " + e);
		}
		...
		try {
			...
			throw new InvalidKeySpecException("Failed to decode parameters: "
					+ e);
		}
	...
	/**
	 * @return a value of a public key (y).
	 */
	...
	/**
	 * @return DSA key parameters (p, q, g).
	 */

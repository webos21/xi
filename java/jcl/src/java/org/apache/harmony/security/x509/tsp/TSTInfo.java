...
/**
 * As defined in Time-Stamp Protocol (TSP) (http://www.ietf.org/rfc/rfc3161.txt)
 * 
 * TSTInfo ::= SEQUENCE { version INTEGER { v1(1) }, policy TSAPolicyId,
 * messageImprint MessageImprint, -- MUST have the same value as the similar
 * field in -- TimeStampReq serialNumber INTEGER, -- Time-Stamping users MUST be
 * ready to accommodate integers -- up to 160 bits. genTime GeneralizedTime,
 * accuracy Accuracy OPTIONAL, ordering BOOLEAN DEFAULT FALSE, nonce INTEGER
 * OPTIONAL, -- MUST be present if the similar field was present -- in
 * TimeStampReq. In that case it MUST have the same value. tsa [0] GeneralName
 * OPTIONAL, extensions [1] IMPLICIT Extensions OPTIONAL }
 * 
...
public class TSTInfo {
	 ...
	 */
	public static final ASN1Sequence ACCURACY = new ASN1Sequence(
			new ASN1Type[] { ASN1Integer.getInstance(),
				...
				if (values[i] != null) {
					...
					if (i > 0 && (accuracy[i] < 0 || accuracy[i] > 999)) {
						throw new RuntimeException(
								"Time-stamp accuracy value is incorrect: "
										+ accuracy[i]);
					}
				}
			}
	...
	public static final ASN1Sequence ASN1 = new ASN1Sequence(new ASN1Type[] {
		...
		protected Object getDecodedObject(BerInputStream in) {
			...
			return new TSTInfo(ASN1Integer.toIntValue(values[0]),
					ObjectIdentifier.toString((int[]) values[1]),
					(MessageImprint) values[2], new BigInteger(
							(byte[]) values[3]), (Date) values[4],
					(int[]) values[5], (Boolean) values[6], nonce,
					(GeneralName) values[8], (Extensions) values[9]);
		}

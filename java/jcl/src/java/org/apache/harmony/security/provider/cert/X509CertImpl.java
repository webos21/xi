...
public class X509CertImpl extends X509Certificate {
	...
	public void verify(PublicKey key) throws CertificateException,
		...
		/*
		 * by jshwang // BEGIN android-added if
		 * (getSigAlgName().endsWith("withRSA")) { fastVerify(key); return; } //
		 * END android-added
		 */
	 ...
	 */
	/*
	 * by jshwang private void fastVerify(PublicKey key) throws
	 * SignatureException, InvalidKeyException, NoSuchAlgorithmException { if
	 * (!(key instanceof RSAPublicKey)) { throw new
	 * InvalidKeyException("key is not an instance of RSAPublicKey"); }
	 * RSAPublicKey rsaKey = (RSAPublicKey) key;
	 * 
	 * String algorithm = getSigAlgName();
	 * 
	 * // We don't support MD2 anymore. This needs to also check for aliases //
	 * and OIDs. if ("MD2withRSA".equalsIgnoreCase(algorithm) ||
	 * "MD2withRSAEncryption".equalsIgnoreCase(algorithm) ||
	 * "1.2.840.113549.1.1.2".equalsIgnoreCase(algorithm) ||
	 * "MD2/RSA".equalsIgnoreCase(algorithm)) { throw new
	 * NoSuchAlgorithmException(algorithm); }
	 * 
	 * int i = algorithm.indexOf("with"); algorithm = algorithm.substring(i + 4)
	 * + "-" + algorithm.substring(0, i);
	 * 
	 * if (tbsCertificate == null) { tbsCertificate = tbsCert.getEncoded(); }
	 * 
	 * byte[] sig = certificate.getSignatureValue(); if
	 * (!NativeCrypto.verifySignature(tbsCertificate, sig, algorithm, rsaKey)) {
	 * throw new SignatureException("Signature was not verified"); } } // END
	 * android-added
	 */

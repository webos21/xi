	...
	public static Certificate[] verifySignature(InputStream signature,
		...
		if (certs[issuerSertIndex].hasUnsupportedCriticalExtension()) {
			throw new SecurityException(
					"Can not recognize a critical extension");
		}
		...
		if (da != null && dea != null) {
			...
			try {
				/*
				 * by jshwang // BEGIN android-added sig =
				 * OpenSSLSignature.getInstance(alg); // END android-removed
				 */
		...
		if (atr == null) {
			...
			// MessageDigest md = MessageDigest.getInstance(sigInfo
			// .getDigestAlgorithm());

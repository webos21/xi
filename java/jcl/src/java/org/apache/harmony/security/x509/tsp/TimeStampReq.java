 ...
 * MessageImprint, --a hash algorithm OID and the hash value of the data to be
 * --time-stamped reqPolicy TSAPolicyId OPTIONAL, nonce INTEGER OPTIONAL,
...
public class TimeStampReq {
	...
	public static final ASN1Sequence ASN1 = new ASN1Sequence(new ASN1Type[] {
		...
		protected Object getDecodedObject(BerInputStream in) {
			...
			if (values[5] == null) {
				return new TimeStampReq(ASN1Integer.toIntValue(values[0]),
						(MessageImprint) values[1], objID, nonce,
						(Boolean) values[4], null, in.getEncoded());
			} else {
				return new TimeStampReq(ASN1Integer.toIntValue(values[0]),
						(MessageImprint) values[1], objID, nonce,
						(Boolean) values[4], (Extensions) values[5],
						in.getEncoded());
			}
		}

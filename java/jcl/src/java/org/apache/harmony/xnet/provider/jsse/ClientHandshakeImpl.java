...
/**
 * Client side handshake protocol implementation. Handshake protocol operates on
 * top of the Record Protocol. It is responsible for session negotiating.
 * 
 * The implementation processes inbound server handshake messages, creates and
 * sends respond messages. Outbound messages are supplied to Record Protocol.
 * Detected errors are reported to the Alert protocol.
 * 
...
public class ClientHandshakeImpl extends HandshakeProtocol {
	...
	public void unwrap(byte[] bytes) {
		if (this.delegatedTaskErr != null) {
			...
			this.fatalAlert(AlertProtocol.HANDSHAKE_FAILURE,
					"Error in delegated task", e);
		}
		int handshakeType;
				...
				case 2: // SERVER_HELLO
					...
					if (isResuming) {
						if (serverHello.session_id.length == 0) {
							// server is not willing to establish the new
							// connection
	...
	/**
	 * Processes SSLv2 Hello message. SSLv2 client hello message message is an
	 * unexpected message for client side of handshake protocol. @ see TLS 1.0
	 * spec., E.1. Version 2 client hello
	 * 
	 * @param bytes
	 */
	@Override
	...
	/**
	 * Processes ServerHelloDone: makes verification of the server messages;
	 * sends client messages, computers masterSecret, sends ChangeCipherSpec
	 */
	void processServerHelloDone() {
		...
		if (certificateRequest != null) {
			...
			String clientAlias = ((X509ExtendedKeyManager) parameters
					.getKeyManager()).chooseClientAlias(
					certificateRequest.getTypesAsString(),
		...
		if (session.cipherSuite.keyExchange == CipherSuite.KeyExchange_RSA
			...
			try {
				...
				if (serverKeyExchange != null) {
					c.init(Cipher.ENCRYPT_MODE,
							serverKeyExchange.getRSAPublicKey());
				} else {
			...
			try {
				...
				if (clientCert != null
						&& serverCert != null
						&& (session.cipherSuite.keyExchange == CipherSuite.KeyExchange_DHE_RSA || session.cipherSuite.keyExchange == CipherSuite.KeyExchange_DHE_DSS)) {
					...
					if (client_pk instanceof DHKey
						...
						if (((DHKey) client_pk).getParams().getG()
								.equals(((DHKey) server_pk).getParams().getG())
								&& ((DHKey) client_pk)
										.getParams()
										.getP()
										.equals(((DHKey) server_pk).getParams()
												.getG())) {
	...
	private void verifyServerCert() {
		...
		try {
			...
			fatalAlert(AlertProtocol.BAD_CERTIFICATE,
					"Not trusted server certificate", e);
			return;
		}

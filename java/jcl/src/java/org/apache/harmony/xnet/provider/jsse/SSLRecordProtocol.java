...
/**
 * This class performs functionality dedicated to SSL record layer. It unpacks
 * and routes income data to the appropriate client protocol (handshake, alert,
 * application data protocols) and packages outcome data into SSL/TLS records.
 * Initially created object has null connection state and does not perform any
 * cryptography computations over the income/outcome data. After handshake
 * protocol agreed upon security parameters they are placed into SSLSessionImpl
 * object and available for record protocol as pending session. The order of
 * setting up of the pending session as an active session differs for client and
 * server modes. So for client mode the parameters are provided by handshake
 * protocol during retrieving of change_cipher_spec message to be sent (by
 * calling of getChangeCipherSpecMesage method). For server side mode record
 * protocol retrieves the parameters from handshake protocol after receiving of
 * client's change_cipher_spec message. After the pending session has been
 * setted up as a curent session, new connectin state object is created and used
 * for encryption/decryption of the messages. Among with base functionality this
 * class provides the information about constrains on the data length, and
 * information about correspondance of plain and encrypted data lengths. For
 * more information on TLS v1 see http://www.ietf.org/rfc/rfc2246.txt, on SSL v3
 * see http://wp.netscape.com/eng/ssl3, on SSL v2 see
...
public class SSLRecordProtocol {
	...
	private ConnectionState activeReadState, activeWriteState,
	...
	/**
	 * Creates an instance of record protocol and tunes up the client protocols
	 * to use ut.
	 * 
	...
	protected SSLRecordProtocol(HandshakeProtocol handshakeProtocol,
			AlertProtocol alertProtocol, SSLInputStream in, Appendable appData) {
	...
	/**
	 * Returns the session obtained during the handshake negotiation. If the
	 * handshake process was not compleated, method returns null.
	 * 
	...
	protected int getMinRecordSize() {
		return (activeReadState == null) ? 6 // type + version + length + 1 byte
												// of data
				: 5 + activeReadState.getMinFragmentSize();
	}

	/**
	 * Returns the record length for the specified incoming data length. If
	 * actual resulting record length is greater than MAX_CIPHERED_DATA_LENGTH,
	 * MAX_CIPHERED_DATA_LENGTH is returned.
	 */
	protected int getRecordSize(int data_size) {
		if (activeWriteState == null) {
			...
			return (res > MAX_CIPHERED_DATA_LENGTH) ? MAX_CIPHERED_DATA_LENGTH // so
																				// the
																				// source
																				// data
																				// should
	 ...
	 * specified length. If the provided record_size is greater or equal to
	 * MAX_CIPHERED_DATA_LENGTH the returned value will be MAX_DATA_LENGTH
	 * counted as for data with MAX_CIPHERED_DATA_LENGTH length.
	 */
	 ...
	 * @return ssl packet created over the current connection state
	 */
	protected byte[] wrap(byte content_type, byte[] fragment, int offset,
			int len) {
		if (logger != null) {
		...
		if (activeWriteState != null) {
			ciphered_fragment = activeWriteState.encrypt(content_type,
					fragment, offset, len);
	...
	private void setSession(SSLSessionImpl session) {
		if (!sessionWasChanged) {
			...
			pendingConnectionState = ((version == null) || (version[1] == 1)) ? (ConnectionState) new ConnectionStateTLS(
					getSession()) : (ConnectionState) new ConnectionStateSSLv3(
					getSession());
	...
	/**
	 * Returns the change cipher spec message to be sent to another peer. The
	 * pending connection state will be built on the base of provided session
	 * object The calling of this method triggers pending write connection state
	 * to be active.
	 * 
	...
	protected byte[] getChangeCipherSpecMesage(SSLSessionImpl session) {
		...
		if (activeWriteState == null) {
			change_cipher_spec_message = new byte[] {
					ContentType.CHANGE_CIPHER_SPEC, version[0], version[1], 0,
					1, 1 };
		} else {
			change_cipher_spec_message = packetize(
					ContentType.CHANGE_CIPHER_SPEC, version,
	...
	/**
	 * Retrieves the fragment field of TLSCiphertext, and than depending on the
	 * established Connection State decrypts and decompresses it. The following
	 * structure is expected on the input at the moment of the call:
	 * 
	 * struct { ContentType type; ProtocolVersion version; uint16 length; select
	 * (CipherSpec.cipher_type) { case stream: GenericStreamCipher; case block:
	 * GenericBlockCipher; } fragment; } TLSCiphertext;
	 * 
	 * (as specified by RFC 2246, TLS v1 Protocol specification)
	 * 
	 * In addition this method can recognize SSLv2 hello message which are often
	 * used to establish the SSL/TLS session.
	 * 
	 * @throws IOException
	 *             if some io errors have been occured
	 * @throws EndOfSourceException
	 *             if underlying input stream has ran out of data.
	 * @throws EndOfBufferException
	 *             if there was not enought data to build complete ssl packet.
	...
	protected int unwrap() throws IOException {
		...
		if (length > MAX_CIPHERED_DATA_LENGTH) {
			throw new AlertException(AlertProtocol.RECORD_OVERFLOW,
					new SSLProtocolException("Received message is too big."));
		}
		byte[] fragment = in.read(length);
		if (logger != null) {
	...
	protected void alert(byte level, byte description) {
		if (logger != null) {
			logger.println("SSLRecordProtocol.allert: " + level + " "
					+ description);
		}
	...
	/**
	 * Sets up the SSL version used in this connection. This method is calling
	 * from the hanshake protocol after it becomes known witch protocol version
	 * will be used.
	 * 
	 * @param ver
	 *            : byte[]
	...
	/**
	 * Shutdown the protocol. It will be impossible to use the instance after

...
/**
 * This class provides functionality for computation of PRF values for TLS
 * (http://www.ietf.org/rfc/rfc2246.txt) and SSL v3
 * (http://wp.netscape.com/eng/ssl3) protocols.
 */
public class PRF {
	 ...
	 * 
	 * @param out
	 *            : the buffer to fill up with the value of the function.
	 * @param secret
	 *            : the buffer containing the secret value to generate prf.
	 * @param seed
	 *            : the seed to be used.
	 */
	static synchronized void computePRF_SSLv3(byte[] out, byte[] secret,
			byte[] seed) {
		if (sha == null) {
			init();
		}
	...
	synchronized static void computePRF(byte[] out, byte[] secret,
		...
		if ((secret == null) || (secret.length == 0)) {
			...
			keySha1 = new SecretKeySpec(secret, length, length + offset,
					"HmacSHA1");
		}
		...
		while (pos < out.length) {
			...
			if (pos + md5_mac_length < out.length) {
				...
				System.arraycopy(md5_mac.doFinal(), 0, out, pos, out.length
						- pos);
				break;
			}
			// make A(i)

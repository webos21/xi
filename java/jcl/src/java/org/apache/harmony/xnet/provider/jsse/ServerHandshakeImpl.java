...
/**
 * Server side handshake protocol implementation. Handshake protocol operates on
 * top of the Record Protocol. It responsible for negotiating a session.
 * 
 * The implementation processes inbound client handshake messages, creates and
...
public class ServerHandshakeImpl extends HandshakeProtocol {
					...
					if (session.cipherSuite.keyExchange == CipherSuite.KeyExchange_RSA
						...
						try {
							...
							if (preMasterSecret.length != 48
								...
								// prevent an attack (see TLS 1.0 spec.,
								// 7.4.7.1.)
	...
	/**
	 * Processes SSLv2 Hello message @ see TLS 1.0 spec., E.1. Version 2 client
	 * hello
	 * 
	 * @param bytes
	 */
	...
	/**
	 * 
	 * Processes Client Hello message. Server responds to client hello message
	 * with server hello and (if necessary) server certificate, server key
	 * exchange, certificate request, and server hello done messages.
	 */
	void processClientHello() {
		...
		if (isResuming) {
			...
			// clientHello.cipher_suites must include at least cipher_suite from
			// the session
			checkCipherSuite: {
			...
			if (cipher_suite == null) {
				fatalAlert(AlertProtocol.HANDSHAKE_FAILURE,
						"HANDSHAKE FAILURE. NO COMMON SUITE");
			}
			if (!parameters.getEnableSessionCreation()) {
				fatalAlert(AlertProtocol.HANDSHAKE_FAILURE,
						"SSL Session may not be created");
			}
			session = new SSLSessionImpl(cipher_suite,
					parameters.getSecureRandom());
		}

		recordProtocol.setVersion(clientHello.client_version);
		session.protocol = ProtocolVersion
				.getByVersion(clientHello.client_version);
		session.clientRandom = clientHello.random;

		// create server hello message
		serverHello = new ServerHello(parameters.getSecureRandom(),
				clientHello.client_version, session.getId(), cipher_suite,
				(byte) 0); // CompressionMethod.null
		...
		if (kpg != null) {
			...
			try {
				...
				if (!cipher_suite.isAnonymous()) { // calculate signed_params
					...
					privKey = kp.getPrivate(); // use emphemeral key for key
												// exchange
				}
			} catch (Exception e) {
				fatalAlert(AlertProtocol.INTERNAL_ERROR, "INTERNAL ERROR", e);
			...
			if (cipher_suite.keyExchange == CipherSuite.KeyExchange_RSA_EXPORT) {
				...
				serverKeyExchange = new ServerKeyExchange(p, g,
						dhkeySpec.getY(), hash);
			}
	...
	protected void makeFinished() {
		byte[] verify_data;
		boolean isTLS = (serverHello.server_version[1] == 1); // TLS 1.0
																// protocol
		if (isTLS) {
	...
	private SSLSessionImpl findSessionToResume(byte[] session_id) {
		return (SSLSessionImpl) parameters.getServerSessionContext()

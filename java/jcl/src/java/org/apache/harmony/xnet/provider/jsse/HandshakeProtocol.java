...
public abstract class HandshakeProtocol {
	...
	/**
	 * Handshake status NEED_TASK - HandshakeProtocol needs the results of
	 * delegated task
	 */
	...
	private byte[] master_secret_bytes = { 109, 97, 115, 116, 101, 114, 32,
	...
	/**
	 * Stops the current session renegotiation process. Such functionality is
	 * needed when it is session renegotiation process and no_renegotiation
	 * alert message is received from another peer.
	 * 
	 * @param session
	 */
	protected void stop() {
	...
	public SSLEngineResult.HandshakeStatus getStatus() {
		if (io_stream.hasData() || needSendCCSpec || needSendHelloRequest
	...
	/**
	 * Computers reference TLS verify_data that is used to verify finished
	 * message
	 * 
	...
	/**
	 * Computer reference SSLv3 verify_data that is used to verify finished
	 * message
	 * 
	...
	protected void computerVerifyDataSSLv3(byte[] sender, byte[] buf) {
		...
		try {
			...
			fatalAlert(AlertProtocol.INTERNAL_ERROR,
					"Could not initialize the Digest Algorithms.", e);
			return;
		}
		try {

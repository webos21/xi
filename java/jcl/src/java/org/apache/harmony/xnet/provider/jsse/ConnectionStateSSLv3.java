 ...
 * (http://wp.netscape.com/eng/ssl3) Record Protocol and provides relating
...
public class ConnectionStateSSLv3 extends ConnectionState {
	...
	/**
	 * Creates the instance of SSL v3 Connection State. All of the security
	 ...
	 *            calculation for the state is done according to the SSL v3
	 *            Protocol specification.
	 *            (http://www.mozilla.org/projects/security
	 *            /pki/nss/ssl/draft302.txt)
	 */
	protected ConnectionStateSSLv3(SSLSessionImpl session) {
		try {
			...
			if (is_exportabe) {
				...
				clientIV = new IvParameterSpec(key_block, 2 * hash_size + 2
						* key_size, iv_size);
				serverIV = new IvParameterSpec(key_block, 2 * hash_size + 2
						* key_size + iv_size, iv_size);
			}
			...
			if (is_client) { // client side
				encCipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(
						client_key, 0, key_size, algName), clientIV);
				decCipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(
						server_key, 0, key_size, algName), serverIV);
	...
	protected byte[] encrypt(byte type, byte[] fragment, int offset, int len) {
		try {
			...
			int padding_length = is_block_cipher ? padding_length = ((8 - (++content_mac_length & 0x07)) & 0x07)
					: 0;
			...
			if (logger != null) {
				logger.println("SSLRecordProtocol.encrypt: "
						+ (is_block_cipher ? "GenericBlockCipher with padding["
								+ padding_length + "]:"
								: "GenericStreamCipher:"));
				logger.print(res);
			}

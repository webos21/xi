 ...
 * 
 * select (SignatureAlgorithm) { case anonymous: struct { }; case rsa:
 * digitally-signed struct { opaque md5_hash[16]; opaque sha_hash[20]; }; case
 * dsa: digitally-signed struct { opaque sha_hash[20]; }; } Signature;
 * 
	...
	public boolean verifySignature(byte[] data) {
		...
		if (cipher != null) {
			...
			if (md5_hash != null && sha_hash != null) {
				...
				System.arraycopy(sha_hash, 0, md5_sha, md5_hash.length,
						sha_hash.length);

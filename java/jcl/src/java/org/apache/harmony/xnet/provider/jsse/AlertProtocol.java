...
/**
 * This class encapsulates the functionality of Alert Protocol. Constant values
 * are taken according to the TLS v1 specification
...
public class AlertProtocol {
	...
	/**
	 * Creates the instance of AlertProtocol. Note that class is not ready to
	 * work without providing of record protocol
	 * 
	...
	/**
	 * Reports an alert to be sent/received by transport. This method is usually
	 * called during processing of the income TSL record: if it contains alert
	 * message from another peer, or if warning alert occured during the
	 * processing of the message and this warning should be sent to another
	 * peer.
	 * 
	 * @param level
	 *            : alert level code
	...
	/**
	 * Resets the protocol to be in "no alert" state. This method shoud be
	 * called after processing of the reported alert.
	 */
	...
	/**
	 * Returns the string representation of occured alert. If no alert has
	...
	/**
	 * Returns the record with reported alert message. The returned array of
	 * bytes is ready to be sent to another peer. Note, that this method does
	 * not automatically set the state of alert protocol in "no alert" state, so
	 * after wrapping the method setProcessed should be called.
	 */
	protected byte[] wrap() {

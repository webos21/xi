 ...
 * (http://www.ietf.org/rfc/rfc2246.txt) Record Protocol and provides relating
 * encryption/decryption functionality. The work functionality is based on the
 * security parameters negotiated during the handshake.
 */
public class ConnectionStateTLS extends ConnectionState {

	// Pre-calculated prf label values:
	// "key expansion".getBytes()
	private static byte[] KEY_EXPANSION_LABEL = { (byte) 0x6B, (byte) 0x65,
			(byte) 0x79, (byte) 0x20, (byte) 0x65, (byte) 0x78, (byte) 0x70,
			(byte) 0x61, (byte) 0x6E, (byte) 0x73, (byte) 0x69, (byte) 0x6F,
			(byte) 0x6E };

	// "client write key".getBytes()
	private static byte[] CLIENT_WRITE_KEY_LABEL = { (byte) 0x63, (byte) 0x6C,
			(byte) 0x69, (byte) 0x65, (byte) 0x6E, (byte) 0x74, (byte) 0x20,
			(byte) 0x77, (byte) 0x72, (byte) 0x69, (byte) 0x74, (byte) 0x65,
			(byte) 0x20, (byte) 0x6B, (byte) 0x65, (byte) 0x79 };

	// "server write key".getBytes()
	private static byte[] SERVER_WRITE_KEY_LABEL = { (byte) 0x73, (byte) 0x65,
			(byte) 0x72, (byte) 0x76, (byte) 0x65, (byte) 0x72, (byte) 0x20,
			(byte) 0x77, (byte) 0x72, (byte) 0x69, (byte) 0x74, (byte) 0x65,
			(byte) 0x20, (byte) 0x6B, (byte) 0x65, (byte) 0x79 };

	// "IV block".getBytes()
	private static byte[] IV_BLOCK_LABEL = { (byte) 0x49, (byte) 0x56,
			(byte) 0x20, (byte) 0x62, (byte) 0x6C, (byte) 0x6F, (byte) 0x63,
			(byte) 0x6B };
	...
	/**
	 * Creates the instance of TLS v1 Connection State. All of the security
	 * parameters are provided by session object.
	 * 
	 * @param session
	 *            : the session object which encapsulates all of the security
	 *            parameters established by handshake protocol. The key
	 *            calculation for the state is done according to the TLS v 1.0
	...
	protected ConnectionStateTLS(SSLSessionImpl session) {
		try {
			...
			int key_size = (is_exportabe) ? cipherSuite.keyMaterial
			...
			byte[] key_block = new byte[2 * hash_size + 2 * key_size + 2
					* iv_size];
			...
			System.arraycopy(key_block, hash_size, server_mac_secret, 0,
					hash_size);
			System.arraycopy(key_block, 2 * hash_size, client_key, 0, key_size);
			System.arraycopy(key_block, 2 * hash_size + key_size, server_key,
					0, key_size);
			...
			if (is_exportabe) {
				...
				System.arraycopy(serverRandom, 0, seed, clientRandom.length,
						serverRandom.length);
				...
				serverIV = new IvParameterSpec(key_block, 2
						* (hash_size + key_size) + iv_size, iv_size);
			}
			...
			if (is_client) { // client side
				encCipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(
						client_key, algName), clientIV);
				decCipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(
						server_key, algName), serverIV);
	...
	/**
	 * Creates the GenericStreamCipher or GenericBlockCipher data structure for
	 * specified data of specified type.
	 * 
	...
	protected byte[] encrypt(byte type, byte[] fragment, int offset, int len) {
		try {
			...
			int padding_length = is_block_cipher ? ((8 - (++content_mac_length & 0x07)) & 0x07)
					: 0;
			...
			if (is_block_cipher) {
				// do padding:
				Arrays.fill(res, content_mac_length - 1, res.length,
						(byte) (padding_length));
			}
			if (logger != null) {
				logger.println("SSLRecordProtocol.do_encryption: Generic"
						+ (is_block_cipher ? "BlockCipher with padding["
								+ padding_length + "]:" : "StreamCipher:"));
				logger.print(res);
			}
	...
	/**
	 * Retrieves the fragment of the Plaintext structure of the specified type
	 * from the provided data representing the Generic[Stream|Block]Cipher
	 * structure.
	 * 
	 * @throws AlertException
	 *             if alert was occurred.
		...
		if (is_block_cipher) {
			...
			for (int i = 0; i < padding_length; i++) {
				if (data[data.length - 2 - i] != padding_length) {
					throw new AlertException(AlertProtocol.DECRYPTION_FAILED,
							new SSLProtocolException(

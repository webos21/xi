...
class SocketChannelImpl extends SocketChannel implements FileDescriptorHandler {
	...
	synchronized public Socket socket() {
		if (socket == null) {
			try {
				...
				socket = new SocketAdapter(new PlainSocketImpl(fd, localPort,
						addr, port), this);
	...
	public boolean connect(SocketAddress socketAddress) throws IOException {
		...
		try {
			if (isBlocking()) {
				...
				finished = networkSystem.connectNonBlocking(fd, normalAddr,
						port);
		...
		if (socket != null) {
			socket.socketImpl().initRemoteAddressAndPort(
	...
	public boolean finishConnect() throws IOException {
		...
		try {
			...
			final int POLL = 0;
			finished = networkSystem.isConnected(fd,
					isBlocking() ? WAIT_FOREVER : POLL);
			isBound = finished;
			initLocalAddressAndPort();

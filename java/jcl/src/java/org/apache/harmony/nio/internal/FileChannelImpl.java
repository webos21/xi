...
public abstract class FileChannelImpl extends FileChannel {
	...
	protected FileLock basicLock(long position, long size, boolean shared,
		...
		if (position < 0 || size < 0) {
			throw new IllegalArgumentException(
					"Lock position and size must be non-negative");
		}
	...
	protected final MappedByteBuffer mapImpl(MapMode mapMode, long position,
	...
	public FileChannel position(long newPosition) throws IOException {
		...
		if (newPosition < 0) {
			throw new IllegalArgumentException(
					"New position must be non-negative");
		}
	...
	public long read(ByteBuffer[] buffers, int offset, int length)
		...
		openCheck();
		int count = FileChannelImpl.calculateTotalRemaining(buffers, offset,
				length, true);
		if (count == 0) {
			return 0;
		}
	...
	private int writeImpl(ByteBuffer buffer) throws IOException {
		...
		synchronized (repositioningLock) {
			if (buffer.isDirect()) {
				...
				try {
					begin();
					bytesWritten = (int) fileSystem.write(handle,
							buffer.array(),
							buffer.arrayOffset() + buffer.position(),
							buffer.remaining());
					completed = true;
	...
	/**
	 * @param copyingIn
	 *            true if we're copying data into the buffers (typically because
	 *            the caller is a file/network read operation), false if we're
	 *            copying data out of the buffers (for a file/network write
	 *            operation).
	 */
	static int calculateTotalRemaining(ByteBuffer[] buffers, int offset,
			int length, boolean copyingIn) {
		int count = 0;

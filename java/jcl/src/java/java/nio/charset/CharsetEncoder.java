...
/**
 * Transforms a sequence of 16-bit Java characters to a byte sequence in some
 * encoding.
 * 
 * <p>
 * The input character sequence is a {@link java.nio.CharBuffer CharBuffer} and
 * the output byte sequence is a {@link java.nio.ByteBuffer ByteBuffer}.
 * 
 * <p>
 * Use {@link #encode(CharBuffer)} to encode an entire {@code CharBuffer} to a
 * new {@code ByteBuffer}, or {@link #encode(CharBuffer, ByteBuffer, boolean)}
 * for more control. When using the latter method, the entire operation proceeds
 * as follows:
 * <ol>
 * <li>Invoke {@link #reset()} to reset the encoder if this instance has been
 * used before.</li>
 * <li>Invoke {@link #encode(CharBuffer, ByteBuffer, boolean) encode} with the
 * {@code endOfInput} parameter set to false until additional input is not
 * needed (as signaled by the return value). The input buffer must be filled and
 * the output buffer must be flushed between invocations.
 * <p>
 ...
 * returned to indicate the current state. The caller should fill the input
 * buffer, flush the output buffer, or recovering from an error and try again,
 * accordingly.</li>
 * <li>Invoke {@link #encode(CharBuffer, ByteBuffer, boolean) encode} for the
 * last time with {@code endOfInput} set to true.</li>
 * <li>Invoke {@link #flush(ByteBuffer)} to flush remaining output.</li>
 * </ol>
 * 
 * <p>
 * There are two classes of encoding error: <i>malformed input</i> signifies
 * that the input character sequence is not legal, while <i>unmappable
 * character</i> signifies that the input is legal but cannot be mapped to a
 * byte sequence (because the charset cannot represent the character, for
 * example).
 * 
 * <p>
 * Errors can be handled in three ways. The default is to
 * {@link CodingErrorAction#REPORT report} the error to the caller. The
 * alternatives are to {@link CodingErrorAction#IGNORE ignore} the error or
 * {@link CodingErrorAction#REPLACE replace} the problematic input with the byte
 * sequence returned by {@link #replacement}. The disposition for each of the
 * two kinds of error can be set independently using the
 * {@link #onMalformedInput} and {@link #onUnmappableCharacter} methods.
 * 
 * <p>
 * The default replacement bytes depend on the charset but can be overridden
 * using the {@link #replaceWith} method.
 * 
 * <p>
 ...
 * {@link #implFlush(ByteBuffer) implFlush} and {@link #implReset() implReset}
 * methods.
 * 
 * <p>
...
public abstract class CharsetEncoder {
	 ...
	 * <code>Charset</code>, replacement byte array, average number and maximum
	 ...
	 *            length cannot be larger than <code>maxBytesPerChar</code>, and
	 *            must be a legal replacement, which can be justified by
	...
	protected CharsetEncoder(Charset cs, float averageBytesPerChar,
		...
		if (this instanceof CharsetEncoderICU) {
			// The RI enforces unnecessary restrictions on the replacement
			// bytes. We trust ICU to
			// know what it's doing. This lets us support EUC-JP, SCSU, and
			// Shift_JIS.
	 ...
	 *             character error is {@link CodingErrorAction#REPORT
	 *             CodingErrorAction.REPORT}. Unmappable means the Unicode
	 *             character sequence at the input buffer's current position
	 *             cannot be mapped to a equivalent byte sequence.
	...
	private void checkCoderResult(CoderResult result)
		...
		if (malformAction == CodingErrorAction.REPORT && result.isMalformed()) {
			throw new MalformedInputException(result.length());
		} else if (unmapAction == CodingErrorAction.REPORT
				&& result.isUnmappable()) {
			throw new UnmappableCharacterException(result.length());
		}
	}
	 ...
	 * <li>{@link CoderResult#UNDERFLOW CoderResult.UNDERFLOW} indicates that as
	 * many characters as possible in the input buffer have been encoded. If
	 ...
	 * input.
	 * <p>
	 * This method invokes the {@link #encodeLoop(CharBuffer, ByteBuffer)
	 * encodeLoop} method to implement the basic encode logic for a specific
	 * charset.
	 * 
	 * @param in
	 *            the input buffer.
	 ...
	 * <code>CoderResult</code> object indicating the result of the current
	 * encoding operation. The rule to construct the <code>CoderResult</code> is
	 * the same as for {@link #encode(CharBuffer, ByteBuffer, boolean) encode}.
	 * When an exception is encountered in the encoding operation, most
	 * implementations of this method will return a relevant result object to
	 * the {@link #encode(CharBuffer, ByteBuffer, boolean) encode} method, and
	 * some performance optimized implementation may handle the exception and
	 ...
	 * <code>CoderResult.OVERFLOW</code> will be returned, and this method must
	 * be called again with a byte buffer that has free space. Otherwise this
	 ...
	 * returns <code>CoderResult.UNDERFLOW</code>; this method can be overridden
	 * if needed.
	 * 
	 * @param out
	 *            the output buffer.
	...
	/**
	 * Notifies that this encoder's <code>CodingErrorAction</code> specified for
	 * malformed input error has been changed. The default implementation does
	...
	/**
	 * Notifies that this encoder's <code>CodingErrorAction</code> specified for
	 * unmappable character error has been changed. The default implementation
	...
	public boolean isLegalReplacement(byte[] replacement) {
		...
		CharBuffer out = CharBuffer
				.allocate((int) (replacement.length * decoder.maxCharsPerByte()));
		CoderResult result = decoder.decode(in, out, true);
		return !result.isError();
	}

	/**
	 * Gets this encoder's <code>CodingErrorAction</code> when a malformed input
	 * error occurred during the encoding process.
	 * 
	 ...
	 * 
	 * This method will call the
	 * {@link #implOnUnmappableCharacter(CodingErrorAction)
	 ...
	 *            the replacement byte array, cannot be null or empty, its
	 *            length cannot be larger than <code>maxBytesPerChar</code>, and
	 *            it must be legal replacement, which can be justified by
	...
	public final CharsetEncoder replaceWith(byte[] replacement) {
		...
		if (replacement.length == 0 || maxBytes < replacement.length) {
			throw new IllegalArgumentException("bad replacement length: "
					+ replacement.length);
		}
		if (!isLegalReplacement(replacement)) {
			throw new IllegalArgumentException("bad replacement: "
					+ Arrays.toString(replacement));
		}
		uncheckedReplaceWith(replacement);
		return this;
	}
	...
	private final void uncheckedReplaceWith(byte[] replacement) {
		// It seems like a bug, but the RI doesn't clone, and we have tests that
		// check we don't.
		replace = replacement;

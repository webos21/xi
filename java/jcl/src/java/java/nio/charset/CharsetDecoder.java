 ...
 * <p>
 * The input byte sequence is wrapped by a {@link java.nio.ByteBuffer
 ...
 * <li>invoking the {@link #decode(ByteBuffer, CharBuffer, boolean) decode}
 * method for the last time, and then the <code>endOfInput</code> parameter must
 * be set to true;</li>
 * <li>invoking the {@link #flush(CharBuffer) flush} method to flush the output.
 * </li>
 * </ol>
 * <p>
 ...
 * {@link #implFlush(CharBuffer) implFlush} method and the {@link #implReset()
...
public abstract class CharsetDecoder {
	 ...
	 *             CodingErrorAction.REPORT}. Unmappable means the byte sequence
	 *             at the input buffer's current position cannot be mapped to a
	 *             Unicode character sequence.
	 ...
	 * following rules:
	 * <ul>
	 * <li>{@link CoderResult#OVERFLOW CoderResult.OVERFLOW} indicates that even
	 * though not all of the input has been processed, the buffer the output is
	 * being written to has reached its capacity. In the event of this code
	 ...
	 * many bytes as possible in the input buffer have been decoded. If there is
	 * no further input and no remaining bytes in the input buffer then this
	 ...
	 * and their number can be got by result's {@link CoderResult#length()
	 * length}. This kind of result can be returned only if the unmappable
	 * character action is {@link CodingErrorAction#REPORT
	 * CodingErrorAction.REPORT}.</li>
	 * </ul>
	 * <p>
	 ...
	 * <p>
	 * This method invokes the {@link #decodeLoop(ByteBuffer, CharBuffer)
	 * decodeLoop} method to implement the basic decode logic for a specific
	 * charset.
	 * 
	 * @param in
	 *            the input buffer.
	 ...
	 * @return a <code>CoderResult</code> instance which indicates the reason of
	 *         termination.
	 * @throws IllegalStateException
	 ...
	 * <code>CoderResult</code> object indicating the result of current decoding
	 * operation. The rules to construct the <code>CoderResult</code> are the
	 * same as for {@link #decode(ByteBuffer, CharBuffer, boolean) decode}. When
	 * an exception is encountered in the decoding operation, most
	 * implementations of this method will return a relevant result object to
	 * the {@link #decode(ByteBuffer, CharBuffer, boolean) decode} method, and
	 ...
	 * <code>UnsupportedOperationException</code>, so it should be overridden by
	 ...
	 * {@link #implFlush(CharBuffer) implFlush} to perform the writing
	 * operation.
	 * <p>
	 ...
	 * be called again with a character buffer that has more remaining space.
	...
	/**
	 * Notifies that this decoder's <code>CodingErrorAction</code> specified for
	 ...
	 * <code>UnsupportedOperationException</code>; it should be overridden by a
	 * subclass if needed.
	 * 
	...
	public final CharsetDecoder replaceWith(String newReplacement) {
		if (newReplacement == null || newReplacement.isEmpty()) {
			throw new IllegalArgumentException(
					"Replacement string cannot be null or empty");
		}
	 ...
	 * 
	 * @return this decoder's <code>CodingErrorAction</code> when an unmappable
	 *         character error occurred during the decoding process.
	 */

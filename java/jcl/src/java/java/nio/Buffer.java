...
public abstract class Buffer {
	...
	/**
	 * For direct buffers, the effective address of the data. This is set on
	 * first use. If the field is zero, this is either not a direct buffer or
	 * the field has not been initialized, and you need to issue the
	 * getEffectiveAddress() call and use the result of that.
	 * 
	...
	/**
	 * Returns the array that backs this buffer (optional operation). The
	 * returned value is the actual array, not a copy, so modifications to the
	 * array write through to the buffer.
	 * 
	 * <p>
	 * Subclasses should override this method with a covariant return type to
	 * provide the exact type of the array.
	 * 
	 * <p>
	 * Use {@code hasArray} to ensure this method won't throw. (A separate call
	 * to {@code isReadOnly} is not necessary.)
	 * 
	 * @return the array
	 * @throws ReadOnlyBufferException
	 *             if the buffer is read-only UnsupportedOperationException if
	 *             the buffer does not expose an array
	 * @since 1.6
	 */
	 ...
	 * element of the buffer (optional operation). The backing array (if there
	 * is one) is not necessarily the same size as the buffer, and position 0 in
	 * the buffer is not necessarily the 0th element in the array. Use
	 * {@code buffer.array()[offset + buffer.arrayOffset()} to access element
	 * {@code offset} in {@code buffer}.
	 * 
	 * <p>
	 * Use {@code hasArray} to ensure this method won't throw. (A separate call
	 * to {@code isReadOnly} is not necessary.)

...
abstract class DirectByteBuffer extends BaseByteBuffer implements DirectBuffer {
	...
	DirectByteBuffer(SafeAddress address, int capacity, int offset) {
		super(capacity);
		/*
		 * by jshwang // BEGIN android-added PlatformAddress baseAddress =
		 * address.address; long baseSize = baseAddress.getSize();
		 * 
		 * if ((baseSize >= 0) && ((offset + capacity) > baseSize)) { throw new
		 * IllegalArgumentException("slice out of range"); } // END
		 * android-added
		 */
	...
	public final void addressValidityCheck() {
		if (!isAddressValid()) {
			throw new IllegalStateException(
					"Cannot use a direct byte buffer after it has been explicitly freed");
		}
	}

...
public abstract class SelectorProvider {
	...
	synchronized public static SelectorProvider provider() {
		if (provider == null) {
			provider = ServiceLoader
					.loadFromSystemProperty(SelectorProvider.class);
			if (provider == null) {
	...
	private static SelectorProvider loadProviderByJar() {
		for (SelectorProvider provider : ServiceLoader.load(
				SelectorProvider.class, null)) {
			return provider;
		}
		return null;
	}
	...
	/**
	 * Returns the channel inherited from the instance that created this virtual
	 * machine.
	 * 
	 * @return the channel.
	 * @throws IOException

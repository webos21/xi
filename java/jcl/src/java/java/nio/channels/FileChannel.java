 ...
 * file <em>position</em>. The position can be manipulated by moving it within
 * the file, and the current position can be queried.
 * <p>
 * FileChannels also have an associated <em>size</em>. The size of the file is
 * the number of bytes that it currently contains. The size can be manipulated
 * by adding more bytes to the end of the file (which increases the size) or
 * truncating the file (which decreases the size). The current size can also be
 * queried.
 * <p>
...
public abstract class FileChannel extends AbstractInterruptibleChannel
	 ...
	 * This method attempts to read all {@code remaining()} bytes from
	 * {@code length} byte buffers, in order, starting at
	 * {@code targets[offset]}. It increases the file position by the number of
	 * bytes actually read. The number of bytes actually read is returned.
	 * <p>
	 * If a read operation is in progress, subsequent threads will block until
	 * the read is completed and will then contend for the ability to read.
	 * 
	 ...
	 * file channel. This method attempts to write all {@code remaining()} bytes
	 * from {@code length} byte buffers, in order, starting at
	 * {@code sources[offset]}. The number of bytes actually written is
	 * returned.
	 * <p>

 ...
 * {@code overlaps(long, long)}) method. Locks held in other processes may
 * overlap locks held in this process. Locks are shared amongst all threads in
 * the acquiring process, and are therefore unsuitable for intra-process
 * synchronization.
 * <p>
 * Once a lock is acquired, it is immutable in all its state except
 * {@code isValid()}. The lock will initially be valid, but may be rendered
 * invalid by explicit removal of the lock, using {@code release()}, or
 * implicitly by closing the channel or exiting the process (terminating the
 * virtual machine).
...
public abstract class FileLock {
	...
	/**
	 * Indicates if the receiver's lock region overlaps the region described in
	 * the parameter list.
	 * 
	 * @param start
	...
	/**
	 * Indicates whether this lock is a valid file lock. The lock is valid
	 * unless the underlying channel has been closed or it has been explicitly
	 * released.
	 * 

 ...
 * any given time. The {@code connect(SocketAddress)} and
 * {@code finishConnect()} methods are synchronized against each other; when
 * they are processing, calls to {@code read} and {@code write} will block.
 */
public abstract class SocketChannel extends AbstractSelectableChannel implements
	...
	/**
	 * Returns the socket assigned to this channel, which does not declare any
	 * public methods that are not declared in {@code Socket}.
	 * 
	 ...
	 * {@code read} and {@code write} operations while connecting.
	 * 
	 * @return {@code true} if the connection is successfully finished,
	 *         {@code false} otherwise.
	 ...
	 * {@code targets[offset]}. The number of bytes actually read is returned.
	 * <p>
	 ...
	 * bytes from {@code length} byte buffers, in order, starting at

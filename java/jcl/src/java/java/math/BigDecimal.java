...
public class BigDecimal extends Number implements Comparable<BigDecimal>,
		...
		if (scale < 0) {
			...
			if (scale < LONG_FIVE_POW.length
				...
				setUnscaledValue(Multiplication.multiplyByFivePow(
						BigInteger.valueOf(mantisa), scale));
			}
		} else { // scale == 0
			smallValue = mantisa;
	...
	private static BigDecimal addAndMult10(BigDecimal thisValue,
			BigDecimal augend, int diffScale) {
	...
	private static BigDecimal divideBigIntegers(BigInteger scaledDividend,
		...
		BigInteger[] quotAndRem = scaledDividend
	...
	private void inplaceRound(MathContext mc) {
		...
		BigInteger sizeOfFraction = Multiplication
				.powerOf10(discardedPrecision);
		BigInteger[] integerAndFraction = getUnscaledValue()

...
class Multiplication {
	 ...
	 * {@code 5^0,5^1,...,5^13})
	 */
	static final int fivePows[] = { 1, 5, 25, 125, 625, 3125, 15625, 78125,
	...
	static void multArraysPAP(int[] aDigits, int aLen, int[] bDigits, int bLen,
		...
		if (aLen == 1) {
			resDigits[bLen] = multiplyByInt(resDigits, bDigits, bLen,
					aDigits[0]);
		} else if (bLen == 1) {
			resDigits[aLen] = multiplyByInt(resDigits, aDigits, aLen,
					bDigits[0]);
		} else {
	...
	/**
	 * Multiplies an array of integers by an integer value and saves the result
	 * in {@code res}.
	 * 
	 * @param a
	...
	static BigInteger multiplyByPositiveInt(BigInteger val, int factor) {
		...
		if (aNumberLength == 1) {
			...
			return ((resHi == 0) ? new BigInteger(resSign, resLo)
		...
		resDigits[aNumberLength] = multiplyByInt(resDigits, aDigits,
				aNumberLength, factor);
		BigInteger result = new BigInteger(resSign, resLength, resDigits);
		result.cutOffLeadingZeroes();
		return result;
	}
	...
	/**
	 * Multiplies a number by a power of ten. This method is used in
	...
	/**
	 * It calculates a power of ten, which exponent could be out of 32-bit
	 * range. Note that internally this method will be used in the worst case
	 * with an exponent equals to: {@code Integer.MAX_VALUE - Integer.MIN_VALUE}
	 * .
	 * 
	 * @param exp
	...
	/**
	 * Multiplies a number by a power of five. This method is used in
	 * {@code BigDecimal} class.
	 * 
	 * @param val
	 *            the number to be multiplied

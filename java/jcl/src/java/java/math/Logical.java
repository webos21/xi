...
class Logical {
	...
	static BigInteger andPositive(BigInteger val, BigInteger that) {
		...
		int i = Math.max(val.getFirstNonzeroDigit(),
				that.getFirstNonzeroDigit());
	...
	static BigInteger andNegative(BigInteger longer, BigInteger shorter) {
		...
		if (digit == 0) {
			...
			if (digit == 0) {
				...
				for (; i < longer.numberLength
						&& (digit = ~longer.digits[i]) == 0; i++)
					;
				if (digit == 0) {
	...
	/**
	 * @return sign = -1, magnitude = -(-negative.magnitude &
	 *         ~positive.magnitude)
	 */
	...
	static BigInteger or(BigInteger val, BigInteger that) {
		if (that.equals(BigInteger.MINUS_ONE)
				|| val.equals(BigInteger.MINUS_ONE)) {
			return BigInteger.MINUS_ONE;
		}
		if (that.sign == 0) {
			return val;

...
public abstract class InputStream extends Object implements Closeable {
	 ...
	 * "without blocking": a read may still block waiting for I/O to
	 * complete&nbsp;&mdash; the guarantee is merely that it won't have to wait
	 * indefinitely for data to be written. The result of this method should not
	 * be used as a license to do I/O on a thread that shouldn't be blocked.
	 * 
	 * <p>
	 ...
	 * implementation that always returns 0 would be correct. In general,
	 * callers should only use this method if they'd be satisfied with treating
	 * the result as a boolean yes or no answer to the question "is there
	 * definitely data ready?".
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * It is particularly important to realize that you <i>must not</i> use this
	 * method to size a container and assume that you can read the entirety of
	 * the stream without needing to resize the container. Such callers should
	 * probably write everything they read to a {@link ByteArrayOutputStream}
	 * and convert that to a byte array. Alternatively, if you're reading from a
	 * file, {@link File#length} returns the current length of the file (though
	 * assuming the file's length can't change may be incorrect, reading a file
	 * is inherently racy).
	 * 
	 * <p>
	 * The default implementation of this method in {@code InputStream} always
	 * returns 0. Subclasses should override this method if they are able to
	 * indicate the number of bytes available.
	 * 
	...
	/**
	 * Skips at most {@code n} bytes in this stream. This method does nothing
	 * and returns 0 if {@code n} is negative.
	 * 
	 * <p>

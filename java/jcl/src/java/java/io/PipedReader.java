...
public class PipedReader extends Reader {
	 ...
	 * range {@code [out, in)} and written to the range {@code [in, out)}. Data
	 * in the buffer is either sequential:
	 * 
	 * <pre>
	 *     { - - - X X X X X X X - - - - - }
	 *             ^             ^
	...
	/**
	 * Constructs a new unconnected {@code PipedReader} with the given buffer
	 * size. The resulting reader must be connected to a {@code PipedWriter}
	...
	public PipedReader(int pipeSize) {
		if (pipeSize <= 0) {
			throw new IllegalArgumentException("pipe size " + pipeSize
					+ " too small");
		}
	...
	/**
	 * Constructs a new {@code PipedReader} connected to the given
	 * {@code PipedWriter}, with the given buffer size. Any data written to the
	 * writer can be read from this reader.
	 * 
	 * @param out
	 *            the {@code PipedWriter} to connect to.
	 * @param pipeSize
	 ...
	 * @throws IOException
	 *             if this reader is closed or already connected, or if
	 *             {@code src} is already connected.
	 */
	...
	public synchronized int read(char[] buffer, int offset, int count)
		 ...
		 * dies while someone is waiting to write an IOException of
		...
		if (out >= in) {
			copyLength = count > this.buffer.length - out ? this.buffer.length
					- out : count;
		...
		copyLength = in - out > count - copyLength ? count - copyLength : in
				- out;
	 ...
	 *             occurs.
	 */
	synchronized void receive(char[] chars, int offset, int count)
			throws IOException {
		if (chars == null) {

...
public class RandomAccessFile implements DataInput, DataOutput, Closeable {
	 ...
	 * <p>
	 * <a id="accessmode"/> {@code mode} may have one of following values:
	 * <table border="0">
	 * <tr>
	 * <td>{@code "r"}</td>
	 ...
	 *            the file to open.
	 * @param mode
	 *            the file access <a href="#accessmode">mode</a>, either
	 *            {@code "r"}, {@code "rw"}, {@code "rws"} or {@code "rwd"}.
	 * @throws FileNotFoundException
	 *             if the file cannot be opened or created according to
	 *             {@code mode}.
	 * @throws IllegalArgumentException
	 *             if {@code mode} is not {@code "r"}, {@code "rw"},
	 *             {@code "rws"} or {@code "rwd"}.
	...
	public RandomAccessFile(File file, String mode)
		...
		} else if (mode.equals("rw") || mode.equals("rws")
				|| mode.equals("rwd")) {
			isReadOnly = false;
	...
	/**
	 * Constructs a new {@code RandomAccessFile} based on the file named
	 * {@code fileName} and opens it according to the access string in
	 * {@code mode}. The file path may be specified absolutely or relative to
	 ...
	 * <p>
	 * The file channel's {@link FileChannel#position() position} is the same as
	 * this file's file pointer offset (see {@link #getFilePointer()}). Any
	...
	/**
	 * Gets the current position within this file. All reads and writes take
	 * place at the current file pointer position.
	 * 
	 ...
	 * {@code offset}. Blocks until {@code count} bytes have been read, the end
	 ...
	 * until two bytes have been read, the end of the file is reached or an
	 * exception is thrown.
	 * 
	...
	/**
	 * Reads a 32-bit float from the current position in this file. Blocks until
	 * four bytes have been read, the end of the file is reached or an exception
	 * is thrown.
	 * 
	 ...
	 * {@code buffer.length} number of bytes have been read, the end of the file
	 ...
	 * {@code offset}. This method blocks until {@code count} number of bytes
	 * have been read.
	 * 
	 * @param buffer
	 *            the buffer to read bytes into.
	 ...
	 * returns it as an integer. Blocks until two bytes have been read, the end
	 * of the file is reached or an exception is thrown.
	 * 
	...
	/**
	 * Writes a 64-bit long to this file, starting at the current file pointer.
	 * The eight bytes of the long are written with the highest byte first.
	 * 
	 * @param val
	...
	/**
	 * Writes a 16-bit short to this file, starting at the current file pointer.
	 * Only the two least significant bytes of the integer {@code val} are
	 * written, with the high byte first.
	 * 
	 * @param val

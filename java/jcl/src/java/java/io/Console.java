...
/**
 * Provides access to the console, if available. The system-wide instance can be
 * accessed via {@link java.lang.System#console}.
 * 
 * @since 1.6
 */
public final class Console implements Flushable {
	...
	/**
	 * Writes a formatted string to the console using the specified format
	...
	/**
	 * Reads a line from this console, using the specified prompt. The prompt is
	 * given as a format string and optional arguments. Note that this can be a
	 * source of errors: if it is possible that your prompt contains {@code %}
	 * characters, you must use the format string {@code "%s"} and pass the
	 * actual prompt as a parameter.
	 * 
	 * @param format
	 *            the format string (see {@link java.util.Formatter#format})
	 * @param args
	...
	/**
	 * Reads a password from the console. The password will not be echoed to the
	 * display.
	 * 
	...
	private static native int setEchoImpl(boolean on, int previousState)
			throws IOException;

	/**
	 * Reads a password from the console. The password will not be echoed to the
	 * display. A formatted prompt is also displayed.
	 * 
	 * @param format
	 *            the format string (see {@link java.util.Formatter#format})
	 * @param args
		...
		public ConsoleReader(InputStream in) throws IOException {
			super(
					new InputStreamReader(in,
							System.getProperty("file.encoding")), 256);
			lock = CONSOLE_LOCK;
		}

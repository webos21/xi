...
public class PrintWriter extends Writer {
	...
	/**
	 * Constructs a new {@code PrintWriter} with {@code file} as its target. The
	 * character set named {@code csn} is used for character encoding. The print
	 * writer does not automatically flush its contents to the target file when
	 * a newline is encountered. The output to the file is buffered.
	 * 
	 * @param file
	 *            the target file. If the file already exists, its contents are
	 *            removed, otherwise a new file is created.
	...
	/**
	 * Constructs a new {@code PrintWriter} with the file identified by
	 * {@code fileName} as its target. The virtual machine's default character
	 * set is used for character encoding. The print writer does not
	 * automatically flush its contents to the target file when a newline is
	 * encountered. The output to the file is buffered.
	 * 
	 * @param fileName
	...
	/**
	 * Formats {@code args} according to the format string {@code format}, and
	 * writes the result to this stream. This method uses the user's default
	 * locale. See "<a href="../util/Locale.html#default_locale
	 * ">Be wary of the default locale</a>". If automatic flushing is enabled
	 * then the buffer is flushed as well.
	 * 
	 * @param format
	 ...
	 * {@code "line.separator"} to the target. Flushes this writer if the
	 * autoflush flag is set to {@code true}.
	 */
	public void println() {
		synchronized (lock) {
			print(lineSeparator);
	 ...
	 * works the same way as {@code PrintWriter.print(csq.toString())}. If
	 * {@code csq} is {@code null}, then the string "null" is written to the
	 * target.
	 * 
	 * @param csq

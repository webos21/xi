...
public class DataOutputStream extends FilterOutputStream implements DataOutput {
	...
	int writeLongToBuffer(long val, byte[] buffer, int offset)
			throws IOException {
	...
	int writeShortToBuffer(int val, byte[] buffer, int offset)
			throws IOException {
		buffer[offset++] = (byte) (val >> 8);
		buffer[offset++] = (byte) val;
		return offset;
	...
	int writeUTFBytesToBuffer(String str, byte[] buffer, int offset)
			throws IOException {
		// BEGIN android-note
		// removed unused parameter count
		// END android-note

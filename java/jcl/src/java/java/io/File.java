...
public class File implements Serializable, Comparable<File> {
	 ...
	 * ('/'). Use of this (rather than hard-coding '/') helps portability to
	 * other operating systems.
	 * 
	 * <p>
	 * This field is initialized from the system property "file.separator".
	 * Later changes to that property will have no effect on this field or this
	 * class.
	 */
	...
	/**
	 * The system-dependent character used to separate components in search
	 * paths (':'). This is used to split such things as the PATH environment
	 * variable and classpath system properties into lists of directories to be
	 * searched.
	 * 
	 * <p>
	...
	private void init(String dirtyPath) {
		...
		boolean isAbsolute = ((cleanPath.length() > 0
				&& cleanPath.charAt(0) == '/') || (cleanPath.length() > 3 &&  (cleanPath.charAt(1) == ':' || cleanPath
						.charAt(2) == ':')));
		if (isAbsolute) {
			if (cleanPath.charAt(2) == ':' && cleanPath.startsWith("/")) {
			...
			if (tg instanceof FilePathRefThreadGroup) {
				...
				String ctxRootPath = fprtg.getContextRootPath();
				if (ctxRootPath != null) {
					userDir = ctxRootPath;
				}
			}
			...
			this.absolutePath = cleanPath.isEmpty() ? userDir : join(userDir,
					cleanPath);
			// We want path to be equal to cleanPath, but we'd like to reuse
			// absolutePath's char[].
			this.path = absolutePath.substring(absolutePath.length()
					- cleanPath.length());
		}
	}
	...
	private void checkURI(URI uri) {
		...
		if (!"file".equals(uri.getScheme())) {
			throw new IllegalArgumentException("Expected file scheme in URI: "
					+ uri);
		}
		String rawPath = uri.getRawPath();
		if (rawPath == null || rawPath.isEmpty()) {
			throw new IllegalArgumentException(
					"Expected non-empty path in URI: " + uri);
		}
	...
	public boolean canExecute() {
		...
		if (security != null) {
			security.checkExec(path); // Seems bogus, but this is what the RI
										// does.
		}
	...
	public String getCanonicalPath() throws IOException {
		...
		if (separatorChar != '/') {
			if (result.charAt(0) == '\\') {
				rootLoc = (result.length() > 1 && result.charAt(1) == '\\') ? 1
						: 0;
			} else {
		...
		for (int i = 1; i <= path.length(); i++) {
			if (i == path.length() || path.charAt(i) == separatorChar) {
				...
				if (linkPath.equals(path)) {
					bytes = path;
					// if there are no symbolic links, truncate the path instead
					// of copying
					if (!done) {
						inPlace = true;
				...
				linkPath = resolveLink(bytes, inPlace ? i : bytes.length(),
						true);
				if (inPlace) {
					// path[i] = '/';
					path = path.substring(0, i)
							+ '/'
							+ (i + 1 < path.length() ? path.substring(i + 1)
									: "");
				}
				last = i + 1;
			}
	 ...
	 */
	private static String resolveLink(String path, int length,
		...
		do {
			...
			if (target.charAt(0) == separatorChar) {
				// The link target was an absolute path, so we may need to start
				// again.
				restart = resolveAbsolute;
				path = target + path.substring(length);
			} else {
				path = path.substring(0,
						path.lastIndexOf(separatorChar, length - 1) + 1)
						+ target;
			}
			length = path.length();
	...
	private static native boolean setExecutableImpl(String path,
	...
	private static native boolean setReadableImpl(String path,
	...
	private static native boolean setWritableImpl(String path,
	 ...
	 * 
	 * <p>
	 * Note that this method does <i>not</i> call {@link #deleteOnExit}.
	 * 
	 * @param prefix
	 *            the prefix to the temp file name.
	 * @param suffix
	...
	public static File createTempFile(String prefix, String suffix,
		...
		if (tmpDirFile == null) {
			String tmpDir = AccessController
					.doPrivileged(new PriviAction<String>("java.io.tmpdir", "."));
			tmpDirFile = new File(tmpDir);
		}
		File result;
		do {
			result = new File(tmpDirFile, prefix + new Random().nextInt()
					+ suffix);
	...
	public URL toURL() throws java.net.MalformedURLException {
		...
		if (!name.startsWith("/")) {
			// start with sep.
			return new URL("file", "", -1, new StringBuilder(name.length() + 1)

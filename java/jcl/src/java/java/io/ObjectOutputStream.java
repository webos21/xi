...
public class ObjectOutputStream extends OutputStream implements ObjectOutput,
	 ...
	 */
	private final ObjectStreamClass proxyClassDesc = ObjectStreamClass
			.lookup(Proxy.class);
	 ...
	 * {@code flush} but the flush is not propagated to the target stream.
	 * 
	 * @throws IOException
	 *             if an error occurs while writing to the target stream.
	 */
	...
	/**
	 * Dumps the parameter {@code obj} only if it is {@code null} or an object
	 * that has already been dumped previously.
	 * 
	 * @param obj
	 ...
	 *         method does nothing). Integer, if {@code obj} is an instance
	 *         which has been dumped already. In this case this method saves the
	 *         cyclic reference.
	 * 
	 * @throws IOException
	 *             If an error occurs attempting to save {@code null} or a
	 *             cyclic reference.
	 */
	...
	/**
	 * Assume object {@code obj} has not been dumped yet, and assign a handle to
	 * it
	 * 
	 * @param obj
	 ...
	 * first with calling {@code enableReplaceObject(true)}. This implementation
	 ...
	 *            {@code PROTOCOL_VERSION_x} constant from
	 *            {@code java.io.ObjectStreamConstants}.
	...
	/**
	 * Write a class descriptor {@code classDesc} (an {@code ObjectStreamClass})
	 * to the stream.
	 * 
	 * @param classDesc
	 *            The class descriptor (an {@code ObjectStreamClass}) to be
	 *            dumped
	 * @param unshared
	 ...
	 * class descriptor {@code classDesc} (an {@code ObjectStreamClass})
	 * 
	 * @param classDesc
	 *            The class descriptor (an {@code ObjectStreamClass}) for which
	 ...
	 * {@code PutField} object, which can be accessed by calling
	 * {@code putFields()}.
	 * 
	 * @throws IOException
	 *             if an error occurs while writing to the target stream.
	 ...
	 *            an {@code EmulatedFieldsForDumping}, concrete subclass of
	 *            {@code PutField}
	 * 
	 * @throws IOException
	 ...
	 * descriptor {@code classDesc} (an {@code ObjectStreamClass}). This is the
	 * default mechanism, when emulated fields (an {@code PutField}) are not
	 * used. Actual values to dump are fetched directly from object {@code obj}.
	 * 
	 * @param obj
	 *            Instance from which to fetch field values to dump.
	 * @param classDesc
	 *            A class descriptor (an {@code ObjectStreamClass}) defining
	 *            which fields should be dumped.
	 * 
	 * @throws IOException
	 *             If an IO exception happened when writing the field values.
	 * 
	...
	private void writeFieldValues(Object obj, ObjectStreamClass classDesc)
		...
		for (ObjectStreamField fieldDesc : fields) {
			try {
				...
				// long fieldID = fieldDesc.getFieldID(accessor,
				// declaringClass);
				...
				if (fieldDesc.isPrimitive()) {
					...
					Object objField = getFieldObj(obj, declaringClass,
							fieldDesc.getName(), fieldDesc.getTypeString());
	 ...
	 * {@code classDesc} and writes the field values corresponding to fields
	 * declared by the corresponding class descriptor. The instance to fetch
	 * field values from is {@code object}. If the class (corresponding to class
	 * descriptor {@code classDesc}) defines private instance method
	 * {@code writeObject} it will be used to dump field values.
	 * 
	 * @param object
	 *            Instance from which to fetch field values to dump.
	 * @param classDesc
	...
	/**
	 * Write array {@code array} of class {@code arrayClass} with component type
	 * {@code componentType} into the receiver. It is assumed the array has not
	 * been dumped yet. Return an {@code Integer} that represents the handle for
	 * this object (array) which is dumped here.
	 * 
	 * @param array
	 *            The array object to dump
	 * @param arrayClass
	 *            A {@code java.lang.Class} representing the class of the array
	 * @param componentType
	 *            A {@code java.lang.Class} representing the array component
	 *            type
	...
	private Integer writeNewArray(Object array, Class<?> arrayClass,
			ObjectStreamClass arrayClDesc, Class<?> componentType,
		...
		if (componentType.isPrimitive()) {
			if (componentType == Integer.TYPE) {
				...
				throw new InvalidClassException("Wrong base type in "
						+ arrayClass.getName());
			}
		} else {
			// Array of Objects
	...
	/**
	 * Write class {@code object} into the receiver. It is assumed the class has
	 * not been dumped yet. Classes are not really dumped, but a class
	 * descriptor ({@code ObjectStreamClass}) that corresponds to them. Return
	 * an {@code Integer} that represents the handle for this object (class)
	 * which is dumped here.
	 * 
	 * @param object
	...
	/**
	 * Write class descriptor {@code classDesc} into the receiver. It is assumed
	 * the class descriptor has not been dumped yet. The class descriptors for
	 * the superclass chain will be dumped as well. Return an {@code Integer}
	 * that represents the handle for this object (class descriptor) which is
	 * dumped here.
	 * 
	 * @param classDesc
	...
	private void writeNewClassDesc(ObjectStreamClass classDesc)
		...
		if (externalizable) {
			if (protocolVersion == PROTOCOL_VERSION_1) {
				...
				// (PROTOCOL_VERSION_1) or in the 1.2 format
				// (PROTOCOL_VERSION_2).
				flags |= SC_BLOCK_DATA;
			}
		}
	...
	/**
	 * Write exception {@code ex} into the receiver. It is assumed the exception
	 * has not been dumped yet. Return an {@code Integer} that represents the
	 * handle for this object (exception) which is dumped here. This is used to
	 * dump the exception instance that happened (if any) when dumping the
	 * original object graph. The set of seen objects will be reset just before
	 * and just after dumping this exception object.
	 * 
	...
	/**
	 * Write object {@code object} of class {@code theClass} into the receiver.
	 * It is assumed the object has not been dumped yet. Return an
	 * {@code Integer} that represents the handle for this object which is
	 * dumped here.
	 * 
	 * If the object implements {@code Externalizable} its {@code writeExternal}
	 * is called. Otherwise, all fields described by the class hierarchy is
	 * dumped. Each class can define how its declared instance fields are dumped
	 * by defining a private method {@code writeObject}
	 * 
	 * @param object
	 *            The object to dump
	 * @param theClass
	 *            A {@code java.lang.Class} representing the class of the object
	 * @param unshared
	 *            Write the object unshared
	...
	private Integer writeNewObject(Object object, Class<?> theClass,
			ObjectStreamClass clDesc, boolean unshared) throws IOException {
	...
	/**
	 * Write String {@code object} into the receiver. It is assumed the String
	 * has not been dumped yet. Return an {@code Integer} that represents the
	 * handle for this object (String) which is dumped here. Strings are saved
	 * encoded with {@link DataInput modified UTF-8}.
	 * 
	 * @param object
	 *            the string to dump.
	...
	private Integer writeObjectInternal(Object object, boolean unshared,
		...
		ObjectStreamClass clDesc = ObjectStreamClass
				.lookupStreamClass(objClass);
		...
		try {
			...
			if (clDesc.isSerializable() && computeClassBasedReplacement) {
				if (clDesc.hasMethodWriteReplace()) {
					...
					Object replObj = null;
					try {
						replObj = methodWriteReplace.invoke(object,
								(Object[]) null);
					...
					if (replObj != object) {
						...
						Integer replacementHandle = writeObjectInternal(
								replObj, false, false, computeStreamReplacement);
			...
			// Is it an Array ?
			if (objClass.isArray()) {
				return writeNewArray(object, objClass, clDesc,
						objClass.getComponentType(), unshared);
			}
	...
	private ObjectStreamClass writeEnumDesc(Class<?> theClass,
			ObjectStreamClass classDesc, boolean unshared) throws IOException {
		...
		if (handle == null) {
			...
			if (null != superClassDesc) {
				...
				writeEnumDesc(superClassDesc.forClass(), superClassDesc,
						unshared);
			} else {
				output.writeByte(TC_NULL);
			}
	...
	private Integer writeNewEnum(Object object, Class<?> theClass,
		...
		if (null != fields && fields.length > 1) {
			// BEGIN android-changed
			String str = (String) getFieldObj(object, declaringClass,
					fields[1].getName(), fields[1].getTypeString());
			// END android-changed

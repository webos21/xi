...
public class ObjectInputStream extends InputStream implements ObjectInput,
	...
	// BEGIN android-added
	/**
	 * Create and return a new instance of class {@code instantiationClass} but
	 * running the constructor defined in class {@code constructorClass} (same
	 * as {@code instantiationClass} or a superclass).
	 * 
	 * Has to be native to avoid visibility rules and to be able to have
	 * {@code instantiationClass} not the same as {@code constructorClass} (no
	 * such API in java.lang.reflect).
	 * 
	...
	private StreamCorruptedException corruptStream(byte tc)
			throws StreamCorruptedException {
			...
			case TC_REFERENCE:
				if (unshared) {
					...
					throw new InvalidObjectException(
							"Unshared read of back reference");
				}
				return readCyclicReference();
			case TC_NULL:
				return null;
	 ...
	 *            an {@code EmulatedFieldsForLoading}, concrete subclass of
	 *            {@code GetField}
	 * 
	 * @throws IOException
	 ...
	 * {@code classDesc} (an {@code ObjectStreamClass}). The values will be used
	 * to set instance fields in object {@code obj}. This is the default
	 * mechanism, when emulated fields (an {@code GetField}) are not used.
	 * Actual values to load are stored directly into the object {@code obj}.
	 * 
	 * @param obj
	 *            Instance in which the fields will be set.
	 * @param classDesc
	 *            A class descriptor (an {@code ObjectStreamClass}) defining
	 *            which fields should be loaded.
	 * 
	 * @throws IOException
	 *             If an IO exception happened when reading the field values.
	 * @throws InvalidClassException
			...
			if (fieldDesc.isPrimitive()) {
				try {
					...
					case 'C':
						...
						break;
					case 'D':
						setFieldDouble(obj, declaringClass,
								fieldDesc.getName(), input.readDouble());
						break;
					case 'F':
				...
				if (fieldDesc != null) {
					if (toSet != null) {
						...
						if (!fieldType.isAssignableFrom(valueType)) {
							throw new ClassCastException(classDesc.getName()
									+ "." + fieldName + " - " + fieldType
									+ " not compatible with " + valueType);
						}
						try {
							// BEGIN android-changed
							setFieldObject(obj, declaringClass, fieldName,
									field.getTypeString(), toSet);
							// END android-changed
	 ...
	 * {@code classDesc} and reads the field values corresponding to fields
	 * declared by the corresponding class descriptor. The instance to store
	 * field values into is {@code object}. If the class (corresponding to class
	 * descriptor {@code classDesc}) defines private instance method
	 * {@code readObject} it will be used to load field values.
	 * 
	 * @param object
	...
	private void readObjectNoData(Object object, Class<?> cl,
			ObjectStreamClass classDesc) throws ObjectStreamException {
	...
	private Object readNewArray(boolean unshared) throws OptionalDataException,
		...
		if (componentType.isPrimitive()) {
			if (componentType == Integer.TYPE) {
				...
				throw new ClassNotFoundException("Wrong base type in "
						+ classDesc.getName());
			}
		} else {
			// Array of Objects
	...
	private ObjectStreamClass readEnumDescInternal() throws IOException,
		...
		if (0L != classDesc.getSerialVersionUID()
			...
			throw new InvalidClassException(superClass.getName(),
					"Incompatible class (SUID): " + superClass
							+ " but expected " + superClass);
		}
		byte tc = nextTC();
	...
	protected ObjectStreamClass readClassDescriptor() throws IOException,
		 ...
		 * might be null.
		 */
		descriptorHandle = (null == descriptorHandle ? nextHandle()
				: descriptorHandle);
	...
	private Class<?> resolveConstructorClass(Class<?> objectClass,
			boolean wasSerializable, boolean wasExternalizable)
			throws OptionalDataException, ClassNotFoundException, IOException {
	...
	private Object readNewObject(boolean unshared)
		...
		if (objectClass != null) {
			...
			// {
			// constructor =
			// accessor.getMethodID(resolveConstructorClass(objectClass,
			// wasSerializable, wasExternalizable), null, new Class[0]);
			// classDesc.setConstructor(constructor);
			// }
			Class constructorClass = resolveConstructorClass(objectClass,
					wasSerializable, wasExternalizable);
			// END android-changed
	...
	private InvalidClassException missingClassDescriptor()
			throws InvalidClassException {
		throw new InvalidClassException(
				"Read null attempting to read class descriptor for object");
	}
	...
	// BEGIN android-added
	private static final ClassLoader bootstrapLoader = Object.class
			.getClassLoader();
	...
	/**
	 * Assume object {@code obj} has been read, and assign a handle to it,
	 * {@code handle}.
	 * 
	 * @param obj
	 ...
	 * allows to perform additional consistency checks before the
	 * {@code readObject()} method of this class returns its result to the
	 * caller. This method can only be called from within the
	 * {@code readObject()} method of a class that implements "special"
	 * deserialization rules. It can be called multiple times. Validation
	 * callbacks are then done in order of decreasing priority, defined by
	 * {@code priority}.
	 * 
	 * @param object
	 ...
	 * <code>loadedStreamClass</code>matches the SUID & the base name of the
	 * corresponding loaded class and init private fields.
	 * 
	...
	private void verifyAndInit(ObjectStreamClass loadedStreamClass)
		...
		if (!loadedClassBaseName.equals(localClassBaseName)) {
			throw new InvalidClassException(
					loadedStreamClass.getName(),
					String.format(
							"Incompatible class (base name): %s but expected %s",

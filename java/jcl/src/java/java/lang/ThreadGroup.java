...
/**
 * {@code ThreadGroup} is a means of organizing threads into a hierarchical
 * structure. This class is obsolete. See <i>Effective Java</i> Item 73,
 * "Avoid thread groups" for details.
 * 
 * @see Thread
 * @see SecurityManager
 */
public class ThreadGroup implements Thread.UncaughtExceptionHandler {
	...
	/**
	 * Constructs a new {@code ThreadGroup} with the given name. The new
	 * {@code ThreadGroup} will be child of the {@code ThreadGroup} to which the
	 * calling thread belongs.
	 * 
	 * @param name
	 *            the name
	 * @throws SecurityException
	 *             if {@code checkAccess()} for the parent group fails with a
	 *             SecurityException
	...
	/**
	 * Constructs a new {@code ThreadGroup} with the given name, as a child of
	 * the given {@code ThreadGroup}.
	 * 
	 * @param parent
	 *            the parent
	 * @param name
	 ...
	 *             if {@code parent} has been destroyed already
	 */
	...
	/**
	 * Initialize the special "system" ThreadGroup. Was "main" in Harmony, but
	 * we have an additional group above that in Android.
	 */
	ThreadGroup() {
		this.name = "system";
		this.setParent(null);
	...
	/**
	 * Returns the number of running {@code Thread}s which are children of this
	 * thread group, directly or indirectly.
	 * 
	...
	/**
	 * Returns the number of {@code ThreadGroup}s which are children of this
	 ...
	 * to class java.lang.Thread, and should only be called when a new Thread is
	 * created and initialized by the constructor.
	 * 
	 * @param thread
	 *            Thread to add
	 * @throws IllegalThreadStateException
	 *             if this group has been destroyed already
	...
	/**
	 * Checks the accessibility of this {@code ThreadGroup} from the perspective
	 * of the caller. If there is a {@code SecurityManager} installed, calls
	...
	/**
	 * Destroys this thread group and recursively all its subgroups. It is only
	 * legal to destroy a {@code ThreadGroup} that has no threads in it. Any
	 * daemon {@code ThreadGroup} is destroyed automatically when it becomes
	 * empty (no threads or thread groups in it).
	 * 
	 * @throws IllegalThreadStateException
	 *             if this thread group or any of its subgroups has been
	 *             destroyed already or if it still contains threads.
	 * @throws SecurityException
	 *             if {@code this.checkAccess()} fails with a SecurityException
	 */
	 ...
	 * 
	 * <p>
	 * Note that this method will silently ignore any threads that don't fit in
	 * the supplied array.
	 * 
	 * @param threads
	 *            the array into which the {@code Thread}s will be copied
	 ...
	 *            the array into which the {@code Thread}s will be copied
	 * @param recurse
	 *            indicates whether {@code Thread}s in subgroups should be
	 ...
	 * fit in the supplied array.
	 * 
	 * @param groups
	 *            the array into which the {@code ThreadGroup}s will be copied
	 ...
	 * sub-groups) and stores the groups in the given array. Returns when the
	 * array is full or no more groups remain, whichever happens first.
	 * 
	 * <p>
	 * Note that this method will silently ignore any thread groups that don't
	 * fit in the supplied array.
	 ...
	 *            the array into which the {@code ThreadGroup}s will be copied
	 * @param recurse
	 *            indicates whether {@code ThreadGroup}s in subgroups should be
	 ...
	 * @param recurse
	 *            Indicates whether subgroups should be enumerated or not
	...
	/**
	 * Returns the maximum allowed priority for a {@code Thread} in this thread
	 * group.
	 * 
	...
	/**
	 * Returns this thread group's parent {@code ThreadGroup}. It can be null if
	 * this is the the root ThreadGroup.
	 * 
	 * @return the parent
	 */
	...
	/**
	 * Outputs to {@code System.out} a text representation of the hierarchy of
	 * {@code Thread}s and {@code ThreadGroup}s in this thread group (and
	 * recursively). Proper indentation is used to show the nesting of groups
	 * inside groups and threads inside groups.
	 */
	public void list() {
	...
	/*
	 * Outputs to {@code System.out}a text representation of the hierarchy of
	 * Threads and ThreadGroups in this thread group (and recursively). The
	 * indentation will be four spaces per level of nesting.
	 * 
	...
	/**
	 * Checks whether this thread group is a direct or indirect parent group of
	 * a given {@code ThreadGroup}.
	 * 
	 * @param g
	 *            the potential child {@code ThreadGroup}
	...
	/**
	 * Removes a {@code Thread} from this group. This should only be visible to
	 * class java.lang.Thread, and should only be called when a Thread dies.
	 * 
	 * @param thread
	 *            Thread to remove
	 * 
	...
	/**
	 * Sets whether this is a daemon {@code ThreadGroup} or not. Daemon thread
	 * groups are automatically destroyed when they become empty.
	 * 
	 * @param isDaemon
	 *            the new value
	 * @throws SecurityException
	...
	/**
	 * Configures the maximum allowed priority for a {@code Thread} in this
	 * group and recursively in all its subgroups.
	 * 
	 * <p>
	 * A caller can never increase the maximum priority of a thread group. Such
	 * an attempt will not result in an exception, it will simply leave the
	 * thread group with its current maximum priority.
	 * 
	 * @param newMax
	 *            the new maximum priority to be set
	 * 
	 * @throws SecurityException
	 *             if {@code checkAccess()} fails with a SecurityException
	 ...
	 * thread group to the parent's collection of immediate children (if
	 * {@code parent} is not {@code null}).
	 * 
	 * @param parent
	 *            The parent ThreadGroup, or null to make this thread group the
	 *            root ThreadGroup
	 * 
	 * @see #getParent
	 * @see #parentOf
	 ...
	 * 
	 * <p>
	 * New code should use {@link Thread#setUncaughtExceptionHandler} instead of
	 * thread groups.
	 * 
	 * @param t
	 ...
	 * 
	 * @throws IllegalThreadStateException
	 *             if the thread has been destroyed already
	 * 

 ...
 * <p>
 * A {@code Throwable} can also include a cause, which is a nested
 * {@code Throwable} that represents the original problem that led to this
 * {@code Throwable}. It is often used for wrapping various types of errors into
 * a common {@code Throwable} without losing the detailed original error
...
public class Throwable implements java.io.Serializable {
	...
	/**
	 * An intermediate representation of the stack trace. This field may be
	 * accessed by the VM; do not rename.
	 */
	// by jshwang
	// private volatile Object stackState;
	private volatile VMThrowable stackState;
	 ...
	 * to this {@code Throwable}. This method is invoked by the
	 * {@code Throwable} constructors.
	 * 
	 * <p>
	 * This method is public so that code (such as an RPC system) which catches
	 * a {@code Throwable} and then re-throws it can replace the
	 * construction-time stack trace with a stack trace from the location where
	 * the exception was re-thrown, by <i>calling</i> {@code fillInStackTrace}.
	 * 
	 * <p>
	 * This method is non-final so that non-Java language implementations can
	 * disable VM stack traces for their language. Filling in the stack trace is
	 * relatively expensive. <i>Overriding</i> this method in the root of a
	 * language's exception hierarchy allows the language to avoid paying for
	...
	public Throwable fillInStackTrace() {
		...
		// by jshwang
		stackState = VMThrowable.fillInStackTrace(this);
	 ...
	 * localized text for the message. Android returns the regular detail
	 * message.
	 * 
	 ...
	 * stored in this {@code Throwable}. will be returned by
	 * {@code getStackTrace()} and printed by {@code printStackTrace()}.
	 * 
	 * @param trace
	...
	/**
	 * Returns an array of StackTraceElement. Each StackTraceElement represents
	...
	private StackTraceElement[] getInternalStackTrace() {
		if (stackTrace == null) {
			// by jshwang
			stackTrace = stackState.getStackTrace(this);
	 ...
	 * storing in the "stackState" field, based on the current thread's call
	 * stack.
	 */
	// by jshwang - replaced to VMThrowable.fillInStackTrace
	// private static native Object nativeFillInStackTrace();

	/*
	 * Creates an array of StackTraceElement objects from the data held in
	 * "stackState".
	 */
	// by jshwang - replaced to VMThrowable.getStackTrace
	// private static native StackTraceElement[] nativeGetStackTrace(Object
	// stackState);
}

 ...
 * 
 * <a name="backing_array"><h3>Backing Arrays</h3></a> This class is implemented
 * using a char[]. The length of the array may exceed the length of the string.
 * For example, the string "Hello" may be backed by the array
 * {@code ['H', 'e', 'l', 'l', 'o', 'W'. 'o', 'r', 'l', 'd']} with offset 0 and
 * length 5.
 * 
 * <p>
 * Multiple strings can share the same char[] because strings are immutable. The
 * {@link #substring} method <strong>always</strong> returns a string that
...
public final class String implements Serializable, Comparable<String>,
	...
	public String(byte[] data, int start, int length) {
		...
		if (start >= 0 && 0 <= length && length <= data.length - start) {
			CharBuffer cb = Charset.defaultCharset().decode(
					ByteBuffer.wrap(data, start, length));
			count = cb.length();
			offset = 0;
			if (count > 0) {
				value = cb.array();
	 ...
	 * 
	 * <p>
	 * The behavior when the bytes cannot be decoded by the named charset is
	 * unspecified. Use {@link java.nio.charset.CharsetDecoder} for more
	 * control.
	 * 
	 * @param data
	 *            the byte array to convert to a string.
	 ...
	 * 
	 * <p>
	 * The behavior when the bytes cannot be decoded by the given charset is to
	 * replace malformed input and unmappable characters with the charset's
	 * default replacement string. Use {@link java.nio.charset.CharsetDecoder}
	 * for more control.
	 * 
	 * @param data
	 *            the byte array to convert to a String
	...
	public String(byte[] data, int start, int length, Charset charset) {
		...
		// We inline UTF-8, ISO-8859-1, and US-ASCII decoders for speed and
		// because 'count' and
		// 'value' are final.
		...
		if (canonicalCharsetName.equals("UTF-8")) {
			...
			outer: while (idx < last) {
				...
				if ((b0 & 0x80) == 0) {
					...
					// Reject chars greater than the Unicode maximum of
					// U+10FFFF.
					if (val > 0x10FFFF) {
						v[s++] = REPLACEMENT_CHAR;
						continue;
			...
			if (count > 0) {
				// We could use cb.array() directly, but that would mean we'd
				// have to trust
				// the CharsetDecoder doesn't hang on to the CharBuffer and
				// mutate it later,
				// which would break String's immutability guarantee. It would
				// also tend to
				// mean that we'd be wasting memory because CharsetDecoder
				// doesn't trim the
				// array. So we copy.
	 ...
	 * {@code toCopy}. The returned string's <a href="#backing_array">backing
	 * array</a> is no larger than necessary.
	 */
	public String(String toCopy) {
		value = (toCopy.value.length == toCopy.count) ? toCopy.value : Arrays
				.copyOfRange(toCopy.value, toCopy.offset, toCopy.offset
						+ toCopy.length());
		offset = 0;
		count = value.length;
	}
	...
	/**
	 * Returns a new byte array containing the characters of this string encoded
	 * using the system's {@link java.nio.charset.Charset#defaultCharset default
	 * charset}.
	 * 
	 * <p>
	 * The behavior when this string cannot be represented in the system's
	 * default charset is unspecified. In practice, when the default charset is
	 ...
	 * is unspecified. Use {@link java.nio.charset.CharsetEncoder} for more
	 * control.
	 * 
	 * @throws UnsupportedEncodingException
	 ...
	 * is to replace malformed input and unmappable characters with the
	 * charset's default replacement byte array. Use
	 * {@link java.nio.charset.CharsetEncoder} for more control.
	 * 
	 * @since 1.6
	 */
	public byte[] getBytes(Charset charset) {
		...
		if (canonicalCharsetName.equals("UTF-8")) {
			...
			CharBuffer chars = CharBuffer.wrap(this.value, this.offset,
					this.count);
	 ...
	 * within the java.lang package only. The caller is responsible for ensuring
	 * that 0 <= start && start <= end && end <= count.
	 */
	...
	public String replace(CharSequence target, CharSequence replacement) {
		...
		// The empty target matches at the start and end and between each
		// character.
	...
	/**
	 * Converts this string to lowercase, using the rules of the user's default
	 * locale. See "<a href="../util/Locale.html#default_locale
	 * ">Be wary of the default locale</a>".
	 * 
	 * @return a new lowercase string, or {@code this} if it's already
	 *         all-lowercase.
	 */
	public String toLowerCase() {
		return CaseMapper.toLowerCase(Locale.getDefault(), this, value, offset,
				count);
	}
	 ...
	 * 
	 * <p>
	 * Most case mappings are unaffected by the language of a {@code Locale}.
	 * Exceptions include dotted and dotless I in Azeri and Turkish locales, and
	 * dotted and dotless I and J in Lithuanian locales. On the other hand, it
	 * isn't necessary to provide a Greek locale to get correct case mapping of
	 * Greek characters: any locale will do.
	 * 
	 * <p>
	 * See <a
	 * href="http://www.unicode.org/Public/UNIDATA/SpecialCasing.txt">http
	 * ://www.unicode.org/Public/UNIDATA/SpecialCasing.txt</a> for full details
	 * of context- and language-specific special cases.
	 * 
	 * @return a new lowercase string, or {@code this} if it's already
	 *         all-lowercase.
	 */
	...
	/**
	 * Converts this this string to uppercase, using the rules of the user's
	 * default locale. See "<a href="../util/Locale.html#default_locale
	 * ">Be wary of the default locale</a>".
	 * 
	 * @return a new uppercase string, or {@code this} if it's already
	 *         all-uppercase.
	 */
	public String toUpperCase() {
		return CaseMapper.toUpperCase(Locale.getDefault(), this, value, offset,
				count);
	}
	...
	/**
	 * Tests whether this string matches the given {@code regularExpression}.
	 * This method returns true only if the regular expression matches the
	 * <i>entire</i> input string. A common mistake is to assume that this
	 * method behaves like {@link #contains}; if you want to match anywhere
	 * within the input string, you need to add {@code .*} to the beginning and
	 * end of your regular expression. See {@link Pattern#matches}.
	 * 
	 * <p>
	 * If the same regular expression is to be used for multiple operations, it
	 * may be more efficient to reuse a compiled {@code Pattern}.
	 * 
	 * @throws PatternSyntaxException
	 *             if the syntax of the supplied regular expression is not
	 *             valid.
	 * @throws NullPointerException
	 *             if {@code regularExpression == null}
	 * @since 1.4
	 */
	...
	/**
	 * Replaces all matches for {@code regularExpression} within this string
	 * with the given {@code replacement}. See {@link Pattern} for regular
	 * expression syntax.
	 * 
	 * <p>
	 * If the same regular expression is to be used for multiple operations, it
	...
	public String replaceAll(String regularExpression, String replacement) {
		return Pattern.compile(regularExpression).matcher(this)
				.replaceAll(replacement);
	}

	/**
	 * Replaces the first match for {@code regularExpression} within this string
	 * with the given {@code replacement}. See {@link Pattern} for regular
	 * expression syntax.
	 * 
	 * <p>
	 ...
	 * Equivalent to {@code split(regularExpression, 0)}. See
	 * {@link Pattern#split(CharSequence, int)} for an explanation of
	 * {@code limit}. See {@link Pattern} for regular expression syntax.
	 * 
	 * <p>
	 * If the same regular expression is to be used for multiple operations, it
	 * may be more efficient to reuse a compiled {@code Pattern}.
	...
	/**
	 * Splits this string using the supplied {@code regularExpression}. See
	 * {@link Pattern#split(CharSequence, int)} for an explanation of
	 * {@code limit}. See {@link Pattern} for regular expression syntax.
	 * 
	 * <p>
	...
	public String[] split(String regularExpression, int limit) {
		String[] result = java.util.regex.Splitter.fastSplit(regularExpression,
				this, limit);
	 ...
	 * @throws IndexOutOfBoundsException
	 *             if
	 *             {@code beginIndex < 0 || endIndex > length() || beginIndex > endIndex}
	...
	public int codePointCount(int beginIndex, int endIndex) {
		...
		return Character.codePointCount(value, offset + beginIndex, endIndex
				- beginIndex);
	}
	 ...
	 * 
	 * <p>
	 * If you're formatting a string other than for human consumption, you
	 * should use the {@code format(Locale, String, Object...)} overload and
	 * supply {@code Locale.US}. See

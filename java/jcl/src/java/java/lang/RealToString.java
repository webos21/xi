...
final class RealToString {
	...
	private static final double invLogOfTenBaseTwo = Math.log(2.0)
			/ Math.log(10.0);

	private int firstK;

	/**
	 * An array of decimal digits, filled by longDigitGenerator or
	...
	private String convertDouble(AbstractStringBuilder sb, double inputNumber) {
		...
		int p = Double.EXPONENT_BIAS + Double.MANTISSA_BITS; // the power offset
																// (precision)
		int pow;
	...
	public String convertFloat(AbstractStringBuilder sb, float inputNumber) {
		...
		int p = Float.EXPONENT_BIAS + Float.MANTISSA_BITS; // the power offset
															// (precision)
		int pow;
	...
	private void freeFormatExponential(AbstractStringBuilder sb,
			boolean positive) {
		int digitIndex = 0;
		if (!positive) {
			sb.append0('-');
	...
	private native void bigIntDigitGenerator(long f, int e,
			boolean isDenormalized, int p);
	...
	private void longDigitGenerator(long f, int e, boolean isDenormalized,
		...
		int U;
		while (true) {
			// Set U to floor(R/S) and R to the remainder, using *unsigned*
			// 64-bit division
			U = 0;
			for (int i = 3; i >= 0; i--) {

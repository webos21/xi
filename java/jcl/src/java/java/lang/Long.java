 ...
 * described in <a href="http://www.hackersdelight.org/">Henry S. Warren, Jr.'s
 * Hacker's Delight, (Addison Wesley, 2002)</a> and <a href=
 * "http://graphics.stanford.edu/~seander/bithacks.html">Sean Anderson's Bit
 * Twiddling Hacks.</a>
 * 
...
public final class Long extends Number implements Comparable<Long> {
	 ...
	 *             {@code radix > Character.MAX_RADIX}, or if {@code string} can
	...
	public static int numberOfTrailingZeros(long v) {
		int low = (int) v;
		return low != 0 ? Integer.numberOfTrailingZeros(low) : 32 + Integer
				.numberOfTrailingZeros((int) (v >>> 32));
	}
	...
	/*
	 * On a modern 64-bit processor with a fast hardware multiply, this is much
	 * faster (assuming you're running a 64-bit VM):
	 * 
	 * // http://chessprogramming.wikispaces.com/Population+Count int bitCount
	 * (long x) { x -= (x >>> 1) & 0x5555555555555555L; x = (x &
	 * 0x3333333333333333L) + ((x >>> 2) & 0x3333333333333333L); x = (x + (x >>>
	 * 4)) & 0x0f0f0f0f0f0f0f0fL; x = (x * 0x0101010101010101L) >>> 56; return
	 * (int) x; }
	 * 
	...
	public static long reverseBytes(long v) {
		...
		v = ((v >>> 8) & 0x00FF00FF00FF00FFL)
				| ((v & 0x00FF00FF00FF00FFL) << 8);
		v = ((v >>> 16) & 0x0000FFFF0000FFFFL)
				| ((v & 0x0000FFFF0000FFFFL) << 16);
		return ((v >>> 32)) | ((v) << 32);
	}
	...
	public static long reverse(long v) {
		...
		// http://graphics.stanford.edu/~seander/bithacks.html
		v = ((v >>> 1) & 0x5555555555555555L)
				| ((v & 0x5555555555555555L) << 1);
		v = ((v >>> 2) & 0x3333333333333333L)
				| ((v & 0x3333333333333333L) << 2);
		v = ((v >>> 4) & 0x0F0F0F0F0F0F0F0FL)
				| ((v & 0x0F0F0F0F0F0F0F0FL) << 4);
		v = ((v >>> 8) & 0x00FF00FF00FF00FFL)
				| ((v & 0x00FF00FF00FF00FFL) << 8);
		v = ((v >>> 16) & 0x0000FFFF0000FFFFL)
				| ((v & 0x0000FFFF0000FFFFL) << 16);
	...
	public static int signum(long v) {
		...
		// END android-changed
		// The following branch-free version is faster on modern
		// desktops/servers

...
public class Thread implements Runnable {
	...
	/**
	 * Holds the thread's ID. We simply count upwards, so each Thread has a
	 * unique ID.
	 */
	private long threadId; // by jshwang - renamed
	...
	/**
	 * Holds the handler for uncaught exceptions in this Thread, in case there
	 * is one.
	 */
	...
	/**
	 * Reflects whether this Thread has already been started. A Thread can only
	 * be started once (no recycling). Also, we need it to deduce the proper
	 * Thread status.
	 */
	 ...
	 * provided. The new {@code Thread} will belong to the same
	 * {@code ThreadGroup} as the {@code Thread} calling this constructor.
	 * 
	 * @param runnable
	 *            a {@code Runnable} whose method <code>run</code> will be
	 ...
	 * name provided. The new {@code Thread} will belong to the same
	 * {@code ThreadGroup} as the {@code Thread} calling this constructor.
	 * 
	 * @param threadName
	 *            the name for the {@code Thread} being created
	...
	/**
	 * Constructs a new {@code Thread} with a {@code Runnable} object and a
	 * newly generated name. The new {@code Thread} will belong to the
	 * {@code ThreadGroup} passed as parameter.
	 * 
	 * @param group
	...
	/**
	 * Package-scope method invoked by Dalvik VM to create "internal" threads or
	 * attach threads created externally.
	 * 
	 * Don't call Thread.currentThread(), since there may not be such a thing
	 * (e.g. for Main).
	 */
	...
	/**
	 * Initializes a new, existing Thread object with a runnable object, the
	 * given name and belonging to the ThreadGroup passed as parameter. This is
	 * the method that the several public constructors delegate their work to.
	 * 
	 * @param group
	 ...
	 * @see java.lang.SecurityManager
	 */
	private void create(ThreadGroup group, Runnable runnable,
		...
		if (smgr != null) {
			...
			/*
			 * Freaky security requirement: If the Thread's class is actually a
			 * subclass of Thread and it tries to override either
			...
			if (getClass() != Thread.class) {
				...
				try {
					getClass().getDeclaredMethod("getContextClassLoader",
							signature);
					smgr.checkPermission(new RuntimePermission(
				...
				try {
					getClass().getDeclaredMethod("setContextClassLoader",
							signature);
					smgr.checkPermission(new RuntimePermission(
							"enableContextClassLoaderOverride"));
				} catch (NoSuchMethodException ex) {
		...
		if (currentThread.inheritableValues != null) {
			inheritableValues = new ThreadLocal.Values(
					currentThread.inheritableValues);
		}

		// store current AccessControlContext as inherited context for this
		// thread
	...
	/**
	 * Returns the number of active {@code Thread}s in the running
	 * {@code Thread}'s group and its subgroups.
	 * 
	 ...
	 */
	/*
	 * by jshwang public static Map<Thread, StackTraceElement[]>
	 * getAllStackTraces() { SecurityManager securityManager =
	 * System.getSecurityManager(); if (securityManager != null) {
	 ...
	 * StackTraceElement[]>();
	 * 
	 * // Find out how many live threads we have. Allocate a bit more // space
	 * than needed, in case new ones are just being created. int count =
	 * ThreadGroup.mSystem.activeCount(); Thread[] threads = new Thread[count +
	 * count / 2];
	 * 
	 * // Enumerate the threads and collect the stacktraces. count =
	 * ThreadGroup.mSystem.enumerate(threads); for (int i = 0; i < count; i++) {
	 ...
	 * <li>the caller's class loader is not the same as the requested context
	...
	public ClassLoader getContextClassLoader() {
		...
		if (sm != null) {
			...
			ClassLoader calling = VMStackWalker.getCallingClassLoader();
	 ...
	 * 
	 * @return an {@link UncaughtExceptionHandler} or <code>null</code> if none
	 *         exists.
	 */
	 ...
	 */
	/*
	 * by jshwang public StackTraceElement[] getStackTrace() { SecurityManager
	 * securityManager = System.getSecurityManager(); if (securityManager !=
	 * null) { securityManager.checkPermission(new
	 * RuntimePermission("getStackTrace")); }
	 * 
	 * StackTraceElement ste[] = VMStack.getThreadStackTrace(this); return ste
	 * != null ? ste : new StackTraceElement[0]; }
	 */
	/**
	 ...
	 * daemon Thread (<code>true</code>) or not (<code>false</code>) A daemon
	 * Thread only runs as long as there are non-daemon Threads running. When
	 * the last non-daemon Thread ends, the whole program ends no matter if it
	 * had daemon Threads still running or not.
	 * 
	 ...
	 * pending interrupt request (<code>true</code>) or not ( <code>false</code>
	 * )
	 * 
	 * @return a <code>boolean</code> indicating the interrupt status
	 * @see Thread#interrupt
	 ...
	 * <p>
	 * The <code>RuntimePermission("setContextClassLoader")</code> is checked
	 * prior to setting the handler.
	 * </p>
	 * 
	 * @param cl
	...
	public final void setDaemon(boolean isDaemon) {
		...
		if (hasBeenStarted) {
			throw new IllegalThreadStateException("Thread already started."); // TODO
																				// Externalize?
		}
	...
	public final void setPriority(int priority) {
		...
		if (priority < Thread.MIN_PRIORITY || priority > Thread.MAX_PRIORITY) {
			throw new IllegalArgumentException("Priority out of range"); // TODO
																			// Externalize?
		}
	...
	/**
	 * Starts the new Thread of execution. The <code>run()</code> method of the
	 * receiver will be called by the receiver Thread itself (and not the Thread
	 ...
	 * <code>throwable()</code>. The Thread is resumed if it was suspended and
	 * awakened if it was sleeping, so that it can proceed to throw the
	 * <code>throwable()</code>.
	 * 
	 * @param throwable
	...
	/* package */void unpark() {
		...
		if (vmt == null) {
			/*
			 * vmThread is null before the thread is start()ed. In this case, we
			 * just go ahead and set the state to PREEMPTIVELY_UNPARKED. Since
			 * this happens before the thread is started, we don't have to worry
			 * about synchronizing with it.
			 */
		...
		synchronized (vmt) {
			...
			case ParkState.PREEMPTIVELY_UNPARKED: {
				/*
				 * Nothing to do in this case: By definition, a preemptively
				 * unparked thread is to remain in the preemptively unparked
				 * state if it is told to unpark.
				 */
				break;
			}
	 ...
	 * 
	 * @param nanos
	 *            number of nanoseconds to park for
	 */
		...
		synchronized (vmt) {
			...
			case ParkState.UNPARKED: {
				...
				try {
					 ...
					 * PREEMPTIVELY_UNPARKED before hitting this code, it should
					 * left in that state.
					 */
	 ...
	 * 
	 * @param time
	 *            absolute milliseconds since the epoch to park until
	 */
	/* package */void parkUntil(long time) {
		...
		synchronized (vmt) {
			/*
			 * Note: This conflates the two time bases of "wall clock" time and
			 * "monotonic uptime" time. However, given that the underlying
			 * system can only wait on monotonic time, it is unclear if there is
			 * any way to avoid the conflation. The downside here is that if,
			 * having calculated the delay, the wall clock gets moved ahead,
			 * this method may not return until well after the wall clock has
			 * reached the originally designated time. The reverse problem (the
			 * wall clock being turned back) isn't a big deal, since this method
			 * is allowed to spuriously return for any reason, and this
			 * situation can safely be construed as just such a spurious return.
			 */

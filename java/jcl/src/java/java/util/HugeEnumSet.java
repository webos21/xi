...
final class HugeEnumSet<E extends Enum<E>> extends EnumSet<E> {
	 ...
	 * 
	 * @param elementType
	 *            non-null; type of the elements
	 * @param enums
	 *            non-null; prepopulated array of constants in ordinal order
	 */
	...
	private class HugeEnumSetIterator implements Iterator<E> {
		...
		/**
		 * The bits yet to be returned for the long in bits[index]. As values
		 * from the current index are returned, their bits are zeroed out. When
		 * this reaches zero, the index must be incremented.
		 */
		...
		/**
		 * Assigns mask and index to the next available value, cycling
		 * currentBits as necessary.
		 */
		void computeNextElement() {
			while (true) {
				if (currentBits != 0) {
					mask = currentBits & -currentBits; // the lowest 1 bit in
														// currentBits
					return;
		...
		public E next() {
			...
			int ordinal = Long.numberOfTrailingZeros(mask) + index
					* BIT_IN_LONG;
			last = enums[ordinal];
	...
	public boolean addAll(Collection<? extends E> collection) {
		...
		if (collection instanceof EnumSet) {
			EnumSet<?> set = (EnumSet) collection; // raw type due to javac bug
													// 6548436

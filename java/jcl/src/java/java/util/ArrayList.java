 ...
 * optional operations including adding, removing, and replacing elements are
 * supported.
 * 
 * <p>
 ...
 * 
 * <p>
 * This class is a good choice as your default {@code List} implementation.
 * {@link Vector} synchronizes all operations, but not necessarily in a way
 * that's meaningful to your application: synchronizing each call to {@code get}
 * , for example, is not equivalent to synchronizing the list and iterating over
 * it (which is probably what you intended).
 * {@link java.util.concurrent.CopyOnWriteArrayList} is intended for the special
 * case of very high concurrency, frequent traversals, and very rare mutations.
 * 
 * @param <E>
...
public class ArrayList<E> extends AbstractList<E> implements Cloneable,
	 ...
	 * gives empirically good results and is arguably consistent with the RI's
	 * specified default initial capacity of 10: instead of 10, we start with 0
	 * (sans allocation) and jump to 12.
	 */
	...
	public <T> T[] toArray(T[] contents) {
		...
		if (contents.length < s) {
			@SuppressWarnings("unchecked")
			T[] newArray = (T[]) Array.newInstance(contents.getClass()
					.getComponentType(), s);
			contents = newArray;
		}
	...
	private void readObject(ObjectInputStream stream) throws IOException,
		...
		if (cap < size) {
			throw new InvalidObjectException("Capacity: " + cap + " < size: "
					+ size);
		}

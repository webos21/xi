...
public final class Currency implements Serializable {
	...
	private Currency(String currencyCode) {
		...
		// Ensure that we throw if the our currency code isn't an ISO currency
		// code.
		String symbol = ICU.getCurrencySymbolNative(Locale.US.toString(),
				currencyCode);
		if (symbol == null) {
			throw badCurrency(currencyCode);
		}

		this.defaultFractionDigits = ICU
				.getCurrencyFractionDigitsNative(currencyCode);
		if (defaultFractionDigits < 0) {
			// In practice, I don't think this can fail because ICU doesn't care
			// whether you give
			// it a valid country code, and will just return a sensible default
			// for the default
			// locale's currency.
			throw badCurrency(currencyCode);
		}
	...
	private IllegalArgumentException badCurrency(String currencyCode) {
		throw new IllegalArgumentException(
				"Not a supported ISO 4217 currency code: " + currencyCode);
	}
	...
	public static Currency getInstance(Locale locale) {
		...
		if (variant.length() > 0
				&& (variant.equals("EURO") || variant.equals("HK") || variant
						.equals("PREEURO"))) {
		...
		if (currencyCode == null) {
			throw new IllegalArgumentException(
					"Not a supported ISO 3166 country: " + locale);
	...
	/**
	 * Returns the localized currency symbol for this currency in the user's
	 ...
	 * . That is, given "USD" and Locale.US, you'd get "$", but given "USD" and
	 * a non-US locale, you'd get "US$".
	 * 
	 * <p>
	 * If the locale only specifies a language rather than a language and a
	 * country (such as {@code Locale.JAPANESE} or {new Locale("en", "")} rather
	 * than {@code Locale.JAPAN} or {new Locale("en", "US")}), the ISO 4217
	 * currency code is returned.
	 * 
	 * <p>
	 * If there is no locale-specific currency symbol, the ISO 4217 currency
	 * code is returned.
	 */
	public String getSymbol(Locale locale) {
		...
		String symbol = ICU.getCurrencySymbolNative(locale.toString(),
				currencyCode);
	...
	public int getDefaultFractionDigits() {
		// BEGIN android-changed
		// return
		// com.ibm.icu.util.Currency.getInstance(currencyCode).getDefaultFractionDigits();
	...
	// TODO: remove this in favor of direct access (and no ResourceBundle
	// cruft).

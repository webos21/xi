 ...
 * 
 * <p>
 * Each timer has one thread on which tasks are executed sequentially. When this
 * thread is busy running a task, runnable tasks may be subject to delays.
 * 
 * <p>
 ...
 * <ul>
 * <li>With the default <strong>fixed-period execution</strong>, each successive
 * run of a task is scheduled relative to the start time of the previous run, so
 * two runs are never fired closer together in time than the specified
 * {@code period}.
 * <li>With <strong>fixed-rate execution</strong>, the start time of each
 * successive run of a task is scheduled without regard for when the previous
 * run took place. This may result in a series of bunched-up runs (one launched
 * immediately after another) if delays prevent the timer from starting tasks on
 * time.
 * </ul>
 * 
 * <p>
...
public class Timer {
	...
	private void scheduleImpl(TimerTask task, long delay, long period,
		...
		synchronized (impl) {
			...
			if (when < 0) {
				throw new IllegalArgumentException(
						"Illegal delay to start the TimerTask: " + when);
			}

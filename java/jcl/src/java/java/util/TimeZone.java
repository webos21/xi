...
/**
 * {@code TimeZone} represents a time zone, primarily used for configuring a
 * {@link Calendar} or {@link java.text.SimpleDateFormat} instance.
 * 
 * <p>
 * Most applications will use {@link #getDefault} which returns a
 * {@code TimeZone} based on the time zone where the program is running.
 * 
 * <p>
 ...
 * methods yourself. Let classes like {@link Calendar} and
 * {@link java.text.SimpleDateFormat} do the date computations for you.
 * 
 * <p>
 * If you do need to do date computations manually, there are two common cases
 * to take into account:
 * <ul>
 * <li>Somewhere like California, where daylight time is used. The
 * {@link #useDaylightTime} method will always return true, and
 * {@link #inDaylightTime} must be used to determine whether or not daylight
 * time applies to a given {@code Date}. The {@link #getRawOffset} method will
 * return a raw offset of (in this case) -8 hours from UTC, which isn't usually
 * very useful. More usefully, the {@link #getOffset} methods return the actual
 * offset from UTC <i>for a given point in time</i>; this is the raw offset plus
 * (if the point in time is {@link #inDaylightTime in daylight time}) the
 * applicable {@link #getDSTSavings DST savings} (usually, but not necessarily,
 * 1 hour).
 * <li>Somewhere like Japan, where daylight time is not used. The
 * {@link #useDaylightTime} and {@link #inDaylightTime} methods both always
 * return false, and the raw and actual offsets will always be the same.
 * </ul>
 * 
 * <p>
 * Note the type returned by the factory methods {@link #getDefault} and
 * {@link #getTimeZone} is implementation dependent. This may introduce
 * serialization incompatibility issues between different implementations.
 * Android returns instances of {@link SimpleTimeZone} so that the bytes
 * serialized by Android can be deserialized successfully on other
...
public abstract class TimeZone implements Serializable, Cloneable {
	...
	static final TimeZone GMT = new SimpleTimeZone(0, "GMT"); // Greenwich Mean
																// Time
	 ...
	 * {@code offsetMillis}. Any of these IDs can be passed to
	 * {@link #getTimeZone} to lookup the corresponding time zone instance.
	 * 
	 ...
	 * 
	 * <p>
	 * Since the user's time zone changes dynamically, avoid caching this value.
	...
	/**
	 * Equivalent to
	 * {@code getDisplayName(false, TimeZone.LONG, Locale.getDefault())}. <a
	 * href="../util/Locale.html#default_locale">Be wary of the default
	 * locale</a>.
	 */
	...
	/**
	 * Equivalent to
	 * {@code getDisplayName(daylightTime, style, Locale.getDefault())}. <a
	 * href="../util/Locale.html#default_locale">Be wary of the default
	 * locale</a>.
	 */
	 ...
	 * @param style
	 *            either {@link TimeZone#LONG} or {@link TimeZone#SHORT}.
	 * @param locale
	 *            the display locale.
	 */
	public String getDisplayName(boolean daylightTime, int style, Locale locale) {
		...
		String result = TimeZones.getDisplayName(getID(), daylightTime, style,
				locale);
		if (result != null) {
			return result;
		}
	 ...
	 * {@code time}. The offset includes daylight savings time if the specified
	 ...
	 * date and time. The offset includes daylight savings time if the date and
	 * time is within the daylight savings time period.
	 * 
	 * <p>
	 * This method is intended to be used by {@link Calendar} to compute
	...
	/**
	 * Returns a {@code TimeZone} suitable for {@code id}, or {@code GMT} on
	 * failure.
	 * 
	 * <p>
	 * An id can be an Olson name of the form <i>Area</i>/<i>Location</i>, such
	 * as {@code America/Los_Angeles}. The {@link #getAvailableIDs} method
	 * returns the supported names.
	 * 
	 * <p>
	 * This method can also create a custom {@code TimeZone} using the following
	 * syntax: {@code GMT[+|-]hh[[:]mm]}. For example,
	 * {@code TimeZone.getTimeZone("GMT+14:00")} would return an object with a
	 * raw offset of +14 hours from UTC, and which does <i>not</i> use daylight
	 * savings. These are rarely useful, because they don't correspond to time
	 * zones actually in use.
	 * 
	 * <p>
	 * Other than the special cases "UTC" and "GMT" (which are synonymous in
	 * this context, both corresponding to UTC), Android does not support the
	 * deprecated three-letter time zone IDs used in Java 1.1.
	 */
	...
	/**
	 * Returns a new SimpleTimeZone for an id of the form "GMT[+|-]hh[[:]mm]",
	 * or null.
	 */
	 ...
	 * 
	 * <p>
	 * The base implementation returns true if both time zones have the same raw
	 * offset.
	 */
	 ...
	 * 
	 * <p>
	 * <strong>Warning</strong>: avoid using this method to use a custom time
	 ...
	 * 
	 * @param timeZone
	 *            a custom time zone, or {@code null} to set the default to the

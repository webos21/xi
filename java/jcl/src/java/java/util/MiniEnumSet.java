...
final class MiniEnumSet<E extends Enum<E>> extends EnumSet<E> {
	...
	private class MiniEnumSetIterator implements Iterator<E> {
		 ...
		 * index are returned, their bits are zeroed out.
		 */
		 ...
		 */
		private long mask = currentBits & -currentBits; // the lowest 1 bit in
														// currentBits
	...
	public boolean containsAll(Collection<?> collection) {
		...
		if (collection instanceof MiniEnumSet) {
			...
			return isValidType(set.elementClass)
					&& ((bits & setBits) == setBits);
		}

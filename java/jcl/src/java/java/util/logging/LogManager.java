 ...
 * {@code Logger} objects.
 * <p>
 * There is only one global {@code LogManager} instance in the application,
 * which can be get by calling static method {@link #getLogManager()}. This
 * instance is created and initialized during class initialization and cannot be
 * changed.
 * <p>
 * The {@code LogManager} class can be specified by java.util.logging.manager
 * system property, if the property is unavailable or invalid, the default class
 * {@link java.util.logging.LogManager} will be used.
 * <p>
 ...
 * {@code LogManager} initialization, so that this object's default constructor
 * can read the initial configuration and define properties for
 * {@code LogManager}.
 * <p>
 ...
 * properties file. The {@code LogManager} will read initial configuration from
 * this file.
 * <p>
 * If neither of these properties is defined, or some exception is thrown during
 * these two properties using, the {@code LogManager} will read its initial
 * configuration from default properties file, as described above.
 * <p>
 ...
 * {@code Handler} and each must have a default constructor, these classes will
 * be loaded, instantiated and registered as handlers on the root {@code Logger}
 * (the {@code Logger} named ""). These {@code Handler}s maybe initialized
 * lazily.</li>
 ...
 * <p>
 * In the {@code LogManager}'s hierarchical namespace, {@code Loggers} are
 * organized based on their dot separated names. For example, "x.y.z" is child
 * of "x.y".
 * <p>
 * Levels for {@code Loggers} can be defined by properties whose name end with
 * ".level". Thus "alogger.level" defines a level for the logger named as
...
public class LogManager {
	...
	static {
		...
		AccessController.doPrivileged(new PrivilegedAction<Object>() {
			public Object run() {
				...
				Logger.getLogger(Logger.GLOBAL_LOGGER_NAME).setParent(root);

				manager.addLogger(root);
				manager.addLogger(Logger.getLogger(Logger.GLOBAL_LOGGER_NAME));
				return null;
			}
		});
	}
	 ...
	 * {@code LogManager.getLogManager()}. This is protected so that application
	...
	/**
	 * Check that the caller has {@code LoggingPermission("control")} so that it
	 * is trusted to modify the configuration for logging framework. If the
	 * check passes, just return, otherwise {@code SecurityException} will be
	 * thrown.
	 * 
	 ...
	 * unexpectedly garbage collected it is necessary for <i>applications</i> to
	...
	public void readConfiguration() throws IOException {
		...
		if (null == configClassName
			...
			if (null == configFile) {
				...
				configFile = System.getProperty("java.home") + File.separator
			...
			try {
				...
				try {
					...
					input = new BufferedInputStream(getClass()
	...
	/**
	 * Add a {@code PropertyChangeListener}, which will be invoked when the
	 * properties are reread.
	 * 
	 * @param l
	...
	/**
	 * Remove a {@code PropertyChangeListener}, do nothing if the given listener
	 * is not found.
	 * 
	 * @param l
	 ...
	 *            the resource bundle to associate, or null for no associated
	 *            resource bundle.
	 */
	 ...
	 * 
	 * @param newLevel
	 *            the new minimum logging threshold. If null, the logger's
	 *            parent level will be used; or {@code Level.INFO} for loggers
	 *            with no parent.
	 */
	synchronized void setLevelRecursively(Logger logger, Level newLevel) {
		...
		if (newLevel == null) {
			logger.levelIntVal = logger.parent != null ? logger.parent.levelIntVal
					: Level.INFO.intValue();
		} else {

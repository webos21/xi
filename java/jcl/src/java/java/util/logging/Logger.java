...
import org.apache.harmony.luni.util.DalvikLogHandler;
import org.apache.harmony.luni.util.DalvikLogging;
...
/**
 * Loggers are used to log records to a variety of destinations such as log
 * files or the console. They use instances of {@link Handler} to actually do
 * the destination-specific operations.
 * 
 * <p>
 ...
 * usually the Java package naming convention. Anonymous loggers do not belong
 * to any namespace.
 * 
 * <p>
 * Developers should use named loggers to enable logging to be controlled on a
 * per-{@code Logger} granularity. The recommended idiom is to create and assign
 * the logger to a {@code static final} field. This ensures that there's always
 * a strong reference to the logger, preventing it from being garbage collected.
 * In particular, {@link LogManager#addLogger(Logger)} will <i>not</i> keep your
 * logger live.
 * 
 * <p>
 ...
 * context {@code ClassLoader}. If that fails, it tries the system
 * {@code ClassLoader}. And if that still fails, it searches up the class stack
 * and uses each class's {@code ClassLoader} to try to locate the resource
 * bundle.
 * <p>
...
public class Logger {
	...
	private static final DalvikLogHandler GENERAL_LOG_HANDLER = new DalvikLogHandler() {
		public void publish(Logger source, String tag, Level level,
				String message) {
	...
	/**
	 * The name of the global logger. Before using this, see the discussion of
	 * how to use {@code Logger} in the class documentation.
	 * 
	 * @since 1.6
	 */
	 ...
	 * 
	 * @deprecated deadlock-prone. Use
	 *             {@code Logger.getLogger(Logger.GLOBAL_LOGGER_NAME)} as a
	 *             direct replacement, but see the discussion of how to use
	 ...
	 * </ol>
	 */
	volatile int levelIntVal = Level.FINEST.intValue();
	...
	/**
	 * Child loggers. Should be accessed only while synchronized on
	 * {@code LogManager.getLogManager()}.
	 */
	 ...
	 * 
	 * <p>
	 * This optimization has been measured to show a 2.75x improvement in
	 ...
	 * 
	 * <p>
	 * Each time the handler configuration changes, either directly or
		...
		if (getClass() != Logger.class) {
			...
			if (h.hasNext()) {
				...
				if (!h.hasNext() && firstHandler instanceof DalvikLogHandler) {
					 ...
					 * satisfied these constraints: 1. This is not a subclass of
					 * logger 2. This is a root logger (no parent) 3. There is
					 * exactly one handler installed 4. That handler is a
					 * DalvikLogHandler
					 */
			 ...
			 * logger is eligible. We've satisfied these constraints: 1. This is
			 * not a subclass of logger 2. our parent exists 3. we have no
			 * handlers of our own 4. we notify our parent's handlers
			 */
	 ...
	 * <p>
	 * Notice : Loggers use a naming hierarchy. Thus "z.x.y" is a child of
	 * "z.x".
	 * 
	 * @param name
	 ...
	 * 
	 * @param resourceBundleName
	 *            the name of the resource bundle to load, cannot be
	 *            {@code null}.
	 ...
	 *             if this logger's resource bundle already exists and is
	 *             different from the resource bundle specified.
	 */
	private synchronized void initResourceBundle(String resourceBundleName) {
		...
		if (current != null) {
			if (current.equals(resourceBundleName)) {
				...
				throw new IllegalArgumentException("Resource bundle name '"
						+ resourceBundleName
						+ "' is inconsistent with the existing '" + current
						+ "'");
			}
		}
	...
	public static Logger getLogger(String name, String resourceBundleName) {
		Logger result = LogManager.getLogManager().getOrCreate(name,
				resourceBundleName);
	...
	void setManager(LogManager manager) {
		...
		if (levelProperty != null) {
			try {
				manager.setLevelRecursively(Logger.this,
						Level.parse(levelProperty));
			} catch (IllegalArgumentException invalidLevel) {
				invalidLevel.printStackTrace();
			}
		}

		String handlersPropertyName = name.isEmpty() ? "handlers" : name
				+ ".handlers";
	 ...
	 * messages. If the value is {@code null}, the parent's resource bundle will
	 * be inherited.
	 * 
	 ...
	 * localize logging messages. If the value is {@code null}, the parent's
	 * resource bundle name will be inherited.
	 * 
	...
	/**
	 * Logs a message of level {@code Level.WARNING}; the message is transmitted
	 * to all subscribed handlers.
	 * 
	 * @param msg
	 *            the message to log.
	...
	/**
	 * Logs a message of level {@code Level.INFO}; the message is transmitted to
	...
	/**
	 * Logs a message of level {@code Level.FINE}; the message is transmitted to
	 * all subscribed handlers.
	 * 
	 * @param msg
	 *            the message to log.
	 ...
	 * handlers for logging. If {@code getUseParentHandlers()} returns
	 * {@code true}, the log record will also be submitted to the handlers of
	 * this logger's parent, potentially recursively up the namespace.
	 * <p>
	 ...
	 * message. If {@code bundleName} is null, the empty string or not valid
	 * then the message is not localized.
	 * 
	 * @param logLevel
	 *            the level of the given message.
	 * @param sourceClass
	 ...
	 * localize the message. If {@code bundleName} is null, the empty string or
	 * not valid then the message is not localized.
	 * 
	 * @param logLevel
	 *            the level of the given message.
	 * @param sourceClass

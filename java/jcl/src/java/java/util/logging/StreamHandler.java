...
public class StreamHandler extends Handler {
	...
	/**
	 * Constructs a {@code StreamHandler} object. The new stream handler does
	 * not have an associated output stream.
	 */
	...
	private void write(String s) {
		try {
			...
			getErrorManager().error(
					"Exception occurred when writing to the output stream", e,
	...
	void close(boolean closeStream) {
		if (null != this.os) {
			...
			try {
				...
				getErrorManager().error(
						"Exception occurred when closing the output stream", e,
	...
	public void flush() {
		if (null != this.os) {
			try {
				...
				getErrorManager().error(
						"Exception occurred when flushing the output stream",
		...
		try {
			if (this.isLoggable(record)) {
				...
				try {
					...
					getErrorManager()
							.error("Exception occurred when formatting the log record",
			...
			getErrorManager().error(
					"Exception occurred when logging the record", e,

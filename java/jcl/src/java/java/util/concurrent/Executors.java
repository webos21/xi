...
public class Executors {
	 ...
	 * be the same as the thread invoking this <tt>privilegedThreadFactory</tt>
	 * method. A new <tt>privilegedThreadFactory</tt> can be created within an
	 ...
	 * initialize worker threads to have the same InheritableThreadLocal
	 * settings as some other designated thread, you can create a custom
	 * ThreadFactory in which that thread waits for and services requests to
	 * create others that will inherit its values.
	 * 
	 * @return a thread factory
	 ...
	 * given <tt>callable</tt> under the current access control context. This
	 * method should normally be invoked within an
	 * {@link AccessController#doPrivileged} action to create callables that
	 * will, if possible, execute under the selected permission settings holding
	 * within that action; or if not possible, throw an associated
	...
	/**
	 * Returns a {@link Callable} object that will, when called, execute the
	 * given <tt>callable</tt> under the current access control context, with
	 * the current context class loader as the context class loader. This method
		...
		PrivilegedCallableUsingCurrentClassLoader(Callable<T> task) {
			...
			if (sm != null) {
				...
				sm.checkPermission(new RuntimePermission(
						"getContextClassLoader")); // android-changed

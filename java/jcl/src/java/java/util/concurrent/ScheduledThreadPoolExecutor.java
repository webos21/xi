 ...
 * Such a cancelled task is not automatically removed from the work queue until
 * its delay elapses. While this enables further inspection and monitoring, it
 * may also cause unbounded retention of cancelled tasks.
 * 
 * <p>
 * Successive executions of a task scheduled via
 * <code>scheduleAtFixedRate</code> or <code>scheduleWithFixedDelay</code> do
 * not overlap. While different executions may be performed by different
 * threads, the effects of prior executions <a

 ...
 * 
 * <p>
 * A <tt>CyclicBarrier</tt> supports an optional {@link Runnable} command that
 * is run once per barrier point, after the last thread in the party arrives,
 * but before any threads are released. This <em>barrier action</em> is useful
 ...
 * the merger determines that a solution has been found then <tt>done()</tt>
 * will return <tt>true</tt> and each worker will terminate.
 * 
 * <p>
 ...
 * 
 * <p>
 * The <tt>CyclicBarrier</tt> uses an all-or-none breakage model for failed
 ...
 * that are part of the barrier action, which in turn <i>happen-before</i>
 * actions following a successful return from the corresponding {@code await()}
 * in other threads.
 * 
 * @since 1.5
 * @see CountDownLatch
...
public class CyclicBarrier {
	 ...
	 * <tt>count</tt> applies) and all the rest are either broken or tripped.
	...
	/**
	 * Creates a new <tt>CyclicBarrier</tt> that will trip when the given number

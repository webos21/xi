 ...
 * 
 * <p>
 * An <tt>ExecutorService</tt> can be shut down, which will cause it to reject
 * new tasks. Two different methods are provided for shutting down an
 * <tt>ExecutorService</tt>. The {@link #shutdown} method will allow previously
 ...
 * unused <tt>ExecutorService</tt> should be shut down to allow reclamation of
 * its resources.
 * 
 * <p>
 * Method <tt>submit</tt> extends base method {@link Executor#execute} by
 * creating and returning a {@link Future} that can be used to cancel execution
 * and/or wait for completion. Methods <tt>invokeAny</tt> and <tt>invokeAll</tt>
 * perform the most commonly useful forms of bulk execution, executing a
 * collection of tasks and then waiting for at least one, or all, to complete.
 * (Class {@link ExecutorCompletionService} can be used to write customized
 ...
 * first by calling <tt>shutdown</tt> to reject incoming tasks, and then calling
 * <tt>shutdownNow</tt>, if necessary, to cancel any lingering tasks:
 * 
 * <pre>
 ...
 * {@code Runnable} or {@code Callable} task to an {@code ExecutorService} <a
 * href="package-summary.html#MemoryVisibility"><i>happen-before</i></a> any
...
public interface ExecutorService extends Executor {
	 ...
	 * <tt>true</tt> for each element of the returned list. Note that a
	 ...
	 * happens first. {@link Future#isDone} is <tt>true</tt> for each element of

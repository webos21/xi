...
public class CopyOnWriteArrayList<E> implements List<E>, RandomAccess,
	 ...
	 * 
	 * @param c
	 *            the collection the elements of which are to be copied into the
	 *            new instance.
	 */
	 ...
	 * 
	 * @param array
	 *            the array the elements of which are to be copied into the new
	 *            instance.
	 */
	 ...
	 * 
	 * @param c
	 *            the collection from which the potential new elements are
	 *            taken.
	 * 
	...
	public Object clone() {
		try {
			CopyOnWriteArrayList thisClone = (CopyOnWriteArrayList) super
					.clone();
			thisClone.setData(this.getData());
			return thisClone;
		} catch (CloneNotSupportedException e) {
			throw new RuntimeException(
					"CloneNotSupportedException is not expected here");
		}
	}
	...
	/**
	 * Removes from the specified range of this list all the elements that are
	...
	/**
	 * Retains only the elements in the specified range of this list that are
	 * contained in the specified collection
	 * 
	 * @return Returns the number of removed elements
	 */
	...
	/**
	 * Returns an array containing all of the elements in the specified range of
	 * the array in proper sequence
	 */
	...
	/**
	 * Returns an array containing all of the elements in the specified range of
	 * the array in proper sequence, stores the result in the array, specified
	 * by first parameter (as for public instance method toArray(Object[] to)
	 */
	 ...
	 * @param data
	 *            array where to search the elements
	 * @param start
	 *            start index
	 * @param size
	 *            size of the range
	...
	/**
	 * Throws <code>IndexOutOfBoundsException</code> if <code>index</code> is
	 * out of the list bounds.
	 * 
	 * @param index
	 *            element index to check.
	 */
	...
	/**
	 * Throws <code>IndexOutOfBoundsException</code> if <code>index</code> is
	 * out of the list bounds. Excluding the last element.
	 * 
	 * @param index
	 *            element index to check.
	 */
	...
	/**
	 * list iterator implementation, when created gets snapshot of the list, so
	...
	private static class ListIteratorImpl implements ListIterator {
		...
		public Object next() {
			...
			throw new NoSuchElementException("pos is " + current + ", size is "
					+ size);
		}
		...
		public Object previous() {
			...
			throw new NoSuchElementException("pos is " + (current - 1)
					+ ", size is " + size);
		}
	...
	/**
	 * Keeps a state of sublist implementation, size and array declared as
	 * final, so we'll never get the unconsistent state
	 */
	...
	static class SubList extends AbstractList {
		 ...
		 * modified in the other way other than via this subList
		 * <p/>
		 * Should be called under lock!
		 */
		...
		public boolean addAll(Collection c) {
			...
			try {
				...
				read = new SubListReadData(read.size + (list.size() - d),
						list.getData());
				return true;
			} finally {
				list.lock.unlock();
		...
		public int lastIndexOf(Object o) {
			...
			int ind = CopyOnWriteArrayList
					.lastIndexOf(o, b.data, start, b.size) - start;
			return ind < 0 ? -1 : ind;
		}
		...
		public boolean removeAll(Collection c) {
			...
			try {
				...
				if (removed > 0) {
					read = new SubListReadData(read.size - removed,
							list.getData());
					return true;
				}
			} finally {
		 ...
		 * <code>SubList</code> gets a snapshot of the sublist, never throws
		...
		private class SubListIterator extends ListIteratorImpl {
			 ...
			 * 
			 * @param index
			 *            index of the first element to iterate.
			 */
	 ...
	 * 
	 * @param oos
	 *            ObjectOutputStream to write object to.
	 * @throws IOException
	 ...
	 * 
	 * @param ois
	 *            ObjectInputStream to read object from.
	 * @throws IOException
	 *             if an I/O error occur.
	 */

 ...
 * {@code null} elements. because some null arguments and return values cannot
 * be reliably distinguished from the absence of elements. Arbitrarily, the
 * {@link Collection#remove} method is mapped to {@code removeFirstOccurrence},
 * and {@link Collection#add} is mapped to {@code addLast}.
 * 
 * <p>
 ...
 * 
 * <p>
 * This class is {@code Serializable}, but relies on default serialization
 * mechanisms. Usually, it is a better idea for any serializable class using a
 * {@code ConcurrentLinkedDeque} to instead serialize a snapshot of the elements
 * obtained by method {@code toArray}.
 * 
 * @hide
 * 
 * @author Doug Lea
...
public class ConcurrentLinkedDeque<E> extends AbstractCollection<E> implements
	 ...
	 * interior removes but not interior insertions, as required to fully
	 * support the Deque interface.
	 * 
	 * We extend the techniques developed for ConcurrentLinkedQueue and
	 * LinkedTransferQueue (see the internal docs for those classes).
	 * 
	 * At any time, there is precisely one "first" active node with a null prev
	 * pointer. Similarly there is one "last" active node with a null next
	 * pointer. New nodes are simply enqueued by null-CASing.
	 * 
	 * A node p is considered "active" if it either contains an element, or is
	 * an end node and neither next nor prev pointers are self-links:
	 * 
	 * p.item != null || (p.prev == null && p.next != p) || (p.next == null &&
	 * p.prev != p)
	 * 
	 * The head and tail pointers are only approximations to the start and end
	 * of the deque. The first node can always be found by following prev
	 * pointers from head; likewise for tail. However, head and tail may be
	 * pointing at deleted nodes that have been unlinked and so may not be
	 * reachable from any live node.
	 * 
	 * There are 3 levels of node deletion: - logical deletion atomically
	 * removes the element - "unlinking" makes a deleted node unreachable from
	 * active nodes, and thus eventually reclaimable by GC - "gc-unlinking"
	 * further does the reverse of making active nodes unreachable from deleted
	 * nodes, making it easier for the GC to reclaim future deleted nodes
	 * 
	 * TODO: find a better name for "gc-unlinked"
	 * 
	 * Logical deletion of a node simply involves CASing its element to null.
	 * Physical deletion is merely an optimization (albeit a critical one), and
	 * can be performed at our convenience. At any time, the set of
	 * non-logically-deleted nodes maintained by prev and next links are
	 * identical, that is the live elements found via next links from the first
	 * node is equal to the elements found via prev links from the last node.
	 * However, this is not true for nodes that have already been logically
	 * deleted - such nodes may only be reachable in one direction.
	 * 
	 * When a node is dequeued at either end, e.g. via poll(), we would like to
	 * break any references from the node to live nodes, to stop old garbage
	 * from causing retention of new garbage with a generational or conservative
	 * GC. We develop further the self-linking trick that was very effective in
	 * other concurrent collection classes. The idea is to replace prev and next
	 * pointers to active nodes with special values that are interpreted to mean
	 * off-the-list-at-one-end. These are approximations, but good enough to
	 * preserve the properties we want in our traversals, e.g. we guarantee that
	 * a traversal will never hit the same element twice, but we don't guarantee
	 * whether a traversal that runs out of elements will be able to see more
	 * elements later after more elements are added at that end. Doing
	 * gc-unlinking safely is particularly tricky, since any node can be in use
	 * indefinitely (for example by an iterator). We must make sure that the
	 * nodes pointed at by head/tail do not get gc-unlinked, since head/tail are
	 * needed to get "back on track" by other nodes that are gc-unlinked.
	 ...
	 * first node, and p.next == null && p.prev == p means that the iteration is
	 * at an end and that p is a (final static) dummy node, NEXT_TERMINATOR, and
	 * not the last active node. Finishing the iteration when encountering such
	 * a TERMINATOR is good enough for read-only traversals. When the last
	 * active node is desired, for example when enqueueing, goto tail and
	 * continue traversal.
	 * 
	 ...
	 * ("forward" direction).
	 * 
	 * There is one desirable property we would like to have, but don't: it is
	 * possible, when an addFirst(A) is racing with pollFirst() removing B, for
	 * an iterating observer to see A B C and subsequently see A C, even though
	 * no interior removes are ever performed. I believe this wart can only be
	 * removed at significant runtime cost.
	 * 
	...
	/**
	 * A node from which the first node on list (that is, the unique node with
	 * node.prev == null) can be reached in O(1) time. Invariants: - the first
	 * node is always O(1) reachable from head via prev links - all live nodes
	 * are reachable from the first node via succ() - head != null - (tmp =
	 * head).next != tmp || tmp != head Non-invariants: - head.item may or may
	 * not be null - head may not be reachable from the first or last node, or
	 * from tail
	 */
	 ...
	 * node is always O(1) reachable from tail via next links - all live nodes
	 * are reachable from the last node via pred() - tail != null
	 * Non-invariants: - tail.item may or may not be null - tail may not be
	 * reachable from the first or last node, or from head
	 */
	...
	static final class Node<E> {
		...
		private static final sun.misc.Unsafe UNSAFE = sun.misc.Unsafe
				.getUnsafe();
		private static final long prevOffset = objectFieldOffset(UNSAFE,
				"prev", Node.class);
		private static final long itemOffset = objectFieldOffset(UNSAFE,
				"item", Node.class);
		private static final long nextOffset = objectFieldOffset(UNSAFE,
				"next", Node.class);
	}
	...
	private final static ThreadLocal<Count> tlc = new ThreadLocal<Count>() {
			...
			if (p.item != null || (q = p.prev) == null) {
				if (hops >= HOPS) {
					...
					if (last.casPrev(prev, p)) {
						...
						if (// shouldGCUnlinkOccasionally() &&
						last.next == null && (p.prev == null || p.item != null)
								&& p.next == last) {
	...
	/**
	 * Returns the first node, the unique node which has a null prev link. The
	 * returned node may or may not be logically deleted. Guarantees that head
	 * is set to the returned node.
	 */
	Node<E> first() {
		retry: for (;;) {
			for (Node<E> h = head, p = h;;) {
	...
	/**
	 * Returns the last node, the unique node which has a null next link. The
	 * returned node may or may not be logically deleted. Guarantees that tail
	 * is set to the returned node.
	 */
	Node<E> last() {
		retry: for (;;) {
	...
	/**
	 * Inserts the specified element at the end of this deque. This is identical
	 * in function to the {@code add} method.
	 * 
	 * @throws NullPointerException
	 *             {@inheritDoc}
	 */
	 ...
	 * @throws NullPointerException
	 *             if {@code c} or any element within it is {@code null}
	 */
	...
	private static final long headOffset = objectFieldOffset(UNSAFE, "head",
			ConcurrentLinkedDeque.class);
	private static final long tailOffset = objectFieldOffset(UNSAFE, "tail",
			ConcurrentLinkedDeque.class);

 ...
 * share access to a common collection. This queue does not permit {@code null}
 * elements.
 * 
 * <p>
...
public class ConcurrentLinkedQueue<E> extends AbstractQueue<E> implements
	 ...
	 * every time one could is a significant optimization (fewer CASes). This is
	 * controlled by local "hops" variables that only trigger helping-CASes
	 * after experiencing multiple lags.
	 * 
	 ...
	 * volatile write to item by using lazySet instead of a normal write. This
	 * allows the cost of enqueue to be "one-and-a-half" CASes.
	 * 
	...
	/**
	 * We don't bother to update head or tail pointers if fewer than HOPS links
	 * from "true" location. We assume that volatile writes are significantly
	 * more expensive than volatile reads.
	 */
	private static final int HOPS = 1;

	/**
	 * Try to CAS head to p. If successful, repoint old head to itself as

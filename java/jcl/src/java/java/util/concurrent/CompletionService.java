 ...
 * consumption of the results of completed tasks. Producers <tt>submit</tt>
 * tasks for execution. Consumers <tt>take</tt> completed tasks and process
 * their results in the order they complete. A <tt>CompletionService</tt> can
 * for example be used to manage asynchronous IO, in which tasks that perform
 ...
 * 
 * <p>
 * Typically, a <tt>CompletionService</tt> relies on a separate {@link Executor}
 * to actually execute the tasks, in which case the <tt>CompletionService</tt>
 ...
 * a {@code CompletionService} <a
 * href="package-summary.html#MemoryVisibility"><i>happen-before</i></a> actions
 * taken by that task, which in turn <i>happen-before</i> actions following a
...
public interface CompletionService<V> {
	 ...
	 *         <tt>get()</tt> method will return the given result value upon
	 *         completion
	 * @throws RejectedExecutionException
	 *             if the task cannot be scheduled for execution
	 * @throws NullPointerException
	 ...
	 * 
	 * @return the Future representing the next completed task, or <tt>null</tt>
	 *         if none are present
	 */
	Future<V> poll();

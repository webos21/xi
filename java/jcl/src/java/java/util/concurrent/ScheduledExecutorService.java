 ...
 * 
 * <p>
 * The <tt>schedule</tt> methods create tasks with various delays and return a
 * task object that can be used to cancel or check execution. The
 * <tt>scheduleAtFixedRate</tt> and <tt>scheduleWithFixedDelay</tt> methods
 * create and execute tasks that run periodically until cancelled.
 * 
 * <p>
 * Commands submitted using the {@link Executor#execute} and
 * {@link ExecutorService} <tt>submit</tt> methods are scheduled with a
 * requested delay of zero. Zero and negative delays (but not periods) are also
 * allowed in <tt>schedule</tt> methods, and are treated as requests for
 * immediate execution.
 * 
 * <p>
 * All <tt>schedule</tt> methods accept <em>relative</em> delays and periods as
 ...
 * For example, to schedule at a certain future <tt>date</tt>, you can use:
 * <tt>schedule(task,
 * date.getTime() - System.currentTimeMillis(),
 * TimeUnit.MILLISECONDS)</tt>. Beware however that expiration of a relative
 * delay need not coincide with the current <tt>Date</tt> at which the task is
 * enabled due to network time synchronization protocols, clock drift, or other
 * factors.
 * 

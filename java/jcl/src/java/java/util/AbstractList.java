 ...
 * implementation does not support adding or replacing. A subclass must
 * implement the abstract methods {@code get()} and {@code size()}, and to
 * create a modifiable {@code List} it's necessary to override the {@code add()}
...
public abstract class AbstractList<E> extends AbstractCollection<E> implements
	...
	private static class SubAbstractList<E> extends AbstractList<E> {
		...
		public ListIterator<E> listIterator(int location) {
			if (modCount == fullList.modCount) {
				if (0 <= location && location <= size) {
					return new SubAbstractListIterator<E>(
							fullList.listIterator(location + offset), this,
							offset, size);
				}
				throw new IndexOutOfBoundsException();
			}

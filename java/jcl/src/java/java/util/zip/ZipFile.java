...
public class ZipFile implements ZipConstants {
	/**
	 * General Purpose Bit Flags, Bit 3. If this bit is set, the fields crc-32,
	 * compressed size and uncompressed size are set to zero in the local
	 * header. The correct values are put in the data descriptor immediately
	 * following the compressed data. (Note: PKZIP version 2.04g for DOS only
	 * recognizes this bit for method 8 compression, newer versions of PKZIP
	 * recognize this bit for any compression method.)
	 */
	static final int GPBF_DATA_DESCRIPTOR_FLAG = 1 << 3; // android-added

	/**
	 * General Purpose Bit Flags, Bit 11. Language encoding flag (EFS). If this
	 * bit is set, the filename and comment fields for this file must be encoded
	 * using UTF-8.
	 */
	...
	public InputStream getInputStream(ZipEntry entry) throws IOException {
		/*
		 * Make sure this ZipEntry is in this Zip file. We run it through the
		 * name lookup.
		 */
	 ...
	 * 
	 * <p>
	 * The central directory can be followed by a variable-length comment field,
	 * so we have to scan through it backwards. The comment is at most 64K, plus
	 * we have 18 bytes for the end-of-central-dir stuff itself, plus apparently
	 * sometimes people throw random junk on the end just for the fun of it.
	 * 
	 * <p>
	 * This is all a little wobbly. If the wrong value ends up in the EOCD area,
	 * we're hosed. This appears to be the way that everybody handles it though,
	 * so we're in good company if this fails.
	 */
	private void readCentralDir() throws IOException {
		/*
		 * Scan back, looking for the End Of Central Directory field. If the
		 * archive doesn't have a comment, we'll hit it on the first try.
		 * 
		 * No need to synchronize mRaf here -- we only do this when we first
		 * open the Zip file.
		 */
		 ...
		 * 
		 * For performance we want to use buffered I/O when reading the file. We
		 * wrap a buffered stream around the random-access file object. If we
		 * just read from the RandomAccessFile we'll be doing a read() system
		 * call every time.
		 */
	...
	static class ZipInflaterInputStream extends InflaterInputStream {
		...
		public int available() throws IOException {
			if (closed) {
				// Our superclass will throw an exception, but there's a jtreg
				// test that

...
public class DeflaterOutputStream extends FilterOutputStream {
	...
	/**
	 * This is the most basic constructor. You only need to pass the
	 * {@code OutputStream} to which the compressed data shall be written to.
	 * The default settings for the {@code Deflater} and internal buffer are
	 * used. In particular the {@code Deflater} produces a ZLIB header in the
	 * output stream.
	 * 
	 * @param os
	 ...
	 * 
	 * <p>
	 * For deflater output streams constructed with Java 7's {@code syncFlush}
	 * parameter set to true (not yet available on Android), this first flushes
	 * all outstanding data so that it may be immediately read by its recipient.
	 * Doing so may degrade compression.
	 */
	@Override

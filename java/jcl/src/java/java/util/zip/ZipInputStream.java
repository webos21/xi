 ...
 * the so called ZIP entries. Therefore when reading from a
 * {@code ZipInputStream} first the entry's attributes will be retrieved with
 * {@code getNextEntry} before its data is read.
 * <p>
...
public class ZipInputStream extends InflaterInputStream implements ZipConstants {
	...
	public void closeEntry() throws IOException {
		 ...
		 * so by: - pushing bytes back into the source stream - reading a data
		 * descriptor footer from the source stream - resetting fields that
		 * manage the entry being closed
		 */
	...
	private void readAndVerifyDataDescriptor(int inB, int out)
			throws IOException {
		if (hasDD) {
	...
	public ZipEntry getNextEntry() throws IOException {
		...
		if (version > ZIPLocalHeaderVersionNeeded) {
			throw new ZipException("Cannot read local header version "
					+ version);
		}
		...
		currentEntry = createZipEntry(ModifiedUtf8.decode(nameBuf, charBuf, 0,
				flen));

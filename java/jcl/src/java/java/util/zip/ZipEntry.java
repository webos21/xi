...
/**
 * An instance of {@code ZipEntry} represents an entry within a
 * <i>ZIP-archive</i>. An entry has attributes such as name (= path) or the size
 * of its data. While an entry identifies data stored in an archive, it does not
 * hold the data itself. For example when reading a <i>ZIP-file</i> you will
 * first retrieve all its entries in a collection and then read the data for a
 * specific entry through an input stream.
 * 
 * @see ZipFile
 * @see ZipOutputStream
 */
public class ZipEntry implements ZipConstants, Cloneable {
	 ...
	 * 
	 * @return {@code true} when this {@code ZipEntry} is a directory,
	 ...
	 * 
	 * @param value
	 *            the compression method, either {@code DEFLATED} or
	 *            {@code STORED}.
	 * @throws IllegalArgumentException
	...
	/**
	 * Constructs a new {@code ZipEntry} using the values obtained from
	 * {@code ze}.
	 * 
	 * @param ze
	...
	/*
	 * Internal constructor. Creates a new ZipEntry by reading the Central
	 * Directory Entry from "in", which must be positioned at the CDE signature.
	 * 
	...
	ZipEntry(LittleEndianReader ler, InputStream in) throws IOException {
		 ...
		 * readIntLE, so we're going to read the entire header at once and then
		 * parse the results out without using any function calls. Uglier, but
		...
		mLocalHeaderRelOffset = (hdrBuf[42] & 0xff)
				| ((hdrBuf[43] & 0xff) << 8) | ((hdrBuf[44] & 0xff) << 16)
		...
		// The RI has always assumed UTF-8. (If GPBF_UTF8_FLAG isn't set, the
		// encoding is
		// actually IBM-437.)
	...
	static long readIntLE(RandomAccessFile raf) throws IOException {
		...
		return b0 | (b1 << 8) | (b2 << 16) | (b3 << 24); // ATTENTION: DOES SIGN
															// EXTENSION: IS
															// THIS WANTED?
	}
	...
	static class LittleEndianReader {
		...
		long readIntLE(InputStream in) throws IOException {
			if (in.read(b, 0, 4) == 4) {
				return (((b[0] & 0XFF)) | ((b[1] & 0XFF) << 8)
						| ((b[2] & 0XFF) << 16) | ((b[3] & 0XFF) << 24)) & 0XFFFFFFFFL; // Here
																						// for
																						// sure
																						// NO
																						// sign

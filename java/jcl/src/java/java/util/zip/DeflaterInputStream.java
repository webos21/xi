...
/**
 * An {@code InputStream} filter to compress data. Callers read compressed data
 * in the "deflate" format from the uncompressed underlying stream.
 * 
 * @since 1.6
 */
public class DeflaterInputStream extends FilterInputStream {
	...
	/**
	 * Constructs a {@code DeflaterInputStream} with a new {@code Deflater} and
	 * an implementation-defined default internal buffer size. {@code in} is a
	 * source of uncompressed data, and this stream will be a source of
	 * compressed data.
	 * 
	 * @param in
	...
	/**
	 * Constructs a {@code DeflaterInputStream} with the given {@code Deflater}
	 * and an implementation-defined default internal buffer size. {@code in} is
	 * a source of uncompressed data, and this stream will be a source of
	 * compressed data.
	 * 
	 * @param in
	 *            the source {@code InputStream}
	 * @param deflater
	 *            the {@code Deflater} to be used for compression
	 */
	...
	/**
	 * Constructs a {@code DeflaterInputStream} with the given {@code Deflater}
	 * and given internal buffer size. {@code in} is a source of uncompressed
	 * data, and this stream will be a source of compressed data.
	 * 
	 * @param in
	 *            the source {@code InputStream}
	 * @param deflater
	...
	/**
	 * Closes the underlying input stream and discards any remaining
	 * uncompressed data.
	 */
	@Override
	...
	/**
	 * Reads a byte from the compressed input stream. The result will be a byte
	 * of compressed data corresponding to an uncompressed byte or bytes read
	...
	/**
	 * Reads compressed data into a byte buffer. The result will be bytes of
	 * compressed data corresponding to an uncompressed byte or bytes read from
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		while (count < len && !def.finished()) {
			...
			int byteCount = def.deflate(buf, 0,
					Math.min(buf.length, len - count));
			if (byteCount == -1) {
				break;
			}

...
public class Deflater {
	 ...
	 * header is added to the output by default; use constructor
	 ...
	 *            maximum number of bytes of compressed data to be written.
	 * @param flush
	 *            one of {@link #NO_FLUSH}, {@link #SYNC_FLUSH} or
	 *            {@link #FULL_FLUSH}.
	...
	private synchronized int deflateImpl(byte[] buf, int off, int nbytes,
			int flush) {
		if (streamHandle == -1) {
	 ...
	 * input or output is discarded. While this method is used by
	 * {@code finalize()}, it can be called explicitly in order to free native
	...
	/**
	 * Indicates to the {@code Deflater} that all uncompressed input has been
	 * provided to it.
	 * 
	 * @see #finished
	 */
	 ...
	 * uncompressed input have been consumed by the {@code Deflater}. If
	 * needsInput() returns true setInput() must be called before deflation can
	 * continue. If all bytes of uncompressed data have been provided to the
	 * {@code Deflater} finish() must be called to ensure the compressed data is
	 * output.
	 * 
	 ...
	 * setDictionary() can only be called if this {@code Deflater} supports the
	 * writing of ZLIB headers. This is the default behaviour but can be
	 * overridden using {@code Deflater(int, boolean)}.
	 * 
	 * @param buf
	...
	/**
	 * Sets the input buffer the {@code Deflater} will use to extract
	 * uncompressed bytes for later compression. Input will be taken from the
	 * buffer region starting at off and ending at nbytes - 1.
	 * 
	 * @param buf
	...
	/**
	 * Returns a long int of total number of bytes read by the {@code Deflater}.
	 * This method performs the same as {@code getTotalIn} except it returns a
	 ...
	 * {@code Deflater}. This method performs the same as {@code getTotalOut}
	 * except it returns a long value instead of an integer
	 * 
	 * @see #getTotalOut()

...
public class ZipOutputStream extends DeflaterOutputStream implements
	...
	public void closeEntry() throws IOException {
		...
		int flags = currentEntry.getMethod() == STORED ? 0
				: ZipFile.GPBF_DATA_DESCRIPTOR_FLAG;
	...
	public void putNextEntry(ZipEntry ze) throws java.io.IOException {
		...
		if (nameLength > 0xffff) {
			throw new IllegalArgumentException("Name too long: " + nameLength
					+ " UTF-8 bytes");
		}
		...
		int flags = currentEntry.getMethod() == STORED ? 0
				: ZipFile.GPBF_DATA_DESCRIPTOR_FLAG;
		// Java always outputs UTF-8 filenames. (Before Java 7, the RI didn't
		// set this flag and used
		// modified UTF-8. From Java 7, it sets this flag and uses normal
		// UTF-8.)
	 ...
	 * method must be one of {@code STORED} (for no compression) or
	 * {@code DEFLATED}.
	 * 
	 * @param method

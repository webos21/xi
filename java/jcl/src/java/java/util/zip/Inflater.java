...
public class Inflater {
	 ...
	 * stream. If deflated bytes remain and {@code needsInput()} returns
	 * {@code true} this method will return {@code false}. This method should be
	 ...
	 * 
	 * @return The <i>Adler32</i> checksum associated with this {@code Inflater}
	 *         .
	 */
	 ...
	 * {@code Inflater}. The result value is limited by
	 ...
	 * should be called with the appropriate dictionary prior to calling
	 * {@code inflate()}.
	 * 
	...
	// BEGIN android-only
	/**
	 * Sets the current input to the region within a file starting at
	 * {@code off} and ending at {@code nbytes - 1}. This method should only be
	 * called if {@code needsInput()} returns {@code true}.
	 * 
	 * @param fd
	 *            the input file.
	...
	// BEGIN android-only
	private native synchronized int setFileInputImpl(FileDescriptor fd,
			long off, int nbytes, long handle);
	// END android-only
}

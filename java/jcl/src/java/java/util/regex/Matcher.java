...
/**
 * The result of applying a {@code Pattern} to a given input. See
 * {@link Pattern} for example uses.
 */
public final class Matcher implements MatchResult {
	...
	/**
	 * Holds the end of the region, or input.length() if the matching should go
	 ...
	 * order of their <i>opening</i> parenthesis, starting at 1. The special
	 * group 0 represents the entire match (as if the entire pattern is
	 * surrounded by an implicit capturing group). For example, "a((b)c)"
	 * matching "abc" would give the following groups:
	 * 
	 * <pre>
	 * 0 "abc"
	 * 1 "bc"
	 ...
	 * successful match (for example, "a(b)?c" matching "ac") returns null. A
	...
	private static native boolean findImpl(int addr, String s, int startIndex,
			int[] offsets);
	...
	private static native boolean lookingAtImpl(int addr, String s,
			int[] offsets);
	...
	private static native void setInputImpl(int addr, String s, int start,
			int end);

...
/**
 * Used to make {@code String.split} fast (and to help {@code Pattern.split}
 * too).
 * 
 * @hide
 */
public class Splitter {
	// The RI allows regular expressions beginning with ] or }, but that's
	// probably a bug.
	...
	/**
	 * Returns a result equivalent to {@code s.split(separator, limit)} if it's
	 * able to compute it more cheaply than ICU, or null if the caller should
	 * fall back to using ICU.
	 */
	public static String[] fastSplit(String re, String input, int limit) {
		...
		// Unlike Perl, which considers the result of splitting the empty string
		// to be the empty
		...
		// Collect text preceding each occurrence of the separator, while
	...
	public static String[] split(Pattern pattern, String re, String input,
			int limit) {
	...
	private static String[] finishSplit(List<String> list, String input,
			int begin, int maxSize, int limit) {
		// Add trailing text.
		if (begin < input.length()) {
			list.add(input.substring(begin));
		} else if (limit != 0) { // No point adding the empty string if limit ==
									// 0, just to remove it below.
			list.add("");
		}

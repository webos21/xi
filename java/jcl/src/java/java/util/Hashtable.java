...
/**
 * Hashtable is a synchronized implementation of {@link Map}. All optional
 * operations are supported.
 * 
 * <p>
 * Neither keys nor values can be null. (Use {@code HashMap} or
 * {@code LinkedHashMap} if you need null keys or values.)
 * 
 * @param <K>
...
public class Hashtable<K, V> extends Dictionary<K, V> implements Map<K, V>,
	...
	private static int capacityForInitSize(int size) {
		...
		return (result & ~(MAXIMUM_CAPACITY - 1)) == 0 ? result
				: MAXIMUM_CAPACITY;
	}
	...
	protected void rehash() {
		/*
		 * This method has no testable semantics, other than that it gets called
		 * from time to time.
		 */
	}
	 ...
	 * backing map. However, this would require them to have a reference to it,
	 * which would cause considerable bloat. Moreover, the RI behaves the same
	 * way.
	 */
	...
	private final class KeyEnumeration extends HashIterator implements
			Enumeration<K> {
	...
	private final class ValueEnumeration extends HashIterator implements
			Enumeration<V> {
		public boolean hasMoreElements() {
			return hasNext();
		}
	...
	/**
	 * A rough estimate of the number of characters per entry, for use when
	 * creating a string buffer in the toString method.
	 */
	 ...
	 * 
	 * @serialField
	 *                  loadFactor float load factor for this Hashtable
	 */

 ...
 * <p>
 * It's relatively rare to use a {@code Formatter} directly. A variety of
 * classes offer convenience methods for accessing formatter functionality. Of
 * these, {@link String#format} is generally the most useful.
 * {@link java.io.PrintStream} and {@link java.io.PrintWriter} both offer
 * {@code format} and {@code printf} methods.
 * <p>
 * <i>Format strings</i> consist of plain text interspersed with format
 * specifiers, such as {@code "name: %s weight: %03dkg\n"}. Being a Java string,
 * the usual Java string literal backslash escapes are of course available.
 * <p>
 * <i>Format specifiers</i> (such as {@code "%s"} or {@code "%03d"} in the
 * example) start with a {@code %} and describe how to format their
 * corresponding argument. It includes an optional argument index, optional
 * flags, an optional width, an optional precision, and a mandatory conversion
 * type. In the example, {@code "%s"} has no flags, no width, and no precision,
 * while {@code "%03d"} has the flag {@code 0}, the width 3, and no precision.
 * <p>
 * Not all combinations of argument index, flags, width, precision, and
 * conversion type are valid.
 * <p>
 * <i>Argument index</i>. Normally, each format specifier consumes the next
 * argument to {@code format}. For convenient localization, it's possible to
 * reorder arguments so that they appear in a different order in the output than
 * the order in which they were supplied. For example, {@code "%4$s"} formats
 * the fourth argument ({@code 4$}) as a string ({@code s}). It's also possible
 * to reuse an argument with {@code <}. For example,
 ...
 * <tr>
 * <td width="5%">{@code  }</td>
 * <td width="25%">A space indicates that non-negative numbers should have a
 * leading space. (Decimal only.)</td>
 ...
 * <td width="30%">{@code format("%-6dx", 5);}<br/>
 * {@code format("%-3C, %3C", 'd', 0x65);}</td>
 * <td width="30%">
 * 
 * <pre>
 * 5      x
 ...
 * <td width="30%">{@code format("%o %#o", 010, 010);}<br/>
 * {@code format("%x %#x", 0x12, 0x12);}</td>
 * <td width="30%">{@code 10 010}<br/>
 * {@code 12 0x12}</td>
 * </tr>
 * </table>
 * <p>
 * <i>Width</i>. The width is a decimal integer specifying the minimum number of
 * characters to be used to represent the argument. If the result would
 * otherwise be shorter than the width, padding will be added (the exact details
 * of which depend on the flags). Note that you can't use width to truncate a
 * field, only to make it wider: see precision for control over the maximum
 * width.
 * <p>
 * <i>Precision</i>. The precision is a {@code .} followed by a decimal integer,
 * giving the minimum number of digits for {@code d}, {@code o}, {@code x}, or
 * {@code X}; the minimum number of digits after the decimal point for {@code a}, {@code A}, {@code e}, {@code E}, {@code f}, or {@code F}; the maximum
 * number of significant digits for {@code g} or {@code G}; or the maximum
 * number of characters for {@code s} or {@code S}.
 * <p>
 * <i>Conversion type</i>. One or two characters describing how to interpret the
 * argument. Most conversions are a single character, but date/time conversions
 * all start with {@code t} and have a single extra character describing the
 * desired output.
 * <p>
 * Many conversion types have a corresponding uppercase variant that converts
 * its result to uppercase using the rules of the relevant locale (either the
 * default or the locale set for this formatter).
 * <p>
 * This table shows the available single-character (non-date/time) conversion
 * types:
 * <table BORDER="1" WIDTH="100%" CELLPADDING="3" CELLSPACING="0" SUMMARY="">
 * <tr BGCOLOR="#CCCCFF" CLASS="TableHeadingColor">
 * <TD COLSPAN=4>
 * <B>String conversions</B> <br>
 * All types are acceptable arguments. Values of type {@link Formattable} have
 * their {@code formatTo} method invoked; all other types use {@code toString}.</TD>
 * </tr>
 * <tr>
 ...
 * <tr BGCOLOR="#CCCCFF" CLASS="TableHeadingColor">
 * <TD COLSPAN=4>
 * <B>Character conversions</B> <br>
 * Byte, Character, Short, and Integer (and primitives that box to those types)
 * are all acceptable as character arguments. Any other type is an error.</TD>
 * </tr>
 * <tr>
 ...
 * <tr BGCOLOR="#CCCCFF" CLASS="TableHeadingColor">
 * <TD COLSPAN=4>
 * <B>Integer conversions</B> <br>
 * Byte, Short, Integer, Long, and BigInteger (and primitives that box to those
 * types) are all acceptable as integer arguments. Any other type is an error.</TD>
 * </tr>
 * <tr>
 ...
 * </tr>
 * <tr BGCOLOR="#CCCCFF" CLASS="TableHeadingColor">
 * <TD COLSPAN=4><B>Floating-point conversions</B> <br>
 * Float, Double, and BigDecimal (and primitives that box to those types) are
 * all acceptable as floating-point arguments. Any other type is an error.</TD>
 * </tr>
 * <tr>
 ...
 * <td width="25%" valign="top">Decimal or engineering, depending on the
 * magnitude of the value.</td>
 ...
 * Accepts Boolean values. {@code null} is considered false, and instances of
 * all other types are considered true.</TD>
 * </tr>
 * <tr>
 * <td width="5%">{@code b}, {@code B}</td>
 * <td width="25%">Boolean.</td>
 * <td width="30%">{@code format("%b %b", true, false);}<br>
 * {@code format("%B %B", true, false);}<br>
 * {@code format("%b", null);}<br>
 * {@code format("%b", "hello");}</td>
 * <td width="30%">{@code true false}<br>
 * {@code TRUE FALSE}<br>
 * {@code false}<br>
 * {@code true}</td>
 * </tr>
 * <tr BGCOLOR="#CCCCFF" CLASS="TableHeadingColor">
 * <TD COLSPAN=4>
 * <B>Hash code conversion</B> <br>
 * Invokes {@code hashCode} on its argument, which may be of any type.</TD>
 * </tr>
 * <tr>
 * <td width="5%">{@code h}, {@code H}</td>
 * <td width="25%">Hexadecimal hash code.</td>
 * <td width="30%">{@code format("%h", this);}<br>
 * {@code format("%H", this);}<br>
 * {@code format("%h", null);}</td>
 * <td width="30%">{@code 190d11}<br>
 * {@code 190D11}<br>
 * {@code null}</td>
 * </tr>
 * <tr BGCOLOR="#CCCCFF" CLASS="TableHeadingColor">
 ...
 * <td width="25%">Newline. (The value of the system property
 * {@code "line.separator"}.)</td>
 ...
 * you should seriously consider using {@link java.text.SimpleDateFormat} via
 * the factory methods in {@link java.text.DateFormat} instead. The facilities
 * offered by {@code Formatter} are low-level and place the burden of
 * localization on the developer. Using
 * {@link java.text.DateFormat#getDateInstance},
 * {@link java.text.DateFormat#getTimeInstance}, and
 * {@link java.text.DateFormat#getDateTimeInstance} is preferable for dates and
 * times that will be presented to a human. Those methods will select the best
 * format strings for the user's locale.
 * <p>
 * The best non-localized form is <a
 * href="http://en.wikipedia.org/wiki/ISO_8601">ISO 8601</a>, which you can get
 * with {@code "%tF"} (2010-01-22), {@code "%tF %tR"} (2010-01-22 13:39),
 * {@code "%tF %tT"} (2010-01-22 13:39:15), or {@code "%tF %tT%z"} (2010-01-22
 * 13:39:15-0800).
 * <p>
 * As with the other conversions, date/time conversion has an uppercase format.
 * Replacing {@code %t} with {@code %T} will uppercase the field according to
 * the rules of the formatter's locale.
 * <p>
 * This table shows the date/time conversions:
 * <table BORDER="1" WIDTH="100%" CELLPADDING="3" CELLSPACING="0" SUMMARY="">
 * <tr BGCOLOR="#CCCCFF" CLASS="TableHeadingColor">
 * <TD COLSPAN=4><B>Date/time conversions</B> <br>
 * Calendar, Date, and Long (representing milliseconds past the epoch) are all
 * acceptable as date/time arguments. Any other type is an error. The epoch is
 * 1970-01-01 00:00:00 UTC.</TD>
 * </tr>
 * <tr>
 ...
 * {@link java.text.DateFormat} for more variations.)</td>
 ...
 * </table>
 * <p>
 * <i>Number localization</i>. Some conversions use localized decimal digits
 * rather than the usual ASCII digits. So formatting {@code 123} with {@code %d}
 * will give 123 in English locales but &#x0661;&#x0662;&#x0663; in appropriate
 * Arabic locales, for example. This number localization occurs for the decimal
 * integer conversion {@code %d}, the floating point conversions {@code %e},
 * {@code %f}, and {@code %g}, and all date/time {@code %t} or {@code %T}
 * conversions, but no other conversions.
 * <p>
 * <i>Thread safety</i>. Formatter is not thread-safe.
 * 
 * @since 1.5
...
public final class Formatter implements Closeable, Flushable {
	...
	private static class CachedDecimalFormat {
		...
		public NativeDecimalFormat update(LocaleData localeData, String pattern) {
			if (decimalFormat == null) {
				...
				decimalFormat = new NativeDecimalFormat(currentPattern,
						currentLocaleData);
			}
	...
	/**
	 * Creates a native peer if we don't already have one, or reconfigures an
	 * existing one. This means we get to reuse the peer in cases like
	 * "x=%.2f y=%.2f".
	 */
	 ...
	 * 
	 * <p>
	 * The {@code Locale} used is the user's default locale. See
	 * "<a href="../util
	 * /Locale.html#default_locale">Be wary of the default locale</a>".
	 */
	public Formatter() {
	 ...
	 * 
	 * @param a
	 *            the output destination of the {@code Formatter}. If {@code a}
	 *            is {@code null}, then a {@code StringBuilder} will be used.
	 */
	 ...
	 * 
	 * @param l
	 *            the {@code Locale} of the {@code Formatter}. If {@code l} is
	...
	/**
	 * Constructs a {@code Formatter} with the specified {@code Locale} and
	 * whose output will be written to the specified {@code Appendable}.
	 * 
	 * @param a
	 *            the output destination of the {@code Formatter}. If {@code a}
	 *            is {@code null}, then a {@code StringBuilder} will be used.
	 ...
	 *             or if a new file cannot be created, or if any error arises
	 *             when opening or creating the file.
	 * @throws SecurityException
	 *             if there is a {@code SecurityManager} in place which denies
	 *             permission to write to the file in
	 ...
	 * 
	 * @param file
	 *            the {@code File} that is used as the output destination for
	 *            the {@code Formatter}. The {@code File} will be truncated to
	 *            zero size if the {@code File} exists, or else a new
	 *            {@code File} will be created. The output of the
	 *            {@code Formatter} is buffered.
	 * @throws FileNotFoundException
	 *             if the {@code File} is not a normal and writable {@code File}
	 *             , or if a new {@code File} cannot be created, or if any error
	 *             rises when opening or creating the {@code File}.
	 * @throws SecurityException
	 *             if there is a {@code SecurityManager} in place which denies
	 *             permission to write to the {@code File} in
	 *             {@code checkWrite(file.getPath())}.
	 */
	...
	/**
	 * Constructs a {@code Formatter} with the given charset, and whose output
	 * is written to the specified {@code File}.
	 * 
	 * <p>
	 * The {@code Locale} used is the user's default locale. See
	 * "<a href="../util
	...
	/**
	 * Constructs a {@code Formatter} with the given charset, and whose output
	 * is written to the specified {@code OutputStream}.
	 * 
	 * <p>
	 * The {@code Locale} used is the user's default locale. See
	 * "<a href="../util
	 ...
	 * 
	 * @param ps
	 *            the {@code PrintStream} used as destination of the
	 ...
	 * 
	 * @return the {@code Locale} for the {@code Formatter} or {@code null} for
	 *         no {@code Locale}.
	...
	/**
	 * Returns the content by calling the {@code toString()} method of the
	 * output destination.
	 * 
	 * @return the content by calling the {@code toString()} method of the
	 *         output destination.
	 * @throws FormatterClosedException
	 *             if the {@code Formatter} has been closed.
	 */
	...
	/**
	 * Closes the {@code Formatter}. If the output destination is
	 * {@link Closeable}, then the method {@code close()} will be called on that
	 * destination.
	 * 
	 * If the {@code Formatter} has been closed, then calling the this method
	 * will have no effect.
	 * 
	...
	/**
	 * Returns the last {@code IOException} thrown by the {@code Formatter}'s
	 * output destination. If the {@code append()} method of the destination
	 * does not throw {@code IOException}s, the {@code ioException()} method
	 * will always return {@code null}.
	 * 
	 * @return the last {@code IOException} thrown by the {@code Formatter}'s
	 *         output destination.
	 */
	 ...
	 *            a format string.
	 * @param args
	 *            the arguments list used in the {@code format()} method. If
	 ...
	 *             arguments, or if fewer arguments are sent than those required
	 *             by the format string, or any other illegal situation.
	 * @throws FormatterClosedException
	 *             if the {@code Formatter} has been closed.
	 */
	 ...
	 *            {@code Locale} as specified during construction, and only
			...
			if (i < length) {
				...
				if (token.requireArgument()) {
					int index = token.getArgIndex() == FormatToken.UNSET ? currentObjectIndex++
							: token.getArgIndex();
					argument = getArgument(args, index, fsp, lastArgument,
							hasLastArgumentSet);
	...
	private Object getArgument(Object[] args, int index,
			FormatSpecifierParser fsp, Object lastArgument,
	...
	/*
	 * Complete details of a single format specifier parsed from a format
	 * string.
	 */
	private static class FormatToken {
		...
		// These have package access for performance. They used to be
		// represented by an int bitmask
		// and accessed via methods, but Android's JIT doesn't yet do a good job
		// of such code.
		...
		// Tests whether there were no flags, no width, and no precision
		// specified.
		boolean isDefault() {
			return !flagComma && !flagMinus && !flagParenthesis && !flagPlus
					&& !flagSharp && !flagSpace && !flagZero && width == UNSET
					&& precision == UNSET;
		}
		...
		boolean setFlag(int ch) {
			...
			if (dupe) {
				// The RI documentation implies we're supposed to report all the
				// flags, not just
				// the first duplicate, but the RI behaves the same as we do.
				throw new DuplicateFormatFlagsException(String.valueOf(ch));
			}
			if (strFlags == null) {
				strFlags = new StringBuilder(7); // There are seven possible
													// flags.
			}
		...
		void checkFlags(Object arg) {
			...
			case 'n':
				...
				break;
			case '%':
				// The only flag allowed is '-', and no argument or precision is
				// allowed.
				allowArgument = false;
				allowPrecision = false;
				break;
			...
			if (mismatch != null) {
				if (conversionType == 'n') {
					...
					throw new FormatFlagsConversionMismatchException(mismatch,
							conversionType);
				}
			}
			...
			// Check that we don't have a precision or width where they're not
			// allowed.
			...
			if (flagPlus && flagSpace) {
				throw new IllegalFormatFlagsException(
						"the '+' and ' ' flags are incompatible");
			}
			if (flagMinus && flagZero) {
				throw new IllegalFormatFlagsException(
						"the '-' and '0' flags are incompatible");
			}
		}
		...
		public UnknownFormatConversionException unknownFormatConversionException() {
			if (conversionType == 't' || conversionType == 'T') {
				throw new UnknownFormatConversionException(String.format(
						"%c%c", conversionType, dateSuffix));
			}
			throw new UnknownFormatConversionException(
					String.valueOf(conversionType));
		}
	}
			...
			case 'd':
				...
				if (out instanceof StringBuilder && !needLocalizedDigits) {
					if (arg instanceof Integer || arg instanceof Short
							|| arg instanceof Byte) {
						IntegralToString.appendInt((StringBuilder) out,
								((Number) arg).intValue());
						return null;
					} else if (arg instanceof Long) {
						IntegralToString.appendLong((StringBuilder) out,
								((Long) arg).longValue());
						return null;
					}
				}
				if (arg instanceof Integer || arg instanceof Long
					...
					return needLocalizedDigits ? localizeDigits(result)
							: result;
				}
			}
		}
	...
	/**
	 * Returns a CharSequence corresponding to {@code s} with all the ASCII
	 * digits replaced by digits appropriate to this formatter's locale. Other
	...
	/**
	 * Inserts the grouping separator every 3 digits. DecimalFormat lets you
	 * configure grouping size, but you can't access that from Formatter, and
	 * the default is every 3 digits.
	 */
	...
	private CharSequence transformFromCharacter() {
		...
		if (arg instanceof Character) {
			...
			CharSequence result = (codePoint < Character.MIN_SUPPLEMENTARY_CODE_POINT) ? String
					.valueOf((char) codePoint) : String.valueOf(Character
					.toChars(codePoint));
			return padding(result, 0);
		} else {
	...
	private CharSequence padding(CharSequence source, int startIndex) {
		...
		if (formatToken.flagZero) {
			if (formatToken.getConversionType() == 'd') {
				...
				paddingChar = '0'; // No localized digits for bases other than
									// decimal.
			}
		} else {
	...
	private StringBuilder toStringBuilder(CharSequence cs) {
		return cs instanceof StringBuilder ? (StringBuilder) cs
	...
	private CharSequence transformFromInteger() {
		...
		if (currentConversionType == 'd') {
			...
			// Undo sign-extension, since we'll be using
			// Long.to(Octal|Hex)String.
	...
	private CharSequence transformFromBigInteger() {
		...
		if (formatToken.flagSharp) {
			...
			if (currentConversionType == 'o') {
				...
				startIndex += 1;
			} else if (currentConversionType == 'x'
					|| currentConversionType == 'X') {
	...
	private boolean appendT(StringBuilder result, char conversion,
			Calendar calendar) {
		switch (conversion) {
		case 'A':
			result.append(localeData.longWeekdayNames[calendar
					.get(Calendar.DAY_OF_WEEK)]);
			return true;
		case 'a':
			result.append(localeData.shortWeekdayNames[calendar
					.get(Calendar.DAY_OF_WEEK)]);
			return true;
		case 'B':
			result.append(localeData.longMonthNames[calendar
					.get(Calendar.MONTH)]);
			return true;
		case 'b':
		case 'h':
			result.append(localeData.shortMonthNames[calendar
					.get(Calendar.MONTH)]);
			return true;
		case 'C':
		...
		case 'M':
			...
			return true;
		case 'N':
			appendLocalized(result,
					calendar.get(Calendar.MILLISECOND) * 1000000L, 9);
			return true;
		case 'Q':
		...
		case 'Z':
			...
			result.append(timeZone.getDisplayName(
	...
	private CharSequence transformFromFloat() {
		if (arg == null) {
			...
			// BigDecimal can't represent NaN or infinities, but its doubleValue
			// method will return
		...
		if (formatToken.flagZero
				&& (firstChar == '+' || firstChar == localeData.minusSign)) {
			startIndex = 1;
		}
	...
	private void transform_e(StringBuilder result) {
		// All zeros in this method are *pattern* characters, so no
		// localization.
		...
		// Unlike %f, %e uses 'e' (regardless of what the DecimalFormatSymbols
		// would have us use).
	...
	private static class FormatSpecifierParser {
		...
		/**
		 * Returns a FormatToken representing the format specifier starting at
		 * 'offset'.
		 * 
		 * @param offset
		...
		/**
		 * Returns a string corresponding to the last format specifier that was
		 * parsed. Used to construct error messages.
		 */
		...
		private FormatToken parseArgumentIndexAndFlags(FormatToken token) {
			...
			if (Character.isDigit(ch)) {
				...
				if (peek() == '$') {
					...
					if (ch == '0') {
						// The digit zero is a format flag, so reparse it as
						// such.
						i = position;
					} else {
						// The number was a width. This means there are no flags
						// to parse.
		...
		private FormatToken parseConversionType(FormatToken token) {
			...
			if (conversionType == 't' || conversionType == 'T') {
				char dateSuffix = advance(); // A date suffix is mandatory for
												// 't' or 'T'.
		...
		// Parses an integer (of arbitrary length, but typically just one
		// digit).
		private int nextInt() {
			long value = 0;
		...
		// Swallow remaining digits to resync our attempted parse, but return
		// failure.

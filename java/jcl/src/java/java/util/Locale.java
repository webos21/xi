...
/**
 * {@code Locale} represents a language/country/variant combination. Locales are
 * used to alter the presentation of information such as numbers or dates to
 * suit the conventions in the region they describe.
 * 
 * <p>
 * The language codes are two-letter lowercase ISO language codes (such as "en")
 * as defined by <a href="http://en.wikipedia.org/wiki/ISO_639-1">ISO 639-1</a>.
 * The country codes are two-letter uppercase ISO country codes (such as "US")
 * as defined by <a href="http://en.wikipedia.org/wiki/ISO_3166-1_alpha-3">ISO
 * 3166-1</a>. The variant codes are unspecified.
 * 
 * <p>
 ...
 * {@code Locale} object, not just for instances returned by the various lookup
 * methods.
 * 
 ...
 * {@code Locale} for languages and countries that don't exist, and you can
 * create instances for combinations that don't exist (such as "de_US" for
 * "German as spoken in the US").
 * 
 * <p>
 ...
 * countries (such as en_GB or es_ES), nor any locales for other languages (such
 * as de_DE). The opposite may well be true for a device sold in Europe.
 * 
 * <p>
 * You can use {@code getDefault} to get an appropriate locale for the
 * <i>user</i> of the device you're running on, or {@code getAvailableLocales}
 * to get a list of all the locales available on the device you're running on.
 * 
 * <a name="locale_data">
 * <h3>Locale data</h3></a>
 * <p>
 * Note that locale data comes solely from ICU. User-supplied locale service
 * providers (using the {@code java.text.spi} or {@code java.util.spi}
 * mechanisms) are not supported.
 * 
 * <p>
 * Here are the versions of ICU (and the corresponding CLDR and Unicode
 * versions) used in various Android releases:
 * <table BORDER="1" WIDTH="100%" CELLPADDING="3" CELLSPACING="0" SUMMARY="">
 * <tr>
 * <td>cupcake/donut/eclair</td>
 * <td>ICU 3.8</td>
 * <td><a href="http://www.unicode.org/press/pr-cldr1.5.html">CLDR 1.5</a></td>
 * <td><a href="http://www.unicode.org/versions/Unicode5.0.0/">Unicode 5.0</a></td>
 * </tr>
 * <tr>
 * <td>froyo</td>
 * <td>ICU 4.2</td>
 * <td><a href="http://www.unicode.org/press/pr-cldr1.7.html">CLDR 1.7</a></td>
 * <td><a href="http://www.unicode.org/versions/Unicode5.1.0/">Unicode 5.1</a></td>
 * </tr>
 * <tr>
 * <td>gingerbread</td>
 * <td>ICU 4.4</td>
 * <td><a href="http://www.unicode.org/press/pr-cldr1.8.html">CLDR 1.8</a></td>
 ...
 * <p>
 * Note that there are many convenience methods that automatically use the
 * default locale, but these may not be as convenient as you imagine. The
 * default locale is appropriate for anything that involves presenting data to
 * the user. You should use the user's date/time formats, number formats, rules
 * for conversion to lowercase, and so on. A common mistake is to implicitly use
 * the default locale when producing output meant to be machine-readable. This
 * tends to work on the developer's test devices but fail when run on a device
 * whose user is in a less conventional locale. For example, if you're
 * formatting integers some locales will use non-ASCII decimal digits. As
 * another example, if you're formatting floating-point numbers some locales
 * will use {@code ','} as the decimal point. That's correct for human-readable
 * output, but likely to cause problems if presented to another computer (
 * {@code Double.parseDouble} can't parse such a number, for example). The best
 * choice for computer-readable output is usually {@code Locale.US}: this locale
 * is guaranteed to be available on all devices, and the combination of no
 * surprising behavior and frequent use (especially for computer-computer
...
public final class Locale implements Cloneable, Serializable {
	...
	/**
	 * Locale constant for the root locale. The root locale has an empty
	 ...
	 * {@code Locale.US}, and usually several others. Most locale-sensitive
	 * classes offer their own {@code getAvailableLocales} method, which should
	 * be preferred over this general purpose method.
	 * 
	...
	public String getDisplayCountry(Locale locale) {
		...
		String result = ICU.getDisplayCountryNative(toString(),
				locale.toString());
		if (result == null) { // TODO: do we need to do this, or does ICU do it
								// for us?
			result = ICU.getDisplayCountryNative(toString(), Locale
					.getDefault().toString());
		}
		return result;
	}
	...
	public String getDisplayLanguage(Locale locale) {
		...
		String result = ICU.getDisplayLanguageNative(toString(),
				locale.toString());
		if (result == null) { // TODO: do we need to do this, or does ICU do it
								// for us?
			result = ICU.getDisplayLanguageNative(toString(), Locale
					.getDefault().toString());
		}
		return result;
	}
	...
	public String getDisplayName(Locale locale) {
		...
		if (!languageCode.isEmpty()) {
			...
			buffer.append(displayLanguage.isEmpty() ? languageCode
					: displayLanguage);
			++count;
		}
		if (!countryCode.isEmpty()) {
		...
		if (!variantCode.isEmpty()) {
			...
			buffer.append(displayVariant.isEmpty() ? variantCode
					: displayVariant);
			++count;
		}
		if (count > 1) {
	...
	public String getDisplayVariant(Locale locale) {
		...
		String result = ICU.getDisplayVariantNative(toString(),
				locale.toString());
		if (result == null) { // TODO: do we need to do this, or does ICU do it
								// for us?
			result = ICU.getDisplayVariantNative(toString(), Locale
					.getDefault().toString());
		}
		return result;
	}
	 ...
	 * the language code, country code and variant separated by underscores. If
	 * the language is missing the string begins with an underscore. If the
	 * country is missing there are 2 underscores between the language and the
	 * variant. The variant cannot stand alone without a language and/or country
	...
	public final String toString() {
		...
		return (result == null) ? (cachedToStringResult = toNewString())
				: result;
	}

	private String toNewString() {
		// The string form of a locale that only has a variant is the empty
		// string.
		if (languageCode.length() == 0 && countryCode.length() == 0) {
			return "";
		}
		// Otherwise, the output format is "ll_cc_variant", where language and
		// country are always
		// two letters, but the variant is an arbitrary length. A size of 11
		// characters has room
		// for "en_US_POSIX", the largest "common" value. (In practice, the
		// string form is almost

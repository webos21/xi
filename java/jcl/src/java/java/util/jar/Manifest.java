...
public class Manifest implements Cloneable {
	 ...
	 * 
	 * @return main {@code Attributes} associated with the source
	 *         {@code JarFile}.
	 */
	...
	/**
	 * Merges name/attribute pairs read from the input stream {@code is} into
	 * this manifest.
	 * 
	 * @param is
	...
	/*
	 * Helper to read the entire contents of the manifest from the given input
	 * stream. Usually we can do this in a single read but we need to account
	 * for 'infinite' streams, by ensuring we have a line feed within a
	...
	/*
	 * Check to see if the buffer contains a newline or carriage return
	 * character within the first 'length' bytes. Used to check the validity of
	 * the manifest input stream.
	 */
	 ...
	 * {@code Manifest}s are equal if they have identical main attributes as
	 * well as identical entry attributes.
	 * 
	 * @param o
		...
		if (version != null) {
			...
			while (entries.hasNext()) {
				...
				if (!name.equals(Attributes.Name.MANIFEST_VERSION)) {
					writeEntry(out, name,
							manifest.mainAttributes.getValue(name), encoder,
							buffer);
				}
			}
		}
	...
	private static void writeEntry(OutputStream os, Attributes.Name name,
		...
		if (out.length > LINE_LENGTH_LIMIT) {
			throw new IOException("Encoded header name " + name
					+ " exceeded maximum length " + LINE_LENGTH_LIMIT);
		}

		...
		while (readHeader()) {
			...
			if (chunks != null) {
				if (chunks.get(entryNameValue) != null) {
					// TODO A bug: there might be several verification chunks
					// for
					// the same name. I believe they should be used to update
					// signature in order of appearance; there are two ways to
					// fix
					// this: either use a list of chunks, or decide on used
					// signature algorithm in advance and reread the chunks
					// while
					// updating the signature; for now a defensive error is
					// thrown
			...
			if (b == ':') {
				...
				if (buf[pos++] != ' ') {
					...
					throw new IOException(String.format(
							"Invalid value for attribute '%s'", name));
				}
		...
		if (i > 0) {
			throw new IOException(
					"Invalid attribute name: "
							+ Arrays.toString(Arrays.copyOfRange(buf, mark,
									buf.length)));
		}
	}

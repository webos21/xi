 ...
 * objects are expected to have a {@code JarVerifier} instance member which can
 * be used to carry out the tasks associated with verifying a signed JAR. These
...
class JarVerifier {
	...
	class VerifierEntry extends OutputStream {
		 ...
		 *             if the digest value stored in the manifest does
		 *             <i>not</i> agree with the decrypted digest as recovered
		 *             from the <code>.SF</code> file.
		 */
		void verify() {
	...
	private SecurityException invalidDigest(String signatureFile, String name,
			String jarName) {
		throw new SecurityException(signatureFile + " has invalid digest for "
				+ name + " in " + jarName);
	}

	private SecurityException failedVerification(String jarName,
			String signatureFile) {
		throw new SecurityException(jarName + " failed verification of "
				+ signatureFile);
	}
	...
	/**
	 * Invoked for each new JAR entry read operation from the input stream. This
	 * method constructs and returns a new {@link VerifierEntry} which contains
	 * the certificates used to sign the entry and its hash value as specified
	 * in the JAR MANIFEST format.
	 * 
	 * @param name
	...
	VerifierEntry initEntry(String name) {
		...
		while (tokens.hasMoreTokens()) {
			...
			try {
				return new VerifierEntry(name,
						MessageDigest.getInstance(algorithm), hashBytes,
						certificatesArray);
	 ...
	 * entry in the {@code META-INF} directory including the manifest file
	 * itself. Files associated with the signing of a JAR would also be added to
	 * this collection.
	 * 
	 * @param name
	 ...
	 *         the associated JAR file has no entries at all in its
	 *         {@code META-INF} directory. This situation is indicative of an
	 *         invalid JAR file.
	 *         <p>
	...
	private void verifyCertificate(String certFile) {
		...
		if (!verify(attributes, digestAttribute, manifest, 0, manifest.length,
			...
			while (it.hasNext()) {
				...
				if (!verify(entry.getValue(), "-Digest", manifest, chunk.start,
						chunk.end, createdBySigntool, false)) {
	 ...
	 * 
	 * @return {@code true} if the JAR is signed, {@code false} otherwise.
	 */
	boolean isSignedJar() {
	 ...
	 * were used to verify the signature on the JAR entry called {@code name}.
	 * 
	 * @param name

 ...
 * {@code Formatter} should implement the {@code Formattable} interface. It
 * gives basic control over formatting objects.
 * 
 * @see Formatter
 */
...
public interface Formattable {
	 ...
	 *            {@code FormattableFlags.LEFT_JUSTIFY},
	 *            {@code FormattableFlags.UPPERCASE}, and
	 *            {@code FormattableFlags.ALTERNATE}. If no such flag is set,
	 *            the output is formatted by the default formatting of the
	 *            implementation.
	 * @param width
	 *            the minimum number of characters that should be written to the
	 *            output. If the length of the converted value is less than
	 *            {@code width} Additional space characters (' ') are added to
	 *            the output if the as needed to make up the difference. These
	 *            spaces are added at the beginning by default unless the flag
	 ...
	 *            before the width padding is applied. If the precision is -1,
	 *            then maximum length is not enforced.

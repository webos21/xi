 ...
 * this map. If a user-supplied comparator is in use, it will be returned by
 * {@link #comparator}.
 * <li>If no user-supplied comparator is supplied, keys will be sorted by their
 * natural order. Keys must be <i>mutually comparable</i>: they must implement
 * {@link Comparable} and {@link Comparable#compareTo compareTo()} must be able
 * to compare each key with any other key in this map. In this case
 * {@link #comparator} will return null.
 * </ul>
 ...
 * 
 * <p>
 * When the ordering is not consistent with equals the behavior of this class is
 * well defined but does not honor the contract specified by {@link Map}.
 * Consider a tree map of case-insensitive strings, an ordering that is not
...
public class TreeMap<K, V> extends AbstractMap<K, V> implements
		SortedMap<K, V>, NavigableMap<K, V>, Cloneable, Serializable {
	 ...
	 * 
	 * <p>
	 * Even if {@code copyFrom} is a {@code SortedMap}, the constructed map
	 * <strong>will not</strong> use {@code copyFrom}'s ordering. This
	 * constructor always creates a naturally-ordered map. Because the
	 * {@code TreeMap} constructor overloads are ambiguous, prefer to construct
	 * a map and populate it in two steps:
	 * 
	 * <pre>
	 * {
	 * 	&#064;code
	 * 	TreeMap&lt;String, Integer&gt; customOrderedMap = new TreeMap&lt;String, Integer&gt;(
	 * 			copyFrom.comparator());
	 ...
	 * {@code copyFrom}. This map's keys may only be null if the
	 * {@code copyFrom}'s ordering permits.
	 * 
	 * <p>
	 * The constructed map <strong>will always use</strong> {@code copyFrom}'s
	 * ordering. Because the {@code TreeMap} constructor overloads are ambigous,
	 * prefer to construct a map and populate it in two steps:
	 * 
	 * <pre>
	 * {
	 * 	&#064;code
	...
	enum Relation {
		LOWER, FLOOR, EQUAL, CREATE, CEILING, HIGHER;

		/**
		 * Returns a possibly-flipped relation for use in descending views.
		 * 
		 * @param ascending
		 *            false to flip; true to return this.
		 */
	...
	Node<K, V> find(K key, Relation relation) {
		if (root == null) {
			if (comparator == NATURAL_ORDER && !(key instanceof Comparable)) {
				throw new ClassCastException(key.getClass().getName()); // NullPointerException
																		// ok
			}
		...
		while (true) {
			int comparison = (comparableKey != null) ? comparableKey
	...
	Node<K, V> findByEntry(Entry<?, ?> entry) {
		...
		boolean valuesEqual = mine != null
				&& Objects.equal(mine.value, entry.getValue());
	 ...
	 * 
	 * @param entry
	 *            . Need this because we allow entry to be null, in which case
	 *            we return a null SimpleImmutableEntry.
	 */
	...
	public NavigableMap<K, V> subMap(K from, boolean fromInclusive, K to,
			boolean toInclusive) {
	...
	class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public Iterator<Entry<K, V>> iterator() {
			return new MapIterator<Entry<K, V>>(root == null ? null
					: root.first()) {
	...
	class KeySet extends AbstractSet<K> implements NavigableSet<K> {
		...
		public NavigableSet<K> subSet(K from, boolean fromInclusive, K to,
				boolean toInclusive) {
			return TreeMap.this.subMap(from, fromInclusive, to, toInclusive)
					.navigableKeySet();
		}

		public SortedSet<K> subSet(K fromInclusive, K toExclusive) {
			return TreeMap.this.subMap(fromInclusive, true, toExclusive, false)
					.navigableKeySet();
		}
	 ...
	 */
	final class BoundedMap extends AbstractMap<K, V> implements
		...
		BoundedMap(boolean ascending, K from, Bound fromBound, K to,
				Bound toBound) {
			/*
		...
		/**
		 * Returns true if the key is in bounds. Use this overload with NO_BOUND
		 * to skip bounds checking on either end.
		 */
		...
		private Node<K, V> bound(Node<K, V> node, Bound fromBound, Bound toBound) {
			return node != null
					&& isInBounds(node.getKey(), fromBound, toBound) ? node
					: null;
		}
		...
		/**
		 * @param first
		 *            true for the first element, false for the last.
		 */
		...
		private Entry<K, V> findBounded(K key, Relation relation) {
			...
			if (toBound != NO_BOUND && (relation == LOWER || relation == FLOOR)) {
				...
				toBoundForCheck = NO_BOUND; // we've already checked the upper
											// bound
			}

			if (fromBound != NO_BOUND
					&& (relation == CEILING || relation == HIGHER)) {
				...
				fromBoundForCheck = NO_BOUND; // we've already checked the lower
												// bound
			}

			return bound(find(key, relation), fromBoundForCheck,
					toBoundForCheck);
		}
		...
		private NavigableMap<K, V> subMap(K from, Bound fromBound, K to,
				Bound toBound) {
			if (!ascending) {
				K fromTmp = from;
			...
			/*
			 * If both the current and requested bounds are exclusive, the
			 * isInBounds check must be inclusive. For example, to create (C..F)
			 * from (A..F), the bound 'F' is in bounds.
			 */
			...
			if (fromBound == NO_BOUND) {
				...
				Bound fromBoundToCheck = fromBound == this.fromBound ? INCLUSIVE
						: this.fromBound;
			...
			if (toBound == NO_BOUND) {
				...
				Bound toBoundToCheck = toBound == this.toBound ? INCLUSIVE
						: this.toBound;
		...
		private IllegalArgumentException outOfBounds(Object value,
				Bound fromBound, Bound toBound) {
		...
		final class BoundedEntrySet extends AbstractSet<Entry<K, V>> {
			...
			public boolean remove(Object o) {
				...
				Entry<?, ?> entry = (Entry<?, ?>) o;
				return isInBounds(entry.getKey())
						&& TreeMap.this.entrySet().remove(entry);
			}
		}

		final class BoundedKeySet extends AbstractSet<K> implements
				NavigableSet<K> {
			@Override
			public int size() {
				return BoundedMap.this.size();
		...
		Object writeReplace() throws ObjectStreamException {
			return ascending ? new AscendingSubMap<K, V>(TreeMap.this, from,
					fromBound, to, toBound) : new DescendingSubMap<K, V>(
					TreeMap.this, from, fromBound, to, toBound);
		}
	}
	...
	private void writeObject(ObjectOutputStream stream) throws IOException {
		stream.putFields().put("comparator",
				comparator != NATURAL_ORDER ? comparator : null);
		stream.writeFields();
	...
	static abstract class NavigableSubMap<K, V> extends AbstractMap<K, V>
		...
		NavigableSubMap(TreeMap<K, V> delegate, K from, Bound fromBound, K to,
				Bound toBound) {
			this.m = delegate;
			this.lo = from;
			this.hi = to;
		...
		protected Object readResolve() throws ObjectStreamException {
			Bound fromBound = fromStart ? NO_BOUND : (loInclusive ? INCLUSIVE
					: EXCLUSIVE);
			Bound toBound = toEnd ? NO_BOUND : (hiInclusive ? INCLUSIVE
					: EXCLUSIVE);
			boolean ascending = !(this instanceof DescendingSubMap);
			return m.new BoundedMap(ascending, (K) lo, fromBound, (K) hi,
					toBound);
		}
	}
	...
	static class DescendingSubMap<K, V> extends NavigableSubMap<K, V> {
		...
		DescendingSubMap(TreeMap<K, V> delegate, K from, Bound fromBound, K to,
				Bound toBound) {
	...
	static class AscendingSubMap<K, V> extends NavigableSubMap<K, V> {
		...
		AscendingSubMap(TreeMap<K, V> delegate, K from, Bound fromBound, K to,
				Bound toBound) {
			super(delegate, from, fromBound, to, toBound);
		}
	}
	...
	class SubMap extends AbstractMap<K, V> implements Serializable {
		...
		protected Object readResolve() throws ObjectStreamException {
			...
			return new BoundedMap(true, (K) fromKey, fromBound, (K) toKey,
					toBound);
		}
	}
}

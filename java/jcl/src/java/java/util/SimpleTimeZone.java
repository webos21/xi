...
/**
 * {@code SimpleTimeZone} is a concrete subclass of {@code TimeZone} that
 * represents a time zone for use with a Gregorian calendar. This class does not
 * handle historical changes.
 * <p>
 * Use a negative value for {@code dayOfWeekInMonth} to indicate that
 * {@code SimpleTimeZone} should count from the end of the month backwards. For
...
public class SimpleTimeZone extends TimeZone {
	...
	// private static com.ibm.icu.util.TimeZone getICUTimeZone(final String
	// name){
	// return AccessController.doPrivileged(new
	// PrivilegedAction<com.ibm.icu.util.TimeZone>(){
	...
	/**
	 * The constant for representing a start or end time in standard local time
	 * mode, based on timezone's raw offset from GMT; does not include Daylight
	 * savings.
	 */
	public static final int STANDARD_TIME = 1;

	/**
	 * The constant for representing a start or end time in local wall clock
	 * time mode, based on timezone's adjusted offset from GMT; includes
	 * Daylight savings.
	 */
	...
	/**
	 * Constructs a {@code SimpleTimeZone} with the given base time zone offset
	 * from GMT and time zone ID. Timezone IDs can be obtained from
	 * {@code TimeZone.getAvailableIDs}. Normally you should use
	 * {@code TimeZone.getDefault} to construct a {@code TimeZone}.
	 * 
	 * @param offset
	...
	/**
	 * Constructs a {@code SimpleTimeZone} with the given base time zone offset
	 * from GMT, time zone ID, and times to start and end the daylight savings
	 * time. Timezone IDs can be obtained from {@code TimeZone.getAvailableIDs}.
	 * Normally you should use {@code TimeZone.getDefault} to create a
	 * {@code TimeZone}. For a time zone that does not use daylight saving time,
	 * do not use this constructor; instead you should use
	 * {@code SimpleTimeZone(rawOffset, ID)}.
	 * <p>
	 * By default, this constructor specifies day-of-week-in-month rules. That
	 * is, if the {@code startDay} is 1, and the {@code startDayOfWeek} is
	 * {@code SUNDAY}, then this indicates the first Sunday in the
	 * {@code startMonth}. A {@code startDay} of -1 likewise indicates the last
	 * Sunday. However, by using negative or zero values for certain parameters,
	 * other types of rules can be specified.
	 * <p>
	 ...
	 * negative. For example, if {@code startDay} is 5 and
	 * {@code startDayOfWeek} is {@code -MONDAY}, this indicates the first
	 * Monday on or after the 5th day of the {@code startMonth}.
	 * <p>
	 ...
	 * week and the day of the month negative. For example, if {@code startDay}
	 * is {@code -21} and {@code startDayOfWeek} is {@code -WEDNESDAY}, this
	 * indicates the last Wednesday on or before the 21st of the
	 * {@code startMonth}.
	 * <p>
	 * The above examples refer to the {@code startMonth}, {@code startDay}, and
	 * {@code startDayOfWeek}; the same applies for the {@code endMonth},
	 * {@code endDay}, and {@code endDayOfWeek}.
	 * <p>
	 * The daylight savings time difference is set to the default value: one
	 * hour.
	 * 
	 * @param offset
	 *            the given base time zone offset to GMT.
	 ...
	 *            {@code TimeZone.getAvailableIDs}.
	 * @param startMonth
	 *            the daylight savings starting month. The month indexing is
	 ...
	 * @param endMonth
	 *            the daylight savings ending month. The month indexing is
	 *            0-based. eg, 0 for January.
	 * @param endDay
	...
	/**
	 * Constructs a {@code SimpleTimeZone} with the given base time zone offset
	 * from GMT, time zone ID, times to start and end the daylight savings time,
	 * and the daylight savings time difference in milliseconds.
	 * 
	 * @param offset
	 *            the given base time zone offset to GMT.
	 * @param name
	 ...
	 *            the daylight savings starting day-of-week-in-month. Please see
	 *            the description of
	 *            {@link #SimpleTimeZone(int, String, int, int, int, int, int, int, int, int)}
	 *            for an example.
	 * @param startDayOfWeek
	 *            the daylight savings starting day-of-week. Please see the
	 *            description of
	 ...
	 *            for an example.
	 * @param endDayOfWeek
	 *            the daylight savings ending day-of-week. Please see the
	 *            description of
	 *            {@link #SimpleTimeZone(int, String, int, int, int, int, int, int, int, int)}
	 *            for an example.
	 * @param endTime
	 *            the daylight savings ending time in local wall time, which is
	 *            daylight time in this case. Please see the description of
	 *            {@link #SimpleTimeZone(int, String, int, int, int, int, int, int, int, int)}
	 *            for an example.
	...
	public SimpleTimeZone(int offset, String name, int startMonth,
		...
		// (com.ibm.icu.util.SimpleTimeZone)icuTZ;
		...
		if (daylightSavings <= 0) {
			throw new IllegalArgumentException("Invalid daylightSavings: "
					+ daylightSavings);
		}
	...
	/**
	 * Construct a {@code SimpleTimeZone} with the given base time zone offset
	 * from GMT, time zone ID, times to start and end the daylight savings time
	 * including a mode specifier, the daylight savings time difference in
	 * milliseconds. The mode specifies either {@link #WALL_TIME},
	 * {@link #STANDARD_TIME}, or {@link #UTC_TIME}.
	 * 
	 * @param offset
	 *            the given base time zone offset to GMT.
	 * @param name
	 ...
	 *            starts, based on the {@code startTimeMode}.
	 * @param startTimeMode
	 ...
	 * @param endTime
	 *            the time of day in milliseconds on which daylight savings time
	 *            ends, based on the {@code endTimeMode}.
	 * @param endTimeMode
	...
	/**
	 * Returns a new {@code SimpleTimeZone} with the same ID, {@code rawOffset}
	...
	/**
	 * Compares the specified object to this {@code SimpleTimeZone} and returns
	 * whether they are equal. The object must be an instance of
	 * {@code SimpleTimeZone} and have the same internal data.
	 * 
	 * @param object
		...
		if (endDay != 0) {
			...
			if (endMode != DOW_IN_MONTH_MODE) {
				...
				if (endDay < -5 || endDay > 5) {
					throw new IllegalArgumentException("Day of week in month: "
							+ endDay);
				}
			}
		}
	 ...
	 * 
	 * @param month
	 *            the {@code Calendar} month in which daylight savings time
	 *            ends.
	 * @param dayOfMonth
	 *            the {@code Calendar} day of the month on which daylight
	 *            savings time ends.
	 * @param time
	 ...
	 *            savings time ends.
	 * @param dayOfWeek
	 *            the {@code Calendar} day of the week on which daylight savings
	 *            time ends.
	 * @param time
	 *            the time of day in milliseconds standard time on which
	 *            daylight savings time ends.
	 ...
	 * {@code SimpleTimeZone}. Years before this start year will always be in
	 * standard time.
	 * 
	 * @param year
	 *            the starting year.

...
/**
 * {@code GregorianCalendar} is a concrete subclass of {@link Calendar} and
 ...
 * 
 * <p>
 * {@code GregorianCalendar} implements <em>proleptic</em> Gregorian and Julian
 * calendars. That is, dates are computed by extrapolating the current rules
 * indefinitely far backward and forward in time. As a result,
 * {@code GregorianCalendar} may be used for all years to generate meaningful
 * and consistent results. However, dates obtained using
 * {@code GregorianCalendar} are historically accurate only from March 1, 4 AD
 * onward, when modern Julian calendar rules were adopted. Before this date,
 ...
 * 
 * <p>
 * Values calculated for the {@code WEEK_OF_YEAR} field range from 1 to 53. Week
 * 1 for a year is the earliest seven day period starting on
 * {@code getFirstDayOfWeek()} that contains at least
 * {@code getMinimalDaysInFirstWeek()} days from that year. It thus depends on
 * the values of {@code getMinimalDaysInFirstWeek()},
 * {@code getFirstDayOfWeek()}, and the day of the week of January 1. Weeks
 * between week 1 of one year and week 1 of the following year are numbered
 * sequentially from 2 to 52 or 53 (as needed).
 * 
 * <p>
 * For example, January 1, 1998 was a Thursday. If {@code getFirstDayOfWeek()}
 * is {@code MONDAY} and {@code getMinimalDaysInFirstWeek()} is 4 (these are the
 * values reflecting ISO 8601 and many national standards), then week 1 of 1998
 * starts on December 29, 1997, and ends on January 4, 1998. If, however,
 * {@code getFirstDayOfWeek()} is {@code SUNDAY}, then week 1 of 1998 starts on
 * January 4, 1998, and ends on January 10, 1998; the first three days of 1998
 * then are part of week 53 of 1997.
 * 
 * <p>
 * Values calculated for the {@code WEEK_OF_MONTH} field range from 0 or 1 to 4
 * or 5. Week 1 of a month (the days with <code>WEEK_OF_MONTH =
 ...
 * {@code getFirstDayOfWeek()}. Unlike week 1 of a year, week 1 of a month may
 * be shorter than 7 days, need not start on {@code getFirstDayOfWeek()}, and
 * will not include days of the previous month. Days of a month before week 1
 * have a {@code WEEK_OF_MONTH} of 0.
 * 
 * <p>
 * For example, if {@code getFirstDayOfWeek()} is {@code SUNDAY} and
 * {@code getMinimalDaysInFirstWeek()} is 4, then the first week of January 1998
 * is Sunday, January 4 through Saturday, January 10. These days have a
 * {@code WEEK_OF_MONTH} of 1. Thursday, January 1 through Saturday, January 3
 * have a {@code WEEK_OF_MONTH} of 0. If {@code getMinimalDaysInFirstWeek()} is
 * changed to 3, then January 1 through January 3 have a {@code WEEK_OF_MONTH}
 * of 1.
 * 
 * <p>
...
public class GregorianCalendar extends Calendar {
	...
	/**
	 * Constructs a new {@code GregorianCalendar} initialized to the current
	 * date and time with the default {@code Locale} and {@code TimeZone}.
	 */
	...
	/**
	 * Constructs a new {@code GregorianCalendar} initialized to midnight in the
	 * default {@code TimeZone} and {@code Locale} on the specified date.
	 * 
	 * @param year
	 *            the year.
	 * @param month
	...
	/**
	 * Constructs a new {@code GregorianCalendar} initialized to the specified
	 * date and time in the default {@code TimeZone} and {@code Locale}.
	 * 
	 * @param year
	 *            the year.
	 * @param month
	...
	/**
	 * Constructs a new {@code GregorianCalendar} initialized to the current
	 * date and time and using the specified {@code Locale} and the default
	 * {@code TimeZone}.
	 * 
	 * @param locale
	 *            the {@code Locale}.
	...
	/**
	 * Constructs a new {@code GregorianCalendar} initialized to the current
	 * date and time and using the specified {@code TimeZone} and the default
	 * {@code Locale}.
	 * 
	 * @param timezone
	 *            the {@code TimeZone}.
	...
	/**
	 * Constructs a new {@code GregorianCalendar} initialized to the current
	 * date and time and using the specified {@code TimeZone} and {@code Locale}
	 * .
	 * 
	 * @param timezone
	 *            the {@code TimeZone}.
	...
	/**
	 * Creates new instance of {@code GregorianCalendar} with the same
	 * properties.
	 * 
	...
	private final void fullFieldsCalc(long timeVal, int millis, int zoneOffset) {
		...
		if (fields[YEAR] == changeYear
				&& gregorianCutover <= timeVal + zoneOffset) {
		...
		if (dstOffset != 0) {
			...
			if (oldDays != days) {
				...
				fields[DAY_OF_YEAR] = dayOfYear;
				if (fields[YEAR] == changeYear
						&& gregorianCutover <= timeVal - zoneOffset + dstOffset) {
					dayOfYear += currentYearSkew;
				}
				month = dayOfYear / 32;
	...
	protected void computeFields() {
		...
		int dstOffset = timeZone.inDaylightTime(new Date(time)) ? timeZone
				.getDSTSavings() : 0;
		...
		// FIXME: I don't think this caching ever really gets used, because it
		// requires that the
		// time zone doesn't use daylight savings (ever). So unless you're
		...
		if (isCached) {
			...
			// FIXME: this has to be wrong; useDaylightTime doesn't mean what
			// they think it means!

...
public class Collections {
	...
	private static final class ReverseComparator2<T> implements Comparator<T>,
			Serializable {
	...
	public static <T> void copy(List<? super T> destination,
		...
		if (destination.size() < source.size()) {
			throw new ArrayIndexOutOfBoundsException("Source size "
					+ source.size() + " does not fit into destination");
		}
	...
	static <E> E checkType(E obj, Class<? extends E> type) {
		if (obj != null && !type.isInstance(obj)) {
			throw new ClassCastException("Attempt to insert element of type "
					+ obj.getClass() + " into collection of type " + type);
		}
		return obj;
	}

...
class ProxySelectorImpl extends ProxySelector {
	...
	private Proxy selectHttpProxy(String uriHost) {
		...
		if (null != host) {
			...
			port = getSystemProperty("socksProxyPort",
					String.valueOf(SOCKS_PROXY_PORT));
		}
	...
	private Proxy selectHttpsProxy() {
		...
		if (null != host) {
			...
			type = Proxy.Type.HTTP;
			port = getSystemProperty("https.proxyPort",
					String.valueOf(HTTPS_PROXY_PORT));
		} else {
	...
	private Proxy selectFtpProxy(String uriHost) {
		...
		if (null != host) {
			...
			type = Proxy.Type.HTTP;
			port = getSystemProperty("ftp.proxyPort",
					String.valueOf(FTP_PROXY_PORT));
		} else {
			host = getSystemProperty("socksProxyHost");
			if (null != host) {
				// case 2: use socks proxy instead
	...
	private String getSystemProperty(final String property,
		...
		if (value == null || value.isEmpty()) {
			value = (netProps != null) ? netProps.getProperty(property,

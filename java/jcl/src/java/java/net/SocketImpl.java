 ...
 * {@code ServerSocket} on a well known port (referred to as listener) used to
...
public abstract class SocketImpl implements SocketOptions {
	 ...
	 * connection requests are queued up to the limit specified by
	 * {@code backlog}. Additional requests are rejected. The method
	 * {@code listen()} may only be invoked on streaming sockets.
	 * 
	 * @param backlog
	...
	public String toString() {
		return new StringBuilder(100).append("Socket[addr=")
				.append(getInetAddress()).append(",port=").append(port)
				.append(",localport=").append(getLocalPort()).append("]")
				.toString();
	}

	/**
	 * In the IP stack, write at most {@code count} bytes on the socket from the
	 * {@code buffer}, from the {@code offset}.
	 * 
	 * @param buffer

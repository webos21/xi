 ...
 * attributes: {@code Domain}, {@code Expires}, {@code Path}, and {@code Secure}
 * . The {@link #getVersion() version} of cookies in this format is {@code 0}.
 * <p>
 * There are no accessors for the {@code Expires} attribute. When parsed,
 * expires attributes are assigned to the {@link #getMaxAge() Max-Age} attribute
 * as an offset from {@link System#currentTimeMillis() now}.
 * <li><a href="http://www.ietf.org/rfc/rfc2109.txt">RFC 2109</a> formalizes the
 * Netscape cookie spec. It replaces the {@code Expires} timestamp with a
 * {@code Max-Age} duration and adds {@code Comment} and {@code Version}
 * attributes. The {@link #getVersion() version} of cookies in this format is
 * {@code 1}.
 * <li><a href="http://www.ietf.org/rfc/rfc2965.txt">RFC 2965</a> refines RFC
 * 2109. It adds {@code Discard}, {@code Port}, and {@code CommentURL}
 * attributes and renames the header from {@code Set-Cookie} to
 * {@code Set-Cookie2}. The {@link #getVersion() version} of cookies in this
 * format is {@code 1}.
 * </ul>
 * 
 * <p>
 * This implementation silently discards unrecognized attributes. In particular,
 * the {@code HttpOnly} attribute is widely served but isn't in any of the above
 * specs. It was introduced by Internet Explorer to prevent server cookies from
 * being exposed in the DOM to JavaScript, etc.
 * 
 * @since 1.6
 */
public final class HttpCookie implements Cloneable {
	 ...
	 * 
	 * @param domainPattern
	 *            a host name (like {@code android.com} or {@code localhost}),
	 *            or a pattern to match subdomains of a domain name (like
	 *            {@code .android.com}). A special case pattern is
	 *            {@code .local}, which matches all hosts without a TLD (like
	 *            {@code localhost}).
	 * @param host
	...
	public static boolean domainMatches(String domainPattern, String host) {
		...
		/*
		 * From the spec: "both host names are IP addresses and their host name
		 * strings match exactly; or both host names are FQDN strings and their
		 * host name strings match exactly"
		 */
		if (a.equals(b)
				&& (isFullyQualifiedDomainName(a, 0) || !InetAddress
						.isHostName(a))) {
			return true;
		}
		...
		/*
		 * Not in the spec! If prefixing a hostname with "." causes it to equal
		 * the domain pattern, then it should match. This is necessary so that
		 * the pattern ".google.com" will match the host "google.com".
		 */
		if (b.length() == 1 + a.length() && b.startsWith(".") && b.endsWith(a)
		...
		return a.length() > b.length()
				&& a.endsWith(b)
				&& ((b.startsWith(".") && isFullyQualifiedDomainName(b, 1)) || b
						.equals(".local"));
	}

	/**
	 * Returns true if {@code cookie} should be sent to or accepted from
	 * {@code uri} with respect to the cookie's path. Cookies match by directory
	 * prefix: URI "/foo" matches cookies "/foo", "/foo/" and "/foo/bar", but
	 * not "/" or "/foobar".
	 */
	...
	/**
	 * Returns true if {@code cookie} should be sent to {@code uri} with respect
	 * to the cookie's secure attribute. Secure cookies should not be sent in
	 * insecure (ie. non-HTTPS) requests.
	 */
	...
	/**
	 * Returns true if {@code cookie} should be sent to {@code uri} with respect
	 * to the cookie's port list.
	 */
	static boolean portMatches(HttpCookie cookie, URI uri) {
		if (cookie.getPortlist() == null) {
			return true;
		}
		return Arrays.asList(cookie.getPortlist().split(",")).contains(
	 ...
	 * 
	 * <p>
	 * Although this implements the cookie spec's definition of FQDN, it is not
	 * general purpose. For example, this returns true for IPv4 addresses.
	 */
	private static boolean isFullyQualifiedDomainName(String s,
			int firstCharacter) {
	...
	static class CookieParser {
		...
		/*
		 * The cookie's version is set based on an overly complex heuristic: If
		 * it has an expires attribute, the version is 0. Otherwise, if it has a
		 * max-age attribute, the version is 1. Otherwise, if the cookie started
		 * with "Set-Cookie2", the version is 1. Otherwise, if it has any
		 * explicit version attributes, use the first one. Otherwise, the
		 * version is 0.
		 */
		...
		public List<HttpCookie> parse() {
			...
			// The RI permits input without either the "Set-Cookie:" or
			// "Set-Cookie2" headers.
			...
			/*
			 * Read a comma-separated list of cookies. Note that the values may
			 * contain commas! <NAME> "=" <VALUE> ( ";" <ATTR NAME> ( "=" <ATTR
			 * VALUE> )? )*
			 */
			while (true) {
				...
				if (name == null) {
					if (cookies.isEmpty()) {
						throw new IllegalArgumentException("No cookies in "
								+ input);
					}
					return cookies;
				}
				...
				/*
				 * Read the attributes of the current cookie. Each iteration of
				 * this loop should enter with input either exhausted or
				 * prefixed with ';' or ',' as in ";path=/" and
				 * ",COOKIE2=value2".
				 */
				while (true) {
					skipWhitespace();
					...
					if (input.charAt(pos) == ',') {
						pos++;
						break; // a true comma delimiter; the current cookie is
								// complete.
					...
					if (attributeName == null) {
						continue; // for empty attribute as in
									// "Set-Cookie: foo=Foo;;path=/"
					}

					/*
					 * Since expires and port attributes commonly include comma
					 * delimiters, always scan until a semicolon when parsing
					 * these attributes.
					 */
					String terminators = pre2965
							|| "expires".equals(attributeName)
		...
		private String readAttributeValue(String terminators) {
			...
			/*
			 * Quoted string: read 'til the close quote. The spec mentions only
			 * "double quotes" but RI bug 6901170 claims that 'single quotes'
			 * are also used.
			 */
			if (pos < input.length()
	 ...
	 * 
	 * @param name
	 *            a non-empty string that contains only printable ASCII, no
	 *            commas or semicolons, and is not prefixed with {@code $}. May
	 *            not be an HTTP attribute name.
	 * @param value
	 *            an opaque value from the HTTP server.
	 * @throws IllegalArgumentException
	...
	private boolean isValidName(String n) {
		...
		boolean isValid = !(n.length() == 0 || n.startsWith("$") || RESERVED_NAMES
	...
	public boolean equals(Object object) {
		...
		if (object instanceof HttpCookie) {
			...
			return name.equalsIgnoreCase(that.getName())
					&& (domain != null ? domain.equalsIgnoreCase(that.domain)
							: that.domain == null)
	...
	private void appendAttribute(StringBuilder builder, String name,
			String value) {

...
public final class URI implements Comparable<URI>, Serializable {
	...
	private final static INetworkSystem NETWORK_SYSTEM = Platform
			.getNetworkSystem();
		...
		if (index != -1 && (index2 >= index || index2 == -1)
			...
			if (schemeSpecificPart.length() == 0) {
				throw new URISyntaxException(uri,
						"Scheme-specific part expected", index + 1);
			}
		} else {
			absolute = false;
			...
			if (temp.startsWith("//")) {
				...
				if (index != -1) {
					...
					if (authority.length() == 0 && query == null
							&& fragment == null) {
						throw new URISyntaxException(uri, "Authority expected",
								uri.length());
					}
	...
	private void validateScheme(String uri, String scheme, int index)
		...
		try {
			...
			throw new URISyntaxException(uri, "Illegal character in scheme",
					index + e.getIndex());
		}
	}
	...
	private void validateSsp(String uri, String ssp, int index)
		...
		try {
			...
			throw new URISyntaxException(uri, e.getReason()
					+ " in schemeSpecificPart", index + e.getIndex());
		}
	}
	...
	private void validatePath(String uri, String path, int index)
		...
		try {
			...
			throw new URISyntaxException(uri, e.getReason() + " in path", index
					+ e.getIndex());
		}
	}
	...
	/**
	 * Parse the authority string into its component parts: user info, host, and
	 * port. This operation doesn't apply to registry URIs, and calling it on
	 * such <i>may</i> result in a syntax exception.
	 * 
	 * @param forceServer
	 *            true to always throw if the authority cannot be parsed. If
	 *            false, this method may still throw for some kinds of errors;
	 *            this unpredictable behaviour is consistent with the RI.
	 */
					...
					if (tempPort < 0) {
						if (forceServer) {
							throw new URISyntaxException(authority,
									"Invalid port number", hostIndex + index
											+ 1);
						}
						return;
					}
	...
	private void validateUserInfo(String uri, String userInfo, int index)
		...
		for (int i = 0; i < userInfo.length(); i++) {
			...
			if (ch == ']' || ch == '[') {
				throw new URISyntaxException(uri,
						"Illegal character in userInfo", index + i);
			}
		}
	}
	 ...
	 * 
	 * @param forceServer
	 *            true to always throw if the host cannot be parsed. If false,
	 *            this method may still throw for some kinds of errors; this
	 ...
	 * 
	 * @param component
	 *            java.lang.String the component to be converted
	 * @param legalSet
	 *            the legal character set allowed in the component
	 ...
	 * identically to the string constructor but throws a different exception on
	 * failure. The constructor fails with a checked {@link URISyntaxException};
	 * this method fails with an unchecked {@link IllegalArgumentException}.
	 */

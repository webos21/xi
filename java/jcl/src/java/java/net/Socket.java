...
public class Socket {
	 ...
	 */
	private void tryAllAddresses(String dstName, int dstPort,
	 ...
	 * <p>
	 * <strong>Implementation note:</strong> this implementation tries each IP
	 * address for the given hostname until it either connects successfully or
	 * it exhausts the set. It will try both IPv4 and IPv6 addresses in the
	 * order specified by the system property
	 ...
	 *             sockets or an appropriate constructor of
	 *             {@code DatagramSocket} for UDP transport.
	 */
	@Deprecated
	 ...
	 *             streaming sockets or an appropriate constructor of
	 *             {@code DatagramSocket} for UDP transport.
	 */
	@Deprecated
	...
	public synchronized void close() throws IOException {
		isClosed = true;
		// RI compatibility: the RI returns the any address (but the original
		// local port) after close.
	 ...
	 * {@code InetAddress.ANY} if the socket is unbound.
	 */
	...
	public int getSoLinger() throws SocketException {
		checkOpenAndCreate(true);
		// The RI explicitly guarantees this idiocy in the
		// SocketOptions.setOption documentation.
	...
	public void setKeepAlive(boolean keepAlive) throws SocketException {
		if (impl != null) {
			checkOpenAndCreate(true);
			impl.setOption(SocketOptions.SO_KEEPALIVE,
	...
	private void startupSocket(InetAddress dstAddress, int dstPort,
		...
		if (localPort < 0 || localPort > 65535) {
			throw new IllegalArgumentException("Local port out of range: "
					+ localPort);
		}
	...
	/**
	 * Returns a {@code String} containing a concise, human-readable description
	 * of the socket.
	 * 
	 ...
	 * {@code SocketAddress} or {@code null} if the socket is not connected.
	 * 
	 ...
	 * 
	 * @return {@code true} if the socket is bound to a local address,
	 *         {@code false} otherwise.
	 */
	...
	/**
	 * Set the appropriate flags for a socket created by
	...
	private void cacheLocalAddress() {
		this.localAddress = Platform.getNetworkSystem().getSocketLocalAddress(
				impl.fd);
	}

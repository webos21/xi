 ...
 * 
 * <p>
 * {@code URLConnection} must be configured before it has connected to the
 ...
 * Resources from the local file system can be loaded using {@code file:} URIs.
 ...
 * 959</a>) is supported, but with no public subclass. FTP connections can be
 * used for input or output but not both.
 * <p>
 * By default, FTP connections will be made using {@code anonymous} as the
 * username and the empty string as the password. Specify alternate usernames
 * and passwords in the URL: {@code ftp://username:password@host/path}.
 * <li><strong>HTTP and HTTPS</strong><br>
 * Refer to the {@link HttpURLConnection} and
 * {@link javax.net.ssl.HttpsURLConnection HttpsURLConnection} subclasses.
...
public abstract class URLConnection {
	 ...
	 * 
	 * @param url
	 *            the URL which represents the resource this
	 *            {@code URLConnection} will point to.
	 */
	...
	/**
	 * Returns the option value which indicates whether user interaction is
	 * allowed on this {@code URLConnection}.
	 * 
	 ...
	 * {@code URLConnection} is connected to. First, it attempts to get the
	 * content type from the method {@code getContentType()} which looks at the
	 * response header field "Content-Type". If none is found it will guess the
	 * content type from the filename extension. If that fails the stream itself
	 * will be used to guess the content type.
	 * 
	 ...
	 * response header field "Content-Type". If none is found it will guess the
	 * content type from the filename extension. If that fails the stream itself
	 * will be used to guess the content type. The content type must match with
	 * one of the list {@code types}.
	 * 
	 * @param types
	 ...
	 * field {@code content-length} or {@code -1} if this field is not set.
	 * 
	...
	/**
	 * Returns the default setting whether this connection allows user
	 * interaction.
	 * 
	 * @return the value of the default setting
	 *         {@code defaultAllowUserInteraction}.
	 * @see #allowUserInteraction
	 */
	...
	/**
	 * Returns the default value for the specified request {@code field} or
	 * {@code null} if the field could not be found. The base implementation of
	...
	/**
	 * Returns the value of the option {@code doInput} which specifies whether
	...
	/**
	 * Returns the header value at the field position {@code pos} or
	 * {@code null} if the header has fewer than {@code pos} fields. The base
	 * implementation of this method returns always {@code null}.
	 * 
	 * <p>
	 * Some implementations (notably {@code HttpURLConnection}) include a
	 * mapping for the null key; in HTTP's case, this maps to the HTTP status
	 * line and is treated as being at position 0 when indexing into the header
	 * fields.
	 * 
	 * @param pos
	...
	/**
	 * Returns the value of the header field specified by {@code key} or
	 * {@code null} if there is no field with this name. The base implementation
	 * of this method returns always {@code null}.
	 * 
	 * <p>
	 * Some implementations (notably {@code HttpURLConnection}) include a
	 * mapping for the null key; in HTTP's case, this maps to the HTTP status
	...
	/**
	 * Returns the specified header value as a date in milliseconds since
	 * January 1, 1970 GMT. Returns the {@code defaultValue} if no such header
	 ...
	 * {@code defaultValue} if no such header field could be found or the value
	 * could not be parsed as an {@code Integer}.
	 * 
	 * @param field
	 *            the header field name whose value is needed.
	 * @param defaultValue
	...
	/**
	 * Returns the name of the header field at the given position {@code posn}
	 * or {@code null} if there are fewer than {@code posn} fields. The base
	 * implementation of this method returns always {@code null}.
	 * 
	 * <p>
	 * Some implementations (notably {@code HttpURLConnection}) include a
	...
	/**
	 * Returns an {@code InputStream} for reading data from the resource pointed
	 * by this {@code URLConnection}. It throws an UnknownServiceException by
	...
	/**
	 * Returns an {@code OutputStream} for writing data to this
	 * {@code URLConnection}. It throws an {@code UnknownServiceException} by
	 * default. This method must be overridden by its subclasses.
	 * 
	...
	/**
	 * Returns a {@code Permission} object representing all needed permissions
	 * to open this connection. The returned permission object depends on the
	 * state of the connection and will be {@code null} if no permissions are
	 ...
	 * field} or {@code null} if there is no field with this name. The base
	 * implementation of this method returns always {@code null}.
	 * 
	 * @param field
	 ...
	 * {@code URLConnection} allows to use caches.
	 * 
	 ...
	 * @deprecated Use {@link #setRequestProperty} of an existing
	 *             {@code URLConnection} instance.
	 */
	@Deprecated
	 ...
	 * the resource pointed by this {@code URLConnection} instance. A
	 * {@code SocketTimeoutException} is thrown if the connection could not be

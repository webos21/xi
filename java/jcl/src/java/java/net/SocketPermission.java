 ...
 * </pre>
 * 
 * represents the permission to connect to and accept connections from
 * {@code www.company.com} on ports in the range {@code 7000} to {@code 65535}.
 */
public final class SocketPermission extends Permission implements Serializable {
	...
	public boolean equals(Object other) {
		...
		if (!hostName.equalsIgnoreCase(sp.hostName)) {
			if (getIPString(true) == null
					|| !ipString.equalsIgnoreCase(sp.getIPString(true))) {
				return false;
			}
		}
	...
	/**
	 * Creates a new {@code PermissionCollection} to store
	 * {@code SocketPermission} objects.
	 * 
	 ...
	 *            the host[:port] one
	 * @param host
	 *            the host name we just get
	 * @throws IllegalArgumentException
	 *             If the port is not a positive number or minPort is not less
	 *             than or equal maxPort
	 */
	private void parsePort(String hostPort, String host)
			throws IllegalArgumentException {
		...
		try {
			...
			if (portMin > portMax) {
				throw new IllegalArgumentException(
						"MinPort is greater than MaxPort: " + port);
			}
	...
	boolean checkHost(SocketPermission sp) {
		...
		// multiple ips
		return (getIPString(false) != null && ipString.equals(sp
				.getIPString(false))) || hostName.equals(sp.hostName);
	}

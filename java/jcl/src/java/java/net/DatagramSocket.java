 ...
 * {@code DatagramPacket}. A {@code DatagramSocket} object can be used for both
...
public class DatagramSocket {
	 ...
	 * address {@code addr} on port {@code aPort}. Valid values for
	 ...
	 * validated, thereafter the only validation on {@code send()} and
	 * {@code receive()} is to check whether the packet address/port matches the
	 * connected target.
	 * 
	 * @param anAddress
	...
	public void connect(InetAddress anAddress, int aPort) {
		if (anAddress == null || aPort < 0 || aPort > 65535) {
			throw new IllegalArgumentException(
					"Address null or destination port out of range");
		}
	 ...
	 *         {@code null} if this socket is closed.
	 */
	 ...
	 * {@code pack}. All fields of {@code pack} must be set according to the
	 * data received. If the received data is longer than the packet buffer size
	 * it is truncated. This method blocks until a packet is received or a
	 * timeout has expired. If a security manager exists, its
	 * {@code checkAccept} method determines whether or not a packet is
	 * discarded. Any packets from unacceptable origins are silently discarded.
	 * 
	 * @param pack
		...
		if (address != null) { // The socket is connected
			...
			if (security != null) {
				if (packAddr.isMulticastAddress()) {
					...
					security.checkConnect(packAddr.getHostName(),
							pack.getPort());
				}
			}
		}
	...
	/**
	 * Sets the {@link SocketOptions#SO_TIMEOUT read timeout} in milliseconds
	 * for this socket. This receive timeout defines the period the socket will
	 * block waiting to receive data before throwing an
	 * {@code InterruptedIOException}. The value {@code 0} (default) is used to
	 * set an infinite timeout. To have effect this option must be set before
	 * the blocking method was called.
	 * 
	 * @param timeout
	 ...
	 * over the lifetime of the application. This factory is used to create a
	 * new datagram socket implementation. If a security manager is set its
	 ...
	 * socket implementation {@code socketImpl}. The created
	 * {@code DatagramSocket} will not be bound.
	 * 
	 * @param socketImpl
	 ...
	 * by the {@code SocketAddress} {@code localAddr} or an unbound
	 * {@code DatagramSocket} if the {@code SocketAddress} is {@code null}.
	 * 
	 * @param localAddr
	...
	public DatagramSocket(SocketAddress localAddr) throws SocketException {
		if (localAddr != null) {
			if (!(localAddr instanceof InetSocketAddress)) {
				throw new IllegalArgumentException(
						"Local address not an InetSocketAddress: "
								+ localAddr.getClass());
			}
	 ...
	 * {@code localAddr}. If this value is {@code null} any free port on a valid
	...
	public void bind(SocketAddress localAddr) throws SocketException {
		...
		if (localAddr != null) {
			...
			if (addr == null) {
				throw new SocketException("Host is unresolved: "
	...
	/**
	 * Sets the {@see SocketOptions#IP_TOS} value for every packet sent by this
	 * socket.
	 * 
	 * @throws SocketException
	 *             if the socket is closed or the option could not be set.

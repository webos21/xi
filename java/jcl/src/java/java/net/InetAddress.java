 ...
 * IPv6 address, and in practice you'll have an instance of either
 * {@code Inet4Address} or {@code Inet6Address} (this class cannot be
 * instantiated directly). Most code does not need to distinguish between the
 * two families, and should use {@code InetAddress}.
 * 
 * <p>
 * An {@code InetAddress} may have a hostname (accessible via
 * {@code getHostName}), but may not, depending on how the {@code InetAddress}
 * was created.
 * 
 * <h4>IPv4 numeric address formats</h4>
 * <p>
 * The {@code getAllByName} method accepts IPv4 addresses in the following
 * forms:
 * <ul>
 ...
 * </ul>
 * <p>
 * In the first three cases, each number is treated as an 8-bit value between 0
 * and 255. In the fourth case, the single number is treated as a 32-bit value
 * representing the entire address.
 * <p>
 * Note that each numeric part can be expressed in decimal (as above) or hex.
 * For example, {@code "0x01020304"} is equivalent to 1.2.3.4 and
 * {@code "0xa.0xb.0xc.0xd"} is equivalent to 10.11.12.13.
 * 
 * <p>
 * Typically, only the four-dot decimal form ({@code "1.2.3.4"}) is ever used.
 * Any method that <i>returns</i> a textual numeric address will use four-dot
 * decimal form.
 * 
 * <h4>IPv6 numeric address formats</h4>
 * <p>
 * The {@code getAllByName} method accepts IPv6 addresses in the following forms
 * (this text comes from <a href="http://www.ietf.org/rfc/rfc2373.txt">RFC
 * 2373</a>, which you should consult for full details of IPv6 addressing):
 * <ul>
 * <li>
 * <p>
 * The preferred form is {@code x:x:x:x:x:x:x:x}, where the 'x's are the
 * hexadecimal values of the eight 16-bit pieces of the address. Note that it is
 * not necessary to write the leading zeros in an individual field, but there
 * must be at least one numeral in every field (except for the case described in
 * the next bullet). Examples:
 * 
 * <pre>
 ...
 * also be used to compress the leading and/or trailing zeros in an address.
 * 
 ...
 * mixed environment of IPv4 and IPv6 nodes is {@code x:x:x:x:x:x:d.d.d.d},
 * where the 'x's are the hexadecimal values of the six high-order 16-bit pieces
 * of the address, and the 'd's are the decimal values of the four low-order
 * 8-bit pieces of the address (standard IPv4 representation). Examples:
 * 
 * <pre>
 ...
 * href="https://www.ietf.org/rfc/rfc4007.txt">RFC 4007</a> for more on IPv6's
 * scoped address architecture.
 * 
 * <h4>DNS caching</h4>
 * <p>
 * On Android, addresses are cached for 600 seconds (10 minutes) by default.
 * Failed lookups are cached for 10 seconds. The underlying C library or OS may
 * cache for longer, but you can control the Java-level caching with the usual
 * {@code "networkaddress.cache.ttl"} and
 * {@code "networkaddress.cache.negative.ttl"} system properties. These are
 * parsed as integer numbers of seconds, where the special value 0 means
 * "don't cache" and -1 means "cache forever".
 * 
 * <p>
 * Note also that on Android &ndash; unlike the RI &ndash; the cache is not
 * unbounded. The current implementation caches around 512 entries, removed on a
 * least-recently-used basis. (Obviously, you should not rely on these details.)
 * 
 * @see Inet4Address
 * @see Inet6Address
 */
public class InetAddress implements Serializable {
	...
	static InetAddress[] bytesToInetAddresses(byte[][] rawAddresses,
			String hostName) {
		// If we prefer IPv4, ignore the RFC3484 ordering we get from
		// getaddrinfo
	 ...
	 */
	static InetAddress[] getAllByNameImpl(String host)
			throws UnknownHostException {
	...
	static boolean preferIPv6Addresses() {
		...
		String propertyValue = AccessController
				.doPrivileged(new PriviAction<String>(propertyName));
	 ...
	 * performing a DNS lookup, and then taking the first returned address. For
	 * devices with multiple network interfaces and/or multiple addresses per
	 * interface, this does not necessarily return the {@code InetAddress} you
	 * want.
	 * 
	 * <p>
	 * Multiple interface/address configurations were relatively rare when this
	 * API was designed, but multiple interfaces are the default for modern
	 * mobile devices (with separate wifi and radio interfaces), and the need to
	 * support both IPv4 and IPv6 has made multiple addresses commonplace. New
	 * code should thus avoid this method except where it's basically being used
	 * to get a loopback address or equivalent.
	 * 
	 * <p>
	 ...
	 * that's actually in use for that connection. (It's not possible to ask the
	 * question "what local address would a connection to a given remote address
	 ...
	 * 
	 * <p>
	 * Note that if the host doesn't have a hostname set&nbsp;&ndash; as Android
	 * devices typically don't&nbsp;&ndash; this method will effectively return
	 * the loopback address, albeit by getting the name {@code localhost} and
	 * then doing a lookup to translate that to {@code 127.0.0.1}.
	 * 
	 ...
	 */
	private static InetAddress[] lookupHostByName(String host)
			throws UnknownHostException {
		// by jshwang
		...
		try {
			InetAddress[] addresses = bytesToInetAddresses(getaddrinfo(host),
					host);
	...
	static String getHostNameInternal(String host, boolean isCheck)
			throws UnknownHostException {
	 ...
	 */
	public boolean isReachable(NetworkInterface networkInterface,
			final int ttl, final int timeout) throws IOException {
		if (ttl < 0 || timeout < 0) {
	...
	private boolean isReachableByMultiThread(NetworkInterface netif,
			final int ttl, final int timeout) throws IOException {
		List<InetAddress> addresses = Collections
				.list(netif.getInetAddresses());
	...
	private boolean isReachableByTCP(InetAddress destination,
			InetAddress source, int timeout) throws IOException {
	 ...
	 * 4 bytes and for IPv6 exactly 16 bytes. If not, an
	 * {@code UnknownHostException} will be thrown.
	 * <p>
	 ...
	 * the given hostname. In the case of an IPv4 address there must be exactly
	 * 4 bytes and for IPv6 exactly 16 bytes. If not, an
	 * {@code UnknownHostException} is thrown. The host name and IP address are
	 * not validated. The hostname either be a machine alias or a valid IPv6 or
	 * IPv4 address format. The high order byte is {@code ipAddress[0]}.
	 * 
	 * @param hostName

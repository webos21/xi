...
class AddressCache {
	/**
	 * When the cache contains more entries than this, we start dropping the
	 * oldest ones. This should be a power of two to avoid wasted space in our
	 * custom map.
	 */
	...
	// Default time-to-live for positive cache entries. 600 seconds (10
	// minutes).
	...
	// Failed lookups are represented in the cache my mappings to this empty
	// array.
	...
	static class AddressCacheEntry {
		...
		/**
		 * The absolute expiry time in nanoseconds. Nanoseconds from
		 * System.nanoTime is ideal because -- unlike System.currentTimeMillis
		 * -- it can never go backwards.
		 * 
		 * Unless we need to cope with DNS TTLs of 292 years, we don't need to
	...
	public AddressCache() {
		// We pass 'true' so removeEldestEntry removes the least-recently
		// accessed entry, rather
		// than the least-recently inserted.
		map = new LinkedHashMap<String, AddressCacheEntry>(0,
				DEFAULT_LOAD_FACTOR, true) {
			@Override
			protected boolean removeEldestEntry(
					Entry<String, AddressCacheEntry> eldest) {
				// By the time this method is called, the new entry has already
				// been inserted and
				// the map will have grown to accommodate it. Using == lets us
				// prevent resizing.
	...
	/**
	 * Returns the cached addresses associated with 'hostname'. Returns null if
	 * nothing is known about 'hostname'. Returns an empty array if 'hostname'
	...
	public InetAddress[] get(String hostname) {
		...
		// No need to remove expired entries: the caller will provide a
		// replacement shortly.
		return null;
	}

	/**
	 * Associates the given 'addresses' with 'hostname'. The association will
	...
	public void put(String hostname, InetAddress[] addresses) {
		...
		String propertyName = isPositive ? "networkaddress.cache.ttl"
				: "networkaddress.cache.negative.ttl";
		long defaultTtlNanos = isPositive ? DEFAULT_POSITIVE_TTL_NANOS
				: DEFAULT_NEGATIVE_TTL_NANOS;
		// Fast-path the default case...
		long expiryNanos = System.nanoTime() + defaultTtlNanos;
		if (System.getSecurityManager() != null
				|| System.getProperty(propertyName, null) != null) {
			// ...and let those using a SecurityManager or custom properties pay
			// full price.
	 ...
	 * (I.e. insert a negative cache entry.)
	 */
	...
	private long customTtl(String propertyName, long defaultTtlNanos) {
		String ttlString = AccessController
				.doPrivileged(new PriviAction<String>(propertyName, null));
		if (ttlString == null) {
			return System.nanoTime() + defaultTtlNanos;
		}
		try {
			long ttlS = Long.parseLong(ttlString);
			// For the system properties, -1 means "cache forever" and 0 means
			// "don't cache".
			if (ttlS == -1) {

...
public class ServerSocket {
	...
	/**
	 * Unspecified constructor needed by
	...
	/**
	 * Checks whether the server may listen for connection requests on
	 * {@code aport}. Throws an exception if the port is outside the valid range
	...
	/**
	 * Sets the {@link SocketOptions#SO_TIMEOUT accept timeout} in milliseconds
	 * for this socket. This accept timeout defines the period the socket will
	 * block waiting to accept a connection before throwing an
	...
	public String toString() {
		...
		return result.append("addr=").append(getInetAddress().getHostName())
				.append("/").append(getInetAddress().getHostAddress())
				.append(",port=0,localport=").append(getLocalPort())
				.append("]").toString();
	}
	 ...
	 * @param backlog
	 *            the maximum number of unaccepted connections. Passing 0 or a
	 *            negative value yields the default backlog of 50.
	 * @throws IllegalArgumentException
	 *             if the {@code SocketAddress} is not supported.
	 * @throws IOException
	 *             if the socket is already bound or a problem occurs during
	 ...
	 * {@code ServerSocketChannel}. The current implementation returns always
	 * {@code null}.
	 * 

...
public class InetSocketAddress extends SocketAddress {
	...
	/**
	 * Creates a socket endpoint with the given port number {@code port} and no
	 * specified address. The range for valid port numbers is between 0 and
	 * 65535 inclusive.
	 * 
	 * @param port
	 ...
	 *             if a {@link SecurityManager} is installed and its
	 *             {@code checkConnect()} method does not allow the resolving of
	 *             the host name.
	 */
	...
	InetSocketAddress(String hostname, int port, boolean needResolved) {
		if (hostname == null || port < 0 || port > 65535) {
			throw new IllegalArgumentException("host=" + hostname + ", port="
					+ port);
		}
	 ...
	 * @return {@code true} if this socket and the given socket object
	 *         {@code socketAddr} are equal, {@code false} otherwise.
	 */
	@Override

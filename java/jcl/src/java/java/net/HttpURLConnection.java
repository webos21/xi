 ...
 * <ol>
 * <li>Obtain a new {@code HttpURLConnection} by calling
 * {@link URL#openConnection() URL.openConnection()} and casting the result to
 ...
 * {@link #setDoOutput(boolean) setDoOutput(true)} if they include a request
 * body. Transmit data by writing to the stream returned by
 ...
 * {@link #getInputStream()}. If the response has no body, that method returns
 * an empty stream.
 * <li>Disconnect. Once the response body has been read, the
 * {@code HttpURLConnection} should be closed by calling {@link #disconnect()}.
 ...
 * 
 * <p>
 * This class attempts to create secure connections using common TLS extensions
 * and SSL deflate compression. Should that fail, the connection will be retried
 * with SSL3 only.
 * 
 ...
 * {@link #getInputStream()} will throw an {@link IOException}. Use
 * {@link #getErrorStream()} to read the error response. The headers can be read
 * in the normal way using {@link #getHeaderFields()},
 * 
 ...
 * {@link #setFixedLengthStreamingMode(int)} when the body length is known in
 * advance, or {@link #setChunkedStreamingMode(int)} when it is not. Otherwise
 * {@code HttpURLConnection} will be forced to buffer the complete request body
 * in memory before it is transmitted, wasting (and possibly exhausting) heap
 ...
 * buffered</strong>. Most callers should wrap the returned streams with
 * {@link java.io.BufferedInputStream BufferedInputStream} or
 * {@link java.io.BufferedOutputStream BufferedOutputStream}. Callers that do
 * only bulk reads or writes may omit buffering.
 * 
 * <p>
 * When transferring large amounts of data to or from a server, use streams to
 * limit how much data is in memory at once. Unless you need the entire body to
 * be in memory at once, process it as a stream (rather than storing the
 * complete body as a single byte array or string).
 * 
 * <p>
 ...
 * server, {@code HttpURLConnection} includes an extensible cookie manager.
 * Enable VM-wide cookie management using {@link CookieHandler} and
 * {@link CookieManager}:
 * 
 * <pre>
 * {
 ...
 * server</a> only. Two other policies are included:
 * {@link CookiePolicy#ACCEPT_ALL} and {@link CookiePolicy#ACCEPT_NONE}.
 * Implement {@link CookiePolicy} to define a custom policy.
 * 
 * <p>
 ...
 * 
 * <p>
 * By default, new instances of {@code HttpCookie} work only with servers that
 * support <a href="http://www.ietf.org/rfc/rfc2965.txt">RFC 2965</a> cookies.
 * Many web servers support only the older specification, <a
 ...
 * <h3>HTTP Methods</h3>
 * <p>
 * {@code HttpURLConnection} uses the {@code GET} method by default. It will use
 * {@code POST} if {@link #setDoOutput setDoOutput(true)} has been called. Other
 * HTTP methods ({@code OPTIONS}, {@code HEAD}, {@code PUT}, {@code DELETE} and
 * {@code TRACE}) can be used with {@link #setRequestMethod}.
 * 
 * <h3>Proxies</h3>
 * By default, this class will connect directly to the <a
 * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec1.html">origin
 * server</a>. It can also connect via an {@link Proxy.Type#HTTP HTTP} or
 * {@link Proxy.Type#SOCKS SOCKS} proxy. To use a proxy, use
 * {@link URL#openConnection(Proxy) URL.openConnection(Proxy)} when creating the
 * connection.
 * 
 * <h3>IPv6 Support</h3>
 * <p>
 ...
 * <p>
 * {@code HttpURLConnection} supports a VM-wide HTTP response cache. Implement
 * {@link ResponseCache} and use {@link ResponseCache#setDefault} to install a
 * custom cache. Implementing this API is onerous: correct implementations
 * should follow all caching rules defined by <a
...
public abstract class HttpURLConnection extends URLConnection {
	 ...
	 */
	/*
	 * by jshwang protected String method = HttpURLConnectionImpl.GET;
	 */
	 ...
	 * with the host and the port number as the target name and
	 * {@code "resolve, connect"} as the action list. If the port number of this
	 * URL instance is lower than {@code 0} the port will be set to {@code 80}.
	 * 
	 ...
	 * each prefixed with a header containing the chunk's size. Setting a large
	 * chunk length requires a large internal buffer, potentially wasting
	 * memory. Setting a small chunk length increases the number of bytes that
	 * must be transmitted because of the header on every chunk. Most caller
	 * should use {@code 0} to get the system default.
	 * 
	 * @see #setFixedLengthStreamingMode
	 * @param chunkLength
	 *            the length to use, or {@code 0} for the default chunk length.
	 * @throws IllegalStateException
	...
	public void setChunkedStreamingMode(int chunkLength) {
		...
		if (chunkLength <= 0) {
			/*
			 * by jshwang this.chunkLength =
			 * HttpURLConnectionImpl.DEFAULT_CHUNK_LENGTH;
			 */

 ...
 * compatible with the SHA1PRNG algorithm on the reference implementation.</li>
 * </ul>
 * 
 * <p>
 * The default algorithm is defined by the first {@code SecureRandomSpi}
 * provider found in the VM's installed security providers. Use {@link Security}
 * to install custom {@link SecureRandomSpi} providers.
 * 
 * <a name="insecure_seed">
 * <h3>Seeding {@code SecureRandom} may be insecure</h3></a> A seed is an array
 * of bytes used to bootstrap random number generation. To produce
 * cryptographically secure random numbers, both the seed and the algorithm must
 * be secure.
 * 
 * <p>
 * By default, instances of this class will generate an initial seed using an
 * internal entropy source, such as {@code /dev/urandom}. This seed is
 * unpredictable and appropriate for secure use.
 * 
 * <p>
 * You may alternatively specify the initial seed explicitly with the
 * {@link #SecureRandom(byte[]) seeded constructor} or by calling
 * {@link #setSeed} before any random numbers have been generated. Specifying a
 * fixed seed will cause the instance to return a predictable sequence of
 * numbers. This may be useful for testing but it is not appropriate for secure
 * use.
 * 
 * <p>
 * Although it is common practice to seed {@link Random} with the current time,
 * that is dangerous with {@code SecureRandom} since that value is predictable
 * to an attacker and not appropriate for secure use.
 * 
 * <p>
 * Calling {@link #setSeed} on a {@code SecureRandom} <i>after</i> it has been
 * used to generate random numbers (ie. calling {#link nextBytes}) will
...
public class SecureRandom extends Random {
	...
	public SecureRandom() {
		...
		if (service == null) {
			...
			try {
				...
				this.secureRandomSpi = (SecureRandomSpi) service
						.newInstance(null);
	...
	public static SecureRandom getInstance(String algorithm)
		...
		synchronized (engine) {
			...
			return new SecureRandom((SecureRandomSpi) engine.spi,
	 ...
	 *             if {@code provider == null || provider.isEmpty()}
	 */
	...
	public static SecureRandom getInstance(String algorithm, Provider provider)
		...
		synchronized (engine) {
			...
			return new SecureRandom((SecureRandomSpi) engine.spi, provider,
					algorithm);
		}
	}
	...
	/**
	 * Seeds this {@code SecureRandom} instance with the specified {@code seed}.
	 * <a href="#insecure_seed">Seeding {@code SecureRandom} may be
	 * insecure</a>.
	 */
	...
	public void setSeed(long seed) {
		...
		byte[] byteSeed = { (byte) ((seed >> 56) & 0xFF),
				(byte) ((seed >> 48) & 0xFF), (byte) ((seed >> 40) & 0xFF),
				(byte) ((seed >> 32) & 0xFF), (byte) ((seed >> 24) & 0xFF),
				(byte) ((seed >> 16) & 0xFF), (byte) ((seed >> 8) & 0xFF),
				(byte) ((seed) & 0xFF) };
		setSeed(byteSeed);
	}

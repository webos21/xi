...
public class CodeSource implements Serializable {
	...
	/**
	 * Returns the hash code value for this {@code CodeSource}. Returns the same
	 * hash code for {@code CodeSource}s that are equal to each other as
	 ...
	 * {@code CodeSource}. Returns {@code true} if all of the following
	 * conditions are {@code true}, otherwise {@code false}:
	 * <p>
	 * <ul>
	 ...
	 * </ul>
	 * <li>if this {@code CodeSource}'s location port != -1 the port of
	 * {@code cs}'s location is equal to this {@code CodeSource}'s location port
	 ...
	 * <li>this {@code CodeSource}'s location reference is equal to to
	 * {@code cs}'s location reference
	 * </ul>
	 * </ul>
	 * <p>
	 ...
	 * <li>if this {@code CodeSource}'s location file ends with {@code "/"},
	 * then {@code cs}'s file must start with {@code CodeSource}'s location file
	 * <li>if this {@code CodeSource}'s location file does not end with
	 * {@code "/"}, then {@code cs}'s file must start with {@code CodeSource}'s
	...
	public boolean implies(CodeSource cs) {
		...
		if (this.location != null) {
			...
			if (thisFile.endsWith("/-")) { // javadoc:3.6."/-"
				if (!thatFile.startsWith(thisFile.substring(0,
						thisFile.length() - 2))) {
					return false;
				}

 ...
 *             {@link Principal}, {@link KeyStore} and the
 *             {@code java.security.cert} package.
 */
@Deprecated
public abstract class IdentityScope extends Identity {
	...
	public static IdentityScope getSystemScope() {
		/*
		 * Test shows that the implementation class name is read from security
		 * property "system.scope", and the class is only loaded from boot
		 * classpath. No default implementation as fallback, i.e., return null
		 * if fails to init an instance.
		 */
		if (systemScope == null) {
			...
			if (className != null) {
				try {
					systemScope = (IdentityScope) Class.forName(className)
							.newInstance();
				} catch (Exception e) {
					e.printStackTrace();
				}
	 ...
	 * 
	 * @param principal
	 *            the {@code Principal} whose name is used to lookup the
	 *            {@code Identity} to be returned.
	 * @return the {@code Identity} with the specified name or {@code null} if
	 *         not present.
	 */
	 ...
	 * @return the {@code Identity} associated with the specified key or
	...
	public String toString() {
		return new StringBuilder(super.toString()).append("[").append(size())
				.append("]").toString();
	}
}

 ...
 * should be resolved lazy and not during initialization time of the
 * {@code Policy}. {@code UnresolvedPermission}s contain all information to be
 * replaced by a concrete typed {@code Permission} right before the access
...
public final class UnresolvedPermission extends Permission implements
	 ...
	 * instance of {@code UnresolvedPermission}, the two
	 * {@code UnresolvedPermission}s must refer to the same type and must have
	 * the same name, the same actions and certificates.
	 * 
	 * @param obj
	 *            object to be compared for equality with this
	 *            {@code UnresolvedPermission}.
	 * @return {@code true} if the specified object is equal to this
	 *         {@code UnresolvedPermission}, otherwise {@code false}.
	 */
	@Override
	 ...
	 * {@code UnresolvedPermission}. The actions, specified in the constructor,
	 * are used when the concrete permission is resolved and created.
	 * 
	 ...
	 * {@code UnresolvedPermission} is resolved to.
	 * 
	 * @return the fully qualified class name of the permission this
	 *         {@code UnresolvedPermission} is resolved to.
	 */
	 ...
	 * {@code UnresolvedPermission} is resolved to.
	 * 
	 * @return the certificates of the permission this
	 *         {@code UnresolvedPermission} is resolved to.
	 */
	 ...
	 * {@code UnresolvedPermission}. {@code UnresolvedPermission} objects imply
	 * nothing since nothing is known about them yet.
	 * <p>
	...
	/**
	 * Returns a new {@code PermissionCollection} for holding
	 * {@code UnresolvedPermission} objects.
	 * 
	 * @return a new PermissionCollection for holding
	 *         {@code UnresolvedPermission} objects.
	 */
	@Override
	 ...
	 * <p>
	 * It is assumed that the class has a proper name (as returned by
	 * {@code getName()} of this unresolved permission), so no check is
	 * performed to verify this. However, the class must have all required
	 * certificates (as per {@code getUnresolvedCerts()}) among the passed
	 * collection of signers. If it does, a zero, one, and/or two-argument
	 * constructor is tried to instantiate a new permission, which is then
	 * returned.
	 * <p>
	...
	Permission resolve(Class targetType) {
		...
		if (PolicyUtils.matchSubset(targetCerts, targetType.getSigners())) {
			try {
				return PolicyUtils.instantiatePermission(targetType, name,
						actions);
	...
	/**
	 * Outputs {@code type},{@code name},{@code actions} fields via default
	 * mechanism; next manually writes certificates in the following format: <br>
	 * 
	 * <ol>
	 ...
	 * 
	 * @see <a
	 *      href="http://java.sun.com/j2se/1.5.0/docs/api/serialized-form.html#java.security.UnresolvedPermission">Java
	 *      Spec</a>
	 */
			...
			for (int i = 0; i < targetCerts.length; i++) {
				try {
					...
					throw (IOException) new NotSerializableException(
							"Cannot encode certificate: " + targetCerts[i])
							.initCause(cee);
				}
			}
		}

...
public abstract class Policy {
	 ...
	 * 
	 * @param type
	 *            - the specified Policy type. So far in Java 6, only
	 *            'JavaPolicy' supported.
	 * @param params
	...
	private static Policy getInstanceImpl(String type,
			Policy.Parameters params, Provider provider)
			throws NoSuchAlgorithmException {
		if (type == null) {
			throw new NullPointerException();
		}
	 ...
	 * domain (i.e. system classes). System classes are always given full
	 * permissions (i.e. AllPermission). This can not be changed by installing a
	 * new policy.
	 * 
	 * @param cs
	 *            the {@code CodeSource} to compute the permissions for.
	 * @return the permissions that are granted to the specified
	 *         {@code CodeSource}.
	 */
	 ...
	 *            verified.
	 * @return {@code true} if the {@code Permission} is implied by the
	 *         {@code ProtectionDomain}, {@code false} otherwise.
	 */
	...
	private boolean defaultImplies(ProtectionDomain domain,
	...
	private static Policy getDefaultProvider() {
		...
		if (defaultClass == null) {
			// TODO log warning
			System.err
					.println("[getDefaultProvider] No policy provider specified. Loading the "

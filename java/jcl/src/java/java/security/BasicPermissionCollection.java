...
final class BasicPermissionCollection extends PermissionCollection {
	...
	// true if this Collection contains a BasicPermission with '*' as its
	// permission name
	...
	public void add(Permission permission) {
		...
		if (permClass != null) {
			...
			// this is the first element provided that another thread did not
			// add
			synchronized (this) {
	 ...
	 * <dd>The {@code BasicPermission}s in this collection. All
	 * {@code BasicPermission}s in the collection must belong to the same class.
	 * The Hashtable is indexed by the {@code BasicPermission} name; the value
	...
	private void readObject(java.io.ObjectInputStream in) throws IOException,
		...
		synchronized (this) {
			permClass = (Class<? extends Permission>) fields.get("permClass",
					null);

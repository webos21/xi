 ...
 * addCertPathChecker} of the {@code PKIXParameters} and
 * {@code PKIXBuilderParameters} class respectively. The
 * {@link #check(Certificate, Collection) check} method will be called for each
 * certificate processed by a {@code CertPathBuilder} of
 ...
 * (from target to trusted CA). The return value of
 * {@code isForwardCheckingSupported} indicates whether forward checking is
 * supported.
 */
public abstract class PKIXCertPathChecker implements Cloneable {
	 ...
	 * 
	 * @param forward
	 *            the direction of the certification path processing,
	 *            {@code true} if the certificates are processed in forward
	 *            direction (from target to trusted CA), {@code false} if
	 *            processed in reverse direction (from trusted CA to target).
	 ...
	 * {@code PKIXCertPathChecker} is able to process.
	 * 
	 * @return the list of extensions of X.509 certificates that this
	 *         {@code PKIXCertPathChecker} is able to process, or {@code null}
	 *         if there are none.
	 */

...
/**
 * A CRL selector ({@code CRLSelector} for selecting {@code X509CRL}s that match
...
public class X509CRLSelector implements CRLSelector {
	 ...
	 * 
	 * @param issuers
	 *            the list of issuer distinguished names to match, or
	...
	public void setIssuerNames(Collection<?> names) throws IOException {
		...
		for (Object name : names) {
			if (name instanceof String) {
				issuerNames.add(new Name((String) name)
						.getName(X500Principal.CANONICAL));
			} else if (name instanceof byte[]) {
				issuerNames.add(new Name((byte[]) name)
						.getName(X500Principal.CANONICAL));
			} else {
	...
	public boolean match(CRL crl) {
		...
		// the search speed depends on the class of issuerNames
				!(issuerNames.contains(crlist.getIssuerX500Principal().getName(
		...
		if ((minCRL != null) || (maxCRL != null)) {
			try {
				...
				BigInteger crlNumber = new BigInteger((byte[]) ASN1Integer
						.getInstance().decode(bytes));

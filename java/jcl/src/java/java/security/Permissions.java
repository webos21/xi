...
public final class Permissions extends PermissionCollection implements
	...
	/**
	 * Adds the given {@code Permission} to this heterogeneous
	 * {@code PermissionCollection}. The {@code permission} is stored in its
	...
	final static class MetaEnumeration implements Enumeration {
		 ...
		 * 
		 * @param outer
		 *            an iterator over external collection of
	...
	private void readObject(java.io.ObjectInputStream in) throws IOException,
		...
		synchronized (klasses) {
			for (Iterator iter = perms.entrySet().iterator(); iter.hasNext();) {
				...
				PermissionCollection pc = (PermissionCollection) entry
						.getValue();
	...
	private void writeObject(java.io.ObjectOutputStream out) throws IOException {
		...
		fields.put("allPermission",
				allEnabled ? klasses.get(AllPermission.class) : null);
		out.writeFields();
	}
}

 ...
 * the {@code CollationKey}, but once the key is created, binary comparisons are
 * fast. This approach is recommended when the same strings are to be compared
 * over and over again.
 * <p>
 * On the other hand, implementations of
 * {@code Collator.compare(String, String)} can examine and process the strings
 * only until the first characters differ in order. This approach is recommended
 * if the strings are to be compared only once.
 * <p>
 * The following example shows how collation keys can be used to sort a list of
 * strings: <blockquote>
 * 
 * <pre>
...
public abstract class CollationKey implements Comparable<CollationKey> {
	 ...
	 * @return a negative value if this key is less than {@code value}, 0 if
	 *         they are equal, and a positive value if this key is greater.
	 */

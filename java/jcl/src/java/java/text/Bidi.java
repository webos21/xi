...
/**
 * Provides the Unicode Bidirectional Algorithm. The algorithm is defined in the
 * Unicode Standard Annex #9, version 13, also described in The Unicode
 * Standard, Version 4.0 .
 * 
 * Use a {@code Bidi} object to get the information on the position reordering
 * of a bidirectional text, such as Arabic or Hebrew. The natural display
 * ordering of horizontal text in these languages is from right to left, while
 * they order numbers from left to right.
 * 
...
public final class Bidi {
	...
	/**
	 * Creates a {@code Bidi} object from the
	 * {@code AttributedCharacterIterator} of a paragraph text. The
	 * RUN_DIRECTION attribute determines the base direction of the
	 * bidirectional text. If it is not specified explicitly, the algorithm uses
	...
	public Bidi(AttributedCharacterIterator paragraph) {
		...
		for (int textLimit = 1, i = 1; i < length; textLimit = paragraph
				.getRunLimit(TextAttribute.BIDI_EMBEDDING) - begin + 1) {
	 ...
	 * @throws IllegalArgumentException
	 *             if {@code textStart}, {@code embStart}, or
	 *             {@code paragraphLength} is negative; if
	...
	public Bidi(char[] text, int textStart, byte[] embeddings, int embStart,
		...
		if (textStart < 0) {
			throw new IllegalArgumentException("Negative textStart value "
					+ textStart);
		}
		if (embStart < 0) {
			throw new IllegalArgumentException("Negative embStart value "
					+ embStart);
		}
		if (paragraphLength < 0) {
			throw new IllegalArgumentException("Negative paragraph length "
					+ paragraphLength);
		}

		long bidi = 0;
		try {
			bidi = createUBiDi(text, textStart, embeddings, embStart,
	...
	private static long createUBiDi(char[] text, int textStart,
		...
		try {
			...
			NativeBidi.ubidi_setPara(bidi, realText, paragraphLength, flags,
					realEmbeddings);
			needsDeletion = false;
		} finally {
			if (needsDeletion) {
	 ...
	 * @throws IllegalArgumentException
	 *             if {@code lineStart < 0}, {@code lineLimit < 0},
	 *             {@code lineStart > lineLimit} or if {@code lineStart} is
	 *             greater than the length of this object's paragraph text.
	 */
	public Bidi createLineBidi(int lineStart, int lineLimit) {
		if (lineStart < 0 || lineLimit < 0 || lineLimit > length
				|| lineStart > lineLimit) {
			throw new IllegalArgumentException("Invalid ranges (start="
					+ lineStart + ", " + "limit=" + lineLimit + ", length="
					+ length + ")");
		}
		...
		try {
			...
			return new Bidi(NativeBidi.ubidi_setLine(parent, lineStart,
					lineLimit));
		} finally {
	 ...
	 * of objects at {@code index} from {@code objectStart} to
	 * {@code objectStart + count} will be reordered according to the range of
	 * levels at {@code index} from {@code levelStart} to
	...
	public String toString() {
		return getClass().getName() + "[direction: " + direction
				+ " baseLevel: " + baseLevel + " length: " + length + " runs: "

 ...
 * manner. Formatting turns a {@link Date} into a {@link String}, and parsing
 * turns a {@code String} into a {@code Date}.
 * 
 * <h4>Time Pattern Syntax</h4>
 * <p>
 * You can supply a pattern describing what strings are produced/accepted, but
 * almost all callers should use {@link DateFormat#getDateInstance},
 * {@link DateFormat#getDateTimeInstance}, or {@link DateFormat#getTimeInstance}
 * to get a ready-made instance suitable for the user's locale.
 * 
 * <p>
 * The main reason you'd create an instance this class directly is because you
 * need to format/parse a specific machine-readable format, in which case you
 * almost certainly want to explicitly ask for {@link Locale#US} to ensure that
 * you get ASCII digits (rather than, say, Arabic digits). (See
 * "<a href="../util
 * /Locale.html#default_locale">Be wary of the default locale</a>".) The most
 * useful non-localized pattern is {@code "yyyy-MM-dd HH:mm:ss.SSSZ"}, which
 * corresponds to the ISO 8601 international standard date format.
 * 
 * <p>
 * To specify the time format, use a <i>time pattern</i> string. In this string,
 * any character from {@code 'A'} to {@code 'Z'} or {@code 'a'} to {@code 'z'}
 * is treated specially. All other characters are passed through verbatim. The
 * interpretation of each of the ASCII letters is given in the table below.
 * ASCII letters not appearing in the table are reserved for future use, and it
 * is an error to attempt to use them.
 * 
 * <p>
 ...
 * 
 * <p>
 * The number of consecutive copies (the "count") of a pattern character further
 * influences the format.
 * <ul>
 * <li><b>Text</b> if the count is 4 or more, use the full form; otherwise use a
 * short or abbreviated form if one exists. So {@code zzzz} might give
 * {@code Pacific Standard Time} whereas {@code z} might give {@code PST}. Note
 * that the count does <i>not</i> specify the exact width of the field.
 * 
 * <li><b>Number</b> the count is the minimum number of digits. Shorter values
 * are zero-padded to this width, longer values overflow this width.
 * 
 * <p>
 * Years are handled specially: {@code yy} truncates to the last 2 digits, but
 * any other number of consecutive {@code y}s does not truncate. So where
 * {@code yyyy} or {@code y} might give {@code 2010}, {@code yy} would give
 * {@code 10}.
 * 
 * <p>
 * Fractional seconds are also handled specially: they're zero-padded on the
 * <i>right</i>.
 * 
 * <li><b>Text/Number</b>: if the count is 3 or more, use text; otherwise use a
 * number. So {@code MM} might give {@code 07} while {@code MMM} gives
 * {@code July}.
 * </ul>
 * 
 * <p>
 * The two pattern characters {@code L} and {@code c} are ICU-compatible
 * extensions, not available in the RI. These are necessary for correct
 * localization in languages such as Russian that distinguish between, say,
 * "June" and "June 2010".
 * 
 * <p>
 ...
 * 		DateFormat.getDateTimeInstance(), DateFormat.getTimeInstance(), };
 ...
 * 
 * <p>
 * Produces this output when run on an {@code en_US} device in the PDT time
 * zone:
 * 
 * <pre>
 * Dec 31, 1969
 ...
 * </pre>
 * 
 * And will produce similarly appropriate localized human-readable output on any
 * user's system.
 * 
 * <p>
 ...
 * 
 * <p>
 * As this example shows, each {@code SimpleDateFormat} instance has a
 * {@link TimeZone}. This is because it's called upon to format instances of
 * {@code Date}, which represents an absolute time in UTC. That is, {@code Date}
 * does not carry time zone information. By default, {@code SimpleDateFormat}
 * will use the system's default time zone. This is appropriate for
 * human-readable output (for which, see the previous sample instead), but
 * generally inappropriate for machine-readable output, where ambiguity is a
 * problem. Note that in this example, the output that included a time but no
 * time zone cannot be parsed back into the original {@code Date}. For this
 * reason it is almost always necessary and desirable to include the timezone in
 * the output. It may also be desirable to set the formatter's time zone to UTC
 * (to ease comparison, or to make logs more readable, for example).
 * 
...
public class SimpleDateFormat extends DateFormat {
	...
	// 'L' and 'c' are ICU-compatible extensions for stand-alone month and
	// stand-alone weekday.
	static final String PATTERN_CHARS = "GyMdkHmsSEDFwWahKzZLc";

	// The index of 'Z' in the PATTERN_CHARS string. This pattern character is
	// supported by the RI,
	// but has no corresponding public constant.
	private static final int RFC_822_TIMEZONE_FIELD = 18;

	// The index of 'L' (cf. 'M') in the PATTERN_CHARS string. This is an
	// ICU-compatible extension
	// necessary for correct localization in various languages
	// (http://b/2633414).
	private static final int STAND_ALONE_MONTH_FIELD = 19;
	// The index of 'c' (cf. 'E') in the PATTERN_CHARS string. This is an
	// ICU-compatible extension
	// necessary for correct localization in various languages
	// (http://b/2633414).
	 ...
	 * See "<a href="../util/Locale.html#default_locale
	 ...
	 * non-localized pattern and the {@code DateFormatSymbols} and
	 * {@code Calendar} for the user's default locale. See
	 * "<a href="../util/Locale
	 * .html#default_locale">Be wary of the default locale</a>".
	 * 
	 * @param pattern
	 *            the pattern.
	...
	/**
	 * Constructs a new {@code SimpleDateFormat} using the specified
	 * non-localized pattern and {@code DateFormatSymbols} and the
	 * {@code Calendar} for the user's default locale. See
	 * "<a href="../util/Locale
	 * .html#default_locale">Be wary of the default locale</a>".
	 * 
	 ...
	 * Constructs a new {@code SimpleDateFormat} using the specified
	 * non-localized pattern and the {@code DateFormatSymbols} and
	 * {@code Calendar} for the specified locale.
	 * 
	 * @param template
	 *            the pattern.
	 * @param locale
	...
	private static String defaultPattern() {
		...
		return localeData.getDateFormat(SHORT) + " "
				+ localeData.getTimeFormat(SHORT);
	}
	 ...
	 * @throws IllegalArgumentException
	 *             if the object cannot be formatted by this simple date format.
	 */
	@Override
	public AttributedCharacterIterator formatToCharacterIterator(Object object) {
		...
		if (object instanceof Number) {
			return formatToCharacterIteratorImpl(new Date(
	 ...
	 * Formats the date.
	 * <p>
	 * If the FieldPosition {@code field} is not null, and the field specified
	 * by this FieldPosition is formatted, set the begin and end index of the
	 * formatted field in the FieldPosition.
	 * <p>
	 * If the Vector {@code fields} is not null, find fields of this date, set
	 * FieldPositions with these fields, and add them to the fields vector.
	 * 
	 * @param date
	 *            Date to Format
	 * @param buffer
	...
	private void append(StringBuffer buffer, FieldPosition position,
		...
		case YEAR_FIELD:
			...
			/*
			 * For 'y' and 'yyy', we're consistent with Unicode and previous
			 * releases of Android. But this means we're inconsistent with the
			 * RI. http://unicode.org/reports/tr35/
			 */
			if (count == 2) {
		...
		case STAND_ALONE_MONTH_FIELD: // L
			...
			appendMonth(buffer, count, formatData.longStandAloneMonths,
					formatData.shortStandAloneMonths);
			break;
		case MONTH_FIELD: // M
			dateFormatField = Field.MONTH;
			appendMonth(buffer, count, formatData.months,
		...
		case STAND_ALONE_DAY_OF_WEEK_FIELD:
			...
			appendDayOfWeek(buffer, count, formatData.longStandAloneWeekdays,
					formatData.shortStandAloneWeekdays);
			break;
		case DAY_OF_WEEK_FIELD:
			dateFormatField = Field.DAY_OF_WEEK;
			appendDayOfWeek(buffer, count, formatData.weekdays,
	...
	private void appendDayOfWeek(StringBuffer buffer, int count,
			String[] longs, String[] shorts) {
	 ...
	 * 
	 * @param count
	 *            the number of z or Z characters in the format string; "zzz"
	 *            would be 3, for example.
	 * @param generalTimeZone
	 *            true if we should use a display name ("PDT") if available;
	 *            false implies that we should use RFC 822 format ("-0800")
	 *            instead. This corresponds to 'z' versus 'Z' in the format
	 *            string.
	 */
	private void appendTimeZone(StringBuffer buffer, int count,
		...
		if (generalTimeZone) {
			...
			if (!formatData.customZoneStrings) {
				buffer.append(tz.getDisplayName(daylight, style,
						formatData.locale));
				return;
			}
			// We can't call TimeZone.getDisplayName() because it would not use
			// the custom DateFormatSymbols of this SimpleDateFormat.
			String custom = TimeZones.lookupDisplayName(formatData.zoneStrings,
					tz.getID(), daylight, style);
			if (custom != null) {
				buffer.append(custom);
				return;
			}
		}
		// We didn't find what we were looking for, so default to a numeric time
		// zone.
	...
	/**
	 * @param generalTimeZone
	 *            "GMT-08:00" rather than "-0800".
	 */
	private void appendNumericTimeZone(StringBuffer buffer,
			boolean generalTimeZone) {
	...
	private void appendNumber(StringBuffer buffer, int count, int value) {
		...
		numberFormat.format(Integer.valueOf(value), buffer,
				new FieldPosition(0));
	 ...
	 * two-digit year values. See {@link #set2DigitYearStart} for details.
	 */
	...
	private int parse(String string, int offset, char format, int count) {
		...
		case STAND_ALONE_MONTH_FIELD:
			return parseMonth(string, offset, count, absolute,
					formatData.longStandAloneMonths,
					formatData.shortStandAloneMonths);
		case MONTH_FIELD:
			return parseMonth(string, offset, count, absolute,
		...
		case MILLISECOND_FIELD:
			...
			break;
		case STAND_ALONE_DAY_OF_WEEK_FIELD:
			return parseDayOfWeek(string, offset,
					formatData.longStandAloneWeekdays,
					formatData.shortStandAloneWeekdays);
		case DAY_OF_WEEK_FIELD:
			return parseDayOfWeek(string, offset, formatData.weekdays,
					formatData.shortWeekdays);
		case DAY_OF_YEAR_FIELD:
			field = Calendar.DAY_OF_YEAR;
			break;
	...
	private int parseDayOfWeek(String string, int offset, String[] longs,
			String[] shorts) {
	...
	private int parseMonth(String string, int offset, int count, int absolute,
			String[] longs, String[] shorts) {
		if (count <= 2) {
	...
	/**
	 * Sets the date which is the start of the one hundred year period for
	 * two-digit year values.
	 * 
	 * <p>
	 * When parsing a date string using the abbreviated year pattern {@code yy},
	 * {@code SimpleDateFormat} must interpret the abbreviated year relative to
	 * some century. It does this by adjusting dates to be within 80 years
	 * before and 20 years after the time the {@code SimpleDateFormat} instance
	 * was created. For example, using a pattern of {@code MM/dd/yy}, an
	 * instance created on Jan 1, 1997 would interpret the string
	 * {@code "01/11/12"} as Jan 11, 2012 but interpret the string
	 * {@code "05/04/64"} as May 4, 1964. During parsing, only strings
	 * consisting of exactly two digits, as defined by
	 * {@link java.lang.Character#isDigit(char)}, will be parsed into the
	 * default century. Any other numeric string, such as a one digit string, a
	 * three or more digit string, or a two digit string that isn't all digits
	 * (for example, {@code "-1"}), is interpreted literally. So using the same
	 * pattern, both {@code "01/02/3"} and {@code "01/02/003"} are parsed as Jan
	 * 2, 3 AD. Similarly, {@code "01/02/-3"} is parsed as Jan 2, 4 BC.
	 * 
	 * <p>
	 * If the year pattern does not have exactly two 'y' characters, the year is
	 * interpreted literally, regardless of the number of digits. So using the
	 * pattern {@code MM/dd/yyyy}, {@code "01/11/12"} is parsed as Jan 11, 12
	 * A.D.
	 */
	...
	public String toLocalizedPattern() {
		return convertPattern(pattern, PATTERN_CHARS,
				formatData.getLocalPatternChars(), false);
	}

	private static String convertPattern(String template, String fromChars,
			String toChars, boolean check) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (!quote && (index = fromChars.indexOf(next)) != -1) {
				...
				throw new IllegalArgumentException(
						"Invalid pattern character '" + next + "' in " + "'"
								+ template + "'");
			} else {
				output.append(next);
			}

...
/**
 * Produces concatenated messages in language-neutral way. New code should
 * probably use {@link java.util.Formatter} instead.
 * <p>
 * {@code MessageFormat} takes a set of objects, formats them and then inserts
 ...
 * {@code Format} classes in that you create a {@code MessageFormat} object with
 * one of its constructors (not with a {@code getInstance} style factory
 * method). The factory methods aren't necessary because {@code MessageFormat}
 * itself doesn't implement locale-specific behavior. Any locale-specific
 * behavior is defined by the pattern that you provide as well as the subformats
 * used for inserted arguments.
 * 
 * <h4><a name="patterns">Patterns and their interpretation</a></h4>
 * 
 * {@code MessageFormat} uses patterns of the following form: <blockquote>
 * 
 * <pre>
 ...
 * may be interpreted by the subformat. For example, {@code "
 * 1,number,$'#',##}"} will produce a number format with the hash-sign quoted,
 * with a result such as: "$#31,45". An <i>UnquotedPattern</i> can contain
 * arbitrary characters except single quotes, but curly braces within it must be
 * balanced. For example, {@code "ab 0} de"} and {@code "ab '}' de"} are valid
 * subformat patterns, but {@code "ab 0'}' de"} and {@code "ab } de"} are not.
 * <dl>
 * <dt><b>Warning:</b></dt>
 ...
 * in {@code MessageFormat}. See the annotations for U+0027 Apostrophe in The
 * Unicode Standard.
 * </dl>
 * <p>
 * The <i>ArgumentIndex</i> value is a non-negative integer written using the
 * digits '0' through '9', and represents an index into the {@code arguments}
 * array passed to the {@code format} methods or the result array returned by
 ...
 * </blockquote>
 * <p>
 * Typically, the message format will come from resources, and the arguments
 * will be dynamically set at runtime.
 * <p>
 ...
 * 
 * <p>
 * For more sophisticated patterns, you can use a {@code ChoiceFormat} to get
 * output such as: <blockquote>
 * 
 * <pre>
 ...
 * {@code ChoiceFormat} in {@code MessageFormat} is treated specially;
 * occurances of '{' are used to indicated subformats, and cause recursion. If
 * you create both a {@code MessageFormat} and {@code ChoiceFormat}
...
public class MessageFormat extends Format {
	 ...
	 * returns an {@code AttributedCharacterIterator} with the formatted message
	 * and attributes. The {@code AttributedCharacterIterator} returned also
	 * includes the attributes from the formats of this message format.
	 * 
	 * @param object
	 *            the object to format.
	 * @return an {@code AttributedCharacterIterator} with the formatted message
	 *         and attributes.
	 * @throws IllegalArgumentException
	 *             if the arguments in the object array cannot be formatted by
	 *             this message format.
	 */
	@Override
	...
	private void handleArgumentField(int begin, int end, int argIndex,
			FieldPosition position, Vector<FieldContainer> fields) {
		if (fields != null) {
			fields.add(new FieldContainer(begin, end, Field.ARGUMENT, Integer
					.valueOf(argIndex)));
		} else {
			if (position != null
	 ...
	 * index specified by {@code position}. If the string is successfully parsed
	 * then the index of the {@code ParsePosition} is updated to the index
	 * following the parsed text. On error, the index is unchanged and the error
	 * index of {@code ParsePosition} is set to the index where the error
	 * occurred.
	 * 
	 * @param string
	 *            the string to parse.
	...
	private int match(String string, ParsePosition position, boolean last,
		...
		for (int i = tokens.length; --i >= 0;) {
			if (string.regionMatches(true, offset, tokens[i], 0,
					tokens[i].length())) {
				token = i;
				break;
			}
			...
			if (format instanceof DecimalFormat) {
				...
				try {
					formats[i] = new SimpleDateFormat(
							((SimpleDateFormat) format).toPattern(), locale);
				} catch (NullPointerException npe) {
					formats[i] = null;
				}
			}
	...
	private void readObject(ObjectInputStream stream) throws IOException,
		...
		if (maxOffset + 1 < strings.length) {
			strings[strings.length - 1] = pattern.substring(last,
					pattern.length());
		}
	}
	...
	public static class Field extends Format.Field {
		...
		protected Object readResolve() throws InvalidObjectException {
			...
			throw new InvalidObjectException(
					"Not a valid MessageFormat.Field, subclass should override readResolve()");
		}
	}
}

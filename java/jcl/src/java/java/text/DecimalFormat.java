 ...
 * <td>Number</td>
 * <td>No</td>
 * <td><strong><font color="red">NEW</font>&nbsp;</strong> Significant digit.</td>
 * </tr>
 ...
 * <td>Exponent</td>
 * <td>Yes</td>
 * <td><strong><font color="red">NEW</font>&nbsp;</strong> Prefix positive
 * exponents with localized plus sign.
 * <em>Does not need to be quoted in prefix or suffix.</em></td>
 * </tr>
 * <tr valign="top" bgcolor="#eeeeff">
 * <td>{@code ;}</td>
 ...
 * <td>Yes</td>
 * <td><strong><font color="red">NEW</font>&nbsp;</strong> Pad escape, precedes
 * pad character.</td>
 * </tr>
 * </table>
 * </blockquote>
 ...
 * <li><font color="red"><strong>NEW</strong>&nbsp;</font> Two grouping
 * intervals are recognized: The one between the decimal point and the first
 * grouping symbol and the one between the first and second grouping symbols.
 ...
 * <li><strong><font color="red">NEW</font>&nbsp;</strong> The pad specifier
 * {@code padSpec} may appear before the prefix, after the prefix, before the
 * suffix, after the suffix or not at all.
 * </ul>
 * <h4>Parsing</h4>
 * <p>
 ...
 * shown. For example, 1997 is formatted as "97" if maximum integer digits is
 * set to 2.
 * <li>If the number of actual integer digits is less than the
 * <em>minimum integer digits</em>, then leading zeros are added. For example,
 * 1997 is formatted as "01997" if minimum integer digits is set to 5.
 * <li>If the number of actual fraction digits exceeds the <em>maximum
 * fraction digits</em>, then half-even rounding is performed to the maximum
 ...
 * 0.125 is formatted as "0.1250" if the minimum fraction digits is set to 4.
 ...
 * <em>maximum fraction</em> digits, and it is unaffected by the maximum integer
 * digits. If this sum is zero, then all significant digits are shown. If
 * significant digits are used in the pattern then the number of integer digits
 * is fixed at one and there is no exponent grouping.
 * <li>Exponential patterns may not contain grouping separators.
 * </ul>
 * <a name="sigdig">
 * <h4><strong><font color="red">NEW</font>&nbsp;</strong> Significant Digits</h4>
 * <p>
 ...
 * specified in any other location, {@link #applyPattern} throws an
 * {@link IllegalArgumentException}. If there is no prefix, before the prefix
 * and after the prefix are equivalent, likewise for the suffix.</li>
 ...
 * pad escape, then the pattern is illegal.</li>
 * </ul>
...
public class DecimalFormat extends NumberFormat {
	 ...
	 * for the user's default locale. See
	 ...
	 * pattern and the {@code DecimalFormatSymbols} for the user's default
	...
	// Used by NumberFormat.getInstance because cloning DecimalFormatSymbols is
	// slow.
	...
	public boolean equals(Object object) {
		...
		return (this.dform == null ? other.dform == null : this.dform
				.equals(other.dform))
				&& getDecimalFormatSymbols().equals(
						other.getDecimalFormatSymbols());
	}
	...
	private void checkBufferAndFieldPosition(StringBuffer buffer,
	...
	public StringBuffer format(double value, StringBuffer buffer,
		...
		if (roundingMode == RoundingMode.UNNECESSARY) {
			...
			try {
				...
				String upResult = format(value, new StringBuffer(),
						new FieldPosition(0)).toString();
				setRoundingMode(RoundingMode.DOWN);
				String downResult = format(value, new StringBuffer(),
						new FieldPosition(0)).toString();
	...
	@Override
	public final StringBuffer format(Object number, StringBuffer buffer,
			FieldPosition position) {
		checkBufferAndFieldPosition(buffer, position);
		if (number instanceof BigInteger) {
			BigInteger bigInteger = (BigInteger) number;
			char[] chars = (bigInteger.bitLength() < 64) ? dform.formatLong(
					bigInteger.longValue(), position) : dform.formatBigInteger(
					bigInteger, position);
			buffer.append(chars);
			return buffer;
	...
	/**
	 * Indicates whether parsing with this decimal format will only return
	 * numbers of type {@code java.lang.Integer}.
	 * 
	...
	public void setDecimalFormatSymbols(DecimalFormatSymbols value) {
		if (value != null) {
			// BEGIN android-changed: the Java object is canonical, and we copy
			// down to native code.
			this.symbols = (DecimalFormatSymbols) value.clone();
	...
	/**
	 * Sets the maximum number of digits after the decimal point. If the value
	 * passed is negative then it is replaced by 0. Regardless of this setting,
	 * no more than 340 digits will be used.
	 * 
	 * @param value
	...
	public void setMaximumFractionDigits(int value) {
		...
		// Changing the maximum fraction digits needs to update ICU4C's rounding
		// configuration.
		setRoundingMode(roundingMode);
	}

	/**
	 * Sets the maximum number of digits before the decimal point. If the value
	 * passed is negative then it is replaced by 0. Regardless of this setting,
	 * no more than 309 digits will be used.
	 * 
	 * @param value
	...
	/**
	 * Sets the minimum number of digits after the decimal point. If the value
	 * passed is negative then it is replaced by 0. Regardless of this setting,
	 * no more than 340 digits will be used.
	 * 
	 * @param value
	...
	/**
	 * Sets the minimum number of digits before the decimal point. If the value
	 * passed is negative then it is replaced by 0. Regardless of this setting,
	 * no more than 309 digits will be used.
	 * 
	 * @param value
	...
	private void readObject(ObjectInputStream stream) throws IOException,
		...
		dform.setDecimalSeparatorAlwaysShown(fields.get(
				"decimalSeparatorAlwaysShown", false));

		setRoundingMode((RoundingMode) fields.get("roundingMode",
				RoundingMode.HALF_EVEN));

		final int maximumIntegerDigits = fields
				.get("maximumIntegerDigits", 309);
		final int minimumIntegerDigits = fields
				.get("minimumIntegerDigits", 309);
		final int maximumFractionDigits = fields.get("maximumFractionDigits",
				340);
		final int minimumFractionDigits = fields.get("minimumFractionDigits",
				340);
		// BEGIN android-changed: tell ICU what we want, then ask it what we can
		// have, and then
		// set that in our Java object. This isn't RI-compatible, but then very
		// little of our
		// behavior in this area is, and it's not obvious how we can
		// second-guess ICU (or tell
		// it to just do exactly what we ask). We only need to do this with
		// maximumIntegerDigits
		// because ICU doesn't seem to have its own ideas about the other
		// options.
	...
	public void setRoundingMode(RoundingMode roundingMode) {
		...
		if (roundingMode != RoundingMode.UNNECESSARY) { // ICU4C doesn't support
														// UNNECESSARY.
			double roundingIncrement = 1.0 / Math.pow(10,
					Math.max(0, getMaximumFractionDigits()));

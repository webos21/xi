...
public class RuleBasedCollator extends Collator {
	 ...
	 * standard Unicode Collation Algorithm (UCA). This differs slightly from
	 * other implementations which work with full {@code rules} specifications
	 * and may result in different behavior.
	 * 
	 * @param rules
	 *            the collation rules.
	...
	public RuleBasedCollator(String rules) throws ParseException {
		...
		try {
			...
			this.icuColl
					.setDecomposition(com.ibm.icu4jni.text.Collator.CANONICAL_DECOMPOSITION);
		} catch (Exception e) {
	...
	public String getRules() {
		return ((com.ibm.icu4jni.text.RuleBasedCollator) this.icuColl)
				.getRules();
	}

	/**
	 * Returns a new collator with the same collation rules, decomposition mode
	 * and strength value as this collator.
	 * 
	 ...
	 * {@code RuleBasedCollator}. See the {@code Collator} class description for
	 * an example of use.
	 * <p>
	 * General recommendation: If comparisons are to be done with the same
	 * strings multiple times, it is more efficient to generate
	 * {@code CollationKey} objects for the strings and use

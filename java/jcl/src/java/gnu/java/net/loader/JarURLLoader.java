...
/**
 * A <code>JarURLLoader</code> is a type of <code>URLLoader</code> only loading
 * from jar url.
 */
public final class JarURLLoader extends URLLoader {
	...
	private JarURLLoader(URLClassLoader classloader,
			URLStreamHandlerCache cache, URLStreamHandlerFactory factory,
			URL baseURL, URL absoluteUrl, Set indexSet) {
	...
	private void initialize() {
		...
		try {
			jarfile = ((JarURLConnection) baseJarURL.openConnection())
					.getJarFile();
			...
			if (indexMap != null) {
				...
				Iterator<Map.Entry<URL, Set<String>>> it = indexMap.entrySet()
						.iterator();
				while (it.hasNext()) {
					...
					if (subURL.equals(baseURL)) {
						...
						JarURLLoader subLoader = new JarURLLoader(classloader,
								cache, factory, subURL, subURL, prefixes);
						// Note that we don't care if the sub-loader itself has
						// an
	...
	public Resource getResource(String name) {
		...
		if (indexSet != null) {
			...
			// in our model this is tricky to implement, as another URLLoader
			// from

...
public final class TimeZones {
	private static final String[] availableTimeZones = TimeZone
			.getAvailableIDs();
	 ...
	 */
	public static String getDisplayName(String id, boolean daylight, int style,
			Locale locale) {
		// If we already have the strings, linear search through them is 10x
		// quicker than
		// calling ICU for just the one we want.
		if (CachedTimeZones.locale.equals(locale)) {
			String result = lookupDisplayName(CachedTimeZones.names, id,
					daylight, style);
			if (result != null) {
				return result;
			}
	...
	public static String lookupDisplayName(String[][] zoneStrings, String id,
	...
	/**
	 * Initialization holder for default time zone names. This class will be
	 * preloaded by the zygote to share the time and space costs of setting up
	 * the list of time zone names, so although it looks like the lazy
	...
	private static String[][] createZoneStringsFor(Locale locale) {
		...
		// Don't be distracted by all the code either side of this line: this is
		// the expensive bit!
		getZoneStringsImpl(arrayToFill, locale.toString());

		// Reorder the entries so we get the expected result.
		// We also take the opportunity to de-duplicate the names
		// (http://b/2672057).
		...
		Logger.getLogger(Logger.GLOBAL_LOGGER_NAME).info(
				"Loaded time zone names for " + locale + " in " + duration
						+ "ms.");
	...
	/**
	 * Returns an array of time zone strings, as used by
	...
	/**
	 * Returns an array containing the time zone ids in use in the country
	 * corresponding to the given locale. This is not necessary for Java API,
	 * but is used by telephony as a fallback.
	 */
	...
	private static native void getZoneStringsImpl(String[][] arrayToFill,
			String locale);

	private static native String getDisplayNameImpl(String id, boolean isDST,
			int style, String locale);
}

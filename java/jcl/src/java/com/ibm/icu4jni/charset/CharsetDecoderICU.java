...
public final class CharsetDecoderICU extends CharsetDecoder {
	 ...
	 * the number of input bytes consumed data[OUTPUT_OFFSET] = on input
	 * contains the start of output and on output the number of output chars
	 * written data[INVALID_BYTES] = number of invalid bytes data[INPUT_HELD] =
	 * number of input bytes held in the converter's state
	 */
	...
	public static CharsetDecoderICU newInstance(Charset cs,
		...
		try {
			...
			float averageCharsPerByte = NativeConverter
					.getAveCharsPerByte(address);
			CharsetDecoderICU result = new CharsetDecoderICU(cs,
					averageCharsPerByte, address);
			address = 0; // CharsetDecoderICU has taken ownership; its finalizer
							// will do the free.
	...
	private CharsetDecoderICU(Charset cs, float averageCharsPerByte,
			long address) {
	...
	/**
	 * Sets this decoders replacement string. Substitutes the string in input if
	 * an unmappable or illegal sequence is encountered
	 * 
	 * @param newReplacement
	 *            to replace the error bytes with
	 * @stable ICU 2.4
	 */
	...
	protected final CoderResult implFlush(CharBuffer out) {
		try {
			...
			ec = NativeConverter.flushByteToChar(converterHandle, /*
																 * Handle to ICU
																 * Converter
																 */
	...
	/**
	 * Decodes one or more bytes. The default behaviour of the converter is stop
	 * and report if an error in input stream is encountered. To set different
	 * behaviour use @see CharsetDecoder.onMalformedInput() This method allows a
	 * buffer by buffer conversion of a data stream. The state of the conversion
	 * is saved between calls to convert. Among other things, this means
	 * multibyte input sequences can be split between calls. If a call to
	 * convert results in an Error, the conversion may be continued by calling
	 * convert again with suitably modified parameters.All conversions should be
	...
	protected CoderResult decodeLoop(ByteBuffer in, CharBuffer out) {
		...
		try {
			ec = NativeConverter.decode(converterHandle, /*
														 * Handle to ICU
														 * Converter
														 */

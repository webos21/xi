...
public final class CharsetEncoderICU extends CharsetEncoder {
	...
	static {
		// ICU has different default replacements to the RI in some cases. There
		// are many
		// additional cases, but this covers all the charsets that Java
		// guarantees will be
		// available, which is where compatibility seems most important. (The RI
		// even uses
		// the byte corresponding to '?' in ASCII as the replacement byte for
		// charsets where that
		// byte corresponds to an entirely different character.)
		// It's odd that UTF-8 doesn't use U+FFFD, given that (unlike ISO-8859-1
		// and US-ASCII) it
	...
	/*
	 * data[INPUT_OFFSET] = on input contains the start of input and on output
	 * the number of input chars consumed data[OUTPUT_OFFSET] = on input
	 * contains the start of output and on output the number of output bytes
	 * written data[INVALID_CHARS] = number of invalid chars data[INPUT_HELD] =
	 * number of input chars held in the converter's state
	 */
	...
	public static CharsetEncoderICU newInstance(Charset cs,
			String icuCanonicalName) {
		// This complexity is necessary to ensure that even if the constructor,
		// superclass
		// constructor, or call to updateCallback throw, we still free the
		// native peer.
		long address = 0;
		try {
			address = NativeConverter.openConverter(icuCanonicalName);
			float averageBytesPerChar = NativeConverter
					.getAveBytesPerChar(address);
			...
			CharsetEncoderICU result = new CharsetEncoderICU(cs,
					averageBytesPerChar, maxBytesPerChar, replacement, address);
			address = 0; // CharsetEncoderICU has taken ownership; its finalizer
							// will do the free.
	...
	private static byte[] makeReplacement(String icuCanonicalName, long address) {
		// We have our own map of RI-compatible default replacements (where ICU
		// disagrees)...
	...
	private CharsetEncoderICU(Charset cs, float averageBytesPerChar,
			float maxBytesPerChar, byte[] replacement, long address) {
		super(cs, averageBytesPerChar, maxBytesPerChar, replacement);
		this.converterHandle = address;
	}

	/**
	 * Sets this encoders replacement string. Substitutes the string in output
	 * if an unmappable or illegal sequence is encountered
	 * 
	 * @param newReplacement
	 *            to replace the error chars with
	 * @stable ICU 2.4
	 */
	protected void implReplaceWith(byte[] newReplacement) {
		if (converterHandle != 0) {
			if (newReplacement.length > NativeConverter
					.getMaxBytesPerChar(converterHandle)) {
				throw new IllegalArgumentException(
						"Number of replacement Bytes are greater than max bytes per char");
			}
			updateCallback();
		}
	}
	...
	protected CoderResult implFlush(ByteBuffer out) {
		try {
			...
			if (ErrorCode.isFailure(ec)) {
				if (ec == ErrorCode.U_BUFFER_OVERFLOW_ERROR) {
				...
				} else if (ec == ErrorCode.U_TRUNCATED_CHAR_FOUND) {// CSDL: add
																	// this
																	// truncated
																	// character
																	// error
					...
					if (data[INPUT_OFFSET] > 0) {
						return CoderResult
								.malformedForLength(data[INPUT_OFFSET]);
					}
				} else {
	...
	/**
	 * Encodes one or more chars. The default behaviour of the converter is stop
	 * and report if an error in input stream is encountered. To set different
	 * behaviour use @see CharsetEncoder.onMalformedInput()
	 * 
	 * @param in
	 *            buffer to decode
	 * @param out
	 *            buffer to populate with decoded result
	 * @return result of decoding action. Returns CoderResult.UNDERFLOW if the
	 *         decoding action succeeds or more input is needed for completing
	 *         the decoding action.
	 * @stable ICU 2.4
	 */
	...
	/**
	 * Ascertains if a given Unicode character can be converted to the target
	 * encoding
	 * 
	 * @param c
	...
	/**
	 * Ascertains if a given Unicode code point (32bit value for handling
	 * surrogates) can be converted to the target encoding. If the caller wants
	 * to test if a surrogate pair can be converted to target encoding then the
	 * responsibility of assembling the int value lies with the caller. For
	 * assembling a code point the caller can use UTF16 class of ICU4J and do
	 * something like:
	 * 
	 * <pre>
	 ...
	 * 
	 * @param codepoint
	 *            Unicode code point as int value
	 * @return true if a character can be converted
	 * @obsolete ICU 2.4
	 * @deprecated ICU 3.4
	 */

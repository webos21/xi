...
public final class NativeDecimalFormat {
	/**
	 * Constants corresponding to the native type UNumberFormatSymbol, for
	 * setSymbol.
	 */
	...
	/**
	 * Constants corresponding to the native type UNumberFormatTextAttribute,
	 * for getTextAttribute/setTextAttribute.
	 */
	...
	/**
	 * The last pattern we gave to ICU, so we can make repeated applications
	 * cheap. This helps in cases like String.format("%.2f,%.2f\n", x, y) where
	 * the DecimalFormat is reused.
	 */
	...
	public NativeDecimalFormat(String pattern, DecimalFormatSymbols dfs) {
		try {
			this.addr = open(pattern, dfs.getCurrencySymbol(),
					dfs.getDecimalSeparator(), dfs.getDigit(),
					dfs.getExponentSeparator(), dfs.getGroupingSeparator(),
					dfs.getInfinity(), dfs.getInternationalCurrencySymbol(),
					dfs.getMinusSign(), dfs.getMonetaryDecimalSeparator(),
					dfs.getNaN(), dfs.getPatternSeparator(), dfs.getPercent(),
					dfs.getPerMill(), dfs.getZeroDigit());
			this.lastPattern = pattern;
		} catch (NullPointerException npe) {
			throw npe;
		} catch (RuntimeException re) {
			throw new IllegalArgumentException("syntax error: "
					+ re.getMessage() + ": " + pattern);
		}
	}

	// Used so java.util.Formatter doesn't need to allocate DecimalFormatSymbols
	// instances.
	public NativeDecimalFormat(String pattern, LocaleData data) {
		this.addr = open(pattern, data.currencySymbol, data.decimalSeparator,
				data.digit, data.exponentSeparator, data.groupingSeparator,
				data.infinity, data.internationalCurrencySymbol,
				data.minusSign, data.monetarySeparator, data.NaN,
				data.patternSeparator, data.percent, data.perMill,
				data.zeroDigit);
		this.lastPattern = pattern;
	}
	...
	// TODO: remove this and just have DecimalFormat.hashCode do the right thing
	// itself.
	@Override
	public int hashCode() {
	...
	/**
	 * Note: this doesn't check that the underlying native DecimalFormat
	 * objects' configured native DecimalFormatSymbols objects are equal. It is
	 * assumed that the caller (DecimalFormat) will check the
	 * DecimalFormatSymbols objects instead, for performance.
	 * 
	 * This is also unreasonably expensive, calling down to JNI multiple times.
	 * 
	 * TODO: remove this and just have DecimalFormat.equals do the right thing
	 * itself.
	 */
	@Override
	public boolean equals(Object object) {
		...
		return obj.toPattern().equals(this.toPattern())
				&& obj.isDecimalSeparatorAlwaysShown() == this
				...
				&& obj.getMaximumIntegerDigits() == this
						.getMaximumIntegerDigits()
				&& obj.getMaximumFractionDigits() == this
						.getMaximumFractionDigits()
				&& obj.getMinimumIntegerDigits() == this
						.getMinimumIntegerDigits()
				&& obj.getMinimumFractionDigits() == this
	...
	public void setDecimalFormatSymbols(final DecimalFormatSymbols dfs) {
		setDecimalFormatSymbols(this.addr, dfs.getCurrencySymbol(),
				dfs.getDecimalSeparator(), dfs.getDigit(),
				dfs.getExponentSeparator(), dfs.getGroupingSeparator(),
				dfs.getInfinity(), dfs.getInternationalCurrencySymbol(),
				dfs.getMinusSign(), dfs.getMonetaryDecimalSeparator(),
	...
	public void setDecimalFormatSymbols(final LocaleData localeData) {
		setDecimalFormatSymbols(this.addr, localeData.currencySymbol,
				localeData.decimalSeparator, localeData.digit,
				localeData.exponentSeparator, localeData.groupingSeparator,
				localeData.infinity, localeData.internationalCurrencySymbol,
				localeData.minusSign, localeData.monetarySeparator,
				localeData.NaN, localeData.patternSeparator,
	...
	public char[] formatBigDecimal(BigDecimal value, FieldPosition field) {
		FieldPositionIterator fpi = FieldPositionIterator
				.forFieldPosition(field);
	...
	public AttributedCharacterIterator formatToCharacterIterator(Object object) {
		...
		if (number instanceof BigInteger || number instanceof BigDecimal) {
			text = new String(formatDigitList(this.addr, number.toString(),
					fpIter));
	...
	public void setCurrency(Currency currency) {
		...
		setSymbol(this.addr, UNUM_INTL_CURRENCY_SYMBOL,
	...
	private static class FieldPositionIterator {
		...
		private static int getNativeFieldPositionId(FieldPosition fp) {
			...
			// Clients can construct FieldPosition combining any attribute with
			// any field
		...
		private static void setFieldPosition(FieldPositionIterator fpi,
				FieldPosition fp) {
			if (fpi != null && fp != null) {
		...
		private static Format.Field fields[] = {
				// The old java field values were 0 for integer and 1 for
				// fraction.
				// The new java field attributes are all objects. ICU assigns
				// the values
				// starting from 0 in the following order; note that integer and
				// fraction positions match the old field values.
				NumberFormat.Field.INTEGER, NumberFormat.Field.FRACTION,
				NumberFormat.Field.DECIMAL_SEPARATOR,
				NumberFormat.Field.EXPONENT_SYMBOL,
				NumberFormat.Field.EXPONENT_SIGN, NumberFormat.Field.EXPONENT,
				NumberFormat.Field.GROUPING_SEPARATOR,
				NumberFormat.Field.CURRENCY, NumberFormat.Field.PERCENT,
	...
	private static native void applyPatternImpl(int addr, boolean localized,
			String pattern);
	...
	private static native char[] formatLong(int addr, long value,
			FieldPositionIterator iter);

	private static native char[] formatDouble(int addr, double value,
			FieldPositionIterator iter);

	private static native char[] formatDigitList(int addr, String value,
			FieldPositionIterator iter);
	...
	private static native int open(String pattern, String currencySymbol,
			char decimalSeparator, char digit, String exponentSeparator,
			char groupingSeparator, String infinity,
			String internationalCurrencySymbol, char minusSign,
			char monetaryDecimalSeparator, String nan, char patternSeparator,
			char percent, char perMill, char zeroDigit);

	private static native Number parse(int addr, String string,
			ParsePosition position, boolean parseBigDecimal);

	private static native void setDecimalFormatSymbols(int addr,
			String currencySymbol, char decimalSeparator, char digit,
	...
	private static native void setRoundingMode(int addr, int roundingMode,
			double roundingIncrement);

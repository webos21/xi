...
public final class RuleBasedCollator extends Collator {
	...
	// No-one uses RuleBasedCollator as a hash key.
	 ...
	 *                thrown if rules are empty or a Runtime error if collator
	 *                can not be created.
	 * @stable ICU 2.4
	 */
	...
	public RuleBasedCollator(String rules, int strength) throws ParseException {
		...
		m_collator_ = NativeCollation.openCollatorFromRules(rules,
				CollationAttribute.VALUE_OFF, strength);
	}
	 ...
	 * com.ibm.icu4jni.text.Normalization. In 2.4, the valid normalizationMode
	 * arguments for this API are CollationAttribute.VALUE_ON and
	 * CollationAttribute.VALUE_OFF.
	 * </p>
	 * 
	 * @param rules
	 ...
	 *                thrown when constructor error occurs
	 * @see #PRIMARY
	 * @see #SECONDARY
	 * @see #TERTIARY
	 * @see #QUATERNARY
	...
	/**
	 * <p>
	 * Sets the decomposition mode of the Collator object on or off. If the
	 * decomposition mode is set to on, string would be decomposed into NFD
	 * format where necessary before sorting.
	 * </p>
	 * </p>
	 * 
	 ...
	 * transformation.
	 * <p>
	 * E.g. with strength == CollationAttribute.VALUE_SECONDARY, the tertiary
	 ...
	 * @exception IllegalArgumentException
	 *                when argument does not belong to any collation strength
	 ...
	 * @param value
	 *            attribute value from CollationAttribute
	 * @stable ICU 2.4
	 */
	...
	/**
	 * Get the collation rules of this Collation object The rules will follow
	 * the rule syntax.
	 * 
	...
	public CollationElementIterator getCollationElementIterator(String source) {
		CollationElementIterator result = new CollationElementIterator(
				NativeCollation
						.getCollationElementIterator(m_collator_, source));
	...
	public CollationElementIterator getCollationElementIterator(
		...
		// We only implement the String-based API, so build a string from the
		// iterator.
	...
	private String characterIteratorToString(CharacterIterator it) {
		...
		for (char ch = it.current(); ch != CharacterIterator.DONE; ch = it
				.next()) {
			result.append(ch);
		}

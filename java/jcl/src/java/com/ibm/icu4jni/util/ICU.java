...
public final class ICU {
	...
	/**
	 * Returns the appropriate {@code Locale} given a {@code String} of the form
	 * returned by {@code toString}. This is very lenient, and doesn't care
	 * what's between the underscores: this method can parse strings that
	 * {@code Locale.toString} won't produce. Used to remove duplication.
	 */
	public static Locale localeFromString(String localeName) {
		...
		if (first == -1) {
			...
			return new Locale(localeName.substring(0, first),
					localeName.substring(first + 1));
		} else {
			// Language and country and variant ("ja_JP_TRADITIONAL").
			return new Locale(localeName.substring(0, first),
					localeName.substring(first + 1, second),
	...
	public static native String getCurrencySymbolNative(String locale,
			String currencyCode);

	public static native String getDisplayCountryNative(String countryCode,
			String locale);

	public static native String getDisplayLanguageNative(String languageCode,
			String locale);

	public static native String getDisplayVariantNative(String variantCode,
			String locale);

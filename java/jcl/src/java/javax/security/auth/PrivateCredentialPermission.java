 ...
 * <p>
 * The principals on their part are specified as the name of the
 * {@code Principal} class followed by the principal name in quotes. For
 * example, the following file may define permission to read the private
 * credentials of a principal named "Bob":
 * "com.sun.PrivateCredential com.sun.Principal \"Bob\""
 * <p>
 * The syntax also allows the use of the wildcard "*" in place of
 * {@code CredentialClass} or {@code PrincipalClass} and/or
 * {@code PrincipalName}.
 * 
 * @see Principal
 */
public final class PrivateCredentialPermission extends Permission {
	...
	PrivateCredentialPermission(String credentialClass,
		...
		for (Principal p : principals) {
			CredOwner element = new CredOwner(p.getClass().getName(),
					p.getName());
	 ...
	 * {@code PrivateCredentialPermission} as a two dimensional array. The first
	...
	public boolean implies(Permission permission) {
		...
		if (!("*".equals(credentialClass) || credentialClass.equals(that
				.getCredentialClass()))) {
			return false;
		}
	...
	private static final class CredOwner implements Serializable {
		...
		// Creates a new CredOwner with the specified Principal Class and
		// Principal Name
		...
		private IllegalArgumentException badPrincipal() {
			throw new IllegalArgumentException(
					"invalid syntax: Principal Class can not be a "

...
public abstract class SSLSocket extends Socket {
	...
	/**
	 * Sets the names of the cipher suites to be enabled. Only cipher suites
	 * returned by {@link #getSupportedCipherSuites()} are allowed.
	 * 
	 * @param suites
	...
	/**
	 * Sets the names of the protocols to be enabled. Only protocols returned by
	 * {@link #getSupportedProtocols()} are allowed.
	 * 
	 * @param protocols
	 ...
	 * handshake will be initiated, in which case this method will block until
	 * the handshake has been established. If the handshake fails, an invalid
	...
	/**
	 * Returns whether this connection requires client authentication. This is
	 * only useful for sockets in server mode.
	 * 
	...
	/**
	 * Returns a new SSLParameters based on this SSLSocket's current cipher
	 * suites, protocols, and client authentication settings.
	 * 
	 * @since 1.6
	 */
	 ...
	 * Specifically, sets the SSLSocket's enabled cipher suites if the
	 * parameter's cipher suites are non-null. Similarly sets the enabled
	 * protocols. If the parameters specify the want or need for client
	 * authentication, those requirements are set on the SSLSocket, otherwise

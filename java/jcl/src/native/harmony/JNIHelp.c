...
/* by jshwang
 ...
 if (clazz == NULL) {
 log_print(XDLOG, "Native registration unable to find class '%s'\n", className);
 return -1;
 }

 int result = 0;
 if ((*env)->RegisterNatives(env, clazz, gMethods, numMethods) < 0) {
 log_print(XDLOG, "RegisterNatives failed for '%s'\n", className);
 result = -1;
 }
...
static void getExceptionSummary(JNIEnv* env, jthrowable exception, char* buf,
	...
	jmethodID classGetNameMethod = (*env)->GetMethodID(env, classClazz,
			"getName", "()Ljava/lang/String;");
	jstring classNameStr = (*env)->CallObjectMethod(env, exceptionClazz,
			classGetNameMethod);
	if (classNameStr != NULL) {
		/* get printable string */
		const char* classNameChars = (*env)->GetStringUTFChars(env,
				classNameStr, NULL);
		if (classNameChars != NULL) {
			/* if the exception has a message string, get that */
			jmethodID throwableGetMessageMethod = (*env)->GetMethodID(env,
					exceptionClazz, "getMessage", "()Ljava/lang/String;");
			jstring messageStr = (*env)->CallObjectMethod(env, exception,
					throwableGetMessageMethod);

			if (messageStr != NULL) {
				const char* messageChars = (*env)->GetStringUTFChars(env,
						messageStr, NULL);
				if (messageChars != NULL) {
					xi_snprintf(buf, bufLen, "%s: %s", classNameChars,
							messageChars);
					(*env)->ReleaseStringUTFChars(env, messageStr, messageChars);
				} else {
					(*env)->ExceptionClear(env); /* clear OOM */
					xi_snprintf(buf, bufLen, "%s: <error getting message>",
							classNameChars);
				}
	...
	if (!success) {
		...
		xi_snprintf(buf, bufLen, "%s", "<error getting class name>");
	}
}
...
static void printStackTrace(JNIEnv* env, jthrowable exception, char* buf,
		size_t bufLen) {
	int success = 0;
	jclass stringWriterClazz = NULL;

	//	log_print(XDLOG, "\n+++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	//	log_print(XDLOG, "++++++++++++         printStackTrace      +++++++++++\n");
	//	log_print(XDLOG, "+++++++++++++++++++++++++++++++++++++++++++++++++++++\n");

	stringWriterClazz = (*env)->FindClass(env, "java/io/StringWriter");
	if (stringWriterClazz != NULL) {
		jmethodID stringWriterCtor = (*env)->GetMethodID(env,
				stringWriterClazz, "<init>", "()V");
		jmethodID stringWriterToStringMethod = (*env)->GetMethodID(env,
				stringWriterClazz, "toString", "()Ljava/lang/String;");

		jclass printWriterClazz = (*env)->FindClass(env, "java/io/PrintWriter");
		if (printWriterClazz != NULL) {
			jmethodID printWriterCtor = (*env)->GetMethodID(env,
					printWriterClazz, "<init>", "(Ljava/io/Writer;)V");

			jobject stringWriterObj = (*env)->NewObject(env, stringWriterClazz,
					stringWriterCtor);
			if (stringWriterObj != NULL) {
				jobject printWriterObj = (*env)->NewObject(env,
						printWriterClazz, printWriterCtor, stringWriterObj);
				if (printWriterObj != NULL) {
					jclass exceptionClazz = (*env)->GetObjectClass(env,
							exception); /* can't fail */
					jmethodID printStackTraceMethod = (*env)->GetMethodID(env,
							exceptionClazz, "printStackTrace",
							"(Ljava/io/PrintWriter;)V");

					(*env)->CallVoidMethod(env, exception,
							printStackTraceMethod, printWriterObj);
					if (!(*env)->ExceptionCheck(env)) {
						jstring messageStr = (*env)->CallObjectMethod(env,
								stringWriterObj, stringWriterToStringMethod);
						if (messageStr != NULL) {
							jsize messageStrLength = (*env)->GetStringLength(
									env, messageStr);
							if (messageStrLength >= (jsize) bufLen) {
								messageStrLength = bufLen - 1;
							}
							(*env)->GetStringUTFRegion(env, messageStr, 0,
									messageStrLength, buf);
							(*env)->DeleteLocalRef(env, messageStr);
...
int jniThrowException(JNIEnv* env, const char* className, const char* msg) {
	...
	if ((*env)->ExceptionCheck(env)) {
		...
		if (exception != NULL) {
			...
			log_error(XDLOG, "Discarding pending exception (%s) to throw %s\n", buf,
					className);
	...
	if (exceptionClass == NULL) {
		log_error(XDLOG, "Unable to find exception class %s\n", className);
		/* ClassNotFoundException now pending */
		return -1;
	}

	if ((*env)->ThrowNew(env, exceptionClass, msg) != JNI_OK) {
		log_error(XDLOG, "Failed throwing '%s' '%s'\n", className, msg);
...
int jniGetFDFromFileDescriptor(JNIEnv* env, jobject fileDescriptor) {
	//	log_print(XDLOG, "\n------------------------------------------------------------\n");
	//	log_print(XDLOG, "----------------- jniGetFDFromFileDescriptor ----------------\n");
	//	log_print(XDLOG, "-------------------------------------------------------------\n");

	jclass fileDescriptorClass = (*env)->FindClass(env,
			"java/io/FileDescriptor");
	jfieldID descriptorField = (*env)->GetFieldID(env, fileDescriptorClass,
			"descriptor", "I");

	return (*env)->GetIntField(env, fileDescriptor, descriptorField);
}
...
void jniSetFileDescriptorOfFD(JNIEnv* env, jobject fileDescriptor, int value) {
	//	log_print(XDLOG, "\n------------------------------------------------------------\n");
	//	log_print(XDLOG, "--------------- jniSetFileDescriptorOfFD [%d] --------------\n", value);
	//	log_print(XDLOG, "-------------------------------------------------------------\n");
	...
	(*env)->SetIntField(env, fileDescriptor, descriptorField, value);
}

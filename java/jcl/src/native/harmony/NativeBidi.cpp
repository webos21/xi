...
#include "org_apache_harmony_text_NativeBidi.h"
...
struct BiDiData {
	BiDiData(UBiDi* biDi) :
		mBiDi(biDi), mEmbeddingLevels(NULL) {
	}
...
Java_org_apache_harmony_text_NativeBidi_ubidi_1open(JNIEnv*, jclass) {
	return reinterpret_cast<uintptr_t> (new BiDiData(ubidi_open()));
}

//static void NativeBidi_ubidi_close(JNIEnv*, jclass, jlong ptr) {
JNIEXPORT void JNICALL
Java_org_apache_harmony_text_NativeBidi_ubidi_1close(JNIEnv*, jclass, jlong ptr) {
	delete biDiData(ptr);
}
...
JNIEXPORT void JNICALL
Java_org_apache_harmony_text_NativeBidi_ubidi_1setPara(JNIEnv* env, jclass,
		jlong ptr, jcharArray text, jint length, jint paraLevel,
		jbyteArray newEmbeddingLevels) {
	...
	ubidi_setPara(data->uBiDi(), (const UChar*) chars.get(), length, paraLevel,
...
JNIEXPORT jlong JNICALL
Java_org_apache_harmony_text_NativeBidi_ubidi_1setLine(JNIEnv* env, jclass,
		jlong ptr, jint start, jint limit) {
	UErrorCode err = U_ZERO_ERROR;
...
Java_org_apache_harmony_text_NativeBidi_ubidi_1getDirection(JNIEnv*, jclass,
		jlong ptr) {
...
JNIEXPORT jint JNICALL
Java_org_apache_harmony_text_NativeBidi_ubidi_1getLength(JNIEnv*, jclass,
		jlong ptr) {
...
Java_org_apache_harmony_text_NativeBidi_ubidi_1getParaLevel(JNIEnv*, jclass,
		jlong ptr) {
...
Java_org_apache_harmony_text_NativeBidi_ubidi_1getLevels(JNIEnv* env, jclass,
		jlong ptr) {
	UErrorCode err = U_ZERO_ERROR;
	...
	env->SetByteArrayRegion(result, 0, len,
			reinterpret_cast<const jbyte*> (levels));
	return result;
}

//static jint NativeBidi_ubidi_countRuns(JNIEnv* env, jclass, jlong ptr) {
JNIEXPORT jint JNICALL
Java_org_apache_harmony_text_NativeBidi_ubidi_1countRuns(JNIEnv* env, jclass,
		jlong ptr) {
	UErrorCode err = U_ZERO_ERROR;
...
Java_org_apache_harmony_text_NativeBidi_ubidi_1getRuns(JNIEnv* env, jclass,
		jlong ptr) {
	...
	if (!bidiRunClass) {
		log_warn(XDLOG, "--------------> Nil bidiRunClass!!\n");
	}
	jmethodID bidiRunConstructor = env->GetMethodID(bidiRunClass, "<init>",
			"(III)V");
	if (!bidiRunConstructor) {
		log_warn(XDLOG, "--------------> Nil bidiRunConstructor!!\n");
	}
	jobjectArray runs = env->NewObjectArray(runCount, bidiRunClass, NULL);
	if (!runs) {
		log_warn(XDLOG, "--------------> Nil runs!!\n");
	}
	...
	for (int i = 0; i < runCount; ++i) {
		...
		jobject run = env->NewObject(bidiRunClass, bidiRunConstructor, start,
				limit, level);
...
Java_org_apache_harmony_text_NativeBidi_ubidi_1reorderVisual(JNIEnv* env,
		jclass, jbyteArray javaLevels, jint length) {
	...
	const UBiDiLevel* levels =
			reinterpret_cast<const UBiDiLevel*> (levelBytes.get());

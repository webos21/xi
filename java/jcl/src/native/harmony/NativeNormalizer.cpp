...
#include "libcore_icu_NativeNormalizer.h"
...
Java_libcore_icu_NativeNormalizer_normalizeImpl(JNIEnv* env, jclass, jstring s,
		jint intMode) {
	...
	return dst.isBogus() ? NULL : env->NewString(
			(const jchar*) dst.getBuffer(), dst.length());
}

//static jboolean NativeNormalizer_isNormalizedImpl(JNIEnv* env, jclass, jstring s, jint intMode) {
JNIEXPORT jboolean JNICALL
Java_libcore_icu_NativeNormalizer_isNormalizedImpl(JNIEnv* env, jclass,
		jstring s, jint intMode) {
	ScopedJavaUnicodeString src(env, s);
	UNormalizationMode mode = static_cast<UNormalizationMode> (intMode);
	UErrorCode errorCode = U_ZERO_ERROR;
	UBool result = Normalizer::isNormalized(src.unicodeString(), mode,
			errorCode);
	icu4jni_error(env, errorCode);
	return result;
}

...
#include "java_nio_charset_Charsets.h"
#include "JNIHelp.h"
...
public:
	...
	bool resize(int newSize) {
		...
		if (mRawArray != NULL) {
			xi_mem_copy(newRawArray, mRawArray, mOffset);
...
Java_java_nio_charset_Charsets_asciiBytesToChars(JNIEnv* env, jclass,
		jbyteArray javaBytes, jint offset, jint length, jcharArray javaChars) {
...
JNIEXPORT void JNICALL
Java_java_nio_charset_Charsets_isoLatin1BytesToChars(JNIEnv* env, jclass,
		jbyteArray javaBytes, jint offset, jint length, jcharArray javaChars) {
	ScopedByteArrayRO bytes(env, javaBytes);
	if (bytes.get() == NULL) {
		return;
 ...
 */
static jbyteArray charsToBytes(JNIEnv* env, jcharArray javaChars, jint offset,
		jint length, jchar maxValidChar) {
...
Java_java_nio_charset_Charsets_toAsciiBytes(JNIEnv* env, jclass,
		jcharArray javaChars, jint offset, jint length) {
...
JNIEXPORT jbyteArray JNICALL
Java_java_nio_charset_Charsets_toIsoLatin1Bytes(JNIEnv* env, jclass,
		jcharArray javaChars, jint offset, jint length) {
...
JNIEXPORT jbyteArray JNICALL
Java_java_nio_charset_Charsets_toUtf8Bytes(JNIEnv* env, jclass,
		jcharArray javaChars, jint offset, jint length) {
	ScopedCharArrayRO chars(env, javaChars);
	if (chars.get() == NULL) {
		return NULL;
	...
	for (int i = offset; i < end; ++i) {
		...
		if (ch < 0x80) {
			...
			// Two bytes.
			if (!out.append((ch >> 6) | 0xc0)
					|| !out.append((ch & 0x3f) | 0x80)) {
				return NULL;
			}

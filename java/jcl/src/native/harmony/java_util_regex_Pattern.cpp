...
static void throwPatternSyntaxException(JNIEnv* env, UErrorCode status, jstring pattern, UParseError error) {
	jclass patternSyntaxExceptionClass = env->FindClass("java/util/regex/PatternSyntaxException");
	static jmethodID method = env->GetMethodID(patternSyntaxExceptionClass,
            "<init>", "(Ljava/lang/String;Ljava/lang/String;I)V");
    jstring message = env->NewStringUTF(u_errorName(status));
    jobject exception = env->NewObject(patternSyntaxExceptionClass, method, message, pattern, error.offset);
    env->Throw(reinterpret_cast<jthrowable>(exception));
}

//static void Pattern_closeImpl(JNIEnv*, jclass, jint addr) {
JNIEXPORT void JNICALL
Java_java_util_regex_Pattern_closeImpl(JNIEnv*, jclass, jint addr) {
    delete toRegexPattern(addr);
}

//static jint Pattern_compileImpl(JNIEnv* env, jclass, jstring javaRegex, jint flags) {
JNIEXPORT jint JNICALL
Java_java_util_regex_Pattern_compileImpl(JNIEnv* env, jclass, jstring javaRegex, jint flags) {

...
#include "com_ibm_icu4jni_text_NativeCollation.h"
...
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_text_NativeCollation_getCollationElementIterator(
		JNIEnv* env, jclass, jint address, jstring source0) {
	ScopedJavaUnicodeString source(env, source0);
	UErrorCode status = U_ZERO_ERROR;
	UCollationElements* result = ucol_openElements(toCollator(address),
			source.unicodeString().getBuffer(),
			source.unicodeString().length(), &status);
	icu4jni_error(env, status);
...
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_text_NativeCollation_getMaxExpansion(JNIEnv*, jclass,
...
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_text_NativeCollation_openCollator(JNIEnv* env, jclass,
		jstring localeName) {
...
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_text_NativeCollation_openCollatorFromRules(JNIEnv* env,
		jclass, jstring rules0, jint mode, jint strength) {
	ScopedJavaUnicodeString rules(env, rules0);
	UErrorCode status = U_ZERO_ERROR;
	UCollator* c = ucol_openRules(rules.unicodeString().getBuffer(),
			rules.unicodeString().length(), UColAttributeValue(mode),
...
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_text_NativeCollation_previous(JNIEnv* env, jclass,
		jint address) {
	UErrorCode status = U_ZERO_ERROR;
...
JNIEXPORT void JNICALL
Java_com_ibm_icu4jni_text_NativeCollation_reset(JNIEnv*, jclass, jint address) {
	ucol_reset(toCollationElements(address));
}

//static jint NativeCollation_safeClone(JNIEnv* env, jclass, jint address) {
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_text_NativeCollation_safeClone(JNIEnv* env, jclass,
		jint address) {
	UErrorCode status = U_ZERO_ERROR;
	jint bufferSize = U_COL_SAFECLONE_BUFFERSIZE;
	UCollator* c = ucol_safeClone(toCollator(address), NULL, &bufferSize,
			&status);
	icu4jni_error(env, status);
	return static_cast<jint> (reinterpret_cast<uintptr_t> (c));
}
...
JNIEXPORT void JNICALL
Java_com_ibm_icu4jni_text_NativeCollation_setText(JNIEnv* env, jclass,
		jint address, jstring source0) {
	ScopedJavaUnicodeString source(env, source0);
	UErrorCode status = U_ZERO_ERROR;

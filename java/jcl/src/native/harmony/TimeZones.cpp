...
#include "libcore_icu_TimeZones.h"
#include "ErrorCode.h"
#include "JNIHelp.h"
...
static jstring formatDate(JNIEnv* env, const SimpleDateFormat& fmt,
		const UDate& when) {
	UnicodeString str;
	fmt.format(when, str);
...
Java_libcore_icu_TimeZones_forCountryCode(JNIEnv* env, jclass,
		jstring countryCode) {
	...
	UniquePtr<StringEnumeration> ids(
			TimeZone::createEnumeration(countryChars.c_str()));
	if (ids.get() == NULL) {
		return NULL;
	}
	...
	jobjectArray result = env->NewObjectArray(idCount, stringClass, NULL);
	for (int32_t i = 0; i < idCount; ++i) {
		...
		ScopedLocalRef<jstring> idString(env,
				env->NewString((const jchar*) id->getBuffer(), id->length()));
...
Java_libcore_icu_TimeZones_getDisplayNameImpl(JNIEnv* env, jclass,
	...
	zone->getDisplayName((UBool) isDST,
			(style == 0 ? TimeZone::SHORT : TimeZone::LONG), locale,
			displayName);
	return env->NewString((const jchar*) displayName.getBuffer(),
			displayName.length());
}

//static void TimeZones_getZoneStringsImpl(JNIEnv* env, jclass, jobjectArray outerArray, jstring localeName) {
JNIEXPORT void JNICALL
Java_libcore_icu_TimeZones_getZoneStringsImpl(JNIEnv* env, jclass,
		jobjectArray outerArray, jstring localeName) {
	...
	jobjectArray longStdArray = (jobjectArray) env->GetObjectArrayElement(
			outerArray, 1);
	jobjectArray shortStdArray = (jobjectArray) env->GetObjectArrayElement(
			outerArray, 2);
	jobjectArray longDstArray = (jobjectArray) env->GetObjectArrayElement(
			outerArray, 3);
	jobjectArray shortDstArray = (jobjectArray) env->GetObjectArrayElement(
			outerArray, 4);
	...
	jobjectArray zoneIds = (jobjectArray) env->GetObjectArrayElement(
			outerArray, 0);
	int zoneIdCount = env->GetArrayLength(zoneIds);
	for (int i = 0; i < zoneIdCount; ++i) {
		ScopedLocalRef<jstring> id(
				env,
				reinterpret_cast<jstring> (env->GetObjectArrayElement(zoneIds,
						i)));
		...
		ScopedLocalRef<jstring> shortStd(env,
				formatDate(env, shortFormat, standardDate));
		env->SetObjectArrayElement(shortStdArray, i, shortStd.get());

		ScopedLocalRef<jstring> longStd(env,
				formatDate(env, longFormat, standardDate));
		env->SetObjectArrayElement(longStdArray, i, longStd.get());

		if (tz->useDaylightTime()) {
			ScopedLocalRef<jstring> shortDst(env,
					formatDate(env, shortFormat, daylightSavingDate));
			env->SetObjectArrayElement(shortDstArray, i, shortDst.get());

			ScopedLocalRef<jstring> longDst(env,
					formatDate(env, longFormat, daylightSavingDate));

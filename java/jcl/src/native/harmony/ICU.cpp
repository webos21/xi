...
#include "com_ibm_icu4jni_util_ICU.h"
...
Java_com_ibm_icu4jni_util_ICU_getCurrencyFractionDigitsNative(JNIEnv* env,
		jclass, jstring javaCurrencyCode) {
...
Java_com_ibm_icu4jni_util_ICU_getCurrencyCodeNative(JNIEnv* env, jclass,
		jstring javaKey) {
	UErrorCode status = U_ZERO_ERROR;
	ScopedResourceBundle supplData(
			ures_openDirect(U_ICUDATA_CURR, "supplementalData", &status));
	if (U_FAILURE(status)) {
		return NULL;
	}

	ScopedResourceBundle currencyMap(
			ures_getByKey(supplData.get(), "CurrencyMap", NULL, &status));
	if (U_FAILURE(status)) {
		return NULL;
	}

	ScopedUtfChars key(env, javaKey);
	ScopedResourceBundle currency(
			ures_getByKey(currencyMap.get(), key.c_str(), NULL, &status));
	if (U_FAILURE(status)) {
		return NULL;
	}

	ScopedResourceBundle currencyElem(
			ures_getByIndex(currency.get(), 0, NULL, &status));
	if (U_FAILURE(status)) {
		return env->NewStringUTF("None");
	}

	// check if there is a 'to' date. If there is, the currency isn't used anymore.
	ScopedResourceBundle currencyTo(
			ures_getByKey(currencyElem.get(), "to", NULL, &status));
	if (!U_FAILURE(status)) {
	...
	ScopedResourceBundle currencyId(
			ures_getByKey(currencyElem.get(), "id", NULL, &status));
	if (U_FAILURE(status)) {
	...
	int length;
	const jchar* id = (const jchar *) ures_getString(currencyId.get(), &length,
			&status);
...
JNIEXPORT jstring JNICALL
Java_com_ibm_icu4jni_util_ICU_getCurrencySymbolNative(JNIEnv* env, jclass,
		jstring locale, jstring currencyCode) {
	ScopedUtfChars localeName(env, locale);
	UErrorCode status = U_ZERO_ERROR;
	ScopedResourceBundle currLoc(
			ures_open(U_ICUDATA_CURR, localeName.c_str(), &status));
	if (U_FAILURE(status)) {
		return NULL;
	}

	ScopedResourceBundle currencies(
			ures_getByKey(currLoc.get(), "Currencies", NULL, &status));
	if (U_FAILURE(status)) {
		return NULL;
	}

	ScopedUtfChars currency(env, currencyCode);
	ScopedResourceBundle currencyElems(
			ures_getByKey(currencies.get(), currency.c_str(), NULL, &status));
	if (U_FAILURE(status)) {
		return NULL;
	}

	int currSymbL;
	const jchar* currSymbU = (const jchar*) ures_getStringByIndex(
			currencyElems.get(), 0, &currSymbL, &status);
	if (U_FAILURE(status)) {
		return NULL;
	}
...
JNIEXPORT jstring JNICALL
Java_com_ibm_icu4jni_util_ICU_getDisplayCountryNative(JNIEnv* env, jclass,
	...
	return env->NewString((const jchar *) str.getBuffer(), str.length());
}

//static jstring ICU_getDisplayLanguageNative(JNIEnv* env, jclass, jstring targetLocale, jstring locale) {
JNIEXPORT jstring JNICALL
Java_com_ibm_icu4jni_util_ICU_getDisplayLanguageNative(JNIEnv* env, jclass,
		jstring targetLocale, jstring locale) {
	Locale loc = getLocale(env, locale);
	Locale targetLoc = getLocale(env, targetLocale);
	UnicodeString str;
...
JNIEXPORT jstring JNICALL
Java_com_ibm_icu4jni_util_ICU_getDisplayVariantNative(JNIEnv* env, jclass,
		jstring targetLocale, jstring locale) {
	Locale loc = getLocale(env, locale);
	Locale targetLoc = getLocale(env, targetLocale);
	UnicodeString str;
...
JNIEXPORT jstring JNICALL
Java_com_ibm_icu4jni_util_ICU_getISO3CountryNative(JNIEnv* env, jclass,
		jstring locale) {
	Locale loc = getLocale(env, locale);
...
JNIEXPORT jstring JNICALL
Java_com_ibm_icu4jni_util_ICU_getISO3LanguageNative(JNIEnv* env, jclass,
		jstring locale) {
	Locale loc = getLocale(env, locale);
...
Java_com_ibm_icu4jni_util_ICU_getISOCountriesNative(JNIEnv* env, jclass) {
	return toStringArray(env, Locale::getISOCountries());
}

//static jobjectArray ICU_getISOLanguagesNative(JNIEnv* env, jclass) {
JNIEXPORT jobjectArray JNICALL
Java_com_ibm_icu4jni_util_ICU_getISOLanguagesNative(JNIEnv* env, jclass) {
	return toStringArray(env, Locale::getISOLanguages());
}

template<typename Counter, typename Getter>
static jobjectArray getAvailableLocales(JNIEnv* env, Counter* counter,
		Getter* getter) {
...
JNIEXPORT jobjectArray JNICALL
Java_com_ibm_icu4jni_util_ICU_getAvailableLocalesNative(JNIEnv* env, jclass) {
...
JNIEXPORT jobjectArray JNICALL
Java_com_ibm_icu4jni_util_ICU_getAvailableBreakIteratorLocalesNative(
		JNIEnv* env, jclass) {
...
JNIEXPORT jobjectArray JNICALL
Java_com_ibm_icu4jni_util_ICU_getAvailableCalendarLocalesNative(JNIEnv* env,
		jclass) {
...
JNIEXPORT jobjectArray JNICALL
Java_com_ibm_icu4jni_util_ICU_getAvailableCollatorLocalesNative(JNIEnv* env,
		jclass) {
...
JNIEXPORT jobjectArray JNICALL
Java_com_ibm_icu4jni_util_ICU_getAvailableDateFormatLocalesNative(JNIEnv* env,
		jclass) {
...
JNIEXPORT jobjectArray JNICALL
Java_com_ibm_icu4jni_util_ICU_getAvailableNumberFormatLocalesNative(
		JNIEnv* env, jclass) {
...
static bool getDayIntVector(JNIEnv*, UResourceBundle* gregorian, int* values) {
	...
	UErrorCode status = U_ZERO_ERROR;
	ScopedResourceBundle gregorianElems(
			ures_getByKey(gregorian, "DateTimeElements", NULL, &status));
	if (U_FAILURE(status)) {
		return false;
	}

	int intVectSize;
	const int* result = ures_getIntVector(gregorianElems.get(), &intVectSize,
			&status);
...
static jobjectArray getAmPmMarkers(JNIEnv* env, UResourceBundle* gregorian) {
	UErrorCode status = U_ZERO_ERROR;
	ScopedResourceBundle gregorianElems(
			ures_getByKey(gregorian, "AmPmMarkers", NULL, &status));
	if (U_FAILURE(status)) {
		return NULL;
	}

	int lengthAm, lengthPm;
	const jchar* am = (const jchar *) ures_getStringByIndex(
			gregorianElems.get(), 0, &lengthAm, &status);
	const jchar* pm = (const jchar *) ures_getStringByIndex(
			gregorianElems.get(), 1, &lengthPm, &status);
	...
	jclass stringClass = env->FindClass("java/lang/String");
	jobjectArray amPmMarkers = env->NewObjectArray(2, stringClass, NULL);
...
static jobjectArray getEras(JNIEnv* env, UResourceBundle* gregorian) {
	UErrorCode status = U_ZERO_ERROR;
	ScopedResourceBundle gregorianElems(
			ures_getByKey(gregorian, "eras", NULL, &status));
	if (U_FAILURE(status)) {
		return NULL;
	}

	ScopedResourceBundle eraElems(
			ures_getByKey(gregorianElems.get(), "abbreviated", NULL, &status));
	if (U_FAILURE(status)) {
		return NULL;
	}
	...
	jclass stringClass = env->FindClass("java/lang/String");
	jobjectArray eras = env->NewObjectArray(eraCount, stringClass, NULL);
	for (int i = 0; i < eraCount; ++i) {
		int eraLength;
		const jchar* era = (const jchar *) ures_getStringByIndex(
				eraElems.get(), i, &eraLength, &status);
		if (U_FAILURE(status)) {
			return NULL;
		}
...
static jobjectArray getNames(JNIEnv* env, UResourceBundle* namesBundle,
		bool months, NameType type, NameWidth width) {
	const char* typeKey = (type == REGULAR) ? "format" : "stand-alone";
	const char* widthKey = (width == LONG) ? "wide" : "abbreviated";
	UErrorCode status = U_ZERO_ERROR;
	ScopedResourceBundle formatBundle(
			ures_getByKey(namesBundle, typeKey, NULL, &status));
	ScopedResourceBundle valuesBundle(
			ures_getByKey(formatBundle.get(), widthKey, NULL, &status));
	if (U_FAILURE(status)) {
		return NULL;
	}
	...
	jclass stringClass = env->FindClass("java/lang/String");
	jobjectArray result = env->NewObjectArray(count + 1, stringClass, NULL);
	env->SetObjectArrayElement(result, months ? count : 0,
			env->NewStringUTF(""));
	int arrayOffset = months ? 0 : 1;
	for (int i = 0; i < count; ++i) {
		int nameLength;
		const jchar* name = (const jchar *) ures_getStringByIndex(
				valuesBundle.get(), i, &nameLength, &status);
		if (U_FAILURE(status)) {
			return NULL;
		}
		ScopedLocalRef<jstring> nameString(env,
				env->NewString(name, nameLength));
...
static jstring getIntCurrencyCode(JNIEnv* env, jstring locale) {
	...
	if (localeChars[3] < 'A' || localeChars[3] > 'Z' || localeChars[4] < 'A'
			|| localeChars[4] > 'Z') {
		return NULL;
	}
	...
	return Java_com_ibm_icu4jni_util_ICU_getCurrencyCodeNative(env, NULL,
			env->NewStringUTF(country));
}

static void setIntegerField(JNIEnv* env, jobject obj, const char* fieldName,
		int value) {
	...
	jfieldID fid = env->GetFieldID(localeDataClass, fieldName,
			"Ljava/lang/Integer;");
	env->SetObjectField(obj, fid, integerValue.get());
}

static void setStringField(JNIEnv* env, jobject obj, const char* fieldName,
		jstring value) {
	jclass localeDataClass = env->FindClass("com/ibm/icu4jni/util/LocaleData");
	jfieldID fid = env->GetFieldID(localeDataClass, fieldName,
			"Ljava/lang/String;");
...
static void setStringArrayField(JNIEnv* env, jobject obj,
		const char* fieldName, jobjectArray value) {
	jclass localeDataClass = env->FindClass("com/ibm/icu4jni/util/LocaleData");
	jfieldID fid = env->GetFieldID(localeDataClass, fieldName,
			"[Ljava/lang/String;");
	env->SetObjectField(obj, fid, value);
...
static void setStringField(JNIEnv* env, jobject obj, const char* fieldName,
		UResourceBundle* bundle, int index) {
	UErrorCode status = U_ZERO_ERROR;
	int charCount;
	const UChar* chars = ures_getStringByIndex(bundle, index, &charCount,
			&status);
	if (U_SUCCESS(status)) {
		setStringField(env, obj, fieldName,
				env->NewString((const jchar *) chars, charCount));
	} else {
		log_error(XDLOG, "Error setting String field %s from ICU resource: %s", fieldName,
				u_errorName(status));
	}
}

static void setCharField(JNIEnv* env, jobject obj, const char* fieldName,
		UResourceBundle* bundle, int index) {
	UErrorCode status = U_ZERO_ERROR;
	int charCount;
	const UChar* chars = ures_getStringByIndex(bundle, index, &charCount,
...
Java_com_ibm_icu4jni_util_ICU_initLocaleDataImpl(JNIEnv* env, jclass,
		jstring locale, jobject localeData) {
	ScopedUtfChars localeName(env, locale);
	UErrorCode status = U_ZERO_ERROR;
	ScopedResourceBundle root(ures_open(NULL, localeName.c_str(), &status));
	if (U_FAILURE(status)) {
		log_error(XDLOG, "Error getting ICU resource bundle: %s", u_errorName(status));
		status = U_ZERO_ERROR;
		return JNI_FALSE;
	}

	ScopedResourceBundle calendar(
			ures_getByKey(root.get(), "calendar", NULL, &status));
	if (U_FAILURE(status)) {
		log_error(XDLOG, "Error getting ICU calendar resource bundle: %s",
				u_errorName(status));
		return JNI_FALSE;
	}

	ScopedResourceBundle gregorian(
			ures_getByKey(calendar.get(), "gregorian", NULL, &status));
	if (U_FAILURE(status)) {
		log_error(XDLOG, "Error getting ICU gregorian resource bundle: %s",
				u_errorName(status));
		return JNI_FALSE;
	}
	...
	if (getDayIntVector(env, gregorian.get(), firstDayVals)) {
		...
		setIntegerField(env, localeData, "minimalDaysInFirstWeek",
				firstDayVals[1]);
	}

	setStringArrayField(env, localeData, "amPm",
			getAmPmMarkers(env, gregorian.get()));
	setStringArrayField(env, localeData, "eras", getEras(env, gregorian.get()));

	ScopedResourceBundle dayNames(
			ures_getByKey(gregorian.get(), "dayNames", NULL, &status));
	ScopedResourceBundle monthNames(
			ures_getByKey(gregorian.get(), "monthNames", NULL, &status));

	// Get the regular month and weekday names.
	jobjectArray longMonthNames = getNames(env, monthNames.get(), true,
			REGULAR, LONG);
	jobjectArray shortMonthNames = getNames(env, monthNames.get(), true,
			REGULAR, SHORT);
	jobjectArray longWeekdayNames = getNames(env, dayNames.get(), false,
			REGULAR, LONG);
	jobjectArray shortWeekdayNames = getNames(env, dayNames.get(), false,
			REGULAR, SHORT);
	...
	jobjectArray longStandAloneMonthNames = getNames(env, monthNames.get(),
			true, STAND_ALONE, LONG);
	if (longStandAloneMonthNames == NULL) {
		longStandAloneMonthNames = longMonthNames;
	}
	jobjectArray shortStandAloneMonthNames = getNames(env, monthNames.get(),
			true, STAND_ALONE, SHORT);
	if (shortStandAloneMonthNames == NULL) {
		shortStandAloneMonthNames = shortMonthNames;
	}
	jobjectArray longStandAloneWeekdayNames = getNames(env, dayNames.get(),
			false, STAND_ALONE, LONG);
	if (longStandAloneWeekdayNames == NULL) {
		longStandAloneWeekdayNames = longWeekdayNames;
	}
	jobjectArray shortStandAloneWeekdayNames = getNames(env, dayNames.get(),
			false, STAND_ALONE, SHORT);
	if (shortStandAloneWeekdayNames == NULL) {
		shortStandAloneWeekdayNames = shortWeekdayNames;
	}
	setStringArrayField(env, localeData, "longStandAloneMonthNames",
			longStandAloneMonthNames);
	setStringArrayField(env, localeData, "shortStandAloneMonthNames",
			shortStandAloneMonthNames);
	setStringArrayField(env, localeData, "longStandAloneWeekdayNames",
			longStandAloneWeekdayNames);
	setStringArrayField(env, localeData, "shortStandAloneWeekdayNames",
			shortStandAloneWeekdayNames);

	ScopedResourceBundle dateTimePatterns(
			ures_getByKey(gregorian.get(), "DateTimePatterns", NULL, &status));
	if (U_SUCCESS(status)) {
		setStringField(env, localeData, "fullTimeFormat",
				dateTimePatterns.get(), 0);
		setStringField(env, localeData, "longTimeFormat",
				dateTimePatterns.get(), 1);
		setStringField(env, localeData, "mediumTimeFormat",
				dateTimePatterns.get(), 2);
		setStringField(env, localeData, "shortTimeFormat",
				dateTimePatterns.get(), 3);
		setStringField(env, localeData, "fullDateFormat",
				dateTimePatterns.get(), 4);
		setStringField(env, localeData, "longDateFormat",
				dateTimePatterns.get(), 5);
		setStringField(env, localeData, "mediumDateFormat",
				dateTimePatterns.get(), 6);
		setStringField(env, localeData, "shortDateFormat",
				dateTimePatterns.get(), 7);
	}
	status = U_ZERO_ERROR;

	ScopedResourceBundle numberElements(
			ures_getByKey(root.get(), "NumberElements", NULL, &status));
	if (U_SUCCESS(status) && ures_getSize(numberElements.get()) >= 11) {
		...
		setCharField(env, localeData, "decimalSeparator", numberElements.get(),
				0);
		setCharField(env, localeData, "groupingSeparator",
				numberElements.get(), 1);
		setCharField(env, localeData, "patternSeparator", numberElements.get(),
				2);
		setCharField(env, localeData, "percent", numberElements.get(), 3);
		setCharField(env, localeData, "perMill", numberElements.get(), 8);
		setCharField(env, localeData, "monetarySeparator",
				numberElements.get(), 0);
		setCharField(env, localeData, "minusSign", numberElements.get(), 6);
		setStringField(env, localeData, "exponentSeparator",
				numberElements.get(), 7);
	...
	if (internationalCurrencySymbol != NULL) {
		...
		currencySymbol = Java_com_ibm_icu4jni_util_ICU_getCurrencySymbolNative(
				env, NULL, locale, internationalCurrencySymbol);
	} else {
	...
	setStringField(env, localeData, "internationalCurrencySymbol",
			internationalCurrencySymbol);

	ScopedResourceBundle numberPatterns(
			ures_getByKey(root.get(), "NumberPatterns", NULL, &status));
	if (U_SUCCESS(status) && ures_getSize(numberPatterns.get()) >= 3) {
		setStringField(env, localeData, "numberPattern", numberPatterns.get(),
				0);
		setStringField(env, localeData, "currencyPattern",
				numberPatterns.get(), 1);
		setStringField(env, localeData, "percentPattern", numberPatterns.get(),
				2);
	}
...
JNIEXPORT jstring JNICALL
Java_com_ibm_icu4jni_util_ICU_toLowerCase(JNIEnv* env, jclass,
		jstring javaString, jstring localeName) {
	...
	return s == original ? javaString : env->NewString(
			(const jchar *) s.getBuffer(), s.length());
}

//static jstring ICU_toUpperCase(JNIEnv* env, jclass, jstring javaString, jstring localeName) {
JNIEXPORT jstring JNICALL
Java_com_ibm_icu4jni_util_ICU_toUpperCase(JNIEnv* env, jclass,
		jstring javaString, jstring localeName) {
	ScopedJavaUnicodeString scopedString(env, javaString);
	UnicodeString& s(scopedString.unicodeString());
	UnicodeString original(s);

...
/* ************************* Defines ************************* */
#if defined(__linux__) || defined(FREEBSD) || defined(__MINGW32__) || defined(__APPLE__)
#define USE_LL
#endif
...
/* ************************ local data ************************ */
static const jdouble double_tens[] = { 1.0, 1.0e1, 1.0e2, 1.0e3, 1.0e4, 1.0e5,
		1.0e6, 1.0e7, 1.0e8, 1.0e9, 1.0e10, 1.0e11, 1.0e12, 1.0e13, 1.0e14,
...
static jdouble doubleAlgorithm(JNIEnv* env, uint64_t * f, int32_t length,
		jint e, jdouble z);
/* *********************************************************** */
 ...
 */
static jdouble doubleAlgorithm(JNIEnv*, uint64_t* f, int32_t length, jint e,
		jdouble z) {
	uint64_t m;
...
static jfloat floatAlgorithm(JNIEnv* env, uint64_t* f, int32_t length, jint e,
		jfloat z);

static const uint32_t float_tens[] = { 0x3f800000, 0x41200000, 0x42c80000,
		0x447a0000, 0x461c4000, 0x47c35000, 0x49742400, 0x4b189680, 0x4cbebc20,
		0x4e6e6b28, 0x501502f9 /* 10 ^ 10 in float */
};
...
static jfloat createFloat1(JNIEnv* env, uint64_t* f, int32_t length, jint e) {
	...
	if (numBits < 25 && e >= 0 && e < FLOAT_LOG5_OF_TWO_TO_THE_N) {
		...
		result = (jfloat) (toDoubleHighPrecision(f, length) * pow(10.0,
				(double) e));
	} else if (e >= 39) {
		/* Convert the partial result to make sure that the
		 * non-exponential part is not zero. This check fixes the case
 ...
 * then return the original approximation.
 */
static jfloat floatAlgorithm(JNIEnv*, uint64_t* f, int32_t length, jint e,
		jfloat z) {
	uint64_t m;
	int32_t k, comparison, comparison2;
	uint64_t* x;

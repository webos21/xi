...
static jobject makeInterfaceAddress(JNIEnv* env, jint interfaceIndex, ifaddrs* ifa) {
	...
	jmethodID constructor = env->GetMethodID(interfaceAddressClass, "<init>",
	...
	return env->NewObject(interfaceAddressClass, constructor,
...
Java_java_net_NetworkInterface_getAllInterfaceAddressesImpl(JNIEnv*, jclass) {
	...
	//	jclass interfaceAddressClass = env->FindClass("java/net/InterfaceAddress");
	//    jobjectArray result =
	//            env->NewObjectArray(interfaceAddressCount, interfaceAddressClass, NULL);
	//    if (result == NULL) {
	//        return NULL;
	//    }
	//
...
JNIEXPORT jboolean JNICALL
Java_java_net_NetworkInterface_isPointToPointImpl(JNIEnv* env, jclass,
		jstring name) {

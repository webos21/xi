...
*/
#define GETTER(FUNCTION_NAME, JNI_C_TYPE, JNI_TYPE_STRING, JNI_GETTER_FUNCTION) \
    JNIEXPORT JNI_C_TYPE JNICALL \
    Java_java_io_ ## FUNCTION_NAME(JNIEnv* env, jclass, jobject instance, jclass declaringClass, \
            jstring javaFieldName) { \
        if (instance == NULL) { \
            return JNI_C_TYPE(); \
        } \
...
Java_java_io_ObjectOutputStream_getFieldObj(JNIEnv* env, jclass, jobject instance,
        jclass declaringClass, jstring javaFieldName, jstring javaFieldTypeName) {
    ScopedUtfChars fieldName(env, javaFieldName);
    if (fieldName.c_str() == NULL) {
        return NULL;

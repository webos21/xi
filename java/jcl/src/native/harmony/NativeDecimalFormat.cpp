...
#include "com_ibm_icu4jni_text_NativeDecimalFormat.h"
...
Java_com_ibm_icu4jni_text_NativeDecimalFormat_setDecimalFormatSymbols(
		JNIEnv* env, jclass, jint addr, jstring currencySymbol,
		jchar decimalSeparator, jchar digit, jstring exponentSeparator,
		jchar groupingSeparator, jstring infinity,
		jstring internationalCurrencySymbol, jchar minusSign,
		jchar monetaryDecimalSeparator, jstring nan, jchar patternSeparator,
		jchar percent, jchar perMill, jchar zeroDigit) {
...
JNIEXPORT void JNICALL
Java_com_ibm_icu4jni_text_NativeDecimalFormat_close(JNIEnv*, jclass, jint addr) {
...
JNIEXPORT void JNICALL
Java_com_ibm_icu4jni_text_NativeDecimalFormat_setSymbol(JNIEnv* env, jclass,
		jint addr, jint javaSymbol, jstring javaValue) {
	...
	unum_setSymbol(toUNumberFormat(addr), symbol, s.getBuffer(), s.length(),
			&status);
	icu4jni_error(env, status);
}
...
JNIEXPORT void JNICALL
Java_com_ibm_icu4jni_text_NativeDecimalFormat_setAttribute(JNIEnv*, jclass,
		jint addr, jint javaAttr, jint value) {
...
JNIEXPORT void JNICALL
Java_com_ibm_icu4jni_text_NativeDecimalFormat_setTextAttribute(JNIEnv* env,
		jclass, jint addr, jint javaAttr, jstring javaValue) {
	ScopedJavaUnicodeString value(env, javaValue);
	UnicodeString& s(value.unicodeString());
	UErrorCode status = U_ZERO_ERROR;
...
Java_com_ibm_icu4jni_text_NativeDecimalFormat_formatLong(JNIEnv* env, jclass,
		jint addr, jlong value, jobject fpIter) {
...
JNIEXPORT jcharArray JNICALL
Java_com_ibm_icu4jni_text_NativeDecimalFormat_formatDouble(JNIEnv* env, jclass,
		jint addr, jdouble value, jobject fpIter) {
	return format(env, addr, fpIter, value);
}

//static jcharArray NativeDecimalFormat_formatDigitList(JNIEnv* env, jclass, jint addr, jstring value, jobject fpIter) {
JNIEXPORT jcharArray JNICALL
Java_com_ibm_icu4jni_text_NativeDecimalFormat_formatDigitList(JNIEnv* env,
		jclass, jint addr, jstring value, jobject fpIter) {
...
static jobject newBigDecimal(JNIEnv* env, const char* value, jsize len) {
	...
	if (!bigDecimalClass) {
		log_error(XDLOG, "--------------> Nil bigDecimalClass!!\n");
	}
	static jmethodID gBigDecimal_init = env->GetMethodID(bigDecimalClass,
			"<init>", "(Ljava/lang/String;)V");
	if (!gBigDecimal_init) {
		log_error(XDLOG, "--------------> Nil gBigDecimal_init!!\n");
	}
	...
	jobject str = env->NewString((const jchar*) tmp.getBuffer(), tmp.length());
	return env->NewObject(bigDecimalClass, gBigDecimal_init, str);
}
...
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_text_NativeDecimalFormat_cloneImpl(JNIEnv*, jclass,
		jint addr) {
	DecimalFormat* fmt = toDecimalFormat(addr);

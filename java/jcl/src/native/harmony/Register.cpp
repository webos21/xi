...
extern int register_org_apache_harmony_luni_platform_OSNetworkSystem(
		JNIEnv* env);
...
static int registerCoreLibrariesJni(JNIEnv* env) {
...
#else
	bool result =
			register_org_apache_harmony_luni_platform_OSNetworkSystem(env)
					!= -1;
#endif
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
	...
	//log_print(XDLOG, "******************** JNI_OnLoad ********************\n");
	if (vm->GetEnv((void**) &env, JNI_VERSION_1_4) != JNI_OK) {
		log_error(XDLOG, "ERROR: GetEnv failed\n");
		goto bail;
	}

	if (jniRegisterSystemMethods(env) < 0) {
		log_error(XDLOG, "jniRegisterSystemMethods failed\n");
		goto bail;
	}

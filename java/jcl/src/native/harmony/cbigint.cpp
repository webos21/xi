...
#if defined(__linux__) || defined(FREEBSD) || defined(ZOS) || defined(MACOSX) || defined(AIX) || defined(__MINGW32__)
#define USE_LL
#endif
...
int32_t addHighPrecision(uint64_t * arg1, int32_t length1, uint64_t * arg2,
		int32_t length2) {
...
void subtractHighPrecision(uint64_t * arg1, int32_t length1, uint64_t * arg2,
		int32_t length2) {
	/* assumes arg1 > arg2 */
	int32_t index;
...
static uint32_t simpleMultiplyHighPrecision(uint64_t* arg1, int32_t length,
		uint64_t arg2) {
	...
	do {
		product = HIGH_IN_U64 (product) + arg2
...
static void simpleMultiplyAddHighPrecision(uint64_t * arg1, int32_t length,
		uint64_t arg2, uint32_t * result) {
	...
	do {
		product = HIGH_IN_U64 (product) + result[at (resultIndex)] + arg2
				* LOW_U32_FROM_PTR (arg1 + index);
...
void multiplyHighPrecision(uint64_t * arg1, int32_t length1, uint64_t * arg2,
		int32_t length2, uint64_t * result, int32_t length) {
	...
	xi_mem_set(result, 0, sizeof(uint64_t) * length);
	...
	for (count = 0; count < length2; ++count) {
		simpleMultiplyAddHighPrecision(arg1, length1, LOW_IN_U64 (arg2[count]),
				resultIn32 + (++index));
#if __BYTE_ORDER == __LITTLE_ENDIAN
		simpleMultiplyAddHighPrecision(arg1, length1, HIGH_IN_U64(arg2[count]),
				resultIn32 + (++index));
#else
...
uint32_t simpleAppendDecimalDigitHighPrecision(uint64_t * arg1, int32_t length,
		uint64_t digit) {
...
int32_t compareHighPrecision(uint64_t * arg1, int32_t length1, uint64_t * arg2,
		int32_t length2) {
...
jdouble toDoubleHighPrecision(uint64_t * arg, int32_t length) {
	...
	if (length == 0)
		...
		if (highBit <= 53) {
			highBit = 53 - highBit;
			if (highBit > 0) {
				mantissa = (arg[length] << highBit) | (arg[length - 1] >> (64
						- highBit));
			} else {
				mantissa = arg[length];
			}
...
static uint64_t simpleMultiplyHighPrecision64(uint64_t* arg1, int32_t length,
		uint64_t arg2);
...
uint64_t simpleMultiplyHighPrecision64(uint64_t * arg1, int32_t length,
		uint64_t arg2) {
	...
	do {
		if ((*pArg1 != 0) || (intermediate != 0)) {
			...
			intermediate = carry1 + HIGH_IN_U64 (intermediate)
					+ static_cast<uint64_t> (HIGH_U32_FROM_VAR (arg2))
							* static_cast<uint64_t> (buf32);
		}
		pArg1++;
	} while (++index < length);
	return intermediate;

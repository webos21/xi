...
#include "libcore_icu_NativeIDN.h"
...
Java_libcore_icu_NativeIDN_convertImpl(JNIEnv* env, jclass, jstring s,
		jint flags, jboolean toAscii) {
	...
	size_t resultLength = toAscii ? uidna_IDNToASCII(src, srcLength, &dst[0],
			sizeof(dst), flags, NULL, &status) : uidna_IDNToUnicode(src,
			srcLength, &dst[0], sizeof(dst), flags, NULL, &status);
	if (U_FAILURE(status)) {
	...
	return env->NewString((const jchar *) &dst[0], resultLength);
}

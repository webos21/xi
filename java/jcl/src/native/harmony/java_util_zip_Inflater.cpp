...
Java_java_util_zip_Inflater_createStream(JNIEnv* env, jobject,
		jboolean noHeader) {
	...
	if (err != Z_OK) {
		throwExceptionForZlibError(env, "java/lang/IllegalArgumentException",
				err);
		return -1;
	}
	return reinterpret_cast<xlong> (jstream.release());
}

//static void Inflater_setInputImpl(JNIEnv* env, jobject, jbyteArray buf, jint off, jint len, jlong handle) {
JNIEXPORT void JNICALL
Java_java_util_zip_Inflater_setInputImpl(JNIEnv* env, jobject, jbyteArray buf,
		jint off, jint len, jlong handle) {
...
Java_java_util_zip_Inflater_setFileInputImpl(JNIEnv* env, jobject,
	...
	int rc = xi_file_seek(fd, off, XI_FILE_SEEK_SET);
	if (rc < 0) {
		jniThrowIOExceptionMsg(env, "Cannot get the position of zip file!!!", rc);
		return 0;
	}
	jint totalByteCount = 0;
	Bytef* dst = reinterpret_cast<Bytef*> (&stream->input[0]);
	ssize_t byteCount;
	while ((byteCount = xi_file_read(fd, dst, len)) > 0) {
		dst += byteCount;
		len -= byteCount;
		totalByteCount += byteCount;
	}
	if (byteCount < 0) {
		jniThrowIOExceptionMsg(env, "Cannot inflate zip file!!!", byteCount);
		return 0;
	}
...
Java_java_util_zip_Inflater_inflateImpl(JNIEnv* env, jobject recv,
		jbyteArray buf, int off, int len, jlong handle) {
	//log_print(XDLOG, "       ===> [BEGIN] Java_java_util_zip_Inflater_inflateImpl\n");
	jclass inflaterClass = env->FindClass("java/util/zip/Inflater");
	jfieldID finished = env->GetFieldID(inflaterClass, "finished", "Z");
	jfieldID inRead = env->GetFieldID(inflaterClass, "inRead", "I");
	jfieldID needsDictionary = env->GetFieldID(inflaterClass,
			"needsDictionary", "Z");
	...
	if (err != Z_OK) {
		...
		if (err == Z_STREAM_END) {
			...
			throwExceptionForZlibError(env,
					"java/util/zip/DataFormatException", err);
			return -1;
		}
	}
	...
	jint inReadValue = env->GetIntField(recv, inRead);
	inReadValue += bytesRead;
	env->SetIntField(recv, inRead, inReadValue);
	//log_print(XDLOG, "       <===> [ END ] Java_java_util_zip_Inflater_inflateImpl\n");
	return bytesWritten;
}

//static jint Inflater_getAdlerImpl(JNIEnv*, jobject, jlong handle) {
JNIEXPORT jint JNICALL
Java_java_util_zip_Inflater_getAdlerImpl(JNIEnv*, jobject, jlong handle) {
	return toNativeZipStream(handle)->stream.adler;
}

//static void Inflater_endImpl(JNIEnv*, jobject, jlong handle) {
JNIEXPORT void JNICALL
Java_java_util_zip_Inflater_endImpl(JNIEnv*, jobject, jlong handle) {
	NativeZipStream* stream = toNativeZipStream(handle);
...
JNIEXPORT void JNICALL
Java_java_util_zip_Inflater_setDictionaryImpl(JNIEnv* env, jobject,
		jbyteArray dict, int off, int len, jlong handle) {
...
JNIEXPORT void JNICALL
Java_java_util_zip_Inflater_resetImpl(JNIEnv* env, jobject, jlong handle) {
...
JNIEXPORT jlong JNICALL
Java_java_util_zip_Inflater_getTotalOutImpl(JNIEnv*, jobject, jlong handle) {
	return toNativeZipStream(handle)->stream.total_out;
}

//static jlong Inflater_getTotalInImpl(JNIEnv*, jobject, jlong handle) {
JNIEXPORT jlong JNICALL
Java_java_util_zip_Inflater_getTotalInImpl(JNIEnv*, jobject, jlong handle) {

...
Java_java_util_zip_Deflater_setDictionaryImpl(JNIEnv* env, jobject,
...
JNIEXPORT jlong JNICALL
Java_java_util_zip_Deflater_createStream(JNIEnv * env, jobject, jint level,
		jint strategy, jboolean noHeader) {
	...
	int
			err =
					deflateInit2(&jstream->stream, level, Z_DEFLATED, wbits, mlevel, strategy);
	if (err != Z_OK) {
...
JNIEXPORT void JNICALL
Java_java_util_zip_Deflater_setInputImpl(JNIEnv* env, jobject, jbyteArray buf,
...
Java_java_util_zip_Deflater_deflateImpl(JNIEnv* env, jobject recv,
		jbyteArray buf, int off, int len, jlong handle, int flushParm) {
	/* We need to get the number of bytes already read */
	jint inBytes;

	//log_print(XDLOG, "    ===> [BEGIN] java_util_zip_Deflater_deflateImpl\n");
	jclass deflaterClass = env->FindClass("java/util/zip/Deflater");
	jfieldID inRead = env->GetFieldID(deflaterClass, "inRead", "I");
	inBytes = env->GetIntField(recv, inRead);
	...
	if (err != Z_OK) {
		...
		if (err == Z_STREAM_END) {
			jfieldID finished = env->GetFieldID(deflaterClass, "finished", "Z");
	...
	if (flushParm != Z_FINISH) {
		...
		env->SetIntField(recv, inRead,
				(jint) stream->stream.total_in - sin + inBytes);
	}
	//log_print(XDLOG, "    <=== [ END ] java_util_zip_Deflater_deflateImpl\n");
	return stream->stream.total_out - sout;
}

//static void Deflater_endImpl(JNIEnv*, jobject, jlong handle) {
JNIEXPORT void JNICALL
Java_java_util_zip_Deflater_endImpl(JNIEnv*, jobject, jlong handle) {
	NativeZipStream* stream = toNativeZipStream(handle);
...
JNIEXPORT void JNICALL
Java_java_util_zip_Deflater_resetImpl(JNIEnv* env, jobject, jlong handle) {
	NativeZipStream* stream = toNativeZipStream(handle);
...
JNIEXPORT void JNICALL
Java_java_util_zip_Deflater_setLevelsImpl(JNIEnv* env, jobject, int level,
		int strategy, jlong handle) {
	if (handle == -1) {

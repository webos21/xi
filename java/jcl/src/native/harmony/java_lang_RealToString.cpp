...
Java_java_lang_RealToString_bigIntDigitGenerator(JNIEnv* env, jobject obj,
		jlong f, jint e, jboolean isDenormalized, jint p) {
	...
	xi_mem_set(S, 0, STemp_SIZE * sizeof(xuint64));
	xi_mem_set(mplus, 0, RM_SIZE * sizeof(xuint64));
	xi_mem_set(mminus, 0, RM_SIZE * sizeof(xuint64));
	xi_mem_set(Temp, 0, STemp_SIZE * sizeof(xuint64));
	...
	xi_mem_set(Temp + RM_SIZE, 0, (STemp_SIZE - RM_SIZE) * sizeof(xuint64));
	xi_mem_copy(Temp, R, RM_SIZE * sizeof(xuint64));
	...
	jclass realToStringClass = env->FindClass("java/lang/RealToString");
	static jfieldID digitsFid = env->GetFieldID(realToStringClass, "digits",
			"[I");
	jintArray javaDigits = reinterpret_cast<jintArray> (env->GetObjectField(
			obj, digitsFid));
	...
	do {
		...
		for (i = 3; i >= 0; --i) {
			...
			Temp[SLength] = 0;
			xi_mem_copy(Temp, S, SLength * sizeof(xuint64));
		...
		xi_mem_set(Temp + RLength, 0, (STemp_SIZE - RLength) * sizeof(xuint64));
		xi_mem_copy(Temp, R, RLength * sizeof(xuint64));
		TempLength = (RLength > mplus_Length ? RLength : mplus_Length) + 1;
		addHighPrecision(Temp, TempLength, mplus, mplus_Length);
	...
	static jfieldID digitCountFid = env->GetFieldID(realToStringClass,
			"digitCount", "I");
	env->SetIntField(obj, digitCountFid, digitCount);

	static jfieldID firstKFid = env->GetFieldID(realToStringClass, "firstK",
			"I");

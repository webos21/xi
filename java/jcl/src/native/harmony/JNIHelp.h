...
/* by jshwang - Prevent warnings from -Wmissing-prototypes.  */
...
inline int jniThrowIOExceptionMsg(JNIEnv* env, const char *msg, int errnum)
{
	char buf[128];
	xi_mem_set(buf, 0, sizeof(buf));
	xi_sprintf(buf, "%s (ret=%d)", msg, errnum);

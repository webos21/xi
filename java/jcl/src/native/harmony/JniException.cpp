...
void jniThrowSocketExceptionMsg(JNIEnv* env, const char* eobj, const char* message, int error) {
	char buf[512];
	xi_sprintf(buf, "Socket Error : %s (err=%d)", message, error);
	jniThrowException(env, eobj, buf);
}

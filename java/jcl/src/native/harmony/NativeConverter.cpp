...
#include "com_ibm_icu4jni_charset_NativeConverter.h"
...
Java_com_ibm_icu4jni_charset_NativeConverter_openConverter(JNIEnv* env, jclass,
...
Java_com_ibm_icu4jni_charset_NativeConverter_closeConverter(JNIEnv*, jclass,
		jlong address) {
...
Java_com_ibm_icu4jni_charset_NativeConverter_encode(JNIEnv* env, jclass,
		jlong address, jcharArray source, jint sourceEnd, jbyteArray target,
		jint targetEnd, jintArray data, jboolean flush) {
	...
	const UChar* mySourceLimit = (const UChar*) uSource.get() + sourceEnd;
	char* cTarget = reinterpret_cast<char*> (uTarget.get() + *targetOffset);
	const char* cTargetLimit = reinterpret_cast<const char*> (uTarget.get()
			+ targetEnd);
	UErrorCode errorCode = U_ZERO_ERROR;
	ucnv_fromUnicode(cnv, &cTarget, cTargetLimit, (const UChar**) &mySource,
 ...
 */
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_decode(JNIEnv* env, jclass,
		jlong address, jbyteArray source, jint sourceEnd, jcharArray target,
		jint targetEnd, jintArray data, jboolean flush) {
	...
	jint* sourceOffset = &myData[0];
	jint* targetOffset = &myData[1];
	const char* mySource = reinterpret_cast<const char*> (uSource.get()
			+ *sourceOffset);
	const char* mySourceLimit = reinterpret_cast<const char*> (uSource.get()
			+ sourceEnd);
	UChar* cTarget = (UChar*) uTarget.get() + *targetOffset;
	const UChar* cTargetLimit = (const UChar*) uTarget.get() + targetEnd;
	UErrorCode errorCode = U_ZERO_ERROR;
	ucnv_toUnicode(cnv, &cTarget, cTargetLimit, &mySource, mySourceLimit, NULL,
			flush, &errorCode);
	*sourceOffset = mySource - reinterpret_cast<const char*> (uSource.get())
			- *sourceOffset;
	*targetOffset = (cTarget - (UChar*) uTarget.get() - *targetOffset);
...
JNIEXPORT void JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_resetByteToChar(JNIEnv*, jclass,
		jlong address) {
	UConverter* cnv = toUConverter(address);
	if (cnv) {
...
JNIEXPORT void JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_resetCharToByte(JNIEnv*, jclass,
		jlong address) {
	UConverter* cnv = toUConverter(address);
	if (cnv) {
...
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_getMaxBytesPerChar(JNIEnv*,
		jclass, jlong address) {
	UConverter* cnv = toUConverter(address);
	int result;
	result = (cnv != NULL) ? ucnv_getMaxCharSize(cnv) : -1;
...
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_getMinBytesPerChar(JNIEnv*,
		jclass, jlong address) {
	UConverter* cnv = toUConverter(address);
...
Java_com_ibm_icu4jni_charset_NativeConverter_getAveBytesPerChar(JNIEnv*,
		jclass, jlong address) {
	UConverter* cnv = toUConverter(address);
	return (cnv != NULL) ? ((ucnv_getMaxCharSize(cnv)
			+ ucnv_getMinCharSize(cnv)) / 2.0) : -1;
}
 ...
 */
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_flushByteToChar(JNIEnv* env,
		jclass, jlong address, jcharArray target, jint targetEnd,
		jintArray data) {
	UConverter* cnv = toUConverter(address);
	if (cnv == NULL) {
		return U_ILLEGAL_ARGUMENT_ERROR;
 ...
 */
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_flushCharToByte(JNIEnv* env,
		jclass, jlong address, jbyteArray target, jint targetEnd,
		jintArray data) {
	UConverter* cnv = toUConverter(address);
	if (cnv == NULL) {
		return U_ILLEGAL_ARGUMENT_ERROR;
	...
	const UChar* mySourceLimit = (const UChar*) &source;
	char* cTarget = reinterpret_cast<char*> (uTarget.get() + *targetOffset);
	const char* cTargetLimit = reinterpret_cast<char*> (uTarget.get()
			+ targetEnd);
	UErrorCode errorCode = U_ZERO_ERROR;
	ucnv_fromUnicode(cnv, &cTarget, cTargetLimit, (const UChar**) &mySource,
...
Java_com_ibm_icu4jni_charset_NativeConverter_canEncode(JNIEnv*, jclass,
		jlong address, jint codeUnit) {
	UErrorCode errorCode = U_ZERO_ERROR;
	UConverter* cnv = toUConverter(address);
	if (cnv == NULL) {
		return JNI_FALSE;
	...
	ucnv_fromUnicode(cnv, &dst, dstLimit, &src, srcLimit, NULL, TRUE,
			&errorCode);
...
static jstring getJavaCanonicalName(JNIEnv* env, const char* icuCanonicalName) {
	...
	if ((cName = ucnv_getStandardName(icuCanonicalName, "MIME", &status))
			!= NULL) {
		return env->NewStringUTF(cName);
	} else if ((cName = ucnv_getStandardName(icuCanonicalName, "IANA", &status))
			!= NULL) {
		return env->NewStringUTF(cName);
	}
...
Java_com_ibm_icu4jni_charset_NativeConverter_getAvailableCharsetNames(
		JNIEnv* env, jclass) {
	int32_t num = ucnv_countAvailable();
	jobjectArray result;

	jclass stringClass = env->FindClass("java/lang/String");
	result = env->NewObjectArray(num, stringClass, NULL);
	for (int i = 0; i < num; ++i) {
		const char* name = ucnv_getAvailableName(i);
		ScopedLocalRef<jstring> javaCanonicalName(env,
				getJavaCanonicalName(env, name));
...
static jobjectArray getAliases(JNIEnv* env, const char* icuCanonicalName) {
	...
	jclass stringClass = env->FindClass("java/lang/String");
	jobjectArray result = env->NewObjectArray(actualAliasCount, stringClass,
			NULL);
...
static const char* getICUCanonicalName(const char* name) {
	...
	if ((canonicalName = ucnv_getCanonicalName(name, "MIME", &error)) != NULL) {
		//log_print(XDLOG, "   getICUCanonicalName --- case 1: %s\n", canonicalName);
		return canonicalName;
	} else if ((canonicalName = ucnv_getCanonicalName(name, "IANA", &error))
			!= NULL) {
		//log_print(XDLOG, "   getICUCanonicalName --- case 2: %s\n", canonicalName);
		return canonicalName;
	} else if ((canonicalName = ucnv_getCanonicalName(name, "", &error))
			!= NULL) {
		//log_print(XDLOG, "   getICUCanonicalName --- case 3: %s\n", canonicalName);
		return canonicalName;
	} else if ((canonicalName = ucnv_getAlias(name, 0, &error)) != NULL) {
		/* we have some aliases in the form x-blah .. match those first */
		//log_print(XDLOG, "   getICUCanonicalName --- case 4: %s\n", canonicalName);
		return canonicalName;
		...
		//log_print(XDLOG, "   getICUCanonicalName --- case 5: %s\n", canonicalName);
		if (cnv.get() != NULL) {
			return name + 2;
		}
	}
	log_error(XDLOG, "   getICUCanonicalName: Err\n");
	return NULL;
}

static void CHARSET_ENCODER_CALLBACK(const void* rawContext,
		UConverterFromUnicodeArgs* args, const UChar* codeUnits,
		int32_t length, UChar32 codePoint, UConverterCallbackReason reason,
		UErrorCode* status) {
	if (!rawContext) {
		return;
	}
	const EncoderCallbackContext* ctx =
			reinterpret_cast<const EncoderCallbackContext*> (rawContext);
	switch (reason) {
	case UCNV_UNASSIGNED:
		ctx->onUnmappableInput(ctx, args, codeUnits, length, codePoint, reason,
				status);
		return;
	case UCNV_ILLEGAL:
	case UCNV_IRREGULAR:
		ctx->onMalformedInput(ctx, args, codeUnits, length, codePoint, reason,
				status);
		return;
	case UCNV_CLOSE:
		delete ctx;
 ...
 */
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_setCallbackEncode(JNIEnv* env,
		jclass, jlong address, jint onMalformedInput, jint onUnmappableInput,
		jbyteArray subBytes) {
	UConverter* cnv = toUConverter(address);
	if (!cnv) {
		return U_ILLEGAL_ARGUMENT_ERROR;
	...
	ucnv_getFromUCallBack(cnv, &fromUOldAction,
			const_cast<const void**> (&fromUOldContext));
	...
	if (fromUOldContext == NULL) {
		...
		fromUNewContext
				= const_cast<EncoderCallbackContext*> (reinterpret_cast<const EncoderCallbackContext*> (fromUOldContext));
	...
	UErrorCode errorCode = U_ZERO_ERROR;
	ucnv_setFromUCallBack(cnv, fromUNewAction, fromUNewContext,
			&fromUOldAction, &fromUOldContext, &errorCode);
	return errorCode;
}

static void decoderIgnoreCallback(const void*, UConverterToUnicodeArgs*,
		const char*, int32_t, UConverterCallbackReason, UErrorCode* err) {
...
static void decoderReplaceCallback(const void* rawContext,
		UConverterToUnicodeArgs* toArgs, const char*, int32_t,
		UConverterCallbackReason, UErrorCode* err) {
	if (!rawContext) {
		return;
	}
	const DecoderCallbackContext* context =
			reinterpret_cast<const DecoderCallbackContext*> (rawContext);
	*err = U_ZERO_ERROR;
...
static void CHARSET_DECODER_CALLBACK(const void* rawContext,
		UConverterToUnicodeArgs* args, const char* codeUnits, int32_t length,
		UConverterCallbackReason reason, UErrorCode* status) {
	if (!rawContext) {
		return;
	}
	const DecoderCallbackContext* ctx =
			reinterpret_cast<const DecoderCallbackContext*> (rawContext);
	switch (reason) {
	case UCNV_UNASSIGNED:
 ...
 */
JNIEXPORT jint JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_setCallbackDecode(JNIEnv* env,
		jclass, jlong address, jint onMalformedInput, jint onUnmappableInput,
		jcharArray subChars) {
	UConverter* cnv = toUConverter(address);

	if (cnv == NULL) {
		log_error(XDLOG, "     [setCallbackDecode] ERROR 1\n");
		return U_ILLEGAL_ARGUMENT_ERROR;
	}
	...
	if (toUOldContext == NULL) {
		...
		toUNewContext
				= const_cast<DecoderCallbackContext*> (reinterpret_cast<const DecoderCallbackContext*> (toUOldContext));
	...
	if (sub.get() == NULL) {
		log_error(XDLOG, "     [setCallbackDecode] ERROR 2 \n");
		return U_ILLEGAL_ARGUMENT_ERROR;
	}
	toUNewContext->length = sub.size();
	u_strncpy(toUNewContext->subUChars, (const UChar*) sub.get(), sub.size());
	UErrorCode errorCode = U_ZERO_ERROR;
	ucnv_setToUCallBack(cnv, toUNewAction, toUNewContext, &toUOldAction,
			&toUOldContext, &errorCode);
	return errorCode;
}

//static jfloat NativeConverter_getAveCharsPerByte(JNIEnv* env, jclass, jlong handle) {
JNIEXPORT jfloat JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_getAveCharsPerByte(JNIEnv* env,
		jclass, jlong handle) {
	// by jshwang
	//    return (1 / (jfloat) NativeConverter_getMaxBytesPerChar(env, NULL, handle));
	return (1
			/ (jfloat) Java_com_ibm_icu4jni_charset_NativeConverter_getMaxBytesPerChar(
					env, NULL, handle));
}

//static jbyteArray NativeConverter_getSubstitutionBytes(JNIEnv* env, jclass, jlong address) {
JNIEXPORT jbyteArray JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_getSubstitutionBytes(JNIEnv* env,
		jclass, jlong address) {
	UConverter* cnv = toUConverter(address);
	if (cnv == NULL) {
		return NULL;
...
JNIEXPORT jboolean JNICALL
Java_com_ibm_icu4jni_charset_NativeConverter_contains(JNIEnv* env, jclass,
	...
	UnicodeSet set1;
	ucnv_getUnicodeSet(converter1.get(), set1.toUSet(), UCNV_ROUNDTRIP_SET,
			&errorCode);

	UniqueUConverter converter2(ucnv_open(name2Chars.c_str(), &errorCode));
	UnicodeSet set2;
	ucnv_getUnicodeSet(converter2.get(), set2.toUSet(), UCNV_ROUNDTRIP_SET,
			&errorCode);
...
Java_com_ibm_icu4jni_charset_NativeConverter_charsetForName(JNIEnv* env,
		jclass, jstring charsetName) {
	//	log_print(XDLOG, "   [BEGIN] com_ibm_icu4jni_charset_NativeConverter_charsetForName\n");
	ScopedUtfChars charsetNameChars(env, charsetName);
	if (charsetNameChars.c_str() == NULL) {
		log_error(XDLOG, "   NativeConverter_charsetForName ---- Err 1\n");
		return NULL;
	}
	// Get ICU's canonical name for this charset.
	const char* icuCanonicalName =
			getICUCanonicalName(charsetNameChars.c_str());
	if (icuCanonicalName == NULL) {
		log_error(XDLOG, "   NativeConverter_charsetForName ---- Err 2\n");
		return NULL;
	}
	// Get Java's canonical name for this charset.
	jstring javaCanonicalName = getJavaCanonicalName(env, icuCanonicalName);
	if (env->ExceptionOccurred()) {
		log_error(XDLOG, "   NativeConverter_charsetForName ---- Err 3\n");
		return NULL;
	}
	...
	if (cnv.get() == NULL) {
		log_error(XDLOG, "   NativeConverter_charsetForName ---- Err 4\n");
		return NULL;
	}
	cnv.reset();
	...
	if (env->ExceptionOccurred()) {
		log_error(XDLOG, "   NativeConverter_charsetForName ---- Err 5\n");
		return NULL;
	}

	// Construct the CharsetICU object.
	jclass charsetICUClass = env->FindClass(
			"com/ibm/icu4jni/charset/CharsetICU");
	jmethodID charsetConstructor = env->GetMethodID(charsetICUClass, "<init>",
			"(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;)V");
	if (env->ExceptionOccurred()) {
		log_error(XDLOG, "   NativeConverter_charsetForName ---- Err 6\n");
		return NULL;
	}
	//	log_print(XDLOG, "   [ END ] com_ibm_icu4jni_charset_NativeConverter_charsetForName\n");
	return env->NewObject(charsetICUClass, charsetConstructor,

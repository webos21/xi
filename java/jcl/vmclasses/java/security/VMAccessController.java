...
final class VMAccessController {
	...
	/**
	 * This is a per-thread stack of AccessControlContext objects (which can be
	 * null) for each call to AccessController.doPrivileged in each thread's
	...
	/**
	 * This is a Boolean that, if set, tells getContext that it has already been
	 * called once, allowing us to handle recursive permission checks caused by
	 * methods getContext calls.
	 */
	...
	static {
		...
		CodeSource source = new CodeSource(null, tmpCert);
		Permissions permissions = new Permissions();
		permissions.add(new AllPermission());
		ProtectionDomain[] domain = new ProtectionDomain[] { new ProtectionDomain(
				source, permissions) };
	 ...
	 * with an access control context. This method is used by
	 * {@link AccessController#doPrivileged(java.security.PrivilegedAction,java.security.AccessControlContext)}
	 * to set up the context that will be returned by {@link #getContext()}.
	 * This method relates the class to the current thread, so contexts pushed
	 * from one thread will not be available to another.
	 * 
	 * @param acc
	...
	/**
	 * Removes the relation of a class to an {@link AccessControlContext}. This
	 * method is used by {@link AccessController} when exiting from a call to
	 * {@link AccessController#doPrivileged(java.security.PrivilegedAction,java.security.AccessControlContext)}
	 * .
	 */
	...
	/**
	 * Examine the method stack of the currently running thread, and create an
	 * {@link AccessControlContext} filled in with the appropriate
	 * {@link ProtectionDomain} objects given this stack.
	 * 
	 * @return The context.
	 */
	static AccessControlContext getContext() {
		...
		for (int i = 3; i < classes.length && privileged < 2; i++) {
			...
			// Create a static snapshot of this domain, which may change over
			// time
			// if the current policy changes.
			domains.add(new ProtectionDomain(domain.getCodeSource(), domain
					.getPermissions()));
		}
		...
		ProtectionDomain[] result = (ProtectionDomain[]) domains
				.toArray(new ProtectionDomain[domains.size()]);
	...
	/**
	 * Returns a snapshot of the current call stack as a pair of arrays: the
	 * first an array of classes in the call stack, the second an array of
	 * strings containing the method names in the call stack. The two arrays
	 * match up, meaning that method <i>i</i> is declared in class <i>i</i>. The
	 * arrays are clean; it will only contain Java methods, and no element of

...
final class VMRuntime {
	...
	/**
	 * Run finalization on all finalizable Objects (even live ones). This should
	 * only be called immediately prior to VM termination.
	 * 
	 * @see #finalize()
	 */
	 ...
	 * 
	 * @param status
	 *            the status to end the process with
	 */
	 ...
	 * it to the path. XXX This method is being replaced by
	 * System.mapLibraryName.
	 * 
	 * @param pathname
	 *            the path
	 * @param libname
	 *            the short version of the library name
	 * @return the full filename
	 */
	static native String mapLibraryName(String libname);

	/**
	 * Execute a process. The command line has already been tokenized, and the
	 * environment should contain name=value mappings. If directory is null, use
	 * the current working directory; otherwise start the process in that
	 * directory. If env is null, then the new process should inherit the
	 * environment of this process.
	 * 
	 * @param cmd
	 *            the non-null command tokens
	 * @param env
	 ...
	 *             if cmd or env have null elements
	 */
	...
	/**
	 * This method is called by Runtime.addShutdownHook() when it is called for
	 * the first time. It enables the VM to lazily setup an exit handler, should
	 * it so desire.
	 */

...
typedef xuintptr ConstantPoolEntry;
...
typedef xuintptr *(*NativeMethod)(Class*, struct methodblock*, xuintptr*);
...
#define ARRAY_DATA(arrayRef, type)   ((type*)(((xuintptr*)(arrayRef+1))+1)) 
#define ARRAY_LEN(arrayRef)          *(xuintptr*)(arrayRef+1)
...
extern Object *allocMultiArray(Class *array_class, int dim, xintptr *count);
extern Object *cloneObject(Object *ob);
extern xuintptr getObjectHashcode(Object *ob);
...
extern xuintptr resolveSingleConstant(Class *class, int index);
...
#define setStackTrace() \
    setStackTrace0(getExecEnv(), XI_INT_MAX)

/* interp */

extern xuintptr *executeJava();
...
extern xuintptr *resolveNativeWrapper(Class *class, MethodBlock *mb,
                                       xuintptr *ostack);
extern void unloaderUnloadDll(xuintptr entry);
...
extern Class *getCallerCallerCallerClass(); // by jshwang
...
extern void jam_fprintf(int fd, const char *fmt, ...);
extern void jamvm_exit(int status);

#define jam_printf(fmt, ...) log_print(XDLOG, fmt, ## __VA_ARGS__)
...
extern int getTimeoutAbsolute(long long millis, long long nanos);
extern int getTimeoutRelative(long long millis, long long nanos);

...
void initialiseException() {
    ...
    if(backtrace == NULL || ste_init_mb == NULL) {
        log_error(XDLOG, "Error initialising VM (initialiseException)\n");
        exitVM(1);
    }
...
void signalChainedExceptionName(char *excep_name, char *message, Object *cause) {
    if(!inited) {
        log_error(XDLOG, "Exception occurred while VM initialising.\n");
        if(message) {
            log_error(XDLOG, "%s: %s\n", excep_name, message);
        } else {
            log_error(XDLOG, "%s\n", excep_name);
...
void printException() {
    ...
    if(excep != NULL) {
        ...
        if(ee->exception) {
            log_error(XDLOG, "Exception occurred while printing exception"
                        " (%s)...\n", CLASS_CB(ee->exception->class)->name);
            log_error(XDLOG, "Original exception was %s\n",
...
Object *setStackTrace0(ExecEnv *ee, int max_depth) {
    ...
    if(last->prev == NULL) {
        array = allocTypeArray(sizeof(xuintptr) == 4 ? T_INT : T_LONG, 0);
        if(array == NULL)
            return NULL;
        goto out2;
    }
...
out:
    array = allocTypeArray(sizeof(xuintptr) == 4 ? T_INT : T_LONG, depth*2);
    if(array == NULL)
        return NULL;

    data = ARRAY_DATA(array, xuintptr);
    depth = 0;
    do {
        for(; bottom->mb != NULL; bottom = bottom->prev) {
            ...
            data[depth++] = (xuintptr)bottom->mb;
            data[depth++] = (xuintptr)bottom->last_pc;
        }
...
void markVMThrowable(Object *vmthrwble, int mark) {
    ...
    if((array = INST_DATA(vmthrwble, Object*, backtrace_offset)) != NULL) {
        xuintptr *src = ARRAY_DATA(array, xuintptr);

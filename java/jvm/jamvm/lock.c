...
void monitorLock(Monitor *mon, Thread *self) {
	if (mon->owner == self) {
		...
		if (xi_thread_mutex_trylock(&mon->lock)) {
			disableSuspend(self);
...
Monitor *findMonitor(Object *obj) {
	xuintptr lockword = LOCKWORD_READ(&obj->lock);
...
static void inflate(Object *obj, Monitor *mon, Thread *self) {
	...
	LOCKWORD_WRITE(&obj->lock, (xuintptr) mon | SHAPE_BIT);
}

void objectLock(Object *obj) {
	Thread *self = threadSelf();
	xuintptr thin_locked = self->id << TID_SHIFT;
	xuintptr entering, lockword;
	Monitor *mon;
	...
	try_again: // goto LABEL
	mon = findMonitor(obj);

	try_again2: // goto LABEL
	...
	if (mon->obj != obj) {
		while (entering = LOCKWORD_READ(&mon->entering), !(LOCKWORD_COMPARE_AND_SWAP(&mon->entering,
				entering, entering-1))) {
			// Nothing to do
		}
		goto try_again;

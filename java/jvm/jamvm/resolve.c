...
Class *resolveClass(Class *class, int cp_index, int init) {
	...
	retry: switch (CP_TYPE(cp, cp_index)) {
	...
	case CONSTANT_Class: {
		...
		MBARRIER();
		CP_INFO(cp, cp_index) = (xuintptr) resolved_class;
		MBARRIER();
...
MethodBlock *resolveMethod(Class *class, int cp_index) {
	...
	case CONSTANT_Methodref: {
		...
		if (mb) {
			...
			CP_TYPE(cp, cp_index) = CONSTANT_Locked;
			MBARRIER();
			CP_INFO(cp, cp_index) = (xuintptr) mb;
			MBARRIER();
...
FieldBlock *resolveField(Class *class, int cp_index) {
	...
	case CONSTANT_Fieldref: {
		...
		if (fb) {
			...
			CP_TYPE(cp, cp_index) = CONSTANT_Locked;
			MBARRIER();
			CP_INFO(cp, cp_index) = (xuintptr) fb;
			MBARRIER();
			CP_TYPE(cp, cp_index) = CONSTANT_Resolved;
		} else
...
xuintptr resolveSingleConstant(Class *class, int cp_index) {
	ConstantPool *cp = &(CLASS_CB(class)->constant_pool);
	...
	case CONSTANT_String: {
		...
		if (string) {
			CP_TYPE(cp, cp_index) = CONSTANT_Locked;
			MBARRIER();
			CP_INFO(cp, cp_index) = (xuintptr) findInternedString(string);
			MBARRIER();
...
MethodBlock *lookupVirtualMethod(Object *ob, MethodBlock *mb) {
	...
	if (CLASS_CB(mb->class)->access_flags & ACC_INTERFACE) {
		...
		for (i = 0; (i < cb->imethod_table_size) && (mb->class
				!= cb->imethod_table[i].interface); i++) {
			// Do nothing
		}

...
int zipHash(char *path) {
	...
	xi_mem_copy(buff, path, path_len);
	buff[path_len] = '\0';
...
int zipComp(char *path1, char *path2) {
	...
	return path1_len == path2_len && !xi_mem_cmp(path1, path2, path1_len);
}
...
ZipFile *processArchive(char *path) {
	...
	if ((fd = xi_file_open(path, XI_FILE_MODE_READ, 0777)) == -1) {
		return NULL;
	}

	/* First 4 bytes must be the signature for the first local file header */
	if (xi_file_read(fd, &magic[0], SIG_LEN) != SIG_LEN || READ_LE_INT(magic)
			!= LOC_FILE_HEADER_SIG) {
		log_error(XDLOG, "Archive magic header is not matched!!\n");
		goto error;
	}

	/* Get the length */
	len = (xint32) xi_file_seek(fd, 0, XI_FILE_SEEK_END);

	/* Mmap the file into memory */
	data = NULL;
	if (xi_mmap_map((xvoid**) &data, len, XI_MMAP_PROT_READ,
			XI_MMAP_TYPE_PRIVATE, fd, 0) != XI_MMAP_RV_OK) {
		log_error(XDLOG, "Cannot mapping the file to memory(size=%d)!!\n", len);
		goto error;
	}
	...
	if (len < END_CEN_LEN) {
		log_error(XDLOG, "File size is not matched!!\n");
		goto error2;
	}
	...
	if (pntr < data) {
		log_error(XDLOG, "Cannot get the signature!!\n");
		goto error2;
	}
	...
	error2: // ERROR GOTO LABEL #2
	xi_mmap_unmap(data, len);
...
int utf8ZipComp(char *path1, char *path2) {
	...
	xi_mem_copy(buff, path2, path2_len);
	buff[path2_len] = '\0';
...
char *findArchiveEntry(char *pathname, ZipFile *zip, int *uncomp_len) {
	...
	*uncomp_len = READ_LE_INT(dir_entry + (CEN_FILE_UNCOMPLEN_OFFSET -
	...
	case COMP_STORED:
		...
		xi_mem_copy(decomp_buff, comp_data, comp_len);

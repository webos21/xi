...
xuintptr *callJNIMethod(void *env, Class *class, char *sig, int num_args,
		xuintptr *ostack, unsigned char *func) {
	ffi_cif cif;
	ffi_status err;
	void *values[num_args];
	ffi_type *types[num_args];
	xuintptr *opntr = ostack;
	...
	SCAN_SIG(sig, DOUBLE, SINGLE);
			...
			err = ffi_prep_cif(&cif, FFI_DEFAULT_ABI, num_args, sig2ffi(*sig),
					types);

...
#define VA_SINGLE(args, sp)                                 \
    if(*sig == 'L' || *sig == '[')                          \
        *sp = va_arg(args, xuintptr) & ~REF_MASK;          \
    else                                                    \
        if(*sig == 'F')                                     \
...
#define ADJUST_RET_ADDR(ret_addr, ret_type) ({              \
    ...
    if(IS_BIG_ENDIAN) {                                     \
        ...
        switch(ret_type) {                                  \
            case 'B': case 'Z':                             \
                size = sizeof(xuintptr) - 1;               \
                break;                                      \
            case 'C': case 'S':                             \
                size = sizeof(xuintptr) - 2;               \
                break;                                      \
            case 'I': case 'F':                             \
                size = sizeof(xuintptr) - 4;               \
                break;                                      \
            default:                                        \
                size = 0;                                   \
                break;                                      \
...
void *executeMethodVaList(Object *ob, Class *class, MethodBlock *mb,
    ...
    if(ob)
        *sp++ = (xuintptr) ob; /* push receiver first */

...
#include "../jamvm/jam.h"
#include "../jamvm/class.h"
#include "../jamvm/symbol.h"
#include "../jamvm/excep.h"

#include "xi/xi_thread.h"

#define BCP_MESSAGE "<jar/zip files and directories separated by "XI_SEP_PATH_S">"
	...
	for (i = 1; i < argc; i++) {
		if (*argv[i] != '-') {
			...
			if ((args->props_count = props_count)) {
				args->commandline_props = sysMalloc(
						props_count * sizeof(Property));
				memcpy(args->commandline_props, &props[0],
						props_count * sizeof(Property));
			}
		...
		if (strcmp(argv[i], "-?") == 0 || strcmp(argv[i], "-help") == 0) {
		...
		else if (strncmp(argv[i], "-ms", 3) == 0 || strncmp(argv[i], "-Xms", 4)
				== 0) {
		...
		} else if (strncmp(argv[i], "-mx", 3) == 0 || strncmp(argv[i], "-Xmx",
				4) == 0) {
		...
		} else if (strncmp(argv[i], "-ss", 3) == 0 || strncmp(argv[i], "-Xss",
				4) == 0) {
		...
		} else if (strcmp(argv[i], "-classpath") == 0 || strcmp(argv[i], "-cp")
				== 0) {
		...
		} else if (strncmp(argv[i], "-Xbootclasspath/a:", 18) == 0 || strncmp(
				argv[i], "-Xbootclasspath/p:", 18) == 0 || strncmp(argv[i],
				"-Xbootclasspath/c:", 18) == 0 || strncmp(argv[i],
			...
			/* Compatibility options */
				...
				|| strcmp(argv[i], "-server") == 0 || strncmp(argv[i],
				"-XX:PermSize=", 13) == 0 || strncmp(argv[i],
				"-XX:MaxPermSize=", 16) == 0) {
			/* Ignore */
		} else {
...
int main(int argc, char *argv[]) {
	...
	if ((system_loader = getSystemClassLoader()) == NULL) {
		jam_fprintf(2, "*********<Jam_main> error: system_loader!!\n");
		goto error;
	}
	...
	// printf("memset ======================================\n");
	memset(userArgv, 0, sizeof(userArgv));
	// printf("copy ========================================\n");
	for (i = 0; i < argc; ++i) {
		strcpy(userArgv[i], argv[i]);
	}
	// printf("replace sep =================================\n");
	for (cpntr = userArgv[class_arg]; *cpntr; cpntr++) {
		if (*cpntr == '.') {
			*cpntr = XI_SEP_FILE_C;
		}
	}
	...
	// printf("main class == %s\n", userArgv[class_arg]);
	main_class = findClassFromClassLoader(userArgv[class_arg], system_loader);
	if (main_class != NULL) {
		initClass(main_class);
	}

	if (exceptionOccurred()) {
		jam_fprintf(2, "*********<Jam_main> error: exceptionOccurred!!\n");
		goto error;
	}
	...
	if (mb == NULL || !(mb->access_flags & ACC_STATIC)) {
		jam_fprintf(2, "*********<Jam_main> error: signalException!!\n");
	...
	if ((array_class = findArrayClass(SYMBOL(array_java_lang_String)))
		...
		for (; i < argc; i++) {
			if (!(args[i] = Cstr2String(userArgv[i]))) {
				break;
			}
		}
	...
	if ((status = exceptionOccurred() ? 1 : 0)) {
		jam_fprintf(2, "*********<Jam_main> error status: %d\n", status);
		uncaughtException();
	}

    ...
    if(srcLength ==-1){
        ...
        for(;;){
            ...
            if((pSrc+nulLen) < pSrcLimit){
                ...
                if(remaining < (nulLen * ANDROID_MB_CUR_MAX)){
                    /* should rarely occur */
                    int32_t len = (pCSrc-pCSave);
                    pCSrc = pCSave;
                    /* we do not have enough room so grow the buffer*/
                    u_growAnyBufferFromStatic(cStack,(void**)&pCSrc,&cStackCap,
                           _BUFFER_CAPACITY_MULTIPLIER*cStackCap+(nulLen*ANDROID_MB_CUR_MAX),len,sizeof(char));
                ...
                if(remaining < (nulLen * ANDROID_MB_CUR_MAX)){
                    ...
                    /* we do not have enough room so grow the buffer*/
                    u_growAnyBufferFromStatic(cStack,(void**)&pCSrc,&cStackCap,
                           cStackCap+(nulLen*ANDROID_MB_CUR_MAX),len,sizeof(char));
